...
**Implementation note (2026-05-17)**: D4 landed in `archive/tickets/SPEC37STOPIPTEN-004.md`. `describe_capabilities().build_info` now includes `validator_registry_hash` over normalized validator source content and `patch_operation_schema_hash` over the shared patch-operation schema manifest. `docs/MACHINE-FACING-LAYER.md` now documents all five build-info fields and replaces the stale validator-bundle "cannot detect" diagnostic with the new fingerprint comparison path. The focused proof was `cd tools/world-mcp && npm run build && node --test dist/tests/tools/describe-capabilities.test.js dist/tests/server/capability-parity.test.js dist/tests/server/dispatch.test.js` with 39 passing tests; the final broad `cd tools/world-mcp && npm test` lane passed with 380 passing tests.
