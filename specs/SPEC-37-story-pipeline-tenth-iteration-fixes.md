...
2. **Test extension** (`tools/validators/tests/structural/expected-witness-coverage.test.ts`): add four tests beyond the existing eight.
   ...
   - `expected_witness_coverage_accepts_public_da_with_indirect_route_bel` — fixture: as above but with an additional BEL whose `basis.source_records: ["DA-1"]` and `basis.access_route: document`. Expect no verdict.
