...
`propose-new-worlds-from-preferences/SKILL.md` Phase 7 enforces a 3-order consequence skeleton, ≥7 institution categories, ≥3 spectator-caste leverage forms, and an equilibrium explanation per NWP card; Phase 12 backstops with 19 tests including One-Sentence Fertility (Test 1), Minimal Departure (Test 2), and Spectator-Caste Leverage (Test 18).
...
**A2. Phase 8 — CF-0001 propagation breadth + Rule 11 leverage at genesis.** Currently Phase 8 requires `domains_affected[]` to be populated with canonical-domain values but sets no minimum spread. Track A2 amends Phase 8 to:
...
Each new test follows Phase 9's existing format: a one-line failure trigger and a one-line repair direction. PASS requires a one-line rationale citing the world's specific element that satisfies the test (bare PASS without rationale is FAIL per Phase 9's existing discipline).
...
1. **`.claude/skills/create-base-world/SKILL.md` §Phase 4** — add prose: "Each section's body MUST materialize ≥1 first-order consequence of CF-0001 in domain-appropriate prose; stubs that defer materialization to later `canon-addition` runs are rejected at Phase 9."
2. **`.claude/skills/create-base-world/SKILL.md` §Phase 8** — amend the CF-0001 schema-population paragraph to add three requirements:
   ...
   - "If CF-0001 introduces or depends on exceptional capability per `requiresExceptionGovernance(cf.type)`, populate Rule 11 leverage with ≥3 distinct ordinary- / mid-tier-actor leverage forms (genesis spectator-caste check)."
3. **`.claude/skills/create-base-world/SKILL.md` §Phase 9** — amend the rejection-test list to append seven new tests in the order specified in §Approach Track A3. Inline the FOUNDATIONS / Pattern source citation per test.
4. **`.claude/skills/create-base-world/SKILL.md` §Validation Rules This Skill Upholds** — add a new entry: "**FOUNDATIONS §Acceptance Tests** — Phase 9 exercises all 11 Acceptance Tests inline (5 net-new in this SPEC); failure on any test routes back to the responsible upstream phase (Phase 2 / 4 / 8 typically)."
...
3. **`.claude/skills/canon-addition/SKILL.md` §PA `body_markdown` Structure** — add `misrecognition_probe` to the Phase 0 sub-section requirement so the PA carries the probe outcome.
...
| Rule 11 (No Spectator Castes by Accident) | aligns | Track A2 brings the Rule 11 leverage check forward to genesis (currently first applied in the per-CF Phase 14a Test 11). |
...
|---|---|
| Phase 9 enumerates all 11 FOUNDATIONS Acceptance Tests | codebase grep-proof — `.claude/skills/create-base-world/SKILL.md` §Phase 9 contains all 11 test names verbatim |
...
| FOUNDATIONS alignment is bidirectional | FOUNDATIONS alignment check — every FOUNDATIONS §Acceptance Test maps 1:1 to a Phase 9 test by name (5 new + 6 existing); the Phase 14a Test 13 maps 1:1 to FOUNDATIONS Acceptance Test #9 |
...
3. **Three-orders-of-consequence at genesis competes with FOUNDATIONS' "world starts thin and grows" stance.** The current `create-base-world` SKILL.md §Phase 4 reads: "The world starts thin and grows via `canon-addition`; one section per concern is sufficient." Track A1 partially contradicts this. Resolution: thin-but-rooted, not stub-and-defer. The world still grows; Phase 4 sections still seed forward. The SPEC-18 amendment sharpens "thin" from "stubs allowed" to "first-order consequences materialized; second / third-order can grow via canon-addition." Phase 4 prose should be amended to say "thin in coverage, not in concrete commitment."
