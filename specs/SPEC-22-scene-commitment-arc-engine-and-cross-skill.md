...
- **Post-SPEC-21 reassessment required (cross-spec dependency on SPEC-20 §H restructure)**: scheduled per user direction 2026-05-07 to reassess this spec once SPEC-21 lands. Two cross-spec items must be reconciled at that time:
  1. **`cadence_policy` word-count fields removed from SPEC-20 §H** (per SPEC-20 reassessment 2026-05-07): the §H block was rebuilt in arc-units only — `default_min_words_between_menus`, `preferred_words_per_arc`, and `max_words_without_player_commitment_soft` were dropped to honor Prose Craft Contract Rule 11 (commit `b28aead`, 2026-05-06, removed word-per-page guidelines after the LLM was observed padding to fill ranges). This invalidates SPEC-22 §Track 4 references to those fields:
     - Bootstrap Phase 7 instruction "max_words is governed by STORY_KERNEL's `cadence_policy.preferred_words_per_arc[1]` upper bound (default 2000)" — needs to drop the cadence_policy reference and route to engine-only `safety_valves.max_words` runaway-defense.
     - Bootstrap Phase 1 premise-derivation heuristic ("slow-paced literary premises lean toward `preferred_words_per_arc[1]: 1500`... action-oriented premises lean toward `preferred_words_per_arc[1]: 2500`...") — needs to be dropped or re-expressed in arc-unit terms (e.g., per-bundle `max_arcs_without_player_commitment_soft` defaults).
     - Health-audit Choice Cadence sub-check ("mean words per arc-page", "mean words between menus", "mean words per arc-page < 700 → warning") — needs replacement metrics in arc-units (mean arcs between menus; menu-emission-ratio retained; the words-per-arc-page warning is dropped because it structurally rewards longer prose, the same anti-pattern Rule 11 was added to prevent).
  2. **`arc_envelope_conformance` validator gap (8th validator)**: SPEC-20 §E lists `arc_envelope_conformance` among the 5 new Phase 9 gates, but SPEC-22 §Track 2's validator list defines only 7 validators — `arc_envelope_conformance` is absent. SPEC-20 §E (post-reassessment) clarifies the gate's check (envelope-conformance against ARC_TRACE `possible_violations[]` evidence at canonical-record-time, deterministic counterpart to Layer 3's LLM critic verdict). The reassessed SPEC-22 should add this validator as the 8th entry in §Track 2's table with the implementation file `tools/validators/src/rules/arc_envelope_conformance.ts`.
