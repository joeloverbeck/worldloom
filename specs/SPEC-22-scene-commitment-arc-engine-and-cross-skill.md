...
The existing `record_schema_compliance` structural validator extends to handle SLT v2 + CHC v2 + ARC_TRACE record types — including extension of the JSON schemas under `tools/validators/src/schemas/` (`story-storylet.schema.json` and `story-choice.schema.json` extended for v2 fields; new `story-arc-trace.schema.json`) and registration in the `RECORD_TYPE_TO_SCHEMA` map (`tools/validators/src/structural/utils.ts`). The existing `storylet_predicate_dsl_parsability` validator's grammar-loading logic is shared with `stop_policy_parsability` (both consume the extended `predicate-dsl.md`).
...
- **Validator implementation effort**: 8 new validators (incl. extension of 2 existing) + extension of `record_schema_compliance` (and the underlying JSON schemas under `tools/validators/src/schemas/`) is substantial. Estimate: 1-2 weeks of focused implementation for an experienced TypeScript engineer with familiarity with the existing validator patterns.
