...
**Depends on:** archive/specs/SPEC-112-manual-story-studio-record-pickers.md (reuses `RecordForm`, `RecordCard`, `RecordPicker`), archive/specs/SPEC-114-manual-story-studio-delete-lifecycle.md (reuses the broad referrer scanner + block-on-referrer delete).
...
2. **Post-Segment Workbench page (new).** Two-pane per-story page:
   - **Left:** accepted segment text (rendered Markdown), segment title, prompt ID, word count, last paragraph, and the included-records list from the segment sidecar (read-only).
...
3. **Reuse the broad referrer scanner.** Extend/expose the `records.ts:248-291` scanner so the workbench's "touches this segment" rail uses the same field coverage as delete-safety (`holder/between/owed_by/subject/held_by/refs.*`), replacing the narrow `refs.characters`-only logic.
4. **Remove `last_reviewed_after_segment`.** Drop the field from the `current-context` schema, read/write/validate paths, and any UI. Keep `last_accepted_segment` (it seeds recent-prose context / workbench navigation, not review status — report §17).
...
- **Routed, not modal.** After save, navigate to the workbench (a real per-story page via `StoryPageNav`), so the author is *in* the maintenance surface, not dismissing a dialog to reach it.
...
- `tools/manual-story-studio/src/schema/current-context.ts` — remove `last_reviewed_after_segment`; keep `last_accepted_segment`.
- `tools/manual-story-studio/src/read/current-context.ts`, write/validate paths, and the GET/PUT route — drop the removed field.
- `tools/manual-story-studio/src/read/records.ts` — export/reuse the broad referrer scan for the workbench (no behavior change to delete).
- `tools/manual-story-studio/web/src/App.tsx` / `StoryPageNav` — mount the workbench page.
...
2. The workbench left pane shows accepted prose, segment title, prompt ID, word count, last paragraph, and sidecar included-records.
...
5. `last_reviewed_after_segment` no longer exists in schema, read/write/validate, route, or UI; a current-context fixture containing the old field still loads (field ignored/dropped) and round-trips without it. `last_accepted_segment` is retained.
