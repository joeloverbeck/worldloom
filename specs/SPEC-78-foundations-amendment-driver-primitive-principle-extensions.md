...
SPEC-76's §4 Out of Scope previously rejected "Updating `docs/FOUNDATIONS.md`." SPEC-78 lands the FOUNDATIONS amendment carrier; SPEC-76's Out-of-Scope bullet is updated in SPEC-78's Slice A (see §8) to point at SPEC-78 instead of rejecting the amendment outright.
...
**Slice A — FOUNDATIONS prose additions + SPEC-76 §4 truthing.**
...
3. `Edit` `specs/SPEC-76-turn-driver-primitive-and-pressure-driven-turn-cycle.md` §4 Out of Scope to replace the FOUNDATIONS-rejection bullet with a forward reference to SPEC-78 as the FOUNDATIONS amendment carrier.
4. Grep verification: the two new principle paragraphs appear in FOUNDATIONS with the exact prose from §3.1 and §3.2; SPEC-76 §4 no longer carries the old rejection bullet.
...
- **Reversal of an earlier triage decision.** The triage at `docs/triage/2026-05-23-slt-chc-overhaul-first-iteration-triage.md` initially placed "Updating `docs/FOUNDATIONS.md`" in "Out of Scope" with the rationale "FOUNDATIONS principles are unchanged… only schema and contract surfaces change." The user's follow-up question about FOUNDATIONS sufficiency surfaced the gaps in §5c and §6b that the initial triage missed. The triage file is updated in Slice A to record the reversal explicitly so the audit trail is intact.
...
- Negative precedents (record-class additions that did not warrant FOUNDATIONS amendments): SPEC-47 (STPLAN), SPEC-48 (STEMO), SPEC-63 (offstage causal packet tier).
