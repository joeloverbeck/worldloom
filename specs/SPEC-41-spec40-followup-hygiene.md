...
- **Considered scope-path-B (spawned-process tests per hook mirroring SPEC-40 D4); chose scope-path-A (src-vs-dist content-hash CI check) for D4 `(structural)`.** Hooks differ structurally from the MCP server: each hook is a short-lived CLI invocation spawned per Claude Code event, not a persistent process. The MCP-server spawned-process test catches stale `dist/` because the server runs across many requests; hooks load `dist/` fresh per event, so the failure mode is different — what matters is that `dist/` matches `src/` at the build moment, not that a deployed process returns matching hashes. A CI dist-currency check (run `npm run build`, compare produced `dist/*` against committed `dist/*`; mismatch fails) covers this cleanly without the per-hook spawn cost.
...
**Change**: Add a CI check script (likely `tools/hooks/scripts/check-dist-currency.sh` or equivalent) that runs `npm run build` against `tools/hooks/`, captures the produced `dist/` content hash (SHA-256 over sorted file content), compares against the committed `dist/` content hash. If they differ, the script fails CI with a clear message instructing the implementer to commit the rebuilt `dist/`. Wire into `.github/workflows/ci-hooks.yml` as a CI gate. Scope-path-A (content-hash check at build time) was chosen over scope-path-B (spawned-process per hook) per §Key design decisions because hooks are one-shot CLI invocations, not persistent processes.

**Ticket**: `tickets/SPEC41FUP-004.md` — see ticket for full Files to Touch, Acceptance Criteria, Test Plan.
