...
|---|---|---|---|---|
| 1 | [`SPEC-78-foundations-amendment-driver-primitive-principle-extensions.md`](SPEC-78-foundations-amendment-driver-primitive-principle-extensions.md) | FOUNDATIONS amendment (docs-only): extend §Story Bundles §5c with "Driver salience is local." paragraph; extend §6b with event-level driver-declaration paragraph; truth SPEC-76 §4 Out of Scope by removing the now-stale FOUNDATIONS-rejection bullet | — | No schema / validator / skill changes. Single `Edit` slice. Lands first because FOUNDATIONS is upstream of contract and schema — the principles SPEC-76 cites are the extended ones. |
| 2 | [`SPEC-76-turn-driver-primitive-and-pressure-driven-turn-cycle.md`](SPEC-76-turn-driver-primitive-and-pressure-driven-turn-cycle.md) | Schema (`story-event.schema.json` — collapse `event_kind`, add `turn_driver` object, extend `selection_source`); shared story-state-contract §4 / §7 / §8; turn-cycle skill Phase 0; bootstrap; health-audit; 4 new validators; page-plan §7a + active-pressure handling discipline | SPEC-78 (FOUNDATIONS principles SPEC-76 cites are the extended ones) | Breaking schema change; existing test bundles (`red-bunny`) must rebuild. New §7a parser shares the §16a SPEC-73 label-parsing pattern. |
| 3 | [`SPEC-77-slt-grounding-provenance-minimal.md`](SPEC-77-slt-grounding-provenance-minimal.md) | Schema (`story-storylet.schema.json` — required `grounding.compatible_turn_drivers[]` + `reason_to_exist`); shared contract §4 SLT subsection; commitment-block-authoring skill Phase 4; 1 new validator (`slt_grounding_minimal_integrity`); turn-cycle Phase 2 compatible-driver filter | SPEC-76 | `compatible_turn_drivers[]` enum values come from SPEC-76's `turn_driver.kind`. The turn-cycle Phase 2 filter slice may be folded into SPEC-76 Slice B if implementation prefers a single skill-update pass. |

## Dependency rationale

- **SPEC-78 first (FOUNDATIONS is upstream).** SPEC-76's `Validation Rules Upheld` table cites the extended §5c ("Driver salience is local.") and the extended §6b (event-level driver declaration). Landing SPEC-76 before SPEC-78 would mean SPEC-76 cites principles that don't yet exist in their extended form. Landing SPEC-78 first makes the citations factual at SPEC-76 implementation time.
