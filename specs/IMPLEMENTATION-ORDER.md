...
|---|---|---|---|---|
| 1 | [`specs/SPEC-73-page-packet-required-because-label-parsing.md`](SPEC-73-page-packet-required-because-label-parsing.md) | Multi-label parsing of §16a `Required because:` so composite values containing `speaker` / `viewpoint` / `voice_shapes_page` trigger the voice-block requirement; parallel fix for `offstage_causal` exact-match drift; closed-vocabulary warning for unknown labels. Validator-only change; no schema, no record mutation. | — | low — single validator change; legacy single-label packets unchanged; composite packets that newly FAIL are exactly the contract-drift cases the fix is designed to catch |
