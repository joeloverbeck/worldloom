...
| 3 | **SPEC-64** — World-system compatibility & artifact-maturity validation | `artifact_maturity_validator` + `world_compatibility_validator` orchestration + index-consistency checks + `continuity-audit` compatibility reporting (triage D5 / Fault 11) | **SPEC-61** (landed schemas + `approval-semantics` validator) + **SPEC-62 §2.1** (FOUNDATIONS §Artifact Authority and Maturity — carried debt; ticketed `SPEC62FOUANDDOC-001`) |

SPEC-61 (archived) and SPEC-62 are independent and may run in parallel; SPEC-62 is docs/FOUNDATIONS only.
SPEC-64 builds on SPEC-61's landed schemas + `approval-semantics` validator and **carries a debt on
SPEC-62 §2.1** (FOUNDATIONS §Artifact Authority and Maturity, ticketed `SPEC62FOUANDDOC-001`): it must
land after that section exists, since its `artifact_maturity_validator` sources the maturity-tier
vocabulary there. Triage **D5** (continuity-audit compatibility / Fault 11) is lifted into SPEC-64.
