...
**Depends on:** **SPEC-96** (timeline segment model + scene/unscened read layer) and **SPEC-97** (frontend shell to host the search modal + branch-map route). Land SPEC-96, then SPEC-97.
**Supersedes:** the page-centric search + branch-map intent of `SPEC-90` (`specs/SPEC-90-story-explorer-branch-map-and-search.md`) and its `not_implemented` placeholder routes (`src/server/routes/search.ts`, `branch-map.ts`). **SPEC-90 is being removed this iteration; this spec carries the live search/branch-map contract.**
...
`search.ts` and `branch-map.ts` currently return `{ kind: "not_implemented", spec: "SPEC-90" }` placeholders; SPEC-90 itself is page-centric (PG graph nodes, page-search modal, page-jump actions, `searchPages` client). With the segment model (SPEC-96) and scene-first frontend (SPEC-97) in place, search and branch-map are implemented against scenes / unscened ranges / state ticks — not pages — and the SPEC-90 placeholders are removed.

## 2. Scope

### In scope

1. **Remove SPEC-90 placeholders**: replace the `not_implemented` bodies in `src/server/routes/search.ts` and `branch-map.ts` with real implementations; remove the `searchPages` client function and SPEC-90 references in tests (`sketch-routes.test.ts`, `capstone-smoke.test.ts`). (Deletion of the `specs/SPEC-90-*.md` file itself is the user's separate action this iteration.)
...
- Backend: `tools/story-explorer/src/server/routes/search.ts`, `branch-map.ts` (real impl); `src/read/` search + branch-map modules; view-models `SearchHit`, `BranchMapGraph` (scene-layer). Tests: rewrite `sketch-routes.test.ts` (remove SPEC-90 assertions), add search/branch-map route tests.
- Frontend: `web/src/components/SearchModal.tsx`, `BranchMapCanvas.tsx`; `web/src/api/client.ts` (`search`, `getBranchMap`; remove `searchPages`); `web/src/app.tsx` (`/search`, `/branch-map` routes); a11y tests for drawer/modal.
...
`tools/story-explorer`: `npm test` (backend `node --test` + `web` vitest). Add route tests for search grouping + branch-map MVP nodes; assert the SPEC-90 `not_implemented` placeholders are gone and no `searchPages` remains.
...
3. The SPEC-90 `not_implemented` placeholder routes + tests + `searchPages` client are removed; both routes return real data.
