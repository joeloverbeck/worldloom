...
- **`PG.state_snapshot.active_records[]` LLM-token cost. (pragmatic — current 12 classes already produce substantial snapshot text; adding 3 more classes adds proportional cost.)** The snapshot is loaded into every storylet-selection decision; adding three more record-class slots adds proportional LLM-token cost. Page-plan §10b inlines only currently-active CLK/STSEC/STQ; the active_records[] entries are bare ID lists per existing convention.
