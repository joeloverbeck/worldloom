...
- **Plan-hash coupling (author ergonomics).** SPEC-72 already made `plan_hash` advisory; SPEC-72 §3 explicitly deferred *removing* `plan.plan_hash`/`prose_plan_path` because that requires a `snapshot_replay_equality` discontinuity clause. With page plans gone, both fields are meaningless for new `PG`s (they point at / hash a non-existent page plan). The author wants them gone (no value in their workflow). This spec writes the discontinuity clause and removes them from new `PG`s while grandfathering existing records.
...
3. **`PG` schema**: make `plan.plan_hash` and `prose_plan_path` **optional** (grandfather existing records; new `PG`s omit them). Remove `prose_plan_path` from the `state_hash` payload for new `PG`s. Add a **`snapshot_replay_equality` discontinuity clause** keyed on field presence/schema epoch: `PG`s authored before SPEC-93 verify under the legacy payload (with `prose_plan_path`); `PG`s authored after verify under the new payload (without). Retain the `state_hash`/`state_hash_parent` chain and the `computePgStateHash`-from-stored-fields tamper check otherwise unchanged.
...
- Existing `PG` records keep their `plan.plan_hash` + `prose_plan_path` (append-only YAML; not rewritten). Their stored `state_hash` stays valid under the **legacy** payload definition (the discontinuity clause). New `PG`s authored after SPEC-93 omit both fields and hash under the **new** payload.
- The discontinuity boundary is detectable from the record itself (presence of `prose_plan_path` / a schema-epoch marker), so `snapshot_replay_equality` selects the correct payload per record without external state.
...
- **`story-record-schemas.md`**: `plan.plan_hash` and `prose_plan_path` → optional; document the dual-payload `state_hash` rule + discontinuity clause; the `compute-pg-hashes` mandate narrows to `state_hash`-only.
...
- **§Story Bundles §4a ("Plan-Authority Boundary")**: reword so authority anchors on the `PG` record; "rendered prose is a rendering of committed state" generalizes to scene prose; no page-plan render artifact is part of the state turn.
...
- **§Story Bundles §9**: drop the page-plan §2/§3/§19 hosting language; the verbatim contract blocks are hosted in scene plans (SPEC-92). **Verbatim-sections note:** the cold-context verbatim-inlining decision ([[feedback-page-plan-verbatim-sections]]) is **preserved, not reversed** — §2/§3/§render-time still inline verbatim into every render artifact; the artifact is now the scene plan rather than the page plan.
- **§Story Bundles §7**: `branching-story-prose-attach` leaves the Skill Category 2c roster (replaced by SPEC-92's scene-prose-attach).
...
- **Validators:** retire `page-plan-verbatim-section-integrity.ts`, `page-plan-stchar-packet-integrity.ts`, `page-plan-body-engine-vocabulary-cleanliness.ts`, `page-plan-section-parser.ts`, `prose-receipt-hash-integrity.ts`, `prose-receipt-stchar-integrity.ts`, `prose-receipt-schema-compliance.ts`, `schemas/prose-receipt.schema.json`; split `page-plan-turn-driver-consistency.ts` + `page-plan-active-pressure.ts` (retire plan-markdown checks, rehome record-based gate-9 logic); retain record-operating `turn-driver-pov-observer-firewall.ts`; update `schemas/story-page.schema.json` (optional `plan_hash`/`prose_plan_path`); registry + `_engine-vocabulary-tokens.ts` cleanup.
- **world-mcp:** `cli/compute-pg-hashes.ts` (state-only), `tools/verify-pg-state-hash.ts` (drop `plan_hash` comparison), `server.ts` + `tools/validate-patch-plan.ts` + `cli/{submit,validate}-patch-plan.ts` (drop `page_plan_drafts`), `tools/plan-story-state-maintenance.ts`, `README.md`.
- **world-index:** `src/hash/content.ts` (dual `state_hash` payload + discontinuity); `enumerate.ts` keeps legacy page directories for read (no removal).
...
| Rule 6 (No Silent Retcons) | N/A | No canon mutation; existing records are append-only and grandfathered, not rewritten; the discontinuity clause is documented, not silent. |
...
1. `branching-story-turn-cycle` produces no `pages-prose-plans/PG-<n>.md`; `branching-story-bootstrap` produces no `pages-prose-plans/PG-1.md`. The page-plan HARD-GATE precondition and `page_plan_drafts` argument are gone.
...
3. New `PG` records carry no `plan.plan_hash`/`prose_plan_path`; `story-page.schema.json` makes both optional; the new `state_hash` payload excludes `prose_plan_path`; the `snapshot_replay_equality` discontinuity clause is documented and tested.
4. Existing pre-SPEC-93 `PG`s still verify (grandfathered under the legacy payload); a new `PG`'s `state_hash` is byte-identical under the new payload across re-runs; tamper detection (hand-edited `PG` state field → FAIL) is retained.
...
- `tools/world-index`: dual-payload `state_hash` — legacy `PG` (with `prose_plan_path`) verifies under the legacy payload; new `PG` (without) verifies under the new payload; the discontinuity selector keys off the record. PASS rationale: each record verifies under its own epoch; cross-epoch recompute does not false-FAIL.
- `tools/world-mcp`: `compute-pg-hashes` emits `state_hash` only (no `plan_hash` stamping); `verify-pg-state-hash` reports only `state_hash` (no `plan_hash_match`); patch submission accepts no `page_plan_drafts`.
