...
This spec deliberately does **not** re-implement the approval-semantics check (SPEC-61 owns it; this spec *invokes* it), and deliberately does **not** route proposal direct-writes through the patch engine (report §9.5 explicitly defers that until direct-write recovery proves painful — no such pain signal exists).
...
**File:** `tools/validators/src/structural/world-compatibility.ts` (+ registry append) and a CLI entry alongside the existing `tools/validators/src/cli/` surface.

A high-level aggregation pass over a world's root files, `_source/`, `characters/`, `diegetic-artifacts/`, `adjudications/`, `proposals/`, `audits/`, `pressure-events/`, and `world-proposals/`. It **invokes the already-landed** `approval-semantics` validator (SPEC-61) and the new `artifact_maturity_validator` (D1), plus schema validation, and reports a consolidated compatibility verdict. Posture per report §10: **fail-fast with detailed incompatibility messages and manual repair** — no migration / backwards-compatibility shim. Blocks canon/hybrid writes when run as a pre-write gate; warns (does not block) when run in read-only audit mode (D4).

### D3 — Index-consistency checks (report §9.5 step 3)

Extend the compatibility pass with `INDEX.md`-to-disk reconciliation for the proposal/audit/pressure/character-proposal surfaces: every artifact on disk is listed in its `INDEX.md` and vice versa; error class `INDEX_DISK_DRIFT` names the missing/orphaned entry. This is the §9.5 step-3 "index consistency checks" prerequisite that the report places *before* any future consideration of engine-routing (step 4, deferred).

### D4 — `continuity-audit` compatibility reporting hook

**File:** `.claude/skills/continuity-audit/SKILL.md` (+ relevant reference file).

Add an optional, **read-only** compatibility-reporting phase to `continuity-audit` that runs `world_compatibility_validator` in warn mode and surfaces maturity/index/approval findings as a compatibility appendix in the `AU-<integer>` report. This is Fault 11's "optional continuity-audit reporting" — it does not let `continuity-audit` mutate any surface (it remains audit-only, writing to `worlds/<slug>/audits/`); compatibility *enforcement* (blocking) stays in the pre-write CLI gate (D2).
...
Fixture-world copy (never mutates real `worlds/<slug>/`); asserts: maturity collapse is caught; a non-CF `direct_user_approval` is still caught (no regression on SPEC-61's check via the orchestration path); index drift is caught; read-only audit mode warns rather than blocks; a clean world passes.
...
| §Canon Fact Record Schema — `direct_user_approval` (355–361) | aligns | D2 surfaces non-CF `direct_user_approval` misuse across all surfaces by invoking SPEC-61's landed `approval-semantics` validator; no re-implementation. |
...
- `artifact_maturity_validator` is registered in `tools/validators/src/public/registry.ts` and flags `ARTIFACT_MATURITY_COLLAPSE` for a `character-proposals/NCP-*` file presenting as a realized dossier; maturity is path/prefix-derived (no new `maturity_level` frontmatter field is added to any schema).
- `world_compatibility_validator` runs as a pre-write gate (blocks) and a standalone read-only CLI (warns), aggregating schema + approval-semantics (SPEC-61, invoked) + maturity (D1) + index-consistency (D3) checks.
...
- The SPEC-64 capstone test passes, including the no-regression assertion on SPEC-61's `direct_user_approval` check via the orchestration path.
- No proposal direct-write surface is converted to a patch-engine op (D6 stays deferred); no `taxonomy_authority_validator` (D3) or `mystery_policy_validator` is added.

## 5. Out of Scope

- **`approval_semantics_validator`** (report §10.3) — already landed by SPEC-61 (`approval-semantics.ts`); SPEC-64 invokes it, never re-implements it.
...
- **`taxonomy_authority_validator`** (report Fault 7 / triage D3) — deferred: low value, heavy, no consumer, and its closed-list authority-class convention does not exist.
...
- **A mandatory `maturity_level` frontmatter field** — rejected in favor of path/prefix derivation (no schema migration).
