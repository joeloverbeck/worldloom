...
2. **`SF.authority` is read but does not exist.** Landed `story-fact-promotion-to-canon` Phase 3.1 and Phase 4.3 branch on `SF.authority == branch_local_counterfactual`, but `SF`'s schema (contract §4.5.3 + `story-fact.schema.json`) has no `authority` field. The promotion pipeline's scope-inflation cap and mystery-firewall counterfactual check are dead logic today.
...
| §Story Bundles §11 (Mystery and Canon Authority) | aligns | D2's `SF.authority` makes the `apparent` / `branch_local_counterfactual` / `canon_candidate` / `canon_linked` discipline schema-backed — the promotion pipeline already assumes it. |
