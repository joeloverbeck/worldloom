...
**Status**: Five hooks implemented. Hooks 1, 2, and 4 landed 2026-04-24; Hooks 3 and 5 landed 2026-04-26 per SPEC-05 Part B. Hooks 6 and 7 were retired by SPEC-93 when page-plan and page-prose-receipt hash guards stopped being produced for new artifacts. Built hook entrypoints land at `dist/src/*.js`.
...
`npm test` builds the package and runs compiled-script tests for Hooks 1 through 5 against synthetic hook payloads and fixture worlds. Hook 5 tests stub the `world-validate` CLI under a temp repo root so the validator runner is exercised without a real world index.
