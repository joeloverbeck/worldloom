...
import { resolveWorldRoot } from "../../src/cli/_resolve-world-root.js";

function makeRoot(): string {
  const root = mkdtempSync(path.join(os.tmpdir(), "world-mcp-root-"));
...
test("resolveWorldRoot: explicit flag wins over env var and cwd discovery", () => {
  ...
  const nested = path.join(cwdRoot, "tools", "world-mcp", "dist");
...
test("resolveWorldRoot: env var wins over auto-discovery when no flag is present", () => {
  const envRoot = makeRoot();
  const cwdRoot = makeRoot();
  const nested = path.join(cwdRoot, "tools", "world-mcp");
  mkdirSync(nested, { recursive: true });
...
test("resolveWorldRoot: auto-discovery walks upward and requires both markers", () => {
  const root = makeRoot();
  const nested = path.join(root, "tools", "world-mcp", "dist");
  mkdirSync(nested, { recursive: true });
...
test("resolveWorldRoot: invalid explicit flag fails without falling back", () => {
  const invalidFlagRoot = mkdtempSync(path.join(os.tmpdir(), "world-mcp-invalid-root-"));
  const envRoot = makeRoot();
...
test("resolveWorldRoot: missing markers produce a clear attempted-path failure", () => {
  const root = mkdtempSync(path.join(os.tmpdir(), "world-mcp-no-markers-"));
