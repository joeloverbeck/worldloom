...
test("validatePatchPlan rejects malformed page_plan_drafts before validator delegation", async () => {
  const root = createTempRepoRoot();
  seedEmptyWorld(root);
  ...
  try {
    const result = await withRepoRoot(root, () =>
      validatePatchPlan({
        ...
        page_plan_drafts: [
          { path: "not-a-page-plan-path.md", content: "draft" } as { path: string; content: string }
        ]
      } as Parameters<typeof validatePatchPlan>[0])
    );

    assert.ok("code" in result);
    assert.equal(result.code, "invalid_input");
    assert.equal(result.message, "page_plan_drafts[0].path must match stories/<slug>/pages-prose-plans/PG-<integer>.md (got 'not-a-page-plan-path.md').");
  } finally {
    destroyTempRepoRoot(root);
  }
});

test("validatePatchPlan rejects duplicate page_plan_drafts paths", async () => {
  const root = createTempRepoRoot();
  seedEmptyWorld(root);
  ...
  try {
    const result = await withRepoRoot(root, () =>
      validatePatchPlan({
        patch_plan: buildValidPatchPlan(),
        page_plan_drafts: [
          { path: "stories/foo/pages-prose-plans/PG-1.md", content: "draft one" },
          { path: "stories/foo/pages-prose-plans/PG-1.md", content: "draft two" }
        ]
      } as Parameters<typeof validatePatchPlan>[0])
    );
    ...
    assert.ok("code" in result);
    assert.equal(result.code, "invalid_input");
    assert.ok(result.message.includes("duplicate path"));
  } finally {
    destroyTempRepoRoot(root);
  }
});

test("validatePatchPlan accepts well-formed page_plan_drafts and runs the underlying validators", async () => {
  const root = createTempRepoRoot();
  seedEmptyWorld(root);
  ...
  try {
    const result = await withRepoRoot(root, () =>
      validatePatchPlan({
        ...
        page_plan_drafts: [
          {
            path: "stories/red-bunny/pages-prose-plans/PG-4.md",
            content: "## 1. Story Kernel Excerpt\n\nMinimal draft body — does not exercise page-plan validators because the seed world has no story bundle.\n"
          }
        ]
      } as Parameters<typeof validatePatchPlan>[0])
    );
    ...
    assert.ok("status" in result);
    // The seeded world has no story bundle, so page-plan validators have nothing to compare against
    // and should not fail. The shape check passed; the underlying flow is exercised.
    assert.equal(result.status, "pass");
  } finally {
    destroyTempRepoRoot(root);
  }
