...
export function createServer(): McpServer {
  ...
  registerToolWithCapability(
    "list_records",
    "list_records: Return indexed records of a supported atomic, hybrid, or story-bundle record type, with optional server-side filters by parsed body field path, field projection, or include_full_body metadata/body records. Hybrid record types include character_record, diegetic_artifact_record, adjudication_record, character_proposal_card, and character_proposal_batch. Story-bundle record types require story_slug and include story_plan_record, story_emotion_record, scene_record, pressure_clock_record, story_secret_record, and story_question_record alongside the other supported story records. Filters accept scalar exact matches or array any-of matches, including dotted paths such as {shape: ['routine_disruption', 'reflection_dilemma'], content_intensity: ['mature', 'tame'], 'scope.visibility': 'global_author_pool'}. Atomic/story-bundle fields accept top-level keys or dotted parsed-body paths such as grounding.compatible_turn_drivers and unknown fields return invalid_input with accepted_projection_keys; hybrid default fields stay limited to compact metadata keys; include_full_body returns full bodies and ignores fields. Bundle-scoped IDs are unique within (world_slug, story_slug).",
  ...
  registerToolWithCapability(
    ...
    "plan_story_state_maintenance: Build a review-only patch-plan envelope for bounded story-bundle state maintenance from a required parent_page_id. It allocates fresh story-scoped IDs, verifies superseded source records through indexed retrieval, emits create_* story-record ops for STEMO/STPLAN/SREL/CHC maintenance, and appends an audit/system repair SE plus a planless forkable maintenance PG. It never submits, signs, writes, or asks the operator to write a page plan. Validate the returned patch_plan, obtain explicit approval, sign that exact plan, and apply it with submit_patch_plan.",
