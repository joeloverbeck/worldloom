...
export const DEFAULT_PACKET_VERSION = 1 as const;
...
export function estimatePacketTokens(packet: ContextPacket): number {
  ...
  total += packet.nucleus.nodes.reduce((sum, node) => sum + estimateNodeTokens(node), 0);
  total += packet.nucleus.why_included.reduce((sum, reason) => sum + estimateTextTokens(reason), 0);
  total += packet.envelope.nodes.reduce((sum, node) => sum + estimateNodeTokens(node), 0);
  total += packet.envelope.why_included.reduce((sum, reason) => sum + estimateTextTokens(reason), 0);
  total += packet.constraints.active_rules.reduce((sum, rule) => sum + estimateTextTokens(rule), 0);
  total += packet.constraints.protected_surfaces.reduce(
    (sum, surface) => sum + estimateTextTokens(surface),
    0
  );
  total += packet.constraints.required_output_schema.reduce(
    (sum, entry) => sum + estimateTextTokens(entry),
    0
  );
  total += packet.constraints.prohibited_moves.reduce(
    (sum, move) => sum + estimateTextTokens(move),
    0
  );
  total += packet.constraints.open_risks.reduce(
    (sum, risk) => sum + estimateTextTokens(JSON.stringify(risk)),
    0
  );
  total += packet.suggested_impact_surfaces.nodes.reduce(
    (sum, node) => sum + estimateNodeTokens(node),
    0
  );
  total += packet.suggested_impact_surfaces.rationale.reduce(
