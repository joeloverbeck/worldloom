...
**Status**: Stdio MCP entrypoint registers 16 tools in `src/server.ts`; `validate_patch_plan` delegates to `@worldloom/validators`; `submit_patch_plan` delegates to `@worldloom/patch-engine`
...
- `mcp__worldloom__get_context_packet(task_type, seed_nodes, token_budget)` — assembles the ranked retrieval packet. When `token_budget` is omitted, `DEFAULT_TOKEN_BUDGET_BY_TASK_TYPE` uses `canon_addition: 16000` and `8000` for other task types. `packet_incomplete_required_classes` errors include `retry_with: { token_budget }` for a single explicit retry with the computed minimum. See `docs/CONTEXT-PACKET-CONTRACT.md` §Index + Follow-Up Retrieval Pattern for the documented retrieval pattern that complements packet assembly.
