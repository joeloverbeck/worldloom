...
- `mcp__worldloom__get_record(record_id, world_slug?, section_path?)` — retrieves one structured record. Atomic records return parsed YAML and reject `section_path`; hybrid records (`CHAR`, `DA`, `PA`) return parsed frontmatter plus body sections. Optional `section_path` narrows hybrid responses: `frontmatter` returns full frontmatter only, `body` returns all body sections only, `frontmatter.<key>` returns one frontmatter field, and `body.<section>` returns one body section. If an unprojected hybrid response would exceed the effective inline ceiling, the tool persists the full JSON response under the tool-results directory (`WORLDLOOM_MCP_TOOL_RESULTS_DIR` override, default `/tmp/worldloom-mcp-tool-results/`) and returns a bounded `delivery_status: "oversize_with_projection_suggestions"` response with `persisted_output_path` plus `suggested_section_paths` for immediate structured retry; `suggested_section_paths_omitted_count` appears only when the inline hint list is shortened to stay under the cap.
...
- `mcp__worldloom__validate_patch_plan(patch_plan)` *(runs `@worldloom/validators` in pre-apply mode and returns `{ status: "pass" | "fail" | "skipped", verdicts, validators_run, reason?, details? }`; `validators_run[]` carries `{ validator_name, status, duration_ms, detail? }` on pass/fail and is empty on skipped; malformed envelopes with multiple shape errors include `details.additional_errors[]`)*
- `mcp__worldloom__submit_patch_plan(patch_plan, approval_token)` *(delegates to SPEC-03 `@worldloom/patch-engine`)*
- `mcp__worldloom__allocate_next_id(world_slug, id_class, story_slug?, audit_id?)` — allocates append-only IDs. Most world-scoped classes allocate from the world's index; `EPE` scans `worlds/<slug>/pressure-events/EPE-*.md` directly because pressure-event cards are hybrid, pre-canon files, and `STORY` scans `worlds/<slug>/stories/*/STORY_KERNEL.md` frontmatter for `story_id` because story bundles are not indexed. Pipeline-scoped proposal IDs use `world_slug: "__pipeline__"` with `id_class: "NWB"` for `world-proposals/batches/NWB-*.md` and `id_class: "NWP"` for `world-proposals/NWP-*.md`. Story-bundle-scoped IDs use `story_slug` and scan `worlds/<slug>/stories/<story_slug>/_source/<class>/`, including `PG`, `SE`, `SF`, `OBL`, `CNSQ`, `THR`, `SREL`, `STINT`, `SLT`, `STLOC`, `STOBJ`, `BR`, `CHC`, `STENT`, and story-local `DA`; `SLB` scans `worlds/<slug>/stories/<story_slug>/storylet-batches/SLB-*.md`, `SAU` scans `worlds/<slug>/stories/<story_slug>/audits/SAU-*.md`, and `SP` scans `worlds/<slug>/stories/<story_slug>/story-promotions/SP-*.md` plus `SP-*-proposal-package.yaml` sidecars. Sub-audit-scoped `RSP` requires both `story_slug` and bare `audit_id` (`SAU-NNNN`) and scans `worlds/<slug>/stories/<story_slug>/audits/<audit_id>/remediation-storylet-proposals/RSP-*.md`.
