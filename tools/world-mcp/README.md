...
**Status**: Stdio MCP entrypoint registers 12 tools in `src/server.ts`; `submit_patch_plan` delegates to `@worldloom/patch-engine`
...
- `mcp__worldloom__validate_patch_plan(patch_plan)` *(returns `validator_unavailable` until SPEC-04 lands)*
- `mcp__worldloom__submit_patch_plan(patch_plan, approval_token)` *(delegates to SPEC-03 `@worldloom/patch-engine`; currently fails closed at `validator_unavailable` before source writes until SPEC-04 validators land)*
