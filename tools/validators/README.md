...
**Status**: package scaffold, framework types, record-class JSON Schemas, the 14 structural validators, the 10 rule-derived/story-scope validators, package-internal structural/rule registries, the `world-validate` CLI, and the public `validatePatchPlan` entry point are present.
