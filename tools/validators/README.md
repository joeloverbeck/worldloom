...
**Status**: package scaffold, framework types, record-class JSON Schemas, the 93 structural validators, the 12 rule-derived/story-scope validators, package-internal structural/rule registries, the `world-validate` CLI, and the public `validatePatchPlan` entry point are present.
