...
test("stemo_agency_effect_compatibility accepts same-event rationale", async () => {
  const event = storyRecord("story_event_record", "SE-2", "events", eventBody("SE-2", "plan_relation:blocks(plan=STPLAN-1)"));
  const verdicts = await stemoAgencyEffectCompatibility.run(
    undefined,
