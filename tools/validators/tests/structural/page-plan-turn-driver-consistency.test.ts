...
test("page_plan_turn_driver_consistency accepts matching page and turn-driver event records without page-plan input", async () => {
  const verdicts = await pagePlanTurnDriverConsistency.run({}, context(records()));

  assert.deepEqual(verdicts, []);
});

test("page_plan_turn_driver_consistency reports a missing resolved turn event", async () => {
  const withoutEvent = records().filter((item) => (item.parsed as { id?: string }).id !== "SE-1");
  const verdicts = await pagePlanTurnDriverConsistency.run({}, context(withoutEvent));
...
test("page_plan_turn_driver_consistency reports page/event ownership mismatches", async () => {
  const mismatched = records({ created_at_page: "PG-2" });
  const verdicts = await pagePlanTurnDriverConsistency.run({}, context(mismatched));
...
test("page_plan_turn_driver_consistency ignores non-turn-resolution events and scopes to story record surfaces", async () => {
  const nonTurn = records({ event_kind: "story_start" });
  const verdicts = await pagePlanTurnDriverConsistency.run({}, context(nonTurn));
  ...
  assert.equal(pagePlanTurnDriverConsistency.applies_to(context([], { run_mode: "incremental", touched_files: [`stories/${STORY}/_source/pages/PG-1.yaml`] })), true);
  assert.equal(pagePlanTurnDriverConsistency.applies_to(context([], { run_mode: "incremental", touched_files: [`stories/${STORY}/pages-prose-plans/PG-1.md`] })), false);
});
