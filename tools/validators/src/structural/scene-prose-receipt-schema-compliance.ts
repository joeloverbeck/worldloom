...
export const sceneProseReceiptSchemaCompliance: Validator = {
  name: "scene_prose_receipt_schema_compliance",
  severity_mode: "fail",
  ...
  run: async (input: unknown, ctx: Context): Promise<Verdict[]> => {
    ...
    for (const receipt of receiptTargets(input, ctx)) {
      ...
      if (parsed === null) {
        verdicts.push({
          validator: "scene_prose_receipt_schema_compliance",
          severity: "fail",
          code: "scene_prose_receipt_schema_compliance.yaml_parse",
          message: `${receipt.node_id} scene prose receipt YAML could not be parsed`,
          location: {
  ...
  skip_reason: "no scene prose receipt files in scope"
};
...
function receiptTargetsFromWorldRoot(worldRoot: string, ctx: Context): ReceiptTarget[] {
  ...
  for (const storyEntry of readdirSync(storiesRoot, { withFileTypes: true })) {
    ...
    const receiptDir = path.join(storiesRoot, storyEntry.name, "scene-prose-receipts");
    if (!existsSync(receiptDir)) {
      continue;
    }
    for (const receiptEntry of readdirSync(receiptDir, { withFileTypes: true })) {
      if (!receiptEntry.isFile() || !/^SCN-(0|[1-9][0-9]*)\.yaml$/.test(receiptEntry.name)) {
        continue;
      }
      const relativePath = toPosixPath(path.join("stories", storyEntry.name, "scene-prose-receipts", receiptEntry.name));
      targets.push({
...
function schemaVerdicts(record: ReceiptTarget, errors: ErrorObject[]): Verdict[] {
  return errors.map((error) => ({
    validator: "scene_prose_receipt_schema_compliance",
    severity: "fail",
    code: `scene_prose_receipt_schema_compliance.${error.keyword}`,
    message: `${record.node_id} scene prose receipt schema violation at ${error.instancePath || "/"}: ${error.message ?? error.keyword}`,
    location: {
...
function nodeIdFromPath(filePath: string): string {
  ...
  return match ? `scene-prose-receipt:${match[1]}:${match[2]}` : toPosixPath(filePath);
}
...
function loadReceiptSchemaValidator(): ValidateFunction {
  ...
  const schema = JSON.parse(readFileSync(path.join(schemaRoot, "scene-prose-receipt.schema.json"), "utf8")) as AnySchema;
