...
const STATIC_RECORD_ID = /^(?:STENT|STCHAR|SF|BEL|SE|OBL|CNSQ|THR|SREL|STINT|STLOC|STOBJ|DA|STSTAT|CLK|STSEC|STQ|STPLAN|STEMO|SLT|CHC|BR|PG|CF|CHAR|ENT)-\d+$/;
...
function collectStaticRecordReferences(value: unknown, path: string, references: StoryReference[]): void {
  if (typeof value === "string") {
    ...
    for (const match of value.matchAll(/\b(?:STENT|STCHAR|SF|BEL|SE|OBL|CNSQ|THR|SREL|STINT|STLOC|STOBJ|DA|STSTAT|CLK|STSEC|STQ|STPLAN|STEMO|SLT|CHC|BR|PG|CF|CHAR|ENT)-\d+\b/g)) {
