...
const STORY_LOCAL_ID = /^(?:STENT|SF|BEL|SE|OBL|CNSQ|THR|SREL|STINT|STLOC|STOBJ|DA|STSTAT|CLK|STSEC|STQ|STPLAN|STEMO|SLT|CHC|BR|PG)-\d+$/;
const WORLD_SCOPE_ID = /^(?:CF|CHAR|ENT)-\d+$/;
const STATIC_RECORD_ID = /^(?:STENT|SF|BEL|SE|OBL|CNSQ|THR|SREL|STINT|STLOC|STOBJ|DA|STSTAT|CLK|STSEC|STQ|STPLAN|STEMO|SLT|CHC|BR|PG|CF|CHAR|ENT)-\d+$/;
...
function collectStaticRecordReferences(value: unknown, path: string, references: StoryReference[]): void {
  if (typeof value === "string") {
    ...
    for (const match of value.matchAll(/\b(?:STENT|SF|BEL|SE|OBL|CNSQ|THR|SREL|STINT|STLOC|STOBJ|DA|STSTAT|CLK|STSEC|STQ|STPLAN|STEMO|SLT|CHC|BR|PG|CF|CHAR|ENT)-\d+\b/g)) {
