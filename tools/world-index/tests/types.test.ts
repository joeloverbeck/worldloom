...
test("node and edge type registries match the spec counts and contain no duplicates", () => {
  ...
  assert.ok(nodeTypes.includes("pressure_event_batch"));
  assert.ok(nodeTypes.includes("world_proposal_card"));
  assert.ok(nodeTypes.includes("world_proposal_batch"));
  ...
  assert.ok(STORY_EDGE_TYPES.includes("scene_branch"));
  assert.ok(STORY_EDGE_TYPES.includes("scene_includes_page"));
  assert.ok(STORY_EDGE_TYPES.includes("scene_emitted_choice"));
  assert.ok(STORY_EDGE_TYPES.includes("scene_previous_scene"));
  ...
  assert.ok(STORY_EDGE_TYPES.includes("storylet_compatible_driver"));
  assert.ok(STORY_EDGE_TYPES.includes("storylet_predicate_pred"));
  assert.ok(STORY_EDGE_TYPES.includes("storylet_predicate_class"));
  assert.ok(STORY_EDGE_TYPES.includes("storylet_action_family"));
  ...
  assert.equal(edgeTypes.length, 98);
