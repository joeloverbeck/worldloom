...
After existing schema checks, call `readCurrentContext(root)`. If `!result.ok`, append a `blocking` finding with code `current-context-yaml-parse-failed` and the standard `path` / `message` / `repair_hint` fields. If `result.ok && result.value === null`, append no finding (absent is not an error). If `result.ok && result.value !== null`, store the parsed ctx for Pass 3 reuse.

### 2. Extend `runReferencePass` in `src/health/compute.ts`

For stories with a valid current-context payload (from Pass 2's stored result, OR re-read here), construct `KnownIds` via `listAllKnownIds(root)` + `metadata.segment_order`, call `validateCurrentContext(ctx, knownIds, segmentOrder)` (from 003). For each finding code, append an `error` HealthFinding under code `current-context-reference-broken` (or `current-context-pov-not-in-cast` for the cross-field invariant), populated with `path`, `message`, `repair_hint`.

### 3. New acceptance test (extend or add) at `test/current-context/health-integration.test.ts`

Inferred from §4 Files-to-touch's `health/compute.ts` modification + AC #11. Covers all four verification layers above using fixture stories with present-valid, present-corrupted, and present-broken-references variants under `test/current-context/fixtures/`.
