...
**Deps**: PROSESPLIT2-001 (canonical-source paths must exist), PROSESPLIT2-002 (skill-side refs migrated so new plans author from the new path), PROSESPLIT2-003 (FOUNDATIONS reference migrated), PROSESPLIT2-004 (legacy report deleted; only the new canonical files remain as authoritative source). All four must land first or the test points at a moving target.
