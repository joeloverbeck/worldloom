...
3. Cross-skill boundary under audit: MCP retrieval surface (`tools/world-mcp/`) ↔ world-index public API (`tools/world-index/`'s post-001 projection columns + new edge types). The new tool reads only the indexed projection surface; it does NOT reach into world-index parser internals (per the `tools/world-mcp` may not reach into `world-index/src/parse/` internals package-boundary rule).
