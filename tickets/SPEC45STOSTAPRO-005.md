...
3. Cross-skill / cross-package boundary under audit: this validator runs against the world-index database edges produced by SPEC45STOSTAPRO-002. The validator-to-indexer contract is the `edges(src, tgt, edge_type)` shape and the specific `edge_type` strings (`state_delta_create`, `state_delta_supersede`, `creation_evidence`). Validator must query the edges by exact edge_type string.
