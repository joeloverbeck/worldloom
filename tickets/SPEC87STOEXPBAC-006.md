...
1. Per-class summary extraction → vitest test (one assertion per class against a representative record body; asserts the summary line matches SPEC-87 §8 mapping)
2. Fallback chain step (1) → vitest test (record body with explicit `title` → summary uses title)
3. Fallback chain step (4) → vitest test (record body with no meaningful string field → summary is `"Untitled <CLASS> record"`)
4. No fabrication → vitest test (asserts no per-class summary returns text not present in the record body — every summary character traces to a body field or to the class-name fallback)
...
1. `cd tools/story-explorer && npm test -- record-card` — per-class summary extraction passes for all 22 indexed classes.
2. `cd tools/story-explorer && npm test -- deterministic-summaries` — fallback chain + no-fabrication invariant pass.
...
1. `cd tools/story-explorer && npm test -- record-card` (targeted)
2. `cd tools/story-explorer && npm test -- deterministic-summaries` (targeted)
