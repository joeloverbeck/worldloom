...
5. **No regression on SPEC-45 edges** → existing edges (`state_delta_create`, `state_delta_supersede`, `creation_evidence` + the other 11 pre-Phase-C edges) continue to emit unchanged from the fixture.
...
Mirror the SPEC-45 integration test pattern at `tools/world-index/tests/integration/spec45-atomic-integration.test.ts`:
- Fixture-world copy via `cpSync` to a temp root (so the real `worlds/<slug>/` tree is untouched).
- Build the index via `build` from `../../src/commands/build.js`.
- Open the resulting `world.db` via better-sqlite3.
- For each of the 22 new edge types, query the edges table and assert the expected row count matches the fixture's source-field count (computed at test start, not hardcoded).
- For each edge type with empty source fields in some fixture records, assert no edges of that type emit for those records (T-8 negative half).
- Assert every new edge row's `story_slug` matches the fixture's story slug (FOUNDATIONS §Rule 4 bundle-isolation discipline).
- Assert the pre-Phase-C edges (`state_delta_create`, `state_delta_supersede`, `creation_evidence`, and the 11 other pre-Phase-C edges) still emit unchanged from the fixture (T-9 cross-phase no-regression sub-check).

### 3. Build the fixture story-bundle

In `tools/world-index/tests/fixtures/` (or a new sub-directory following the SPEC-45 fixture pattern at `tests/helpers/atomic-fixture.ts`), construct a story-bundle fixture that exercises records of every Phase C class with populated and empty source fields per edge type. Include:
...
5. Pre-Phase-C edge preservation: the 14 pre-Phase-C edges still emit unchanged from the fixture.
6. `npm test --prefix tools/world-index` passes for the full world-index test suite including the new integration test.
7. `npm run build --prefix tools/world-index` typechecks cleanly.
...
2. Per-edge expected counts in the integration test are computed from the fixture's source-field state at test start; hardcoded counts are forbidden because they become stale as the fixture grows (spec §Spec-Integration Ticket Shape discipline).
3. The fixture-world copy is rooted in a temp directory via `cpSync` so the real `worlds/<slug>/` tree is untouched — parallels SPEC-45's integration test discipline.

## Test Plan

### New/Modified Tests

1. `tools/world-index/tests/types.test.ts` — modify or extend with registry-completeness assertion per T-6.
2. `tools/world-index/tests/integration/spec46-story-bundle-edges-integration.test.ts` — new capstone integration test following SPEC-45 fixture pattern; exercises every Phase C edge type with populated and empty source fields; asserts T-6 + T-8 + bundle isolation + pre-Phase-C edge preservation.

### Commands

1. `npm test --prefix tools/world-index` (targeted: full world-index test suite passes including the new T-6 registry assertion AND the new integration test)
2. `npm run build --prefix tools/world-index` (typechecks the test file)
3. `node tools/world-index/dist/src/cli.js build <fixture-world-path>` (manual smoke: confirm the CLI build command still works against an out-of-test fixture; the integration test invokes `build` programmatically, but the CLI surface is the operator-facing entry point)
