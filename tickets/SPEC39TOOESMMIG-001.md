...
**Deps**: `archive/tickets/SPEC39TOOESMMIG-004.md`, `tickets/SPEC39TOOESMMIG-002.md` (downstream TypeScript consumers must be ESM before they can compile against an ESM patch-engine package)

## Problem

`tools/patch-engine` is already authored for the modern Node16/Node16 module form (tsconfig uses `module: "Node16"` + `moduleResolution: "Node16"`; all relative imports already use the `.js` suffix discipline). The only remaining patch-engine-local gap is the `package.json` `"type": "commonjs"` declaration, which tells Node to emit and load the compiled JavaScript as CommonJS. Flipping it to `"type": "module"` aligns patch-engine with the SPEC-39 end-state (all five `tools/` packages on ESM), but it is not independently landable ahead of its TypeScript consumers: a live proof run showed `tools/validators` cannot compile as CommonJS while importing an ESM patch-engine package under Node16 TypeScript semantics.

Patch-engine is the cleanest entry point for the SPEC-39 batch because it has zero `__dirname`/`__filename` usage (verified via grep across `src/`) and zero extensionless relative imports — the entire migration surface is a single `package.json` flip plus a clean rebuild.

## Assumption Reassessment (2026-05-17)

1. **Codebase**: `tools/patch-engine/package.json` currently declares `"type": "commonjs"` and `tools/patch-engine/tsconfig.json` already uses `module: "Node16"` + `moduleResolution: "Node16"` — verified via `jq` and direct read. All relative imports in `tools/patch-engine/src/**/*.ts` and `tools/patch-engine/tests/**/*.ts` use the `.js` suffix discipline (verified: 0 extensionless relative imports). No `__dirname` / `__filename` / `require(` usage in `src/` (verified via grep). The migration surface is genuinely a single field change.
...
1. **Build produces no deprecation warnings or interop errors** → codebase grep-proof: `npm --prefix tools/patch-engine run build 2>&1 | grep -E "deprecat|TS1479|TS5107"` returns empty.
...
Change the `"type"` field from `"commonjs"` to `"module"`. No other fields in `package.json` change.

### 2. Rebuild and re-verify

Run `npm --prefix tools/patch-engine run clean && npm --prefix tools/patch-engine run build` to regenerate `dist/` under the new emission target. Then `npm --prefix tools/patch-engine test` (plus `test:compile-reject` and `test:integration`) to confirm all tests pass.

### 3. Downstream consumer smoke

Rebuild `tools/validators` and `tools/world-mcp` against the new patch-engine `dist/` (`npm --prefix tools/validators run build`; `npm --prefix tools/world-mcp run build`) to confirm the ESM emission doesn't break the file-dependency import chain.
