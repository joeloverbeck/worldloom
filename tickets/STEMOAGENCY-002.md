...
**Priority**: MEDIUM
**Effort**: Medium
**Engine Changes**: None — story-bundle content repair through existing patch-engine / turn-cycle workflow.
**Deps**: `archive/tickets/STEMOAGENCY-001.md`
...
3. The repair owner is a story-bundle restamp/turn-cycle cleanup for `worlds/erotica-world/stories/red-bunny`, not a direct source edit from this ticket.
...
2. Engine discipline preserved -> repair is performed through the existing patch-engine / turn-cycle workflow, not direct `_source` edits.

## What to Change

### 1. Restamp the affected story-bundle state

Use the approved story workflow to repair red-bunny `STEMO-3` and `STEMO-4` so each constraining affect has matching downstream grounding, or no longer claims `agency_effect: constraining`.

## Files to Touch

- `worlds/erotica-world/stories/red-bunny/_source/**` (engine-routed modify/create as determined by the repair plan)
- `worlds/erotica-world/stories/red-bunny/INDEX.md` (modify if the story workflow requires an index update)
