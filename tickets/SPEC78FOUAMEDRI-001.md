...
1. The §5c "Driver salience is local." paragraph exists at the end of §Story Bundles §5c, after the existing "No global drama manager." paragraph → **codebase grep-proof**: `grep -B1 -A4 "Driver salience is local" docs/FOUNDATIONS.md` returns the new paragraph within §5c with one line of leading context.
...
Edit `docs/FOUNDATIONS.md` to insert a new paragraph at the end of §Story Bundles §5c, AFTER the existing "No global drama manager." paragraph that closes with "The architecture already embodies this; the principle exists to keep it from drifting." Insert the following verbatim (from SPEC-78 §3.1):
...
Edit `docs/FOUNDATIONS.md` to insert a new middle paragraph in §Story Bundles §6b, AFTER the existing opening paragraph (which ends with "...gives that actor an access route to the load-bearing information.") and BEFORE the existing closing paragraph (which begins with "This firewall governs move and choice generation."). Insert the following verbatim (from SPEC-78 §3.2):
...
Per SPEC-78 §8 Slice A step 3, the SPEC-76 §4 Out of Scope bullet was rewritten to point at SPEC-78 during the source brainstorm reversal flow (2026-05-23). This ticket does NOT re-apply that edit; it verifies the bullet's presence as part of acceptance:

```
grep -c "Carried separately by \[SPEC-78\]" specs/SPEC-76-turn-driver-primitive-and-pressure-driven-turn-cycle.md
```

Expected output: `1`. If the count differs, surface as an Issue before landing the FOUNDATIONS edits — the pre-applied state may have drifted between the brainstorm reversal session (2026-05-23) and this ticket's implementation.

## Files to Touch

- `docs/FOUNDATIONS.md` (modify) — insert two new paragraphs per §What to Change items 1 + 2.
...
- **`git commit`.** The ticket lands edits in the working tree; the user reviews the diff and commits.

## Acceptance Criteria

### Tests That Must Pass

1. `grep -B1 -A4 "Driver salience is local" docs/FOUNDATIONS.md` returns the new §5c paragraph in full, with one line of leading context confirming placement within §Story Bundles §5c (the leading context line should be the closing sentence of the "No global drama manager." paragraph: "The architecture already embodies this; the principle exists to keep it from drifting.").
...
1. `grep -B1 -A4 "Driver salience is local" docs/FOUNDATIONS.md` — verifies the §5c extension landed within §Story Bundles §5c with correct leading context.
