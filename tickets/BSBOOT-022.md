...
BSBOOT-011 made `branching-story-bootstrap` compute the bootstrap seed `target_pool_size` from `intended_scale` + state complexity, with `storylet_pool_seed_size` as an explicit override. That changed the bootstrap caller contract, but `storylet-pool-authoring` still contains stale downstream wording:

- `.claude/skills/storylet-pool-authoring/SKILL.md:3` describes "`seed` mode (~20 storylets, invoked by branching-story-bootstrap Phase 6 as a no-write sub-routine)".
- `.claude/skills/storylet-pool-authoring/SKILL.md:19` says `target_pool_size` defaults to "~20 in seed mode".
- `.claude/skills/storylet-pool-authoring/SKILL.md:195` says the seed-mode default is calibrated for fresh story bundles, explicitly naming the `parent_skill_invocation: true` bootstrap sub-routine path.

That wording now conflates two different callers:
...
Leaving the stale wording in place would make operators think bootstrap still asks `storylet-pool-authoring` for a fixed 20-ish seed pool even though the bootstrap-side formula has landed.
...
2. `.claude/skills/storylet-pool-authoring/SKILL.md:3,19,195` still carries fixed `~20` bootstrap-seed/default wording. The stale surface is prose only; no code, schema, validator, or engine op consumes the phrase.
...
- In the skill description, replace "`seed` mode (~20 storylets, invoked by branching-story-bootstrap Phase 6 as a no-write sub-routine)" with wording that distinguishes:
...
- In the `target_pool_size` argument doc, replace "Default: ~20 in seed mode" with wording that says:
...
- In Inputs §`target_pool_size`, remove the statement that the seed-mode default is calibrated for fresh bootstrap bundles. Preserve the existing direct seed/top-up rules for existing bundles, and add a bootstrap-parent sentence pointing to `branching-story-bootstrap/references/phase-6-storylet-pool-seed.md` §Computing target_pool_size.
