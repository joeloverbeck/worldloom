...
3. Cross-skill boundary under audit: the capstone test exercises the pipeline end-to-end — patch engine (commit STPLAN/STEMO records via tickets 003/004 surfaces) + validators (tickets 005/006/007 + 008/009) + MCP retrieval (ticket 011) + world-index edges (ticket 013) + page-plan rendering (ticket 015) + skill prose (ticket 016) + Hook 3 (no-code-change verification per D-A9). Per the §Spec-Integration Ticket Shape: "parallel-branch leaf set" applies — the upstream DAG has parallel branches (skill prose ticket 016 reaches 005/006/007/008/009/011/013/015 transitively but NOT 002/004/010/012/014 which are parallel docs/contract surfaces). Capstone `Deps` enumerates the leaf set: 002 (contract inventory + FOUNDATIONS §6), 004 (patch-engine wiring), 010 (contract docs §5+§5a), 012 (capability + CONTEXT-PACKET-CONTRACT), 014 (MACHINE-FACING-LAYER docs), 016 (skill prose) — collectively transitively cover all 16 prior tickets.
