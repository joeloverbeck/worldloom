...
- Remove the `inactive_default` branch and its member from the `DeleteResult` union. Default path: unreferenced → `hard_deleted` (unlink); referenced → a new `blocked` outcome carrying `referrers: Array<{recordClass, summary}>` from the archived SPEC114MANSTOSTU-001 `resolveReferrerSummaries` implementation. Never write `active:false` / `retired_reason` on delete.
