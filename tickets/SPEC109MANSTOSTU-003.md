...
Implement `validateCurrentContext(ctx: CurrentContext, knownIds: KnownIds, knownSegmentIds: string[]): ValidationResult`:
...
### 2. New acceptance test at `test/current-context/current-context-validate.test.ts`

Cover each finding-code case + positive control. Use in-memory `KnownIds` + `knownSegmentIds` constructed by the test (no fixture-disk-IO required).
