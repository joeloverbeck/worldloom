...
- The backend payload route — SPEC117MANSTOSTU-003.
...
1. `None for new test files — web verification is `tsc --noEmit` typecheck (`npm --prefix web test`) plus the SPEC-117 §7 manual two-pane smoke check; the broad-scan payload behavior is covered by SPEC117MANSTOSTU-003's `post-segment-workbench.test.ts`.`
