...
2. SPEC-84 §4.1 specifies the topology exactly: 2 branches (BR-1: root → PG-1 → PG-3 → PG-5; BR-2: root → PG-1 → PG-2 → PG-4), 5 SLTs (SLT-1/SLT-2 with `scope.visibility: global_author_pool`; SLT-3 with `scope.visibility: global_author_pool` plus hard precondition `{ pred: plan_active, plan_id: STPLAN-99 }`; SLT-4 with `scope.visibility: branch_scoped` and `scope.branch_id: BR-2`; SLT-5 with `scope.visibility: branch_prefix_scoped` and `scope.branch_path_prefix: [PG-1, PG-3]`). Per spec §4.1 SLT-2 paragraph, the live-pool semantics is structural and no per-SLT creation-index wiring is load-bearing for the test; an optional `created_at_page: PG-4` may document author intent without affecting retrieval (only `SE.created_at_page` is read by the cooldown-tracking code at `tools/world-mcp/src/tools/select-storylet-candidates.ts` line 443, not `SLT.created_at_page`). The SLT schema's required-field set is authoritative per `tools/validators/src/schemas/story-storylet.schema.json` (`scope`, `provenance`, `preconditions`, `effects`, plus the title/move_family/saliency/mystery_policy fields).
...
Create `tools/validators/tests/fixtures/spec84-replay-and-branch-scope/` as a new directory under the validators package's test fixtures root, parallel to the existing `tools/validators/tests/fixtures/red-kiln-ambush/` directory.

### 2. Author README.md documenting topology

Author `tools/validators/tests/fixtures/spec84-replay-and-branch-scope/README.md` covering:
...
- **SLT inventory** with scope + expected behavior per case:
  ...
  - SLT-5: `scope.visibility: branch_prefix_scoped` with `scope.branch_path_prefix: [PG-1, PG-3]`. Proves prefix-match positive from BR-1 fork at PG-5 (whose `branch_path` is `[PG-1, PG-3, PG-5]` — prefix matches) AND wrong-prefix negative from BR-2 fork at PG-4 (whose `branch_path` is `[PG-1, PG-2, PG-4]` — prefix does not match).
...
### 3. Author fixture.json

Author `tools/validators/tests/fixtures/spec84-replay-and-branch-scope/fixture.json` as a single JSON file with this top-level shape:
...
- `records[]` array containing:
  ...
  - **Two BR records** (BR-1, BR-2) with their `branch_path` arrays declaring their ancestor chains.
  ...
  - **Five SLT records** per the inventory above, each with full required schema fields per `tools/validators/src/schemas/story-storylet.schema.json`:
    - `id`, `title`, `move_family`, `scope` (with `visibility` + conditional `branch_id` / `branch_path_prefix`), `preconditions` (`hard` array; `soft` array; empty for SLTs 1/2/4/5), `effects` (`create` / `supersede` / `close` arrays; minimum viable for fixture parse), `provenance` (`origin`), `saliency` (`urgency` + `cooldown_pages`), `mystery_policy` (`allowed_authority`).
...
Target size ~300-400 lines total JSON.
