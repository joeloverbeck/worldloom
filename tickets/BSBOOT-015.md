...
**Deps**: archive/tickets/BSBOOT-003.md (relies on the renamed STINT field for the `stent_id` discipline check), BSBOOT-009 (relies on the conditional CNSQ discipline so the validator does not require unused records), BSBOOT-010 (relies on the new SF visibility default + basis enum), BSBOOT-013 (relies on the new CHC `continuation_capacity` block).
