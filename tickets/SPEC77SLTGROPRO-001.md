...
**Priority**: HIGH
**Effort**: Large
**Engine Changes**: Yes — extends `tools/validators/src/schemas/story-storylet.schema.json`; adds new utility module `tools/validators/src/structural/slt-grounding-utils.ts`; amends shared-record-schemas contract at `_shared-templates/story-record-schemas.md` §4.4; migrates inline-fixture builders across `tools/validators/tests/{integration,structural,cli,rules,fixtures}/`
**Deps**: None

## Problem

The `SLT` (commitment block) JSON Schema at `tools/validators/src/schemas/story-storylet.schema.json` accepts schema-valid blocks whose presence is justified only by *"dramatic variety"* or similar generic placeholders, which `docs/FOUNDATIONS.md` §Story Bundles §5a forbids semantically (*"a bad block says: advance Act II"*) but not yet structurally. The schema also has no driver-kind compatibility surface, so storylets cannot declare which `SE.turn_driver.kind` values they can serve at selection time (SPEC-76 landed the closed `turn_driver.kind` enum but storylets have no field to reference it).
...
4. **Inline-fixture builders supply `grounding` at every SLT-construction site** → codebase grep-proof: `grep -rln 'record_kind.*storylet_record\|create_slt_record' tools/validators/tests/` returns the migrated test files (12 sites + 2 JSON fixtures), and every site populates `grounding` (verified by post-migration test run passing `npm test`).
...
Append `grounding` to the root `required` array (after `provenance`). Append a `grounding` property to `properties` with this shape:
...
New file exporting the closed banned-phrase list as a frozen constant array so the Slice B validator and its test suite both consume the same source of truth:
...
Append two sub-paths to the SLT field list (at the location currently ending with `provenance.origin`):
...
Add a change note: `# Added in SPEC-77: grounding object (compatible_turn_drivers + reason_to_exist). 5 dropped grounding fields documented in spec §4 Out of Scope.`

### 4. Schema-level negative tests — `tools/validators/tests/schemas/`

New test file (e.g., `story-storylet-grounding.test.ts`) covering SPEC-77 §6.1's five negative cases:
...
Add minimal `grounding` to every SLT-construction site enumerated at SPEC-77 §6.3:
...
- `structural/state-delta-class-integrity.test.ts:51`
...
Where inline-fixture-builder helpers exist (e.g., `storyRecord("storylet_record", ...)` calls in tests that wrap a shared helper), prefer updating the helper to supply default `grounding` automatically; per-test overrides target only tests whose assertions depend on `grounding` shape (e.g., the new schema-tests at §6.1 above explicitly override `grounding` to inject negative cases).
...
7. `tools/validators/tests/structural/state-delta-class-integrity.test.ts` — modified to populate `grounding` in the SLT helper call at line ~51.
8. `tools/validators/tests/structural/branch-isolation.test.ts` — modified to populate `grounding` in the `storyRecord("storylet_record", …)` helper call at line ~208.
9. `tools/validators/tests/structural/recursive-reference-closure.test.ts` — modified to populate `grounding` in the SLT helper call at line ~113.
10. `tools/validators/tests/structural/record-schema-compliance.test.ts` — modified to populate `grounding` in the SLT helper call at line ~904.
11. `tools/validators/tests/rules/rule_storylet_predicate_dsl_parsability.test.ts` — modified to populate `grounding` in the SLT helper call at line ~399.
12. `tools/validators/tests/fixtures/patch-plan-complete-slt.json` — modified to populate `grounding` in the SLT record.
13. `tools/validators/tests/fixtures/patch-plan-missing-mystery-safety-slt.json` — modified to populate `grounding` in the SLT record.
