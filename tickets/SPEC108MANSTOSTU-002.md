...
5. (was template item 7 — route behavior change blast radius): the legacy `PUT /api/.../segments/:segmentId` and `DELETE /api/.../segments/:segmentId` routes previously accepted requests unconditionally; they now return 405 without `?mode=repair`. Pipeline-wide grep confirms the only consumers of these routes are inside `tools/manual-story-studio/` (the frontend `web/src/api/segments.ts` and the existing tests under `test/server/segments-routes.test.ts`); both consumers are updated by ticket 003 and ticket 008 respectively. No external CI / scripts / sibling tools call these routes (verified by reassessment §8 Assumption #1).
...
2. `cd tools/manual-story-studio && npm test` continues to pass after ticket 008 lands (route tests cover the mode-gate behavior; function-level tests at `test/write/segments.test.ts` remain unchanged).
...
1. Test coverage for the changes in this ticket lands in ticket 008 (new `test/segments/segment-lifecycle.test.ts` plus updates to `test/server/segments-routes.test.ts`). This ticket's edits cause type-check failures in the existing tests (which call the routes without `?mode=repair` and expect 200); those failures resolve when ticket 008 lands. Land tickets 002 and 008 together (or 002 first, 008 immediately after) to avoid an intermediate breaking-tests state.

### Commands

1. `cd tools/manual-story-studio && npm run build:backend` — typecheck for the new field + error class.
2. `cd tools/manual-story-studio && npm test` — full test suite after ticket 008 lands.
