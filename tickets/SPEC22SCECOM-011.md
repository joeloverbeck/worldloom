...
SPEC-22 §Track 4's branching-story-health-audit deliverable extends the SAU report with 3 new metric sections (Choice Cadence, Arc Conformance, Commitment-Class Coverage), extends the audit_focus enum with 3 new values, extends Pre-flight to whole-class load ARC_TRACE records via `mcp__worldloom__list_records`, extends the Phase 4 recursive reference closure walk to ARC_TRACE references, extends Phase 7 self-check structural floors with ARC_TRACE evidence-alignment and envelope-violation severity, extends `choice_pair_distance` for v2 strong-axis collective difference, extends `choice_continuation_capacity` for v2 CHC → arc references, and extends the RSP card schema with `target_commitment_class` + `target_arc_archetype`.
...
1. `audit_focus` enum has 25 values (22 existing + 3 new) → grep audit_focus enum in SKILL.md.
...
4. Phase 4 recursive walk extends to ARC_TRACE references → grep `ARC_TRACE.created_at_page`, `ARC_TRACE.arc_realized`, `ARC_TRACE.observed_actions[].actor`, `ARC_TRACE.effect_evidence[].effect_ref`, `ARC_TRACE.stop_condition_hit.id` in SKILL.md Phase 4.
...
In SKILL.md frontmatter argument enum: add `arc_conformance`, `choice_cadence`, `commitment_class_coverage`. Each follows existing focus-narrowing semantics.

### 2. Pre-flight World-State Prerequisites: ARC_TRACE whole-class load

In SKILL.md Pre-flight section: additionally load all ARC_TRACE records for audited branches via `mcp__worldloom__list_records(world_slug, record_type='arc_trace_record', story_slug=<story_slug>, include_full_body=true)`. Whole-class load bounded by `story_slug`; `include_full_body=true` required for Phase 3's arc-conformance sub-check.
...
In `templates/remediation-storylet-proposal-card.md`: add header-block fields:
...
In SKILL.md SAU report section (or `templates/story-audit-report.md` if shape lives there): add `Choice Cadence`, `Arc Conformance`, `Commitment-Class Coverage` sections per the metrics described in §3 above.
...
- `.claude/skills/branching-story-health-audit/templates/story-audit-report.md` (modify if SAU shape lives here)
...
1. `audit_focus` enum has 25 values (22 existing + `arc_conformance` + `choice_cadence` + `commitment_class_coverage`) → grep enum in SKILL.md.
...
4. Phase 4 recursive walk lists 5 new ARC_TRACE reference checks (created_at_page / arc_realized / observed_actions.actor / effect_evidence.effect_ref / stop_condition_hit.id).
...
3. `grep -nE 'target_commitment_class|target_arc_archetype' .claude/skills/branching-story-health-audit/templates/`
