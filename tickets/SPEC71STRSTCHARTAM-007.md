...
**Priority**: MEDIUM
**Effort**: Large
**Engine Changes**: Yes — `branching-story-bootstrap`, `story-character-profile`, `branching-story-turn-cycle`, `branching-story-prose-attach`, `branching-story-health-audit` SKILL.md prose.
**Deps**: None

## Problem

The five story-pipeline skills still instruct the LLM to compute and stamp the four hashes (SPEC-71 §1.3 skill rows). This ticket removes those instructions so the skills no longer author the hashes: bootstrap/story-character-profile stop stamping STCHAR frontmatter hashes; bootstrap/turn-cycle stop emitting the §16a `Hashes:` line; prose-attach stops the §16a `stchar_authority` hash verdicts + the `compute-stchar-hashes` invocation; health-audit drops the `source_drift` mode and the `page_plan_stchar_hash_mismatch` finding. Prose-only; coherent with the contract docs (005).
...
1. No SKILL.md instructs `compute-stchar-hashes` or stamping `profile_hash`/`voice_block_hash`/`page_packet_hash` → grep-proof across the five skills.
...
Remove the STCHAR-phase `profile_hash`/`voice_block_hash`/`page_packet_hash` compute+stamp instructions; remove the §16a `Hashes:` authoring line (bootstrap).

### 2. turn-cycle
Phase 7 §16a authoring: remove the `Hashes:` line. Leave Phase 9 `plan_hash`/`state_hash` stamping intact (Job A).

### 3. prose-attach
Phase 3 `stchar_authority`: remove the profile/voice/`page_packet_hash` expected-vs-observed verdicts and the `compute-stchar-hashes` invocation; retain `packet_present`/`active_in_snapshot`/`required_because`.

### 4. health-audit
Remove the `source_drift` mode (the `mode` enum value across description + args + Phase 2n `:363` + the `stchar_source_drift` finding `:369`) and the `page_plan_stchar_hash_mismatch` finding from Phase 2m (`:356`); keep the rest of Phase 2m STCHAR authority health.
...
1. `None — skill-prose ticket; verification is grep-proof across the five SKILL.md files + manual review of prose-attach Phase 3 and health-audit mode list.`

### Commands

1. `grep -rnE "compute-stchar-hashes|profile_hash|voice_block_hash|page_packet_hash|source_char_hash" .claude/skills/branching-story-{bootstrap,turn-cycle,prose-attach,health-audit}/SKILL.md .claude/skills/story-character-profile/SKILL.md`
