...
2. **Cross-skill / cross-artifact boundary under audit**: this ticket is the create-base-world consumer-side of SPEC09CANSAFEXP-002's structural-schema enforcement. create-base-world emits `create_cf_record` ops via the same patch engine that canon-addition uses (per CLAUDE.md §Machine-facing layer integration); the same conditional-presence enforcement applies. Skill prose at create-base-world must guide the user to populate the new blocks at genesis, parallel to canon-addition Phase 13a (delivered by SPEC09CANSAFEXP-004).
...
2. Reusing the SPEC09CANSAFEXP-002 helpers (`requiresEpistemicProfile`, `requiresExceptionGovernance`) avoids duplicating the type-taxonomy in skill prose. Single source of truth for the conditional decision.
...
4. Patch-engine pre-apply validator (delivered by SPEC09CANSAFEXP-002) accepts the genesis CF-0001 with the new blocks — schema validation via the structural validator.
...
- Schema extension (delivered by SPEC09CANSAFEXP-002; this ticket consumes the schema)
- Validator implementation (delivered by SPEC09CANSAFEXP-003)
- canon-addition Phase 13a updates (delivered by SPEC09CANSAFEXP-004; parallel mechanism for non-genesis CFs)
...
2. `grep -n "requiresEpistemicProfile\|requiresExceptionGovernance" .claude/skills/create-base-world/SKILL.md` returns ≥1 match referencing the SPEC09CANSAFEXP-002 helpers.
...
4. n_a-form rationale at genesis must contain a fact-type keyword from FOUNDATIONS §Ontology Categories (enforced by SPEC09CANSAFEXP-002's structural validator).
