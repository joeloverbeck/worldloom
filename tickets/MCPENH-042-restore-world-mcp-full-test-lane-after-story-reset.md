...
**Priority**: MEDIUM
**Effort**: Medium
**Engine Changes**: Yes - `tools/world-mcp` tests and story-bundle record-schema surfaces; possible matching updates in `tools/world-mcp` handlers if live code still exposes retired ARC_TRACE vocabulary.
**Deps**: `archive/tickets/MCPENH-041-rename-legacy-story-pipeline-task-types.md`

## Problem

`cd tools/world-mcp && npm test` currently builds, then fails 6 tests after the greenfield story-family reset. MCPENH-041 intentionally used a focused proof lane for the task-type rename, but the package-wide lane remains a broken handoff surface.

The failures are not caused by the task-type rename. They are stale package tests and schema surfaces still straddling the retired SPEC-22 ARC_TRACE/page-cycle family and the new BEL/no-ARC_TRACE story contract.
...
6. `tools/world-mcp/tests/tools/validate-patch-plan.test.ts` expects `validatePatchPlan` to accept `create_bel_record` through pre-apply validation, but the current result is `fail` instead of `pass`.
...
8. This is a separate package-maintenance bug exposed by the MCPENH-041 review. It should not block the archived task-type rename, but it should be fixed before future tickets treat `tools/world-mcp && npm test` as a clean broad proof lane.
...
3. Patch-plan BEL pre-apply behavior current -> focused `validate-patch-plan` test proves `create_bel_record` produces `pass` when the generated plan is valid, or the test is corrected to the intended failure with concrete validator evidence.
...
### 1. Retire or rewrite stale SPEC-22 capstone tests

Update `tools/world-mcp/tests/integration/spec22-capstone.test.ts` so it no longer reads deleted legacy skill files or asserts ARC_TRACE ingestion as current behavior. Preserve only still-current invariants, such as rejection of retired `create_arc_trace_record`, or replace the file with greenfield story-contract coverage.

### 2. Align record-schema support with BEL/no-ARC_TRACE

Update `tools/world-mcp/src/tools/get-record-schema.ts`, `tools/world-mcp/tests/tools/get-record-schema.test.ts`, and related public docs or server descriptions so current story-bundle schemas expose `belief_record` / `story-belief.schema.json` and no longer advertise missing `arc_trace_node` support unless the code intentionally returns an unsupported-node error.

### 3. Diagnose the BEL pre-apply failure

Fix the `validatePatchPlan accepts create_bel_record through pre-apply validation` test or implementation after identifying the actual validator verdict that changes the status to `fail`. Prefer correcting the fixture if it is under-specified; change implementation only if the live BEL plan is valid and the handler is wrongly rejecting it.
...
- `tools/world-mcp/src/server.ts` (modify if public tool descriptions mention retired ARC_TRACE schema support)
- `tools/world-mcp/README.md` and `docs/MACHINE-FACING-LAYER.md` (modify if public schema vocabulary docs require sync)
...
2. `cd tools/world-mcp && node --test dist/tests/tools/get-record-schema.test.js dist/tests/tools/validate-patch-plan.test.js` passes.
3. `rg -n 'story-arc-trace.schema.json|arc_trace_node|branching-story-page-cycle|branching-story-bootstrap/references/phase-9-validation-gates.md' tools/world-mcp/src tools/world-mcp/tests tools/world-mcp/README.md docs/MACHINE-FACING-LAYER.md` returns no current-contract stale hits, except intentional negative/rejection coverage that names retired ARC_TRACE behavior.
...
3. `tools/world-mcp/tests/tools/validate-patch-plan.test.ts` - prove valid `create_bel_record` pre-apply validation or document the corrected expected failure with exact validator evidence.

### Commands

1. `cd tools/world-mcp && npm run build`
2. `cd tools/world-mcp && node --test dist/tests/tools/get-record-schema.test.js dist/tests/tools/validate-patch-plan.test.js`
