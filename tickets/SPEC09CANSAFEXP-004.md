...
6. **Schema extension consumer**: canon-addition Phase 13a populates the new blocks. Use the exported `requiresEpistemicProfile` / `requiresExceptionGovernance` helpers from `tools/validators/src/structural/record-schema-compliance.ts` (per SPEC09CANSAFEXP-002 export plan) to drive the conditional-presence decision at skill prose level. If a block doesn't apply, emit the n_a-with-fact-type-rationale form; if ambiguous, surface the ambiguity to the user rather than defaulting to n_a.
...
- Structural-schema enforcement of block presence (delivered by SPEC09CANSAFEXP-002)
