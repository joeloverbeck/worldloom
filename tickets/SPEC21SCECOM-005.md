...
**Deps**: `archive/tickets/SPEC21SCECOM-001.md` (archetype excerpts fed to the LLM prompt); SPEC21SCECOM-004 (arc seeds consumed as Phase 3 input)
...
1. The current file at `.claude/skills/storylet-pool-authoring/references/phase-3-structured-drafting.md` (verified during SPEC-21 reassessment 2026-05-08) describes v1 envelope filling. The rewrite per SPEC-21 §C replaces the LLM prompt assembly to consume the 11-field arc seed (from SPEC21SCECOM-004), produce all seven v2 structural blocks, and let the engine wrap with schema scaffolding + DSL parsability + visibility-scope assignment.
