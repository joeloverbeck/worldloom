...
Post-review of FOUNDATIONS-003 confirmed the skill-facing notation cleanup is otherwise complete, but the same FOUNDATIONS-002 unpadded natural-integer ID contract still has stale examples in non-skill active documentation surfaces:

- `docs/HARD-GATE-DISCIPLINE.md` still says `pages-prose-receipts/PG-NNNN.yaml`.
- `tools/world-mcp/README.md` still uses concrete padded retrieval examples such as `SEC-ELF-001` and `CF-0042`.
- `docs/CONTEXT-PACKET-CONTRACT.md` still carries padded or placeholder examples such as `CF-0033`, `CF-0044`, and `M-NNNN`.
...
2. `tickets/FOUNDATIONS-003.md` refreshed skill-facing notation and intentionally scoped out package/runtime code and non-skill documentation; its archival remains blocked until a stale active-ticket-path reference in a touched skill is corrected under implementation authority.
...
Replace padded or fixed-width placeholder record IDs in current docs with `<integer>` notation or concrete unpadded examples, preserving meaning.

### 2. Package README example cleanup

Update `tools/world-mcp/README.md` retrieval examples so they demonstrate the current unpadded ID convention.
