...
1. `tools/world-index/src/parse/entities.ts` currently derives `aliasTexts` only from the `slug` field on `character_record` and `character_proposal_card`, and never from explicit alias declarations.
2. Live `animalia` already contains a concrete example where an obvious exact alternate exists in author intent: proposal `NCP-0006` has `title: "Althea Greystone — ..."` while the canonical `name` is `Canon Althea Greystone`. The current system intentionally refuses to infer `Althea Greystone`, but it also provides no explicit declaration path.
3. Shared boundary under audit: exact alias declarations on whole-file authority-bearing records, plus the downstream `entity_aliases` and `mentions_entity` behavior that should consume them.
4. `docs/FOUNDATIONS.md` `Core Principle` and `Tooling Recommendation` support explicit structured declarations; they do not support heuristic title stripping.
5. This ticket must stay exact-only. If a world wants an alternate, it must declare it.
...
1. Declared exact alternates become `entity_aliases` rows. -> unit test coverage.
2. Declared aliases resolve exact mentions to the canonical entity without creating a second canonical row. -> entity-extractor and integration tests.
...
Merge explicit `aliases: []` entries with existing structured-field-derived aliases (currently only `slug` on character/proposal records) and deduplicate by **normalized alias text** — apply `normalizeSurface` then `normalizeLookupKey` per `tools/world-index/src/parse/entities.ts:635-641` before dedup comparison. The merged result carries `alias_kind='exact_structured'`.
...
- `tools/world-index/tests/integration/build-animalia.test.ts` if a live fixture path is added (modify)
...
4. A fixture whose `aliases: []` contains an entry identical (after `normalizeSurface`/`normalizeLookupKey`) to the record's `slug` produces exactly one `exact_structured` alias row, not two.
5. A fixture whose `aliases: []` contains an entry identical (after normalization) to `canonical_name` produces zero additional alias rows (existing behavior preserved).
4. `cd tools/world-index && npm run build`
5. `cd tools/world-index && node --test dist/tests/entities.test.js`
...
1. `tools/world-index/tests/entities.test.ts` — declared exact alias coverage and negative undeclared-alternate coverage.
