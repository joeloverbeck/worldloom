# FOUNDATIONS-006: §Canonical Storage Layer §Read discipline hybrid-record enumeration omits `STCHAR` and `story_character_authority_record`
...
`docs/FOUNDATIONS.md` §Canonical Storage Layer §Read discipline at line 584 enumerates hybrid records as `(CHAR-<integer>, DA-<integer>, PA-<integer>, NCP-<integer>, NCB-<integer>)` retrievable via `get_record(record_id)` with optional `section_path` projection, and lists the whole-class enumeration `list_records(record_type='character_record'|'diegetic_artifact_record'|'adjudication_record'|'character_proposal_card'|'character_proposal_batch')`. Both enumerations omit `STCHAR-<integer>` / `story_character_authority_record`, even though SPEC-56 added STCHAR as a hybrid story-bundle authority artifact (shared contract `_shared-templates/story-state-contract.md` §3 lists it as the sixth hybrid record class) AND the pipeline already supports it:
...
The contract semantics ("hybrid records retrievable via `get_record` with `section_path` projection; whole-class enumeration via `list_records`") implicitly cover STCHAR — the pipeline behavior is correct. Only the per-class enumeration list in FOUNDATIONS §Canonical Storage Layer §Read discipline is incomplete.

Session evidence (`commitment-block-authoring direct_batch` on red-bunny, this Claude session): for two STCHAR profiles whose persona, voice, and pressure behavior were load-bearing for SLT-22 (Jon-side recovery) and SLT-24 (Ane-led negotiation) beat authoring, the operator defaulted to Bash `grep` (find section headers) + `Read` (with offset/limit) instead of `mcp__worldloom__get_record(record_id='STCHAR-1', section_path='body.<section>')` despite the MCP tool supporting STCHAR projection at HEAD. The skill prose at `.claude/skills/commitment-block-authoring/SKILL.md` §Pre-flight step 6 says "retrieve the relevant full or projected STCHAR sections" without naming `get_record`, and FOUNDATIONS — the contract source — does not enumerate STCHAR among the projectable hybrid classes; both surfaces converge to leave the MCP path unsignposted at the point of retrieval choice.
...
3. Shared-contract consistency → manual review: confirm `_shared-templates/story-state-contract.md` §3 hybrid-record listing matches the updated FOUNDATIONS enumeration (STCHAR present in both).

## What to Change

### 1. FOUNDATIONS §Canonical Storage Layer §Read discipline — add STCHAR to both enumerations

In `docs/FOUNDATIONS.md` at line 584 (the **Read discipline** paragraph of §Canonical Storage Layer):

- **Hybrid-record enumeration**: change `Hybrid records (\`CHAR-<integer>\`, \`DA-<integer>\`, \`PA-<integer>\`, \`NCP-<integer>\`, \`NCB-<integer>\`)` to `Hybrid records (\`CHAR-<integer>\`, \`STCHAR-<integer>\`, \`DA-<integer>\`, \`PA-<integer>\`, \`NCP-<integer>\`, \`NCB-<integer>\`)`.
- **`list_records` record_type enum example**: change `list_records(record_type='character_record'|'diegetic_artifact_record'|'adjudication_record'|'character_proposal_card'|'character_proposal_batch')` to `list_records(record_type='character_record'|'story_character_authority_record'|'diegetic_artifact_record'|'adjudication_record'|'character_proposal_card'|'character_proposal_batch')`.
...
- Any change to `_shared-templates/story-state-contract.md` — §3 already correctly lists STCHAR as the sixth hybrid story-bundle class.
...
3. `grep -niE 'Hybrid records.*STCHAR|story_character_authority_record' docs/FOUNDATIONS.md _shared-templates/story-state-contract.md` — both documents enumerate STCHAR among hybrid records (FOUNDATIONS line 584; shared contract §3).
