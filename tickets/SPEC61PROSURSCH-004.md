...
1. A dedicated validator over all non-CF classes is the only mechanism that satisfies §4's universal criterion — the schema-only `not`-prohibition (SPEC61PROSURSCH-001) covers only the eight new schemas and would leave NCP/NCB/CHAR/DA uncovered. Centralizing the rule in one validator avoids per-schema duplication and a coverage gap.
...
- The per-schema `not`-prohibition (SPEC61PROSURSCH-001 — defense-in-depth only).
