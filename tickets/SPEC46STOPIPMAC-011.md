...
3. Cross-skill boundary: the world-index edge extraction is consumed by MCP graph-walking helpers and by the future dramatic-irony packet (deferred per SPEC-46 §Out of Scope item 5). The placeholder-skip convention preserves the same semantics as SPEC46STOPIPMAC-010: abstract holders (`group:<name>`, `narrator`) remain on the source record retrievable via `get_record`; only STENT-bound holders produce edges.
...
1. Per-class helper pattern matches SPEC46STOPIPMAC-006's `edgesForStoryBelief` and SPEC46STOPIPMAC-010's placeholder-skip pattern. `edgesForStorySecret` reuses the placeholder-check helper (inline or factored from SPEC46STOPIPMAC-010) for `holders[]`. The clue-carrier iteration projects the nested `record` field per array element. Alternative considered: emit additional edges per clue_carrier sub-field (`kind`, `discovered_by`, `audience_visible`, `status`) — rejected because edges capture record-to-record relations; sub-field payload stays on the source record per the SPEC-45 / SPEC-46 §Tick-history granularity convention.
2. No backwards-compatibility aliasing or shims introduced. Reuses the placeholder-skip helper from SPEC46STOPIPMAC-010 if factored, or duplicates the inline check.
...
- `tools/world-index/src/parse/atomic.ts` (modify — add `edgesForStorySecret` + dispatch wiring; reuses placeholder-skip helper from SPEC46STOPIPMAC-010 if factored)
