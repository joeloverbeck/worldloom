...
3. Save Segment in create mode: POST /segments → backend returns 201 + `{ segment_id, sidecar, checklist_payload }` → PasteProse renders ticket 012's StateUpdateChecklist as modal with `payload` prop → component test
...
Import `StateUpdateChecklist` from ticket 012's component file (`web/src/components/StateUpdateChecklist.tsx`). Render conditionally when `checklistPayload != null`.
...
- The StateUpdateChecklist component implementation (covered by ticket 012; this ticket imports it)
...
1. PasteProse never directly mutates any record file under `<manualStoryRoot>/records/` — it only POST/PUT/DELETE-s to `/segments` endpoints (which are governed by ticket 004's no-record-mutation invariant).
