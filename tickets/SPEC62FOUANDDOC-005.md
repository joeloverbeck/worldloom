...
`diegetic-artifact-generation/SKILL.md` says a DA's claims are "contested canon (FOUNDATIONS §Canon Layers) at their strongest." This conflates a DA's raw in-world assertions with the **accepted** Contested Canon layer (`status: contested_canon` on a CF). The wording wrongly implies a DA's statements have a canon-layer standing they do not have. SPEC-62 §2.6 replaces the imprecise prose without touching the `claim_map.canon_status` field or the Phase 7 firewall.

## Assumption Reassessment (2026-05-21)

1. Verified against current `.claude/skills/diegetic-artifact-generation/SKILL.md`: 2 "contested canon" prose sites (≈line 106: "The artifact's claims are *contested canon* (FOUNDATIONS §Canon Layers) at their strongest"; ≈line 178: "The artifact is contested canon — an in-world voice, not world-level truth"). The `claim_map.canon_status` enum (`canonically_true | canonically_false | partially_true | contested | mystery_adjacent | prohibited_for_this_artifact`) lives in `templates/diegetic-artifact.md:82` and the phase references — it never uses `contested_canon`, so the field name is already precise and is NOT renamed.
...
1. Neither prose site still equates DA claims with the accepted Contested Canon layer "at their strongest" → codebase grep-proof (`grep -n "contested canon" SKILL.md` shows the corrected in-world-assertions wording).
...
Replace with precise wording: a DA's claims are **in-world assertions**, not canon; the artifact may exist as a world artifact and may cite or distort accepted CFs, but its statements become world truth only if a later `canon-addition` accepts a CF about their existence, circulation, belief, disputed status, or truth. Apply to both sites consistently.
...
1. `grep -n "contested canon" .claude/skills/diegetic-artifact-generation/SKILL.md` shows no remaining "at their strongest"-style equation of DA claims with the accepted Contested Canon layer; the in-world-assertions wording is present at both former sites.
