...
`mcp__worldloom__get_records(record_ids, world_slug?)` returns full record bodies for a known-id batch. For batches of `≤ 5-7` records, the response fits within the harness inline cap. For larger batches, the response exceeds the cap and is redirected to a persisted file requiring `jq`-on-file recovery.
...
Neither matches the common Phase 7a / 7b / 7c shape: "for these N records, give me these M fields." A batched field-projection tool would let the operator request `get_records_field(record_ids=[...10 invariant ids], field_path='statement')` in one call, then `get_records_field(record_ids=[...10 invariant ids], field_path='break_conditions')` in a second call — two calls totaling ~5-10 KB instead of one 61-KB blast.
...
1. `tools/world-mcp/src/tools/get-records.ts` (per archive/tickets/WMCP-001 and subsequent WMCP work) implements known-id batch retrieval. `tools/world-mcp/src/tools/get-record-field.ts` implements single-record-single-field projection (mentioned in `tools/world-mcp/README.md` and `docs/MACHINE-FACING-LAYER.md`; live in current MCP capability set). Combining these — batch + projection — is not yet a tool.
2. `mcp__worldloom__get_record(record_id, section_path?)` accepts a `section_path` for hybrid-record projection (per `mcp__worldloom__get_record` schema description: "Optional section_path projects a hybrid record subset, e.g. 'frontmatter.world_consistency' or 'body.Capabilities'"). The `field_path` semantics for atomic records via `get_record_field` use dot-notation (e.g., `distribution.who_can_do_it`). The new `get_records_field` tool inherits the dot-notation contract from `get_record_field`.
3. Cross-artifact boundary: this is a new MCP tool. It does not modify existing tools' responses. Skills that consume it (`character-generation`, `diegetic-artifact-generation`, `canon-addition`, `continuity-audit`, `propose-new-canon-facts`, `propose-new-characters`, `canon-facts-from-diegetic-artifacts`, `propose-new-worlds-from-preferences`, `emergent-pressure-events`) cite it in their `references/world-state-prerequisites.md` (or equivalent) targeted-retrieval section.
...
6. Adjacent contradictions: `mcp__worldloom__get_firewall_content(world_slug)` already implements a bulk firewall-fields projection across all M records (per `.claude/skills/character-generation/references/world-state-prerequisites.md:17`). It is the world-wide bulk variant of what `get_records_field(record_ids=[all M ids], field_path='disallowed_cheap_answers')` would do for an explicit id list. Both should coexist: `get_firewall_content` for the canonical Phase 7b bulk audit; `get_records_field` for arbitrary id-batch projections (e.g., a subset of M records the auditor wants to re-check for a specific revision).
7. Per-record error handling: `get_records` returns a per-id success/error wrapper (`{records: [{record_id, found, record?, error?}]}`). `get_records_field` should mirror this shape: `{records: [{record_id, found, field_value?, error?}]}` so a partial-failure batch returns successes alongside per-id errors without aborting the batch.
...
2. **Projecting at the SQL/index layer is structurally cleaner than projecting in TypeScript post-processing.** The world index already supports field-path queries via `get_record_field`'s implementation. The new tool delegates to the same retrieval layer per record, avoiding a full-body load + post-processing slice.
...
2. Field projection at SQL layer -> schema validation: response with `field_path='distribution'` for a CF id list returns only the `distribution` field per record, not the full body. Response size scales linearly with `len(field_value) × len(record_ids)`, not with full-record-body size.
...
export interface GetRecordsFieldResponse {
  ...
  field_path: string;  // echo input for downstream auditing
}
```

Implementation: for each `record_id`, delegate to the same retrieval path `get_record_field` uses, collecting per-id results into the response array. Shape mirrors `get_records` but the per-entry payload is the projected `field_value` (any JSON type) instead of the full `record`.

### 2. Register the tool in the MCP server

`tools/world-mcp/src/server.ts` exposes `get_records_field` in the tool registry and capability metadata. Input schema: `world_slug: string` (min 1), `record_ids: string[]` (min 1 item), `field_path: string` (min 1).
...
- Multi-field projection per call (deferred per §What to Change item 4).
- Replacing `get_records` or `get_record_field` (the new tool extends the family; existing tools remain).
- Extending the field-path syntax beyond what `get_record_field` already supports (dot-notation is the established contract).
...
1. `cd tools/world-mcp && npm run build && node --test dist/tests/tools/get-records-field.test.js` — proves the handler returns per-id projections, mirrors `get_records` partial-failure shape, and respects field-path dot-notation.
2. `cd tools/world-mcp && npm run build && node --test dist/tests/server/dispatch.test.js` — proves MCP dispatch routes the new tool name correctly.
3. `cd tools/world-mcp && npm test` — full package suite passes.

### Invariants

1. `get_records_field(world_slug, record_ids, field_path)` returns `{records: [{record_id, found, field_value?, error?}], field_path}` with one entry per requested id, in request order.
...
1. `tools/world-mcp/tests/tools/get-records-field.test.ts` — cases: (a) batch of valid record_ids returns projected `field_value` per id; (b) mix of valid + invalid record_ids returns per-id errors alongside successes; (c) invalid `field_path` returns per-id error with descriptive message; (d) atomic record (`CF-NNNN`) and hybrid record (`CHAR-NNNN`) field-paths both work; (e) response size stays bounded for a 17-id batch with a typical field path (asserts the harness-cap regression risk is closed).
...
2. `cd tools/world-mcp && node --test dist/tests/tools/get-records-field.test.js dist/tests/server/dispatch.test.js` — targeted verification of the new tool.
