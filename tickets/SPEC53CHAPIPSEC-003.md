...
`character-memorability-structure.ts`'s `proposalVerdicts` requires the `## Rejected Directions Audit` heading only when `origin_kind === "upgraded_seed"` — it does not cover `user_seed`, although the `origin_kind` enum and the deepening skill both treat user-seed cards identically. And the deepening skill's "≥3 rejected directions" requirement is not structurally enforced. After SPEC53CHAPIPSEC-001 makes the object-shaped audit schema-valid, the structural layer should enforce the ≥3-entry minimum so a clear per-field message is emitted alongside the AJV failure.
...
- Replace the `origin_kind === "upgraded_seed"` guard with membership in `{upgraded_seed, user_seed}` for the `## Rejected Directions Audit` heading check.

### 2. Rejected-directions ≥3 structural check

- For `origin_kind ∈ {upgraded_seed, user_seed}`, emit a `rejected_directions_audit_min_items` verdict when `upgrade_lineage.rejected_directions_audit` is not an array of ≥3 entries. Keep the message actionable (name the field + required count).
...
1. A `user_seed` NCP without `## Rejected Directions Audit` fails with `missing_rejected_directions_audit` (or equivalent code).
