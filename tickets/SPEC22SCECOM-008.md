...
4. `get_record_schema(record_type='arc_trace_record')` returns the v2 schema metadata — consumes the JSON schema added in 002 (`tools/validators/src/schemas/story-arc-trace.schema.json`) OR a parallel TypeScript schema metadata source in `world-index/src/public/types.ts` (extended in 007). Soft dep on 002 for the JSON schema source-of-truth.
