...
Current Phase 1 Path B (write-in handling) routes free-form `manual_action_text` via four-way routing (`ACCEPT` / `ACCEPT_BUT_TRANSFORM` / `TREAT_AS_ATTEMPT` / `REFUSE_ONLY_THROUGH_WORLD_LOGIC`). Under the scene-commitment-arc pivot, write-ins must additionally classify into the `commitment_class` enum so Phase 4's hard filter 6 (`arc.arc_contract.commitment_class` matches the chosen CHC's commitment_class OR the classified write-in commitment-class) can apply uniformly across Path A (CHC selection) and Path B (write-in). Without the classifier, write-ins would bypass arc-cadence discipline — a write-in is a commitment, not a beat-action. SPEC-20 §G specifies the additional classification step.
...
2. Classifier output is enum-bound → schema validation deferred to runtime; this ticket's surface is documentation.
3. REFUSE_ONLY_THROUGH_WORLD_LOGIC fallback → codebase grep-proof in the same file for the fallback rule.

## What to Change

### 1. §Write-In Commitment-Class Classification (NEW subsection)

In `.claude/skills/branching-story-page-cycle/references/phase-1-choice-resolution.md`, add a new subsection after the existing four-way routing prose:

> ## Write-In Commitment-Class Classification (after four-way routing)
>
...
>
> The classified `commitment_class` is then handed to Phase 4 as an arc-selection filter (Phase 4 hard filter 6 — see `phase-4-storylet-and-mystery-authority.md`). This ensures write-ins do not bypass the arc-cadence discipline — a write-in is a commitment, not a beat-action.

### 2. Cross-reference Phase 4

Add a cross-reference pointer at the end of the new subsection: `See phase-4-storylet-and-mystery-authority.md §Phase 4 hard filter 6 for the consumer-side filter.`
...
1. Skill dry-run: `branching-story-page-cycle` invoked with a write-in `manual_action_text` on a v2 fixture; Phase 1 produces a routing verdict AND a classified `commitment_class`; the classified value is one of the 20 enum entries.
2. Classification failure fallback: a fixture write-in that does not fit any commitment class (e.g., a self-contradictory directive) routes via `REFUSE_ONLY_THROUGH_WORLD_LOGIC` despite the four-way routing returning `ACCEPT`.
...
1. `grep -n "Write-In Commitment-Class Classification" .claude/skills/branching-story-page-cycle/references/phase-1-choice-resolution.md` — confirms NEW subsection anchor lands.
2. `grep -n "REFUSE_ONLY_THROUGH_WORLD_LOGIC" .claude/skills/branching-story-page-cycle/references/phase-1-choice-resolution.md` — confirms fallback rule documented.
3. `grep -n "phase-4-storylet-and-mystery-authority.md" .claude/skills/branching-story-page-cycle/references/phase-1-choice-resolution.md` — confirms cross-reference pointer to Phase 4 lands.
