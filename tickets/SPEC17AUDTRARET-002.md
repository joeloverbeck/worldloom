...
**Priority**: MEDIUM
**Effort**: Small
**Engine Changes**: None — documentation-only changes to `docs/FOUNDATIONS.md`, `docs/CONTEXT-PACKET-CONTRACT.md`, `tools/world-mcp/README.md`, and `docs/MACHINE-FACING-LAYER.md`. No code change.
...
`mcp__worldloom__get_context_packet(task_type, seed_nodes, token_budget)` returns nodes with `body_preview` truncated at ~282 chars; full bodies are retrieved via follow-up `mcp__worldloom__get_record(record_id)` or `mcp__worldloom__get_record_field(record_id, field_path)` calls. Live evidence: the 2026-04-26 PR-0015 packet at 16000-token budget covered 132 nodes but every node body was truncated; reasoning required ~7 follow-up `get_record` calls for the load-bearing CFs/SECs.

`docs/FOUNDATIONS.md` §Tooling Recommendation (lines 426-438) reads as if full content is delivered in one call: *"They should always receive: current World Kernel, current Invariants, relevant canon fact records, affected domain files, unresolved contradictions list, mystery reserve entries..."* The plain reading is "agents receive content, not pointers to content." The actual operational pattern is INDEX (packet) + targeted retrieval (`get_record` / `get_record_field`).
...
1. Verified `mcp__worldloom__get_record_field` registered: `tools/world-mcp/src/tool-names.ts:5` defines `get_record_field: "mcp__worldloom__get_record_field"`; the FOUNDATIONS amendment cross-references this tool, so the tool MUST exist when the prose lands. Verified `tools/world-mcp/README.md` documents `get_record_field` at line 14 as "returns a single field from a parsed record without loading the full body." Verified `docs/MACHINE-FACING-LAYER.md` already lists `get_record_field` in its §Retrieval Tool Scope table at line 65 with usage note "A single field of a parsed atomic record. Use when the field is small and the record body is large, such as `touched_by_cf` on a large SEC record." Verified `docs/CONTEXT-PACKET-CONTRACT.md` exists with 6 layer-semantic H3 sections (`task_header` + 5 content layers `local_authority` / `exact_record_links` / `scoped_local_context` / `governing_world_context` / `impact_surfaces`) at lines 44, 55, 67, 73, 85, 99; NO existing §Index + Follow-Up Retrieval Pattern subsection (this ticket creates it).
...
1. FOUNDATIONS amendment landed → codebase grep-proof: `grep -A3 "should always receive" docs/FOUNDATIONS.md` returns the amended prose including the "or via the documented context-packet + targeted-retrieval pattern" clause AND the `mcp__worldloom__get_record_field` reference inside the "completeness guarantees" sentence.
...
1. `grep -A3 "should always receive" docs/FOUNDATIONS.md` returns the amended prose including the "or via the documented context-packet + targeted-retrieval pattern" clause AND the `mcp__worldloom__get_record_field` reference.
...
1. `grep -A3 "should always receive" docs/FOUNDATIONS.md` — FOUNDATIONS amendment verification (must show the amended bullet list and the `get_record_field` reference).
2. `for f in "docs/CONTEXT-PACKET-CONTRACT.md" "docs/MACHINE-FACING-LAYER.md" "tools/world-mcp/README.md"; do echo "=== $f ==="; grep -nE "Index \+ Follow-Up Retrieval Pattern|Recommended composition|five content layers" "$f"; done` — cross-document landing verification (one combined command across the three non-FOUNDATIONS surfaces).
