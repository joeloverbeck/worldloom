...
**Plan-Authority invariant (§4a, FOUNDATIONS-level verification)**: snapshot `<manualStoryRoot>/records/` directory mtime + file list before each save/edit/delete operation; assert unchanged after. This is the integration-level enforcement of the no-record-mutation invariant tickets 004 / 008 assert at the unit level.
