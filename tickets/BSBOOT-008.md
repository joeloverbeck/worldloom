...
**Priority**: HIGH
**Effort**: Small
**Engine Changes**: None — `branching-story-bootstrap/references/phase-9-validation-gates.md` only.
**Deps**: none

## Problem

Phase 9 gate 12 currently roots its closure traversal at `state_snapshot`:
...
But several PG-0001 fields critical to runtime correctness are **peer fields** of `state_snapshot`, not children of it:
...
- Replace gate 12's "Check" cell:
  ...
  **After:**
  ...
  ```
  | 12 | State_snapshot completeness + recursive reference closure | `current_location`, `entity_status`, `relationships_current`, and the epistemic-faceted fact lists populated on `state_snapshot`; closure traversal roots at PG-0001 itself, expanding through `state_snapshot` ∪ `storylet_realized` ∪ `applied_event_ops` ∪ `emitted_choices`, then through every CHC's `likely_effects` / `uses_fact` / `target` / `actor`; every non-PG story-local ID encountered either has `created_at_page == null` (globally legal — author-pool storylets only) or `created_at_page == PG-0001`; every PG reference is allowed when that PG id is in the root `branch_path` (`[PG-0001]`) because the page record's own id is its branch anchor | Phase 7 |
  ```

### 2. `.claude/skills/branching-story-bootstrap/references/phase-7-root-page-render.md`

- Update the dual-validation-trace mapping note (lines 97-110) to clarify that the PG-0001 `validation_trace.recursive_reference_closure` key records the broader closure result, while `state_snapshot_integrity` records the field-population subset of the gate.
...
1. `grep -nE "PG-0001 itself|emitted_choices.*storylet_realized|likely_effects" .claude/skills/branching-story-bootstrap/references/phase-9-validation-gates.md` returns matches in gate 12's row.
2. `grep -nE "reachable from .state_snapshot." .claude/skills/branching-story-bootstrap/references/phase-9-validation-gates.md` returns no matches as the SOLE closure-root specifier (the new wording names PG-0001 as the root, with state_snapshot as one expansion edge among several).
...
1. `grep -nE "closure traversal|closure root|recursive reference" .claude/skills/branching-story-bootstrap/references/phase-9-validation-gates.md` — confirms broader wording.
2. `grep -n "recursive_reference_closure\|state_snapshot_integrity" .claude/skills/branching-story-bootstrap/references/phase-7-root-page-render.md` — confirms dual-trace mapping is still aligned.
