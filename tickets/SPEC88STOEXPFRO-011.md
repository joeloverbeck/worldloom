...
**Deps**: archive/tickets/SPEC88STOEXPFRO-001.md, archive/tickets/SPEC88STOEXPFRO-005.md, SPEC88STOEXPFRO-006, SPEC88STOEXPFRO-008
...
2. SPEC-88 §9 (post-reassessment) names the matrix: 6 `indexStatus.kind` cases (4 from original spec + `open_failed` added by the reassessment audit) + Page-Not-Found (404) + Backend-Unreachable (network error). World-list-empty and Story-list-empty cases are owned by T005 / T006 respectively (they're empty-state UI, not degraded backend state). 404 surface: "Polished 404 with link to story root." Backend-unreachable: "Polished error with retry button." Both are global error pages — implemented as `<ErrorBoundary renderFallback>` content at route level, dispatched by `ApiError` status code or fetch failure.
3. Cross-skill boundary: this ticket spans 4 files — `routes/{worlds,stories,page-read}.tsx` + `app.tsx` (for the global 404 / backend-unreachable fallbacks). The route modifications are mechanical (add IndexStatusBanner integration at the top). The global error surfaces use T004's `<ErrorBoundary>` primitive with route-specific `renderFallback` content. The §Intra-batch shared-file create-then-modify chains rule applies — T011 modifies `routes/{worlds,stories,page-read}.tsx` files that T005/T006/T008 created `(new)`; Deps declared accordingly.
...
2. **404 as a route-level ErrorBoundary fallback** — when `ApiError.status === 404`, the route's `<ErrorBoundary renderFallback={(err, retry) => <NotFoundPage worldSlug={...} storySlug={...} />}>` renders a polished 404 page with a link back to the story root (or world picker, when story slug unavailable). The 404 surface is route-aware — the link target adapts based on which route caught the error.
3. **Backend-unreachable as a global fallback** — top-level `<ErrorBoundary>` at app.tsx catches network errors (fetch throws before status); renders a polished retry surface with a button that re-mounts the route subtree.
4. **`<NotFoundPage>` and `<BackendUnreachablePage>` as new shared components** — both live in `components/` alongside the cross-cutting infrastructure. Used by route-level ErrorBoundary fallbacks.
...
Add `useIndexStatusBanner` at top — covers `getPageDetail()`'s envelope. Wrap the route in `<ErrorBoundary renderFallback={...}>` whose fallback dispatches by error type:
```tsx
renderFallback={(error, retry) => {
  if (error instanceof ApiError && error.status === 404) {
    return <NotFoundPage worldSlug={worldSlug} storySlug={storySlug} resourceLabel="page" />;
  }
  if (isNetworkError(error)) {
    return <BackendUnreachablePage onRetry={retry} />;
  }
  return <GenericErrorPage error={error} onRetry={retry} />; // catchall
}}
```

### 7. Modify `tools/story-explorer/web/src/app.tsx`

Wrap the entire app in a top-level `<ErrorBoundary>` whose fallback dispatches network errors to `<BackendUnreachablePage>`. Route-level boundaries handle status-code errors; the app-level boundary catches anything route-level missed.
...
- `tools/story-explorer/web/src/routes/page-read.tsx` (modify — adds banner + ErrorBoundary fallbacks)
- `tools/story-explorer/web/src/app.tsx` (modify — adds top-level ErrorBoundary for backend-unreachable)
...
4. Updates to T005/T006/T008's existing route tests (add banner-integration assertions).
