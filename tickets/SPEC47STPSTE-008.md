...
Add 6 new entries to PREDICATE_NAMES (the closed-enum array) following the existing per-predicate registration pattern. Order: alphabetical or grouped with related predicates (e.g., `plan_active` next to `intention_active`; `emotion_active` next to `belief_record` if the file groups by class).

Add 6 new entries to PREDICATE_ARG_SCHEMAS with arg-shape definitions per the SPEC-47 §Approach §B predicate table:

```typescript
plan_active: { required: ["holder"], optional: ["plan"] },
plan_blocked: { required: ["holder"] },
any_plan_active: { required: ["alias"], optional: ["holder_role"] },
emotion_active: { required: ["holder"], optional: ["kind", "min_intensity"] },
any_emotion_active: { required: ["alias"], optional: ["holder_role", "kind", "min_intensity"] },
emotion_pressure: { required: ["holder", "pressure"] },
```

Closed-enum constraint on filter args:
...
### 2. Extend `tools/validators/src/schemas/story-storylet.schema.json`

Update the predicate-discovery surface to enumerate the 6 new predicates (so SLT authors see them via the schema-discovery path).

### 3. Extend `tools/validators/src/schemas/predicate-dsl-grammar.schema.json`

Update the meta-schema (if it enumerates valid predicate names) to include the 6 new names.
...
1. `tools/validators/tests/rules/_shared/predicate-dsl-grammar-stplan-stemo.test.ts` (new) — per-predicate positive + negative case tests for all 6 new predicates.
2. `tools/validators/tests/rules/_shared/predicate-dsl-grammar-parity.test.ts` (modify, if exists) — extend to assert PREDICATE_NAMES.length === 42 (33 + 6 + 3 combinators) and PREDICATE_NAMES is internally consistent with PREDICATE_ARG_SCHEMAS.

### Commands

1. `npm --prefix tools/validators run build && npm --prefix tools/validators test` (full validator package tests pass)
2. `npm --prefix tools/validators test -- --test-name-pattern "predicate-dsl-grammar"` (only predicate-DSL tests run)
