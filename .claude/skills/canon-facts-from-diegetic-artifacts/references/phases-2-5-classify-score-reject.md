...
For each claim in the ledger, determine its relation to existing canon by cross-referencing against CF records (`mcp__worldloom__search_nodes(node_type='canon_fact', filters={domain: ...})` + `mcp__worldloom__get_record`), INV records (`search_nodes(node_type='invariant')` + `get_record`), and `SEC-*` atomic-record prose (`search_nodes(node_type='section', filters={file_class: ...})` + `get_record`). Five classification buckets:
