...
10. **`stchar_authority` block** — for every §16a packet required by the shared story-state contract (viewpoint character, speaker, major actor, direct target, emotionally salient character, or any character whose behavior/voice materially shapes the page), emit one `stchar_authority[]` entry with `STENT` / `STCHAR` / display name, `required_because`, `packet_present`, `active_in_snapshot`, the expected-vs-observed `profile_hash`, `voice_block_hash`, and `page_packet_hash`, and a `deterministic_verdict`. `required_because` is the §16a packet's `Required because:` value copied **verbatim**, including every comma-separated qualifier (e.g. `direct_target, emotionally_salient, behavior_shapes_page`) — not an abbreviation to the first qualifier; `prose_receipt_stchar_integrity` FAILs on any divergence. For `profile_hash` and `voice_block_hash`, `expected` is the `STCHAR` record's stored frontmatter hash and `observed` is the §16a page-plan packet's declared hash. For `page_packet_hash`, `expected` is the §16a page-plan packet's declared page-local hash and `observed` is the recomputed hash of that exact packet projection. STCHAR body-content integrity — that the stored `profile_hash` / `voice_block_hash` actually reproduce from the STCHAR body — is enforced separately by the `stchar_body_integrity` validator (`tools/validators/src/structural/stchar-body-integrity.ts`), not by prose-attach. Missing packets, inactive `STCHAR` ids in `PG.state_snapshot.active_records.STCHAR`, or any hash mismatch force `deterministic_verdict: FAIL`. Omit or leave the array empty only when no qualifying character exists for the page.
