...
- `.claude/skills/_shared-templates/story-record-schemas.md` §4.5.17 / §4.5.18 and `.claude/skills/_shared-templates/story-state-contract.md` §5a / §8 — STPLAN/STEMO record labels, structured SE fields, and page-plan §9b / §9c constraints prose-attach validates when present
...
2. **`engine_jargon_leak`** (`PASS | WARN | FAIL`) — scan the prose body for engine-vocabulary tokens. The closed token source is shared with `page_plan_body_engine_vocabulary_cleanliness` at `tools/validators/src/structural/_engine-vocabulary-tokens.ts`; the same record-id regex, schema-field literals, and predicate-DSL literals scan rendered prose here and upstream page-plan body sections outside the page-plan allow-list. Engine vocabulary legitimately appears in plan §15 frontmatter and verbatim-inlined plan §2 / §3 / §19 — those are NOT scanned. Hits in the rendered prose body are `WARN` if isolated (single occurrence), `FAIL` if pervasive (≥3 occurrences across different tokens).
...
4. **`required_event_rendered`** (`PASS | WARN | FAIL`) — verify plan §7 (selected event + outcome_route) is dramatized in the prose. If the event is implied but ambiguous (the reader could miss it on first read), `WARN`. If absent or actively contradicted, `FAIL`. Verification scans for plan §8 beat keywords + plan §7 actor / target references appearing in the prose body.
   ...
   - **`story_question_payoff_undisclosed`** — for each STQ opened, narrowed, answered, paid off, abandoned, inherited, or superseded by plan §7 / `SE.state_delta`, verify the prose reflects the transition using the question/setup wording, `payoff_of`, `answer_records[]`, §10b render requirement, or another unmistakable setup/payoff cue from the plan. If the STQ transition is omitted but the selected event is otherwise rendered, set `required_event_rendered: WARN` and add a `notes[]` entry beginning `story_question_payoff_undisclosed:`. If the prose contradicts the committed STQ status, implies an answered question remains open, or answers a question the committed state did not answer, set `required_event_rendered: FAIL`.
   ...
   - **`state_relation_undisclosed`** — when plan §9b names this page's `SE.state_relations[]` posture for an active `STPLAN`, verify the prose dramatizes the relation through the plan holder, objective, current step, blockers, resources, or success condition named in §9b. If the selected event is otherwise rendered but the plan movement is easy to miss, set `required_event_rendered: WARN` and add a `notes[]` entry beginning `state_relation_undisclosed:`. If prose contradicts the committed relation, set `required_event_rendered: FAIL`.
...
5. **`choice_consequence_visibility`** (`PASS | WARN | FAIL`) — verify the prose makes the selected action, route, and immediate consequence legible while staying within `STORY_KERNEL.md` `## Player Agency Contract`. For non-accept routes, compare the prose against `SE.resolution.player_visible_feedback` from plan §7. For `accept` routes, do not consult `SE.resolution` (it is absent by schema); compare the rendered prose against the selected `CHC.likely_state_pressure`, `CHC.grounded_in.records[]`, page-plan §13 choice preview, and the committed `SE.state_delta` / `SE.state_relations[]` from plan §7. `PASS` means the consequence has some reader-visible trace and does not imply player control outside the agency surface / write-in envelope / viewpoint limits. `WARN` means the action occurred but the route outcome, consequence, or agency boundary is easy to miss. `FAIL` means the prose obscures, contradicts, or omits the consequence, or implies a broader/narrower agency surface than the contract permits, especially for `attempt`, `accommodate`, `world_block`, `promotion_hold`, or `terminal` routes. Presence is deterministic; sufficiency remains a judgment note, so this check must not force exposition dumps when the committed consequence is already legible.
