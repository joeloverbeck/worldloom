...
6. Phase 6: Commit / Write — HARD-GATE fires.
   ...
   3. Present the complete deliverable summary to the user:
      ...
      - `moment_fit_lanes` (`direct_batch`): per-lane `lane_id` + `source` + `addressed_by_blocks` (when Pass B emitted lanes); explicit `none — pool fit to moment` when Pass B emitted no lanes.
