...
(a) Pre-flight Check has completed: bundle resolved at `worlds/<world_slug>/stories/<story_slug>/`; mode validated; for `direct_batch`, the latest committed parent `PG-<integer>` resolved, the pool-wide SLT inventory loaded through `mcp__worldloom__list_records(record_type='storylet_record', story_slug=<story_slug>, filters={'scope.visibility': ['global_author_pool', 'branch_prefix_scoped']}, fields=['move_family', 'grounding.compatible_turn_drivers', 'preconditions', 'exit_options'])` as projection records keyed by `move_family`, the dotted `grounding.compatible_turn_drivers` response key, and the parent objects needed to compute predicate-class and action-family coverage for Phase 1 coverage-gap diagnosis, AND, when existing-block mutation planning is in scope, per-page eligibility shortlist additionally loaded through `mcp__worldloom__select_storylet_candidates(world_slug=<world_slug>, story_slug=<story_slug>, parent_page_id=<latest committed PG>, turn_driver=<derived_or_player_default>, max_candidates=<pool_size>, include_rejection_summary=true)` for `replace` / `extend` mutation targeting; for `audit_repair`, audit + RSP cards loaded from `audits/<audit_id>-*.md` + `audits/<audit_id>/remediation-storylet-proposals/RSP-*.md`; active STCHAR summaries loaded from `story_bundle_context.active_story_characters` and full/projected STCHAR sections retrieved via `mcp__worldloom__get_record(record_id='STCHAR-<integer>', section_path='body.<section-name>')` when a block's eligibility, beats, effects, pressure behavior, relationship conduct, persona, or voice depends on a specific character; SLT ids and one SLB id allocated via `mcp__worldloom__allocate_next_id`; world canon context packet loaded via `mcp__worldloom__get_context_packet(world_slug, task_type='commitment_block_authoring', ...)`.
...
- pool-wide SLT inventory via `mcp__worldloom__list_records(record_type='storylet_record', story_slug=<story_slug>, filters={'scope.visibility': ['global_author_pool', 'branch_prefix_scoped']}, fields=['move_family', 'grounding.compatible_turn_drivers', 'preconditions', 'exit_options'])` (`direct_batch` only; may be empty post-bootstrap if `seed_commitment_blocks: none`) — this is the primary path for Phase 1's pool-wide coverage-gap diagnosis across the 17 coverage targets. The projected response preserves the requested dotted key as `grounding.compatible_turn_drivers`; derive predicate-class coverage from `preconditions.hard[]` / `preconditions.soft[]` and action-family coverage from `exit_options[]` rather than requesting non-schema projection fields. Additionally, when existing-block mutation planning is in scope, per-page eligibility shortlist via `mcp__worldloom__select_storylet_candidates(world_slug=<world_slug>, story_slug=<story_slug>, parent_page_id=<latest committed PG>, turn_driver=<derived_or_player_default>, max_candidates=<pool_size>, include_rejection_summary=true)` — the response supplies `shortlisted_projection_records[]`, `filter_trace`, and `requires_full_body_ids[]`; retrieve full SLT bodies via `mcp__worldloom__get_records(...)` only for records selected for `replace` / `extend` mutation planning. Pool inventory and per-page eligibility serve distinct authoring concerns: `list_records` returns all entries with projection or parent-object fields needed for coverage diagnosis; `select_storylet_candidates` is filter-narrowed to a single parent page + driver (correct for picking which existing block to extend at that page, but under-counts the pool when used for coverage).
...
Mystery Reserve and Invariants — already loaded in Pre-flight step 6 above
...
**Context - what Phase 1 already does.** Phase 1 currently enumerates 15 coverage targets (14 causal-function + target #15 cast-role coverage) and names pool-inventory-based gap diagnosis below (`grounding.compatible_turn_drivers` for driver-kind coverage; `preconditions` for active-record-class coverage). What is missing is the structured trigger maps (SPEC-80 §3.1, §3.2), the joint composition rule (SPEC-80 §3.3), and a structured YAML output that the Phase 2-4 block generators can read.
...
(b) **SLT-pool load** (the SUPPLY side — projection of current pool storylets to check coverage against demand): use the pool-wide inventory `mcp__worldloom__list_records(record_type='storylet_record', story_slug=<story_slug>, filters={'scope.visibility': ['global_author_pool', 'branch_prefix_scoped']}, fields=['move_family', 'grounding.compatible_turn_drivers', 'preconditions', 'exit_options'])` already loaded in Pre-flight step 4 (i) for `direct_batch`. This is the correct tool for pool-wide coverage projection: it returns all bundle SLTs with the same projected/parent-object fields and is not filter-narrowed to one parent page. The `select_storylet_candidates` per-page eligibility shortlist (Pre-flight step 4 (ii) when mutation planning is in scope) is the right tool for selecting WHICH existing block to extend at a specific page, not for pool-wide coverage diagnosis — its filter pipeline narrows the projection to eligibility at the named parent page + driver and so under-counts the pool when used for coverage. Use `include_full_body=true` only when full SLT bodies are needed for mutation-targeted analysis beyond the projected and parent-object fields.
...
```yaml
coverage_diagnosis:
  # one entry per coverage target (targets #1–#17 enumerated above); status: covered | gap; addressed_by_blocks lists the new SLT ids that resolve each gap (empty list when status is covered).
  - {target_id: 1, target_name: "recovery", status: gap, addressed_by_blocks: [SLT-NEW-3]}
  - {target_id: 2, target_name: "belief_repair", status: covered, addressed_by_blocks: []}
  - {target_id: 4, target_name: "bond_or_status_shift", status: gap, addressed_by_blocks: [SLT-NEW-1]}
  - {target_id: 10, target_name: "opposition_refusal", status: gap, addressed_by_blocks: [SLT-NEW-2]}
  - {target_id: 12, target_name: "clock_advancing", status: gap, addressed_by_blocks: [SLT-NEW-4]}
  - {target_id: 13, target_name: "clue_discovering", status: gap, addressed_by_blocks: [SLT-NEW-5]}
  - {target_id: 14, target_name: "setup_paying_off", status: gap, addressed_by_blocks: [SLT-NEW-6]}
  - {target_id: 15, target_name: "cast_role_coverage", status: covered, addressed_by_blocks: []}
  # ... (one entry per target; covered entries also recorded with empty addressed_by_blocks for completeness)
