...
**`forbidden_resolutions[]` defensive-inclusion heuristic**: include `M-<integer>` for any forbidden-status M whose territory the block's `move_family` thematically touches, even when the block's effects do not directly resolve the mystery. Phase 3 gate 4 only catches *effects that resolve* a listed mystery; an empty `forbidden_resolutions: []` always passes gate 4 because empty cannot conflict. Defensive inclusion strengthens the per-block firewall by surfacing a thematic-territory match the gate would otherwise miss. The heuristic uses the M's `domains_touched` / `knowns` (loaded whole-class at pre-flight step 6, per CBAUTH-001) against the block's beats and exit-options:
