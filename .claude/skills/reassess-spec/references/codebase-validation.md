...
> **Story-scope-validator carve-out**: new read-only structural validators that enforce *story-bundle* (Skill Category 2c) integrity — page-plan / prose-receipt / STCHAR / STENT / cast-list checks — do NOT by themselves trigger 3.9. They mediate no world-canon reads/writes, change no canon-validator thresholds, and resolve no Mystery Reserve entries. "validator thresholds" in the trigger above means *world-canon* validator thresholds, not the existence of any story-scope validator. Apply 3.9 to such a spec only for the story-mystery-firewall angle (does any validator's pass silently narrow a forbidden-status `M`?). Parallels the §3.10 story-bundle carve-out below.
