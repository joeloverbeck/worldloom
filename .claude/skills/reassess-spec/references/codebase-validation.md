...
> **Non-canon read-only structural validator carve-out**: new read-only structural validators that enforce the integrity of a *non-canon surface* — either *story-bundle* (Skill Category 2c) artifacts (page-plan / prose-receipt / STCHAR / STENT / cast-list checks) OR *world-system proposal/audit/pressure/world-proposal* artifacts (PR / BATCH / EPE / EPE-sidecar / NCP / AU / RP / NWP / NWB maturity, index-consistency, or approval-field checks) — do NOT by themselves trigger 3.9. They mediate no world-canon reads/writes, change no canon-validator thresholds, and resolve no Mystery Reserve entries. "validator thresholds" in the trigger above means *world-canon* validator thresholds, not the existence of any non-canon-surface validator. Apply 3.9 to such a spec only for the story-mystery-firewall angle (does any validator's pass silently narrow a forbidden-status `M`?). **A validator that gates canon/hybrid writes — e.g., a pre-apply compatibility gate that blocks a patch plan — is NOT carved out**: it touches the patch-engine write path and engages 3.9 normally. Parallels the §3.10 story-bundle carve-out below.
