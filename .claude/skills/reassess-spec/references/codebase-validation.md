...
2. **Enumerate** the source document's claims / recommendations: numbered findings (`F-01`, `R-7`), P-tier items (Priority 0 must-do, Priority 1 should-do), surgical-hole lists, "deterministic validators to strengthen" tables, or any other claim-set the document presents as actionable. For oversized source documents (token-limit exceeded on a single Read), prefer section-targeted reads of the claim-enumerating sections (grep section headers first, then read the relevant lists) over chunked end-to-end reads. **Use permissive grep anchoring for section-header lookup** — e.g., `grep -n '6\.13'` over the full file or `grep -nE '^#+ .*6\.13'` rather than strict heading-depth patterns like `^### 6.13` — since source reports may use bold-wrapped headings (`### **6.13 Branch-aware STCHAR supersession fix**`), alternate heading depths (a section the auditor expects at `###` may live at `####`), or non-heading anchors (indented bulleted lists with numeric prefixes, table-row labels, paragraph-leading bold spans) that strict-anchored greps miss. When the strict pattern returns zero matches but the section is known to exist by the spec's citation, fall back to the permissive pattern before concluding the section is absent — a zero-matches result from a strict pattern is not evidence the section doesn't exist, only that the operator's grep anchor was over-specific.
