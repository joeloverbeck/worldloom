...
> **Non-canon read-only structural validator carve-out**: adding read-only structural validators that enforce the integrity of a *non-canon surface* — *story-bundle* (Skill Category 2c) artifacts OR *world-system proposal/audit/pressure/world-proposal* artifacts (PR / BATCH / EPE / NCP / AU / RP / NWP / NWB maturity, index-consistency, or approval-field checks) — is not "modifying canon-pipeline semantics" — such validators mediate no world-canon reads/writes and change no canon-validator thresholds. The `### Canon-Pipeline Impact Rule` section is N/A for a spec whose only pipeline touch is new non-canon-surface read-only validators. **A validator that gates canon/hybrid writes (e.g., a pre-apply compatibility gate that blocks a patch plan) is NOT carved out** — it engages the Canon-Pipeline Impact Rule normally. (Parallels the §3.9 non-canon read-only structural validator carve-out in `references/codebase-validation.md`.)
