...
**Auto-stop carve-out**: Under auto-stop (the user has pre-authorized silent progression and explicitly waived the per-section approval gate — typically by directive language like *"work without stopping for clarifying questions"* or *"make the reasonable call and continue"*), the Phase breakdown exception's structured-batch requirement RELAXES — Section 6 must still be presented as 6a + 6b within the batch (the seam itself preserves the design's pre/post HARD-GATE structure), but Section 6 + Sections 7–9 MAY be collapsed into a single global single-message batch provided the Notes-on-the-shape discipline is preserved per section (rationale notes still appear for each section in the global batch). The exception's underlying purpose (preserve user review window per section) is structurally unsatisfied under auto-stop either way — the user has waived the interactive review cycle and the per-section Notes-on-the-shape rationales become the audit-trail substitute for staged approval. Parallel to the gap-filler interview's `Auto mode compression` sub-rule (per `references/gap-filler-interview.md` §Protocol), which relaxes the one-substantive-question-per-message rule under auto-mode for the same reason. Worked precedent: skill-streamlining-audit's compile run (this session) — auto-stop active; the Phase breakdown exception's strict reading would have forced two batches (Section 6 + Sections 7–9) with no approval pause between them, providing zero benefit since the user had waived approval; under this carve-out, all sections were emitted as one global batch with Section 6 structured as 6a + 6b internally, preserving the seam discipline while honoring the auto-stop directive.

**Phase breakdown exception**: Phase breakdowns routinely exceed 10 phases plus branch logic across all classes (canon-mutating consequence-propagation branches, canon-reading multi-sub-check Canon Safety phases, meta-tooling per-severity repair menus). When the phase breakdown exceeds ~8 phases OR contains explicit branch logic (accept/non-accept, pre/post-audit, pre/post Canon Safety Check), split into at most 2 presentations along a natural pipeline seam — pre/post Canon Safety Check [canon-reading with in-world output; canon-mutating], pre/post adjudication [canon-mutating], pre/post escalation gate [canon-mutating with multi-critic phases], or pre/post HARD-GATE [canon-mutating OR meta-tooling-with-HARD-GATE skills with single primary gate at the analysis-to-write boundary — e.g., a skill that gathers proposal evidence and runs critics, then gates a write transaction; the HARD-GATE itself is the natural seam between the analysis cluster and the write cluster. Worked precedents: `spec-to-tickets` (Step 4 analysis / Step 5 writes), `mcp-integration-audit` (Phase 6 analysis / Phase 7 writes)]. Never split at an arbitrary phase number.
...
[Worked precedent: branching-story-page-cycle's §Output section uses the Table shape across 14+ classes ranging from "Always" (PG-<integer>, SE-<integer>, rendered prose) to "IF Phase 4 JIT expansion fired" (SLT-<integer>) to "IF a new story-local location is introduced this turn" (STLOC-<integer>) — Flat-list would have collapsed the per-turn conditionality that maintainers need to read off the runtime contract.]
...
<Multi-directory aggregate pattern — optional sub-section. If the skill's Pre-flight
 ...
 `adjudications/PA-<integer>-accept*.md` across all worlds), declare the registry as
...
<Precondition checks that run before any pipeline phase:
 ...
 - for skills with monotonic-ID outputs at pipeline scope (e.g., NWP-<integer>, NWB-<integer>, or any pipeline-scoped class whose output lives at root-level rather than under `worlds/<slug>/`): allocate via `mcp__worldloom__allocate_next_id(world_slug='__pipeline__', id_class=...)`; if the index does not yet support the `__pipeline__` sentinel, fall back to a manual scan of the pipeline-scoped output directory and increment
 - for skills with monotonic-ID outputs at sub-world scope (e.g., per-story-bundle classes like PG-<integer> / SE-<integer> / OBL-<integer> whose output lives nested under `worlds/<slug>/stories/<story-slug>/_source/<class>/` — uniqueness is per-bundle, not per-world or per-pipeline): allocate via `mcp__worldloom__allocate_next_id(world_slug, id_class, story_slug=...)`. Distinct from world-scoped (e.g., CHAR-<integer>, AU-<integer>) which scans `worlds/<slug>/<class>/` and from pipeline-scoped (NWP-<integer>, NWB-<integer>) which uses the `__pipeline__` sentinel.
 - **Deferred-allocation pattern (acceptable for both pipeline-scoped and sub-world-scoped monotonic IDs)**: when the count of records is unknown until a downstream phase (e.g., `F-NN` findings allocated dynamically at consolidation per `story-skill-internal-coherence` Phase 7; ticket numbers allocated after HARD-GATE per-tier per `skill-streamlining-audit` Phase 9), Pre-flight reserves namespace context (prefix identification, source-directory readability) but defers per-record number allocation to the downstream phase. The skill's prose must explicitly name the allocation phase and the rationale in the Pre-flight Check section. The skill's HARD-GATE clauses must include the deferred-allocation step in their phase-range coverage (e.g., `Phase N has allocated the next free <PREFIX>-NNN integer per-tier`). Parallel to `references/foundations-conformance-check.md`'s ID allocation discipline acknowledgment.
...
## HARD-GATE drafting — numerical-citation and phase-range-citation discipline

When the HARD-GATE clause names a numerical count (e.g., "all N gates pass", "all M axes pass", "every one of N principles", "all P checks record PASS"), recount against the proposal's source enumeration BEFORE presenting Section 2.

**Phase-range citations**: when the HARD-GATE clause cites a phase range (e.g., "Phases X-Y have completed") AND the same condition asserts state that depends on work performed at later phases (e.g., "every finding records its `<field-set>`"), expand the range to cover all phases the condition depends on. Walk each clause's asserted state back to the phase that produces it; the phase range must cover every producing phase. The conformance check's Process Flow consistency check (per `references/foundations-conformance-check.md` §Universal Checks) catches phase-name drift between Process Flow ASCII and phase bodies but does NOT catch HARD-GATE-clause phase-range drift when the ASCII and bodies agree. Worked precedent: story-skill-internal-coherence's compile run drafted HARD-GATE condition (b) as "Phases 1-6 have completed... every finding records its `finding_id`, severity..."; field recording runs at Phase 7 consolidation; the conformance-check Process Flow consistency check did not catch the drift (the Process Flow ASCII and phase bodies both correctly named Phases 1-9 and matched each other; the drift was in the HARD-GATE clause's phase-range citation alone); reconciliation at write-time updated the citation to "Phases 1-7 have completed: Phases 1-6 detect findings; Phase 7 consolidates and populates fields." Catching at draft time would have foreclosed the reconciliation backstop firing.

Numerical citations that drift from the proposal will be caught later by the conformance check's Process Flow consistency check (per `references/foundations-conformance-check.md` §Universal Checks) and by Procedure §6's reconciliation precondition (per `SKILL.md`), but catching at draft time avoids the reconciliation-at-write-time backstop firing for purely arithmetic drift. The recount discipline applies to gate counts (Phase N's per-storylet validation gates), axis counts (Phase M's batch-level diversity audit axes), principle counts (FOUNDATIONS Validation Rules cited), and any other enumerated quantity the HARD-GATE clause references — wherever the count is sourced from a proposal table or list, re-walk the source and tally explicitly rather than transcribing a previously-stated count. Worked precedent: storylet-pool-authoring's Section 2 / Section 3 initially said "8 Phase 4 gates"; Section 6b's gate-table draft re-tallied to 9 (the proposal's Phase 4 table enumerates Mystery firewall, Resolution-authority declaration, Invariant compatibility, Consequence capacity, Dedup, Content-intensity coherence, Predicate DSL parsability, Branch-contamination, and Schema completeness — 9 gates); the reconciliation precondition resolved the meaning at write time, but a draft-time recount would have produced the correct count at Section 2 presentation. Distinct from the reconciliation precondition (which is the recovery-side backstop catching drift at write time per `SKILL.md` Procedure §6); this discipline is the prevention-side draft-time check.
