...
- [ ] **ID allocation discipline (pipeline-scoped output)**: if outputs use monotonic IDs at pipeline scope (e.g., NWP-<integer>, NWB-<integer>, or any class whose output lives at a root-level surface rather than under `worlds/<slug>/`): IDs allocated via `mcp__worldloom__allocate_next_id(world_slug='__pipeline__', id_class=...)` with a documented manual-scan fallback (scan the pipeline-scoped output directory and increment) for cases where the index does not yet support the `__pipeline__` sentinel; collisions abort with a specific-id error; dropped IDs (from user drop-list at commit) become permanent gaps and are never reused. Allocation may happen at Pre-flight OR at a downstream phase by design when the count of records is unknown until that phase (parallel to the meta-tooling §Pre-flight Check deferred-allocation rule below — e.g., ticket numbers allocated after HARD-GATE per-tier per `skill-streamlining-audit` Phase 9). N/A when outputs are world-scoped, sub-world-scoped (e.g., per-story-bundle IDs nested under `worlds/<slug>/stories/<story-slug>/_source/<class>/` such as PG-<integer> / SE-<integer> / OBL-<integer> — uniqueness is per-bundle, not per-world or per-pipeline), or non-monotonic. Sub-world-scoped story-bundle IDs allocate via `mcp__worldloom__allocate_next_id(world_slug, id_class, story_slug=...)`.
...
- [ ] If the skill reads world state OR allocates monotonic IDs for output artifacts: includes a **Pre-flight Check** section before Phase 0 that loads `docs/FOUNDATIONS.md` and verifies required world files are readable. ID allocation may happen at Pre-flight OR at a downstream phase by design — the count of records is sometimes unknown until that downstream phase (e.g., `F-NN` findings allocated dynamically at consolidation per `story-skill-internal-coherence` Phase 7; ticket numbers allocated after HARD-GATE per-tier per `skill-streamlining-audit` Phase 9). When deferred, the skill's prose must explicitly name the allocation phase and the rationale, and the HARD-GATE clauses must include the deferred-allocation step in their phase-range coverage.
...
- [ ] **ID allocation discipline** (if outputs use monotonic IDs like AU-<integer>, RP-<integer>): IDs allocated at pre-flight by scanning the existing output directory OR at a downstream phase by design when the count of records is unknown until that phase (parallel to the §Pre-flight Check deferred-allocation rule above); collisions abort with a specific-id error; dropped IDs (from user drop-list at commit) become permanent gaps and are never reused. Pipeline-scoped variant (root-level surfaces like NWP-<integer>, NWB-<integer>): see canon-reading §ID allocation discipline (pipeline-scoped output) above for allocation pattern. Sub-world-scoped story-bundle IDs allocate via `mcp__worldloom__allocate_next_id(world_slug, id_class, story_slug=...)`.
