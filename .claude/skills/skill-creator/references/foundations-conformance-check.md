...
- [ ] **ID allocation discipline (pipeline-scoped output)**: if outputs use monotonic IDs at pipeline scope (e.g., NWP-NNNN, NWB-NNNN, or any class whose output lives at a root-level surface rather than under `worlds/<slug>/`): IDs allocated via `mcp__worldloom__allocate_next_id(world_slug='__pipeline__', id_class=...)` with a documented manual-scan fallback (scan the pipeline-scoped output directory and increment) for cases where the index does not yet support the `__pipeline__` sentinel; collisions abort with a specific-id error; dropped IDs (from user drop-list at commit) become permanent gaps and are never reused. N/A when outputs are world-scoped, sub-world-scoped (e.g., per-story-bundle IDs nested under `worlds/<slug>/stories/<story-slug>/_source/<class>/` such as PG-NNNN / SE-NNNN / OBL-NNNN — uniqueness is per-bundle, not per-world or per-pipeline), or non-monotonic. For sub-world-scoped IDs, the manual-scan allocator scans the bundle subdirectory rather than the world `_source/` or pipeline-root surface; engine-routed allocation requires extending `mcp__worldloom__allocate_next_id` with a sub-world-key argument (precedent: `tickets/MCPENH-011` for the story-bundle ID-class extension supporting the branching-story skill family).
