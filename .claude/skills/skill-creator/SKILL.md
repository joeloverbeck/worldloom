...
- **Generated skill's runtime supporting files** — any non-world files the generated skill reads at runtime beyond `docs/FOUNDATIONS.md` (templates, configs, hook scripts, validator fixtures, archival-policy docs)? Each must appear in the generated skill's Pre-flight Check as a readability precondition with a clear abort-on-missing error, parallel to the World-State Prerequisites block but distinct in origin (pipeline infrastructure rather than world canon). This closes the failure mode where a generated skill silently fails at first use because a required supporting file was never written or was moved.
...
<Precondition checks that run before any pipeline phase:
 ...
 - for skills with runtime supporting files declared at Step 4 (Generated skill's runtime supporting files gap): verify each supporting file exists and is readable (e.g., `tickets/_TEMPLATE.md`, `tickets/README.md`, validator fixtures, hook scripts) — abort with a clear missing-file error on fail
