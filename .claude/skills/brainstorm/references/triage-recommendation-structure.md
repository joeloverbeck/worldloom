...
**User-elected skip (auditor-offered, user-declined-this-batch)**: when the triage offers an item as actionable (an `accept` / `accept-with-modification` candidate, or an optional bundle the operator recommends) and the user — via the close-out `AskUserQuestion` (§Close-out decision tree) or a direct instruction — declines it *for this batch* (e.g., "skip the polish bundle for now"), record it as `defer` with `deferred_to: "user declined this batch; re-raisable any time"`. Do NOT coin a separate "skipped at user request" verdict bucket — the seven verdicts are closed (per §Verdict types and the Cross-item fold / subsume disposition note below); a user-elected skip is a `defer` whose trigger is the user's batch-scoping choice rather than a codebase lift-condition. If the operator wants user-declined items grouped for reviewer visibility in the durable companion triage file, a labeled sub-grouping *within* the `defer` bucket (e.g., a "user-declined this batch" sub-heading) is acceptable, but it remains `defer`, not a new top-level bucket.
