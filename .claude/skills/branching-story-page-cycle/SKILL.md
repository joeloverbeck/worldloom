...
- **Future siblings (named in proposal; not yet shipping)**:
  - **`storylet-pool-authoring`** — ships at `.claude/skills/storylet-pool-authoring/` as the production authority for `seed` and `focus` mode batch authoring (the seed-mode entrypoint for `branching-story-bootstrap`'s Phase 6, currently consumed via inline shape until BSBOOT-002 lands). The `jit`-mode entrypoint for Phase 4 fallback storylet generation is pending BSPAG-001, which both extends the storylet skill with a `jit` mode and refactors this skill's Phase 4 to delegate. Until BSPAG-001 lands, this skill inlines the minimal JIT shape per the proposal's discipline.
