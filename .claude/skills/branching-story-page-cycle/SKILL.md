...
Pre-flight (resolve worlds/<world-slug>/stories/<story-slug>/;
            ...
            allocate next PG-NNNN via manual scan; detect fork →
...
- Allocate the next `PG-NNNN` for this story by manual filesystem scan over `_source/pages/PG-*.yaml` and incrementing the highest existing id. **Engine-routed allocation deferred per `tickets/MCPENH-011`**; the future MCP `allocate_next_id(world_slug, id_class='PG', story_slug)` API will replace the manual scan without skill rewriting.
- Detect fork: scan `_source/pages/PG-*.yaml` for any descendant whose `branch_path[..-1] == parent_page.branch_path` AND `branch_id == parent_page.branch_id`. If any descendant exists on the parent's branch, this run produces a NEW branch — allocate the next `BR-NNNN` via manual scan. Otherwise this run extends `parent_page.branch_id` (BR is updated via supersession at Phase 11, not new-allocated).
...
- Load premise-bounded world-canon retrieval via `mcp__worldloom__get_context_packet(world_slug, task_type='story_page_cycle', seed_nodes=[<resolved ids>], token_budget=18000)`. On `task_type='other'` fallback (per `tickets/MCPENH-012` deferral), include the same seed-node set explicitly. Apply the packet-too-large fallback per §World-State Prerequisites if the response signals overflow.
...
JIT generation is not free — it expands the engine prompt budget and may produce lower-quality storylets than the author pool. **Future-sibling seam**: when `storylet-pool-authoring` ships, refactor JIT generation to delegate to its JIT-mode entrypoint; until then, this skill inlines the minimal JIT shape per the proposal. `branching-story-health-audit` (also deferred) consumes the `flagged_for_audit` and high-JIT-rate signals.
...
**Future-sibling seam**: `story-fact-promotion-to-canon` does not yet exist on disk. Until it ships, a `canon_candidate` resolution at runtime aborts the page-cycle with a clear "blocked on `story-fact-promotion-to-canon` — pick a different choice or wait for the sibling skill" message. This is Shape (a) inline-with-seams discipline; aborting on `canon_candidate` rather than silently degrading to `apparent` preserves the canon-mutation HARD-GATE invariant.
...
| 5 | ID uniqueness | Allocated IDs do not collide with any existing record in this story (manual-scan defense; `tickets/MCPENH-011` will replace with engine-routed allocation that detects collisions at submit time) | Pre-flight |
...
- **Known integration debt** (deferred per Shape A; design exploits these once landed):
  - **`tickets/MCPENH-011`** — extend `mcp__worldloom__allocate_next_id` to support story-bundle ID classes (PG, SE, SF, OBL, CNSQ, THR, SREL, STINT, SLT, STLOC, STOBJ, BR, CHC, story-local DA). Until landed, manual filesystem scan inside `worlds/<slug>/stories/<story-slug>/_source/<class>/` is the allocator.
  - **`tickets/MCPENH-012`** — register `task_type='story_page_cycle'` in the context-packet retrieval profile registry. Until landed, `task_type='other'` with explicit entity-seed assembly is the fallback.
- **Future siblings (named in proposal; not yet shipping)**:
  - **`storylet-pool-authoring`** — the JIT-mode entrypoint for Phase 4 fallback storylet generation, and the seed-mode entrypoint for `branching-story-bootstrap`'s Phase 6. Until shipping, this skill inlines the minimal JIT shape per the proposal's discipline.
