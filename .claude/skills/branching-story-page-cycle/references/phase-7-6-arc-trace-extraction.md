...
Phase 7.6 runs after Phase 7's plan-completeness check and before Phase 8 decides whether a choice surface is needed. Phase 7.6 at plan-commit runs ONLY **Layer 1 (deterministic structural validation)** over the plan's frontmatter (`declared_intended_beats[]`, `declared_stop_condition`, `forbidden_resolutions[]`, `chosen_variant_id`, `required_effects[]`) and the Phase-4-selected arc record held in engine context. The plan body's §15 must inline the resolvable SLT id plus prose-direction translation, but it does not need to inline the full SLT schema body. **Layer 2 (post-render trace extraction)** and **Layer 3 (semantic conformance critic)** require rendered prose to extract evidence_spans from — they are DEFERRED to `branching-story-page-prose-finalize` Phase 4.
...
- The Phase 7 plan buffer: the populated copy of `.claude/skills/_shared-templates/page-plan.md` (frontmatter + body) not yet written to disk. §15 body must contain the selected `SLT-NNNN` id and prose-direction translation.
- The Phase-4-selected arc record (`SLT-NNNN`) from engine context, including `arc.beat_plan`, `arc.execution_envelope`, `arc.stop_policy`, and `arc.effect_model.variants[]`.
...
| Branch-scope legality | Every inlined record id (SF, OBL, CNSQ, THR, SREL, STINT, SLT, STLOC, STOBJ, BR) in the plan body either has `created_at_page == null` (globally legal — author-pool storylets only) OR `created_at_page ∈ this_page.branch_path`. For §15, the SLT id in the prose-direction translation is the inlined body reference; the full SLT schema body is not required. Sibling-branch references are rejected. | Route to Phase 5; the illegal reference indicates state-mutation drift. |
...
- `arc_envelope_conformance` — Layer-1-only at plan-commit; the deterministic check that the plan's frontmatter and Phase-4-selected arc record are internally consistent. The §15 body provides the resolvable SLT id and prose-direction translation, not the full arc schema.
