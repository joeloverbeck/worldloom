...
Phase 7: Canon Safety Check
         ...
         7b: Mystery Reserve firewall       (vs M-<integer> records; explicit list)
...
Normalize `world_slug` (strip `worlds/` prefix; verify `[a-z0-9-]+`). Allocate the next character id: `mcp__worldloom__allocate_next_id(world_slug, 'CHAR')` → `CHAR-<integer>`. Load the context packet plus the whole-class INV / M Phase 7 firewall sets (per §World-State Prerequisites). Read `worlds/<slug>/ONTOLOGY.md` and `worlds/<slug>/WORLD_KERNEL.md` directly — both remain primary-authored at the world root.
