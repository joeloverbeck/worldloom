...
`docs/FOUNDATIONS.md` plus the world-state slice the brief touches load via `mcp__worldloom__get_context_packet(task_type='character_generation', seed_nodes=[<brief-derived seed nodes>], token_budget=10000)` per `docs/CONTEXT-PACKET-CONTRACT.md`. The packet delivers Kernel + invariants + relevant CFs + Mystery Reserve entries touching the character's domain + named-entity neighbors + section context with completeness guarantees. Direct `Read` of `_source/<subdir>/` is redirected to MCP retrieval by Hook 2 — do not bulk-read. For specific records, use `mcp__worldloom__get_record(record_id)`. For domain-filtered CF lookups during Phase 5 / 7c, use `mcp__worldloom__search_nodes(node_type='canon_fact', filters=...)`. For named-entity binding during Phase 0 (resolve current_location / place_of_origin / institution names), use `mcp__worldloom__find_named_entities(names)` followed by `get_neighbors` for the resolved nodes. ONTOLOGY categories load via `Read worlds/<slug>/ONTOLOGY.md` (primary-authored, unchanged by SPEC-13).
