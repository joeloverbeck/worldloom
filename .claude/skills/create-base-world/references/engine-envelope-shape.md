...
| `stale_index` (retrieval-time) | Returned by `mcp__worldloom__get_context_packet`, `get_record`, `find_named_entities`, and other retrieval tools when the world index is stale relative to source files. `details.drifted_files[]` (or `divergent_files[]`) names the stale paths. The staleness can pre-date the current skill's run — e.g., a prior session edited a hybrid-file frontmatter without index sync. Pre-flight retrieval is the more common firing point because it is the first MCP call most skills make. | Run `node tools/world-index/dist/src/cli.js sync <world-slug>` and retry the retrieval call. The recovery is identical to the submit-time `index_stale` handling above. |
