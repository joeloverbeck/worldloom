...
- Invariant / Mystery / Open Question / Named Entity / Section schemas — engine-owned, validated by `record_schema_compliance`; FOUNDATIONS.md §Invariants and §Mystery Reserve are the authoritative human-facing references. Engine schema source: `tools/patch-engine/src/ops/types.ts` (and per-op modules under `tools/patch-engine/src/ops/`). Operationally-relevant fields per record type:
  ...
  - **Open Question** (`oq_record`): `id` (e.g., `OQ-0001`); `topic`; `body`; `when_to_resolve`; `caution` (optional, may be empty string); `extensions: []`.
  - **Named Entity** (`ent_record`): `id` (e.g., `ENT-0001`); `canonical_name`; `entity_kind` (per FOUNDATIONS §Ontology Categories: `region` / `polity` / `species` / `place` / `institution` / `route` / etc.); `aliases[]`; `originating_cf` (`null` for genesis entities; `CF-NNNN` for entities introduced by later canon-addition runs); `scope_notes`.
  - **Prose Section** (`sec_record`): `id` (e.g., `SEC-GEO-001`); `file_class` (UPPER_SNAKE: `GEOGRAPHY` / `PEOPLES_AND_SPECIES` / `INSTITUTIONS` / `ECONOMY_AND_RESOURCES` / `MAGIC_OR_TECH_SYSTEMS` / `EVERYDAY_LIFE` / `TIMELINE`); `order` (1-based ordinal within file class; `1` for genesis sections); `heading`; `heading_level` (typically `2` for top-level H2 sections); `body` (markdown prose); `extensions: []`; `touched_by_cf[]` (CF IDs whose `required_world_updates` lists this `file_class`; bidirectional pointer enforced by `touched_by_cf_completeness` validator).
