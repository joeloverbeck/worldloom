...
|-----------|-------|-----------|
| Tooling Recommendation (non-negotiable) | Pre-flight | `docs/FOUNDATIONS.md` + `WORLD_KERNEL.md` + `ONTOLOGY.md` (direct Read at world root) + atomic-record world-state slice via `mcp__worldloom__get_context_packet(task_type='propose_new_characters', ...)` (Kernel concepts + INV records + relevant CFs + named-entity neighbors + section context); Person Registry from `list_records(record_type='character_record'|'diegetic_artifact_record'|'adjudication_record', include_full_body=true)` frontmatter/body-section metadata, with `find_named_entities` for named-persona resolution during Phases 1–10 |
...
| Canonical Storage Layer (SPEC-13) | Pre-flight + Phase 1 | All `_source/<subdir>/*.yaml` reads via MCP retrieval — no bulk reads of atomic-record subdirectories. Existing world-level hybrid records (`characters/`, `diegetic-artifacts/`, `adjudications/`) enumerate through `list_records`; targeted direct reads remain permitted for authored proposal outputs and file-collision checks under Hook 3's hybrid-file allowlist. |
