...
After the engine returns a `PatchReceipt`, iterate `receipt.files_written[]` and verify `new_hash != prior_hash` for every file the envelope intended to mutate (every PG file targeted by an `update_record_field` op; newly-created SE / ARCTRACE files are exempt because `prior_hash` is empty for creates). On match, HALT before §INDEX.md Edit with the citation `submit_silent_no_op: file <path> prior_hash == new_hash despite envelope intending mutation; engine accepted ops but staging produced no byte-level change. Likely a staging-cache bug (see SKILL.md §Known Integration Debt for PEENH-006 if the affected file is a story-bundle record). Investigate before flipping INDEX.md.` The receipt's `ops_applied` count alone is not sufficient: the staging-cache bug at `tools/patch-engine/src/commit/temp-file.ts:141-164` produces `ops_applied: 1` (engine counts the staged write) while the file content is byte-identical to the prior version.
...
| Phase 7 §5 submit succeeds but `new_hash == prior_hash` for an intended-mutation file | `_source/` YAML accepted by engine but byte-identical to prior version; INDEX.md not yet edited; temp envelope + token persist | Inspect `receipt.files_written[]`. For each intended-mutation file with `new_hash == prior_hash`, the engine's staging-cache likely has a bug (PEENH-006 for story-bundle records). Halt before §INDEX.md Edit. Investigate the staging path; if the file is a story-bundle record and PEENH-006 hasn't landed, apply the §Known Integration Debt workaround (single-op envelopes with namespaced `target_record_id`) in `SKILL.md`. |
