...
3. **Name the delta in the frontmatter `date` field**: "approximately N years before CHAR-<integer> dossier-present" plus the TIMELINE-layer anchor. This makes the back-projection reconstructable for any future reader without requiring them to re-derive it from two files.
...
**Chronology-coherence revisions during Phase 6 composition.** When the Phase 6 draft introduces narrator-cadence numerals (counts of years, repeat-event counts, mourning-interval lengths) that derive from dossier-present-counted intervals and don't compute at artifact-date, revise each numeral to either (a) the artifact-date-correct count, or (b) a marker that doesn't depend on a specific count (e.g., *"since he died"* in place of *"in six winters"*; *"the times I have done it I can count"* in place of *"four times in thirty years"*). Record each revision in `notes` under a *"Chronology-coherence revisions during Phase 6"* line, framed as *"preserved: <brief intent> / sacrificed: <numeral>"* parallel to the Phase 7f repair format. **These revisions are categorically distinct from Phase 7f canon-safety repairs** — they are back-projection consequences during composition, not canon-violation fixes. The Phase 9 deliverable summary reports them as a separate item from Phase 7f repairs (see `SKILL.md` §Procedure step 7) so the user can distinguish *"no canon-safety repairs fired"* from *"revisions of any kind fired"*.

**Voice-author + framing-author sub-pattern.** When the artifact_type is interview-transcription, oral-history record, dictated-monologue letter, ethnographic interview, prison confession recorded by a clerk, or any artifact_type that captures a SPEAKER's voice within an EDITORIAL FRAME by a different person, the speaker-voice and the framing-author are categorically distinct and Phase 0b handles them separately:

(a) **Speaker is the Phase 0b author for voice purposes.** When `character_path` is provided, treat the dossier as the SPEAKER. Lift the 15-field profile from the dossier per the lift table above. Set `author_character_id` to the speaker's CHAR-id.

(b) **Framing-author is constructed as Phase 0c cast.** The framing-author (the editor / interviewer / clerk / publisher who carried the speaker's voice into the artifact's material form) is constructed under Phase 0c sizing/naming/recording rules — a single named figure at author-personal-scope, no new institutions / polities / species clusters / ritual systems introduced. Sized to the brief's anchor (count-of-one for a single named editor-author; the brief specifies the framing-author's profession, prior work register, and audience-standing).

(c) **Frontmatter `author` field names both.** Use the convention *"<speaker> (as recorded by <framing-author>)"* or *"<speaker> (in <framing-author>'s book *<book-title>*)"*. The frontmatter `author_character_id` binds to the speaker (the voice). The framing-author byline appears in `statement_of_existence` and `notes` only — NOT in the artifact body.

(d) **Phase 5 carries TWO bias layers.** The speaker's bias (lifted from dossier) AND the framing-author's editorial bias (what they kept, what they cut, what they translated, which questions they asked in an interview, which audience-framing they imposed). Both layers shape what appears in the body. See `references/phases-4-6-text-composition.md` §Phase 5 for the dual-bias treatment and §Phase 6 for composing in the speaker's voice within the framing-author's editorial conventions.

(e) **Phase 7d.1 author-personal-scope allowance covers the framing-author.** A single named editor / interviewer / publisher does not violate Phase 7d.1 (no silent canon creation) — they are a bounded personal-scope figure, not a larger-scope canon entity. The book's title (if invented) follows the same author-personal-scope rule.
