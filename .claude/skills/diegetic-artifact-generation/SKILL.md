...
Normalize `world_slug` (strip `worlds/` prefix; verify `[a-z0-9-]+`). Allocate the next artifact id: `mcp__worldloom__allocate_next_id(world_slug, 'DA')` → `DA-NNNN`. Load the context packet plus the whole-class INV / M Phase 7 firewall sets (per §World-State Prerequisites). Read `worlds/<slug>/ONTOLOGY.md` and `worlds/<slug>/WORLD_KERNEL.md` directly. Read the template `.claude/skills/diegetic-artifact-generation/templates/diegetic-artifact.md` to anchor the frontmatter schema for the Phase 9 write.
