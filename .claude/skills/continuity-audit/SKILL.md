...
1. **Pre-flight.** Normalize `world_slug` (strip `worlds/` prefix; verify `[a-z0-9-]+`). Allocate `AU-NNNN` via `mcp__worldloom__allocate_next_id(world_slug, 'AU')`; do NOT pre-allocate RP ids — allocate lazily at Phase 8 per emitted card. Load the context packet (per §World-State Prerequisites). Look up canonical vocabularies via `mcp__worldloom__get_canonical_vocabulary({class})` for `domain`, `verdict`, `mystery_status`, `mystery_resolution_safety`, `invariant_category`, `entity_kind`, `sec_file_class`, `change_type`, `mystery_reserve_effect`, and `revision_difficulty` so any retcon card's `domains_affected`, change-log/proposal repair type, Mystery Reserve effect classification, and narrowed-mystery extension references are validated at reasoning time. Read existing `audits/INDEX.md` if present; record its current content for Phase 13 append.
