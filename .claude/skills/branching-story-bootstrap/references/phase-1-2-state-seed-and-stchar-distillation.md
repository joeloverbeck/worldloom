...
3. For retained `Stable Source Material Inventory` rows, set `operational_home` to exactly one operational STCHAR H2 section name from this closed list: `Story-Facing Identity`, `Stable Persona Core`, `Emotional Appraisal Map`, `Pressure Behavior`, `Voice Bible / Dialogue Authority`, `Page-Plan Voice Block`, `Perception and Embodiment`, `Agency and Planning Tendencies`, `Relationship-Specific Behavior`, `Story-State Derivation Guide`, or `Prose Rendering Constraints`. Do not use compounds such as `Stable Persona Core; Emotional Appraisal Map`, parentheticals such as `Prose Rendering Constraints (Signature scene behaviors)`, abbreviations such as `Agency limits`, or the non-operational sections `Source Distillation` and `Validation / Audit Anchors`.
4. Draft frontmatter against `tools/validators/src/schemas/story-character-authority.schema.json`: `source_kind: world_char`, `source_char_id: <CHAR-id>`, `source_char_sections_used[]`, `source_operational_fact_map[]`, `generated_at_page: story_bootstrap`, `created_by_skill: branching-story-bootstrap`, `supersedes: null`, `status: active`, `bound_stent_ids: [<future STENT id>]`, `profile_revision: 1`, and `body_schema_version: stchar.v1`. For each present structured `dramatic_core` field, include one `source_operational_fact_map` entry with disposition `copied`, `transformed`, `compressed`, `omitted_with_rationale`, or `story_irrelevant`; retained facts must name exactly one operational STCHAR `target_section` from the same 11-name list used for `operational_home`, while omitted or story-irrelevant facts must carry a rationale.
5. For the `cannot_be_swapped_out_because` source field, normally use disposition `compressed` or `transformed` with `target_section: "Stable Persona Core"` because it summarizes the character's world-grounded irreplaceability. If the field is genuinely not operational for the story, omit it with a lawful rationale. Never target `Validation / Audit Anchors`.
6. For `source_kind: world_char`, include both preservation layers: `source_operational_fact_map` for the 10 `dramatic_core` fields and `Stable Source Material Inventory` for stable operational material from all loaded sections.
7. Validate the full STCHAR profile before proceeding. If any required selected cast member cannot produce a schema-valid STCHAR with all required body sections, abort before creating any story state or direct-write artifact.
