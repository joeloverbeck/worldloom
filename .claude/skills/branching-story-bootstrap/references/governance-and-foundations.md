...
|---|---|---|
| Rule 1 (No Floating Facts) | mirror-facts, page-plan, validation | Shared contract §4 record schemas; validation step's plan-grounding gate (gate 7). |
...
| §Story Bundles §4a (Plan-Authority Boundary) | root-event-and-page, page-plan, commit | Story state is authoritative at PG-1 commit; no `pages-prose/PG-1.md` is written by this skill; the page snapshot is the fork primitive. No ARC_TRACE record emitted. |
...
| §Story Bundles §6.1 (Story-Local Character Authority) | stchar-distillation, beliefs, root-event-and-page, page-plan, validation | Bootstrap distills selected cast `CHAR` into STCHAR before state creation; non-background `STENT` uses `bound_stchar_id`; `PG.active_records.STCHAR` and §16a packets carry runtime authority; `source_char_id` remains provenance only. |
...
- **Verbatim §2 / §3 / §19 of the page plan.** The page-plan-authoring phase inlines `docs/prose-renderer-contract/content-policy.md` / `docs/prose-renderer-contract/prose-craft-contract.md` / `docs/prose-renderer-contract/render-time-instruction.md` verbatim; see `references/phase-8-9-page-plan-and-choices.md` for the cold-context rationale. Compacting these sections would defeat the self-contained-plan contract.
