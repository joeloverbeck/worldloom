...
| 11 | `plan_self_containment` | The plan inlines (rather than bare-references) every CF / CHAR / SF / OBL / THR / SREL / STINT / STLOC / STOBJ / SLT / DA / M / INV id that appears in any plan section. Default: referenced record bodies are inlined verbatim. For canonical prose-direction sections (§10 OBL, §11 THR, §12 CNSQ, §15 SLT in selected-arc page-cycle plans), the record id plus the template-specified prose-direction translation satisfies self-containment; dumping engine-only fields is not required. A bare `CF-NNNN` reference in §5 without the CF record body inlined fails; a bare `OBL-NNNN` reference in §10 without the OBL id plus prose-direction obligation context fails; etc. The plan IS the prompt — the external prose renderer reads only the plan file, so unrendered record context corrupts prose silently. | Phase 7 |
