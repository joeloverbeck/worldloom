...
**Initial intention snapshot per major character**: emit `STINT-NNNN.yaml` (bare-numeric id per the patch engine's `^STINT-\d{4}$` contract; `stent_id` points to the story entity this snapshot drives, with `world_character_id` carrying the world CHAR anchor or `null` for story-only cast) with `goals`, `fears`, `secrets[SF-NNNN]`, `beliefs[SF-NNNN]`, `relationships{STENT-id: state}`, `emotional_state`, `current_pressure: 0..10`, `traits`, `values{axis: weight}`, `created_at_page: PG-0001`.
