...
Use `storylet-pool-authoring` as an in-memory sub-routine to generate `target_pool_size` (computed below) approved `SLT-NNNN` records for `_source/storylets/`. Every returned seed is a scene-commitment arc (`record_version: 2`, `shape: scene_commitment_arc`) with an `arc_contract.commitment_family`, closed base `arc_contract.commitment_class`, and optional `arc_contract.commitment_detail`.
