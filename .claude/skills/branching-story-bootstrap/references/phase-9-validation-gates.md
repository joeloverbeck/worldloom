...
Reference for `branching-story-bootstrap` Phase 9 — the 17-gate canon-safety audit that gates HARD-GATE Phase 10 approval. Each gate must record PASS with a one-line rationale into `STORY_KERNEL.md`'s `validation_trace` field. Any FAIL halts the bootstrap and routes to the responsible upstream phase. A bare "PASS" without rationale is treated as FAIL per the FOUNDATIONS skill discipline.
...
| 13 | `arc_envelope_conformance` | PG-0001 root special case: no arc has been selected, so the gate auto-PASSes with rationale `"PG-0001 root special case — no arc selected"`. Non-root enforcement is owned by page-cycle Phase 9. | Phase 7 |
| 14 | `effect_model_replay_safety` | PG-0001 root-page exception accepts `state_snapshot.applied_effect_variant: null` when `id == PG-0001`; no effect model replay occurs at the scene-setter. | Phase 7 |
| 15 | `arc_trace_evidence_alignment` | PG-0001 root special case: no ARC_TRACE is emitted, so the gate auto-PASSes with rationale that there is no trace evidence to align. | Phase 7 |
| 16 | `narrative_point_classification` | PG-0001 defaults to `NATURAL_COMMITMENT_HINGE` because it is the first commitment surface; the closed-enum value is recorded on `state_snapshot.narrative_point_classification`. | Phase 8 |
| 17 | `choice_worthiness_completeness` | Every `choice_kind: scene_commitment` CHC emitted at PG-0001 has non-empty `likely_effects` and a populated `choice_worthiness` block; the displayed menu collectively covers at least two distinct `strong_axes`. | Phase 8 |
