...
Consume the Distillation Boundary Ledger before root `PG` and choice authoring. Every opening-current fact identified as temporal state must be represented in the appropriate initial record class, omitted as genuinely non-load-bearing, or rejected as unsupported; it must not be smuggled into STCHAR as durable authority.
...
- scene-facing presentation and current voice modulation -> downstream scene plan, grounded in active records
