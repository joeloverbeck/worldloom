...
Bootstrap a new branching-story bundle inside an existing worldloom world — initialize root causal state, commit the root page snapshot, and emit the first choices. Bootstrap does not author prose plans, does not render prose, and does not establish any prose-rendering lifecycle.

<HARD-GATE>
Do NOT write any of `worlds/<world_slug>/stories/<story_slug>/STORY_KERNEL.md`, `worlds/<world_slug>/stories/<story_slug>/INDEX.md`, or `worlds/<world_slug>/stories/INDEX.md`, AND do NOT submit any patch plan to `mcp__worldloom__submit_patch_plan`, until:

(a) Pre-flight Check has completed: world resolved, story-slug collision-free against `worlds/<world_slug>/stories/<story_slug>/`, selected cast resolved to existing `CHAR-*` dossiers, all bootstrap ids allocated in one ordered `mcp__worldloom__allocate_many_ids(world_slug, allocations=[...])` call when available (including one `STCHAR` id for every selected non-background cast member, optional root pressure / secret / open-setup ids when premise-warranted, `STORY`, and every remaining story-bundle class to be drafted), falling back to equivalent ordered `mcp__worldloom__allocate_next_id` calls only if the running server has not been restarted with `allocate_many_ids`, context packet loaded via `mcp__worldloom__get_context_packet(world_slug, task_type='story_bootstrap', ...)`, and the canonical contract sources (`docs/FOUNDATIONS.md`, `.claude/skills/_shared-templates/story-state-contract.md`, `.claude/skills/_shared-templates/story-record-schemas.md`, `tools/validators/src/schemas/story-character-authority.schema.json`) are loaded.

(b) Phases 1-8 have completed in working memory: state seed normalized; Phase 1b Distillation Boundary Ledger drafted and checked before any STCHAR or opening state record is finalized; one schema-valid `STCHAR` profile drafted from the ledger's Stable -> STCHAR row and validated for every selected non-background cast member before any STENT, temporal, page, choice, or direct-write artifact is finalized; one `STENT` per cast member drafted with `bound_stchar_id` for every non-background role; one `STSTAT` per active `STENT` plus STINT / SF / BEL / OBL / CNSQ / THR / SREL / STLOC / STOBJ / (optional CLK / STSEC / STQ / STPLAN / STEMO / DA) / (optional SLT) seed records drafted from the ledger's opening-current routing rows; `SE-1` drafted; planless `PG-1` drafted with full `state_snapshot`, `active_records.STCHAR`, and `validation_trace`; `STORY_KERNEL.md` drafted with the required section set including `## Player Agency Contract`; 3-5 `CHC` records drafted.

(c) Phase 9 has validated all 8 shared hard gates per `.claude/skills/_shared-templates/story-state-contract.md` §7 with a one-line PASS rationale per gate on `PG-1.validation_trace`, plus the 5 bootstrap-additional checks (cast resolution to existing `CHAR` dossiers; every required STCHAR generated and validated before any story state is created; no mirrored SF globalizes its parent CF scope; continuation capacity satisfied — at least one eligible seed SLT or a planned runtime JIT path; terminal root rejected as authoring error; pool coverage (cast-role + driver-kind + pressure-source-class + composition per SPEC-80 §3) — fails when any axis is unmet at `seed_commitment_blocks: minimal | standard`, bypassed at `seed_commitment_blocks: none`).

(d) The user has explicitly approved the deliverable summary (bundle path, cast roster with `STENT` / `STCHAR` / source-`CHAR` provenance, record inventory by class, STCHAR profile inventory, emitted choices list).
...
```
Pre-flight Check (FOUNDATIONS + shared contracts loaded;
...
Phase 8: Generate first choices → CHC records (in memory)
        |
        v
Phase 9: Validate against shared 8 hard gates + 5 bootstrap-additional checks;
...
1. Load `references/pre-flight-and-prerequisites.md` and run the Pre-flight Check (FOUNDATIONS + shared contracts loaded; world resolved; cast verified; bundle-target collision-free; STCHAR + all other ids allocated; context packet loaded). Abort on any precondition failure before drafting begins.
...
6. Load `references/phase-7-root-event-and-page.md`. Draft `SE-1` (`event_kind: story_start`) and planless `PG-1` (root page snapshot with `branch_path: ["PG-1"]`, full `state_snapshot`, and placeholder `state_hash` to be finalized at Phase 9) in working memory (Phase 7). In parallel, draft `STORY_KERNEL.md` per `references/story-kernel-contract.md`.
   - PG-1 `SE-1` keeps `event_kind: story_start` for the standard opening pattern where PG-1 emits choices and waits for player input. Because `story_start` forbids `turn_driver`, PG-1 carries no turn-driver / initiative trace in this standard case (the former page-plan §7a "Turn driver / initiative trace" section is retired with the 19-section page-plan contract — see shared contract §8; dispositions now live on `SE.turn_driver`).
...
7. Load `references/phase-8-9-page-plan-and-choices.md`. Emit 3-5 `CHC` records covering distinct commitment axes (no placeholder write-in CHC — write-in is a turn-cycle skill input via `manual_action_text`, orthogonal to the CHC pool), each with `grounded_in.records` populated from `PG-1.state_snapshot.active_records` (Phase 8).
8. Load `references/phase-10-validation.md`. Run the 8 shared hard gates (record one-line PASS rationale per gate on `PG-1.validation_trace`) plus the 5 bootstrap-additional checks, including shared contract §5a.3 public-BEL / non-propagation discharge for triggered witness coverage and SPEC-80 pool coverage (recorded in working memory). After every gate and check passes, assemble the envelope JSON, extract `patches[N].payload.record` for `PG-1` to a JSON scratch file, and compute final `PG-1.state_hash` via `tools/world-mcp/dist/src/cli/compute-pg-hashes.js --pg <PG-1-record.json>` (Phase 9). Abort before commit on any gate, additional-check, hash failure, or YAML `--pg` input.
9. **Phase 11: Commit / Write — HARD-GATE fires.**
   ...
   6. On patch success, write `STORY_KERNEL.md`.
   7. Update bundle `INDEX.md`, then per-world `stories/INDEX.md` (first-run create or append). The bundle `INDEX.md` at first run contains: a Bundle Identity table (World / Story slug / Story ID / Root branch / Root page / Genesis event / Created), a Cast Roster table (STENT / STCHAR / source CHAR provenance / Display name / Role in story), a Story Character Authority table (STCHAR / status / source kind / source CHAR provenance / bound STENT / profile revision / hashes), and one section per non-empty record class — Branches / Pages / Active Threads / Open Obligations / Pending Consequences / Active Clocks / Story Secrets / Open Setups / Relationships / Story-Local Facts / Story-Local Beliefs / Locations / Objects / Commitment Block Pool / `## Emitted Choices at PG-1` / Mystery Reserve at Bundle Scope. The per-world `stories/INDEX.md` at first run contains a Bundles table listing the new bundle's slug, story_id, root page, and created date; subsequent bundles append rows. Bootstrap defines this convention; downstream skills (`branching-story-turn-cycle` and others) inherit and extend it.
   8. Report bundle path + record inventory to the user. Do NOT `git commit`.
...
- **Never write rendered prose or render plans at bootstrap.** Page prose and scene prose artifacts are authored by downstream render workflows; bootstrap writes state records, `STORY_KERNEL.md`, and indexes only.
...
Bootstrap initializes a story bundle's root causal state without authoring a render plan, rendering prose, or establishing any prose-rendering lifecycle — the page snapshot is the fork primitive, and prose is the downstream scene-render layer's artifact, not bootstrap's responsibility.
