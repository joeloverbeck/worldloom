...
- `tools/world-mcp/dist/src/cli/compute-pg-hashes.js` — canonical CLI for deterministic PG hash computation per shared contract §4.2a "Tooling" subsection; consumed at Phase 10. See Phase 10 step 1 for the shared-helper contract and the no-hand-rolling rule.
...
4. Validate the full STCHAR profile before proceeding. If any required selected cast member cannot produce a schema-valid STCHAR with all required body sections, abort before creating any story state or direct-write artifact.
...
- **Verbatim §2 / §3 / §19 of the page plan.** Phase 8 inlines `reports/prose-quality-instructions.md` §Content Policy / §Prose Craft Contract / §Render-Time Instruction Template verbatim; see Phase 8 for the cold-context rationale. Compacting these sections would defeat the self-contained-plan contract.
