...
3. Load `references/phase-3-4-facts-beliefs-da.md`. Mirror load-bearing world facts into `SF` records (Phase 3); consume the Distillation Boundary Ledger to create initial `BEL` records, one `STENT` per cast member with the canonical `role_in_story` and `bound_stchar_id`, and one initial `STSTAT` per active STENT; apply DA triage at opening (Phase 4).
4. Load `references/phase-5-debts-and-optional-seeds.md`. Consume the Distillation Boundary Ledger to create 1-3 `THR` plus the constrained `OBL` / `CNSQ` / `SREL` records, applying the cross-class-provenance rule and the good-debt-vs-bad-debt filter; seed any premise-warranted optional CLK / STSEC / STQ / STPLAN / STEMO records (Phase 5).
