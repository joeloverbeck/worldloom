...
7. Load `references/phase-8-9-page-plan-and-choices.md`. Author the root page plan as future `pages-prose-plans/PG-1.md` bytes — all 19 numbered sections plus optional §9b / §9c / §10b when relevant active records exist, mandatory §16a STCHAR packets that project stable STCHAR authority through active opening state, verbatim §2 / §3 / §19 from `docs/prose-renderer-contract/content-policy.md`, `docs/prose-renderer-contract/prose-craft-contract.md`, and `docs/prose-renderer-contract/render-time-instruction.md` respectively (Phase 8); then emit 3-5 `CHC` records covering distinct commitment axes (no placeholder write-in CHC — write-in is a turn-cycle skill input via `manual_action_text`, orthogonal to the CHC pool), each with `grounded_in.records` populated from `PG-1.state_snapshot.active_records` (Phase 9).
8. Load `references/phase-10-validation.md`. Run the 8 shared hard gates (record one-line PASS rationale per gate on `PG-1.validation_trace`) plus the 6 bootstrap-additional checks, including shared contract §5a.3 public-BEL / non-propagation discharge for triggered witness coverage and SPEC-80 pool coverage (recorded in working memory). After every gate and check passes, assemble the envelope JSON, extract `patches[N].payload.record` for `PG-1` to a JSON scratch file, and compute final `PG-1.plan.plan_hash` and `PG-1.state_hash` via `tools/world-mcp/dist/src/cli/compute-pg-hashes.js` (Phase 10). Abort before commit on any gate, additional-check, hash failure, or YAML `--pg` input.
...
- **Verbatim §2 / §3 / §19 of the page plan** — inlined from `docs/prose-renderer-contract/content-policy.md`, `docs/prose-renderer-contract/prose-craft-contract.md`, and `docs/prose-renderer-contract/render-time-instruction.md` respectively so each cold-context render is self-contained.
