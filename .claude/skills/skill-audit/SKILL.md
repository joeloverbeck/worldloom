...
1. **Read the target skill** — Read the SKILL.md file at the provided path. Parse its name, description, and full content. Load `references/audit-execution-discipline.md` for read-satisfaction rules, path-resolution fallbacks, auxiliary-investigation permissions, and announcement/batching rules — these govern every tool call from here through Step 8.
...
4. **Cross-check alignment** — For each finding from step 3, check whether the skill contradicts or fails to implement:
   ...
   - Apply the Skill Category Classification (Categories 1, 2, 2b, 3) from `references/cross-skill-consistency.md` to determine whether FOUNDATIONS-alignment is N/A (Category 1, meta-tooling) or detailed alignment is required (Categories 2 / 2b / 3).
