...
6. **Severity-tag each finding** — CRITICAL / HIGH / MEDIUM / LOW. Use this rubric:
   ...
   **Convention-restatement check (companion trigger)** — when a Suggestion uses primary+cascade framing AND the targeted Primary section contains an enumeration list (categorized examples, named classes, ordered rules), grep the same file at audit time for OTHER enumeration sites that name the same items — see `references/audit-execution-discipline.md` §Convention-restatement check. Distinct from the §Site-counting trigger above: §Site-counting keys on Suggestion-field phrasing (parallel-instantiation patterns); §Convention-restatement keys on the targeted file's structural restatement pattern. Each restatement site should appear in the Suggestion as a co-equal primary (`<finding>.a / <finding>.b / ...`) rather than left for implementation-phase scope-expansion.
