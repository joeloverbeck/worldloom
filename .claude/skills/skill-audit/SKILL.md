...
6. **Severity-tag each finding** — CRITICAL / HIGH / MEDIUM / LOW. Use this rubric:
   ...
   **Pre-finalization verification trigger check** — for each finding tagged MEDIUM or higher, scan its Suggestion and Skill-gap fields for the absence-claim trigger phrasings enumerated in `references/audit-execution-discipline.md` §Pre-finalization verification (`"Add X"`, `"Document the missing Y"`, `"There is no documented Z"`, `"no fallback exists"`, `"the skill never mentions W"`, `"undefined"`, `"doesn't reference"`, `"not documented"`, mis-cited section/line at MEDIUM-or-higher, plus pipeline-state absence claims like `"does not exist at HEAD"` or `"no hook/validator/op covers X"`). When any trigger fires, run the verification (Read or grep of the cited file/section) BEFORE finalizing the finding's wording. The 30-second-per-finding cost prevents the implementation-phase Re-evaluation retraction cycle the rule warns about: catching a falsified premise at audit time keeps the audit report's premise true to current state, while catching it at implementation phase forces a Re-evaluation amendment that the user reads only after they have already authorized implementation. Pre-finalization verification is the prevention; Re-evaluation is the recovery.
   ...
   **Convention-restatement check (companion trigger)** — when a Suggestion uses primary+cascade framing AND the targeted Primary section contains an enumeration list (categorized examples, named classes, ordered rules, OR FOUNDATIONS-Alignment-style Phase-by-Principle mapping rows), grep the same file at audit time for OTHER enumeration sites that name the same items OR for FOUNDATIONS Alignment / Phase-by-Principle mapping tables whose Phase / Mechanism columns would need syncing when the primary edit changes which Phases enforce a specific Principle — see `references/audit-execution-discipline.md` §Convention-restatement check §Extended trigger for the Phase-by-Principle sub-trigger and worked precedent. Distinct from the §Site-counting trigger above: §Site-counting keys on Suggestion-field phrasing (parallel-instantiation patterns); §Convention-restatement keys on the targeted file's structural restatement pattern. Each restatement site should appear in the Suggestion as a co-equal primary (`<finding>.a / <finding>.b / ...`) rather than left for implementation-phase scope-expansion.

   **Declarative-vs-procedural restatement check (companion trigger)** — when a Suggestion changes a documented data-loading / retrieval / output convention at one Phase-N procedural site AND the target SKILL.md has a §World-State Prerequisites (or equivalently-named declarative-prerequisites) section, grep the declarative section for the same convention's terminology — see `references/audit-execution-discipline.md` §Declarative-vs-procedural restatement check. Distinct from the §Site-counting trigger above (which keys on Suggestion-field phrasing) and from the §Convention-restatement check above (which keys on the targeted Primary section being an enumeration list); this trigger keys on the target SKILL.md's structural pattern of restating data-loading conventions across declarative-prerequisites and procedural-phase registers. When a parallel mention is found, the declarative-section site is a co-equal primary — list as `<finding>.a` (procedural) and `<finding>.b` (declarative) in the Suggestion field.

7. **Present the report** — Read `references/report-conventions.md` (**Required regardless of prior context** — the rules govern report structure, suggestion specificity, severity-count double-check, restructure-encompassing audit suggestions, and "implement all" tagging defaults; skip only via the same in-context-from-prior-session criterion as Step 2's alignment-document load, with the same explicit-mechanism-naming requirement when skipping). Output the structured report using the template below, applying the loaded conventions. The "Required regardless" framing parallels Step 1b's discipline-reference load mandate — both reference loads are normative for their procedural phase, and both must be either explicitly executed or explicitly skipped-with-named-mechanism so the audit trail records what conventions the report was authored against.
...
>
> 1. **Default: OMIT this section** when no finding affects shared surfaces with sibling skills. Include it ONLY when a sibling scan was executed, a cross-skill cascade was identified, OR a finding introduces a forward cross-reference to a sibling skill's section / file / documented convention (form (5) — per `references/cross-skill-consistency.md` §Concrete shared-surface triggers, "Cross-references introduced to a sibling skill's section, file, or documented convention" IS a shared-surface trigger that requires audit-trail recording even when no scan ran and no cascade was identified).
