...
- Stale meta-narrative / dev-cycle commentary — paragraphs describing what the skill "currently does" or "is missing," typically left over from a prior audit cycle, that subsequent additions to the skill have made factually wrong (e.g., a "Context — what Phase 1 already does" paragraph naming missing coverage targets that the next audit added below it). Remove on the same pass as the redundancy it describes.
...
**Major-rewrite preserve list (mandatory pre-write check)**: BEFORE drafting any edits, scan the planned changes for any source block that qualifies as a major rewrite — defined as either (a) ≥50 lines being restructured wholesale (e.g., entire ASCII diagrams, full sections collapsed), OR (b) any source block carrying ≥~8 distinct instructions being restructured, regardless of line count (a dense mega-bullet — one or a few "lines" packing many instructions, the characteristic shape of heavily-audited skills — is the common low-line-count / high-instruction-count case). If any qualifies, enumerate the distinctive terms / load-bearing phrases from the source block as an explicit preserve list NOW, before writing the new version. Step 8's spot-check grep then verifies each item; the pre-write enumeration prevents drops rather than detecting them. Worked example: a Phase N sub-step occupying one paragraph that specifies actions A through Z in semicolon-separated clauses (a ~25-instruction megaparagraph covering token signing + submit-path selection + output reading + replay protection) easily exceeds the 8-instruction threshold under condition (b); enumerate the ~25 phrases BEFORE restructuring into sub-bullets, then Step 8 greps each against the rewritten output.
