...
For `world_char` profiles, the body and frontmatter must satisfy the Semantic Preservation Contract: each present structured `dramatic_core` source field needs a `source_operational_fact_map` entry, and retained facts must land in an operational STCHAR home. Each retained `source_operational_fact_map[].target_section` must be exactly one member of the same 11-name operational-section list used for `operational_home`; compounds, parentheticals, abbreviations, `Source Distillation`, and `Validation / Audit Anchors` are invalid targets. The `cannot_be_swapped_out_because` source field is a provenance / irreplaceability statement with no native audit-section operational home: normally disposition it as `compressed` or `transformed` into `Stable Persona Core`, where the world-grounding it summarizes already lives, or omit it with a lawful rationale. Never target `Validation / Audit Anchors` for this field.
