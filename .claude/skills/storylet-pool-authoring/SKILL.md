...
**Schema transition note:** `templates/storylet-record.yaml` is the SPEC-19 SLT v2 scene-commitment-arc template (`record_version: 2`, `shape: scene_commitment_arc`). The operational authoring phases below still describe the pre-SPEC-21 v1 beat-storylet workflow where they mention shape distribution, `choice_templates`, or v1 diversity gates. SPEC-21 owns the full operational rewrite; until that lands, treat the template as the forward schema authority and this note as the boundary disclosure.
