...
6. **Phase 5b: Engine Pre-Validation** — execute the inline §Phase 5b block below for direct invocation only. Assemble the draft patch plan with `approval_token: "placeholder"` and call `mcp__worldloom__validate_patch_plan(envelope)`. Coverage: `yaml_parse_integrity`, `id_uniqueness`, `cross_file_reference`, `record_schema_compliance`, `touched_by_cf_completeness`, Rules 1-7, `storylet_predicate_dsl_parsability`, `rule11_action_space`, `rule12_redundancy`. Treat as a defensive pre-submit check, not a complete gate (does NOT cover approval-token verification or id-allocation race — both are submit-only). Fold the validators' verdict into the Phase 6 HARD-GATE summary's VALIDATION VERDICTS block. Skip when `parent_skill_invocation: true`; the parent skill's own pre-write validation surface governs.
