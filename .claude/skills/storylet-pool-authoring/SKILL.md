...
3. **Phase 2: Generation Seeds** — load `references/phase-2-generation-seeds.md`. Produce N seeds (with the +30% replacement buffer for seed/focus batches; mode-specific sizing for jit/audit).
4. **Phase 3: Structured Drafting** — load `references/phase-3-structured-drafting.md`. For each seed, assemble the LLM prompt (content_policy FIRST), produce a structured SLT proposal, and let the engine wrap with schema scaffolding + DSL parsability + visibility-scope assignment.
