...
Phase 1: Coverage Diagnosis     (scan current pool + open-state for
                                 ...
                                 commitment_family / commitment_class arcs;
                                 active THRs without escalation family/class
                                 arcs; under-
                                 represented content_intensity bands relative
                                 to story baseline; under-represented
                                 commitment_family, commitment_class, and
                                 arc_archetype distributions; mysteries_in_play[]
                                 entries with no touching arc; recent-history
...
Phase 2: Generation Seeds       (produce target_pool_size + ~30% arc seeds —
                                 each names: commitment_family,
                                 commitment_class, optional commitment_detail,
                                 arc_archetype,
...
Phase 3: Structured Drafting    (per seed: assemble LLM prompt with
                                 ...
                                 excerpt + SLT scaffold; LLM produces
...
Phase 4: Per-Storylet           (each candidate SLT runs all 14 gates
      ...
      |
      v
Phase 5: Diversity Audit        (across surviving SLTs: commitment_family
         (Canon Safety Check    broad coverage plus commitment_class
          phase, batch-level)   distribution ≤30% per class, with documented
                                 small-batch relaxation when applicable;
...
Phase 6: Approval / Return      (direct invocation: HARD-GATE summary
                                 ...
                                 (title, commitment_family, commitment_class,
                                 optional commitment_detail, arc_archetype,
...
- `worlds/<world-slug>/stories/<story-slug>/INDEX.md` — updated in place; storylet-pool section receives new total count + per-commitment_family distribution + per-commitment_class distribution + per-arc_archetype distribution + per-content_intensity distribution.
...
5. **Phases 4-5: Canon Safety Checks** — load `references/phase-4-5-canon-safety-checks.md`. Run all 14 Phase 4 per-storylet gates against every candidate (HARD-REJECT or revise per gate); then run Phase 5's mode-appropriate diversity-axis checks + batch-level branch-contamination audit + audit-mode RSP visibility-match for direct seed/focus batches and the audit-mode subset for direct audit batches; bypass diversity for jit/audit per the rules in the reference.
   Body-language discipline: cast-agnostic storylet `notes:` must use
   character-agnostic gestural language; cast-locked clothing detail requires
   Material Reality verification. See `references/phase-3-structured-drafting.md`
...
OBL coverage:              <count of currently-open OBLs engaged> / <total open>  (rule ≥60% in seed mode; or every `source_obligations` hit in focus mode — PASS)
...
4. `Edit` `worlds/<world-slug>/stories/<story-slug>/INDEX.md` storylet-pool section: update total count, per-commitment_family distribution table, per-commitment_class distribution table, per-arc_archetype distribution table, and per-content_intensity distribution. The INDEX.md edit is the LAST write — `INDEX.md` is NOT under `_source/`, so direct `Edit` is the correct surface (Hook 3's match pattern doesn't cover it).
...
A storylet pool is not authored because storylets were generated. It is authored only when the HARD-GATE block at the top of this file says the candidate pool may proceed. That block is the authoritative pass-condition source; this closing rule is only the discipline it protects. A brittle pool produces a brittle story, so the skill's final act is to preserve the structural gate instead of summarizing it.
