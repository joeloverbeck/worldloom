...
|---|---|---|
| `commitment_class` distribution | <=30% per class for established/top-up batches; <=40% is permitted for batches with `target_pool_size < 20` when recorded as a small-batch relaxation. | `shape` is degenerate because every record has `shape: scene_commitment_arc`. |
