...
Phase 4 is the per-arc Canon Safety Check for candidate v2 SLT records. Phase 5 is the batch-level diversity and branch-contamination audit. Under the scene-commitment-arc schema, every candidate is a multi-beat arc (`shape: scene_commitment_arc`) and Phase 4 runs **14 gates** per candidate.
...
| 3. | Invariant compatibility (Rule 4) | Every legacy `fact_effects` / `relationship_effects` entry and every `effect_model.variants[].required_effects[]` entry respects every INV record's `break_conditions` from the whole-class INV load. | HARD-REJECT |
...
| 5. | Dedup | Candidate is not a near-duplicate of an existing pool entry. Under v2, dedup is keyed primarily by `(arc_contract.commitment_class, arc_contract.arc_archetype, target_obligation)` plus overlap in hard/soft predicate forms, tone tags, theme tags, and required cast STENT ids. | reject; replace with under-represented seed from Phase 1's gap matrix |
...
| 9. | Schema completeness (Rule 1 + Rule 7) | All mandatory fields per `templates/storylet-record.yaml` are present, including legacy required blocks (`mystery_safety`, `provenance`, `visibility`) and all seven v2 structural blocks: `arc_contract`, `dramatic_unit`, `beat_plan`, `execution_envelope`, `stop_policy`, `effect_model`, and `exit_portfolio`. Legacy fact-template create entries still declare `visible_to_reader` and `reader_visibility_basis`; `visible_to_reader: true` requires a positive basis, while `unrevealed_objective_truth` is valid only with `visible_to_reader: false`. SPEC-22's `arc_schema_compliance` validator backstops required v2 sub-field completeness in Phase 5b. | revise (re-prompt LLM with explicit constraint) |
| 10. | Arc envelope conformance | `execution_envelope.invariants[]` and `execution_envelope.required_functions[]` are kebab-case strings, not free-form prose paragraphs, and reference the documented open-vocabulary contract for envelope items. `allowed_tactics`, `prohibited_actions`, `style_directives`, and `mystery_preservation` must be structurally populated and must not contradict gates 1, 3, 6, or 8. This gate is skill-internal-only for v1; SPEC-21 routes the engine-validator gap back to SPEC-22 reassessment. | HARD-REJECT |
...
| 13. | Exit-portfolio completeness | `exit_portfolio.native_seeds[]` has at least one native seed, each native seed has a `commitment_class`, `strategy_cluster`, `expected_state_delta`, and `continuation_arc_selector`, and `engine_discovered_exit_budget` is present with min/max/allowed_sources. Required sub-field minimums are also enforced by SPEC-22's `arc_schema_compliance` validator. | HARD-REJECT |
...
The v1 `mystery_safety` field and the v2 `execution_envelope.mystery_preservation` field are not redundant.
...
Across the surviving SLT records in the batch, verify the batch-level checks that apply to its mode. Seed/focus batches require all v2 diversity-axis checks plus one batch-level branch-contamination audit before Phase 6. Audit batches bypass the diversity axes but still require batch-level branch-contamination and RSP visibility-match PASS.
...
|---|---|---|
| `commitment_class` distribution | <=30% per class for established/top-up batches; <=40% is permitted for batches with `target_pool_size < 20` when recorded as a small-batch relaxation. | Replaces v1 shape distribution. `shape` is degenerate under v2 because every record has `shape: scene_commitment_arc`. |
| `arc_archetype` distribution | <=25% per archetype. | New finer-grained structural axis. |
| Tone distribution | <=40% per dominant tone tag. | Preserved from v1. |
| Theme distribution | <=50% per theme tag. | Preserved from v1. Structural source-OBL concentration may exceed 50% only with a PASS WITH RATIONALE notation that names the source-OBL relationship, confirms secondary themes fragment across families, and records that the override is structural rather than authoring-redundancy. |
...
| Cast usage | No protagonist, major, or antagonist STENT from `STORY_KERNEL.cast_bind_list` has zero engagement. | Engagement means appearance in required/optional cast fields or v2 actor/target/effect surfaces. Supporting and foil cast may be unengaged in a given batch. |
...
- Replace overrepresented entries with under-represented `commitment_class`, `arc_archetype`, tone, theme, content-intensity, OBL, cast, or strong-axis seeds drawn from Phase 1's gap matrix.
...
- `templates/storylet-record.yaml` is the v2 SLT structural authority.
