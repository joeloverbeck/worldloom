...
Required fields are marked `*`. Fields not listed are not part of the schema. Optional legacy fields are explicitly marked when they survive only for grandfathered records. All YAML strings supporting natural language remain free-form unless an enum is named.

**`record_kind` (cross-class note).** None of the field lists below include `record_kind`: it is **optional** — not `required` in any `tools/validators/src/schemas/story-*.schema.json`, and not injected by the patch engine. The world-index derives the displayed `record_kind` from the record class for retrieval (`get_record` responses carry it whether or not the on-disk file does), so it is not needed for downstream reads. Bootstrap authored it onto some classes (`SE` → `story_event_record`, `BEL` → `belief_record`, `STEMO` → `story_emotion_record`, `SREL` → `relationship_record_story`, etc.) but omitted it on `PG` and `CHC`. When authoring a record, match the existing on-disk convention for that class. Note that any field you do author becomes part of that record's content — and for `PG` specifically, part of its `state_hash` input (§4.2a) — so include or omit `record_kind` consistently within a class rather than mixing. All 22 story-bundle record classes listed in §3 have field schemas defined below: §4.1-§4.4 cover the four classes with pre-existing closed schemas, §4.5 covers the 18 additional classes, §4.6 covers the legacy prose receipt direct-write artifact, and §4.7 covers the scene-prose receipt direct-write artifact.
