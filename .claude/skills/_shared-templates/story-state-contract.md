...
The two STCHAR-global hashes plus this page's local packet hash are load-bearing: prose-attach uses them to validate the rendered receipt against the active STCHAR profile, its voice block, and this page-specific packet. `page_packet_hash` is computed from the full §16a packet projection after replacing only its own `page_packet_hash=sha256:<64 lowercase hex>` value with the fixed placeholder `page_packet_hash=sha256:<page_packet_hash>`; the sibling `profile_hash` / `voice_block_hash` declarations and the packet authority prose remain in the hashed projection. A verified §16a packet is sufficient authority for prose and prose-attach validation, but it is not the default authority for new character-dependent state creation; that requires full or projected STCHAR section retrieval. §16a is the renderer's character voice and behavior authority; it does not replace §5 entity status, §9 relationship/belief context, §9b active plans, §9c emotional transition, §16 cast material reality projection, or §17 style/register notes. Page plans must not cite world `CHAR-*` as operational authority for characterization after STCHAR exists; world `CHAR` may appear only as non-operational provenance on the STCHAR itself or in explicit authoring/promotion/adjudication flows.
