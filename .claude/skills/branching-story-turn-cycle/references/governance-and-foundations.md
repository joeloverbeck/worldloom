...
| §Story Bundles §6.1 (Story-Local Character Authority) | Pre-flight, Phase 3, 7, 8 | Runtime characterization consumes active STCHAR through `STENT.bound_stchar_id`, `PG.state_snapshot.active_records.STCHAR`, targeted story-scoped retrieval, and CHC/STPLAN/STEMO/SREL grounding. World `CHAR-*` is not runtime authority; `STCHAR.source_char_id` remains provenance only. Complex new non-background characters route to `story-character-profile` via `blocked_requires_stchar` before meaningful STENT/SE/PG/CHC state is committed. |
