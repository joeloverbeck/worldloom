...
Note: the deterministic `turn_driver_pov_observer_firewall` validator treats only hidden `STSEC` and offstage `STPLAN` driver records as "hidden" for the `perceived_directly` leak check; FOUNDATIONS §Story Bundles §6b is worded more broadly. The reconciliation of that wording with the implemented scope is tracked in `tickets/TDPOV-001` — until it lands, follow the rule above (it matches the enforced validator).
