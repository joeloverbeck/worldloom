...
7. **Phase 7 — Author the page plan.** Write `pages-prose-plans/PG-<integer>.md` per shared contract §8 — the 19 numbered sections, including §7a as the render-side projection of `SE.turn_driver` and the active-pressure disposition table, §2 / §3 / §19 inlined verbatim from `reports/prose-quality-instructions.md`, plus optional per-page §9b active actor plans, §9c emotional causality, §10b new-class visibility blocks when active STPLAN/STEMO/CLK/STSEC/STQ records matter for the render, and mandatory §16a STCHAR authority packets for every relevant viewpoint/speaker/major actor/direct target/emotionally salient or behavior-shaping character. Load `references/phase-7-page-plan.md`.
