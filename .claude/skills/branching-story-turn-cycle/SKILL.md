...
9. **Phase 9 — Validate.** Run the 8 shared hard gates plus Gate 9 (Turn-Driver Lawfulness) with one-line PASS rationale on `PG-<integer>.validation_trace`, plus the 15 turn-cycle-additional checks; compute final PG hashes via `tools/world-mcp/dist/src/cli/compute-pg-hashes.js`. Load `references/phase-9-validation-gates.md`.
