...
- `mcp__worldloom__list_records(world_slug, record_type='invariant_record', include_full_body=true)` / `mcp__worldloom__list_records(world_slug, record_type='mystery_record', include_full_body=true)` for Phase 6 whole-class firewall loads where every INV / M body must be tested
...
| Tooling Recommendation (context-packet + targeted retrieval) | Pre-flight + Phase 1 + Phase 6 | Pre-flight loads atomic records via `mcp__worldloom__list_records`; Phase 6 uses `include_full_body=true` for whole-class INV / M firewall loads; targeted follow-up uses `search_nodes` / `get_record` / `get_neighbors`. Direct `_source/` reads redirected by Hook 2. |
