...
- List `<skill-dir>/references/` if it exists. Skim each existing reference doc's headers to understand what themes are already extracted. Read in full when planning to merge new content into an existing doc (per Step 5 grouping rule "merge into existing"); a header-skim plus targeted grep for §-anchor / path / terminology references suffices when leaving an existing doc unchanged. The full-read instruction can consume substantial context for large reference docs (e.g., 280+ lines) without proportional benefit when no merge is planned — the merge / no-merge decision determines depth.
...
- **Calibration anchor for ref-doc count**: typical phase-router refactors land at ~1 reference doc per 60-100 lines of source content (e.g., 600-line source → 7-10 docs; 1100-line source → 11-15 docs). Fewer docs is acceptable when adjacent phases form natural units (Phase 4 + 4.5 sub-phases, Phase 6 + 6.5 governor recompute → closure detection); more docs is acceptable when phases are structurally distinct enough that combining would obscure their separate concerns. The thin SKILL.md size-target proportions (Step 7) cross-check the calibration: a thin SKILL.md hitting ~50% of original on a phase-router skill typically means ~10 docs is in range. Worked precedents: branching-story-page-cycle 1140 lines → 11 reference docs (~104 lines of source per doc); storylet-pool-authoring 637 lines → 7 reference docs (~91 lines/doc); branching-story-bootstrap 520 lines → 10 reference docs (~52 lines/doc). The line-count-to-ref-count ratio is calibration, not a rule — diagram-heavy or HARD-GATE-heavy skills correctly land lower (more inline content); checklist-heavy skills correctly land higher (more extractable content).
...
- **Update inter-block §-anchor cross-references when blocks move to different reference docs.** When extracted content contains §-anchor cross-references that previously pointed to other H2/H3 sections in the same SKILL.md (e.g., "see Guardrails §Cross-skill consistency", "per the §Cascade edits sub-rule"), rewrite each to point to the reference doc the target now lives in (e.g., "see `references/cross-skill-consistency.md`", "per `references/cascade-and-summary-discipline.md` §Cascade edits"). The rewrite may happen either as a single-pass sweep AFTER all reference docs are written and named, OR inline as each reference doc is authored (when the operator already knows the destination structure for the new reference docs); both rhythms are acceptable provided every §-anchor cross-reference is resolved before Step 8's content-preservation verification. **Verification recipe**: after Step 6 completes (regardless of which rhythm was used), run `grep -nE '§[A-Z]' <skill-dir>/references/*.md` to enumerate every remaining §-anchor string and hand-check each for resolvability; remaining anchors should point to either (i) sections within the same reference doc, (ii) sections in other reference docs by file path (`references/foo.md §Bar`), (iii) sections in the new thin SKILL.md, or (iv) sections in external skills/files (`<sibling-skill>/references/baz.md §Qux`). Step 8's standard scan verifies header presence, not anchor resolvability — this recipe closes that gap.
...
Before overwriting `<skill-dir>/SKILL.md` in Step 7, capture the list of H2/H3 headers from the original — a single `grep -nE '^##? '` over the in-context content suffices; stash the list in your working notes. After Step 7's writes complete, run the verification: for each captured header, **case-insensitive** grep (e.g., `grep -il -F <header>` or `grep -iE`) against `<skill-dir>/references/*.md` + the new `<skill-dir>/SKILL.md` and report any header that no file contains. Use case-insensitive matching because extracted H2 sections commonly become reference doc H1s with title-case promotion (e.g., original H2 `## Phase 6: Seed commitment blocks (optional)` → reference H1 `# Phase 6: Seed Commitment Blocks (Optional)`); case-sensitive grep silently false-positive-zero-matches on every title-case-promoted header, forcing a re-run with `-i` to confirm content was actually preserved. The pre-overwrite capture is load-bearing — Step 7's `Write` overwrites SKILL.md, so the original header list is gone after the rewrite and cannot be reconstructed from disk.

### 9. Cross-Skill Reference Check

Run two separate greps across `.claude/skills/*/SKILL.md` and `.claude/skills/*/references/*.md` (sibling skills increasingly ship reference docs that may carry their own cross-skill citations; scoping the scan to SKILL.md only misses citations living in sibling references/): (a) for the target skill's name verbatim (e.g., `canon-addition`), (b) for its directory path prefix in EITHER form — full-prefix `.claude/skills/<name>/` OR sibling-relative `<name>/(references|templates)/` OR sibling-relative `<name>/SKILL.md`. The sibling-relative form is the dominant cross-skill citation pattern in worldloom — siblings cite each other using shortened paths like `branching-story-bootstrap/references/engine-envelope-shape.md §5` rather than the full `.claude/skills/...` prefix — so a grep that only checks the full-prefix form will silently false-negative on most real cross-skill path references. The `<name>/SKILL.md` alternation (no trailing slash; matches `.md` directly) catches shortened-path citations to the target's main file, which a `<name>/SKILL/` form alone would miss. Either run two sub-greps under (b) and merge the results, OR use a single regex alternation: `grep -rnE '(\.claude/skills/<name>/|<name>/(references|templates)/|<name>/SKILL\.md)' .claude/skills/*/SKILL.md .claude/skills/*/references/*.md`. Report each result independently — two zero-match lines are more auditable than a single compound alternation negative. If the target skill has uniquely named sections, run a third grep for those. Skip generic headers (e.g., "Hard Rules", "Procedure") that would produce false positives. Always record the result in the Step 10 output summary, not just when matches are found: zero-match runs produce a `Cross-skill references: 0 matches` line so the audit trail reflects that the scan ran; match runs list the files (e.g., `Cross-skill references: 3 matches in foo/SKILL.md, bar/SKILL.md`) and flag them as references that may need updating so the user can fix external pointers. Name-only matches (grep (a)) in sibling YAML frontmatter descriptions are pipeline-documentation references and typically do NOT require action on internal refactors; path-prefix matches (grep (b)) usually do require attention since they cite internal file structure. State the distinction in the Step 10 summary when the two grep results diverge.

**§-anchor citation investigation** (parallel to Step 6's internal §-anchor cross-reference rule, applied to EXTERNAL §-anchors from sibling skills): run a third grep BROADLY across all sibling SKILL.md and references/*.md files for §-anchor citation patterns referencing the target's structure — `§<SectionName>`, `§Phase N`, `§<bold-rule-name>`, or quoted phase / sub-section labels paired with the target's path prefix or bare name. **Run this third grep independently of grep (b)'s match status** — a sibling can cite the target via bare `<target> §Foo` form (no path prefix at all) without also citing `<target>/<subdir>/`, and gating the §-anchor investigation on grep (b)'s path-prefix matches would silently miss every such bare-form citation. **Concrete grep recipe**: `grep -rnE '<target-skill-name>(/[A-Za-z._-]+)*(\.md)?\s*§' .claude/skills/*/SKILL.md .claude/skills/*/references/*.md` catches target-prefixed §-anchors in their common forms (`<target>/SKILL.md §Foo`, `<target>/references/bar.md §Baz`, bare `<target> §Foo`); the `(/[A-Za-z._-]+)*(\.md)?` group makes the path-suffix optional so the bare form is included in the same recipe. Broaden the regex if a sibling uses non-standard punctuation between the target reference and the §-anchor (e.g., parenthesized references, line-wrapped citations). For each cited section, cross-check whether it still resolves in (i) the new thin SKILL.md or (ii) one of the new `references/*.md` files. Report each broken §-anchor in the Step 10 summary as a sibling pointer requiring update, naming the sibling file, the cited section, and the reference doc the section now lives in (e.g., `branching-story-bootstrap/SKILL.md cites §Phase 2 §Bootstrap-mix shape weighting which now lives in references/phase-2-generation-seeds.md`). The broad-scope third grep prevents the audit-trail-defect where the audit reports a §-anchor citation as resolved (or never notices it) because grep (b) didn't surface the sibling and the §-anchor recipe was never run on it; siblings that cite specific §-anchors of the refactored skill are exactly the case where shared-surface attention pays off most. When grep (b) produced a path-prefix match in a sibling but the §-anchor third grep returns no hits in that sibling (e.g., the sibling only references `<target>/templates/<file>.yaml` or the bare `<target>/SKILL.md` path without naming sections), report the path-prefix match alone in Step 10 — no §-anchor breakage to enumerate.
