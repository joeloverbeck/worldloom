...
5. **Adjudication record** at `worlds/<world-slug>/adjudications/PA-NNNN-<verdict>.md` — original proposal + full Phase 0–11 analysis + verdict + phase-cited justifications + declined repair options + (if escalation fired) six critic sub-agent reports verbatim. Populate the Discovery section at the top of the report (see `templates/adjudication-report.md`) with the five canonical fields: `mystery_reserve_touched` (M-N ids substantively touched — firewall extensions, narrowings, new entry additions, or explicit cross-application commitments — NOT entries merely checked-and-preserved), `invariants_touched` (ONT-N / CAU-N / DIS-N / SOC-N / AES-N ids substantively touched — clarification annotations, firewall additions, extension blocks, or explicit stabilizer-language commitments — NOT invariants merely tested-and-found-compatible), `cf_records_touched` (CF-NNNN ids, new + modified this adjudication), `open_questions_touched` (OPEN_QUESTIONS.md sections committed, newly added, or pressured — per the UNCHANGED/PRESSURED/NEW/RESOLVED disposition categories in `references/counterfactual-and-verdict.md` — NOT UNCHANGED items merely visited during analysis), `change_id` (the CH-NNNN this adjudication produced — accept branches only). The **"substantively touched" semantics is uniform across all four `_touched` fields** — list only items affected by this proposal's language-level commitments, not items merely visited during analysis. Use these literal field names — ad-hoc alternatives break the cross-adjudication grep-discoverability pattern that enables future canon-addition runs to surface prior guidance on the same surfaces. The template (`templates/adjudication-report.md`) is authoritative if it evolves. Populate the Phase 14a Validation Checklist section (see template) before the verdict; this becomes the auditable record of validation pass/fail per test.
...
3. **CANON_LEDGER.md last**. Within this file, apply three sub-steps in strict order — this makes partial-failure detection tractable:
   1. **CF qualifications first** — apply all in-place YAML edits with `notes`-field modification trace and `modification_history` entries. Completing this before appending new CFs means an interrupted run has consistent qualifications without unresolved new CFs referencing them. **Empty-array edge case**: if the target CF's current `modification_history` is in empty inline-flow form (`modification_history: []`), replace the entire line with block-form — `modification_history:` on its own line followed by indented `-` entries. Do not leave both `[]` and block entries coexisting; the result is invalid YAML and breaks future ledger tooling that parses the field. Conversely, when the target CF already has a populated `modification_history:` block-form array, append the new entry to the array; do NOT reset to `[]` first. **Fence-boundary preservation (common failure mode)**: `CANON_LEDGER.md` separates adjacent CF records with markdown code-fence pairs (closing `\`\`\`` of the current CF, blank line, opening `\`\`\`yaml` of the next CF). When the `modification_history: []` line is the last non-fence line of its YAML block, the Edit tool's `old_string` must be composed carefully — if `old_string` spans `modification_history: []\n\`\`\`\n\n\`\`\`yaml\nid: CF-<next>` (the empty-array line PLUS the fence boundary PLUS the next CF's opener), the `new_string` MUST re-emit the `\`\`\`\n\n\`\`\`yaml` fence pair between the block-form array and the next CF's `id:` line. A `new_string` that writes the block-form array but drops the fence pair leaves the next CF record fence-less, breaks the markdown code-fence structure, and makes downstream YAML parsers treat the next CF as a continuation of the current one. Safest pattern: scope `old_string` to lines strictly BEFORE the current CF's closing `\`\`\`` fence so the fence boundary stays untouched; if the fence boundary must be spanned, verify in `new_string` that the `\`\`\`` close + `\`\`\`yaml` open pair is preserved exactly. This failure mode is distinct from the empty-vs-block coexistence failure above; both can occur simultaneously on a single Edit.
...
4. **Post-write structural-integrity verification (required)**. Before reporting completion, re-read each modified file (full read for short files; targeted Reads of edited regions with flanking context for `CANON_LEDGER.md` and other large files) and confirm structural integrity as a single pass. Checks:
   1. **YAML block-fence integrity**: every touched YAML-block record in `CANON_LEDGER.md` is bounded by a `\`\`\`yaml` opener at the top and a `\`\`\`` closer at the bottom; no CF record has lost its fence pair during editing (the common failure mode documented under sub-step 3.i Fence-boundary preservation). Grep `^\`\`\`yaml` and `^\`\`\`` in the file tail around each touched CF; the ratio and adjacency must match the expected pattern.
   2. **CF id enumeration contiguity**: grep `^id: CF-\d+` in `CANON_LEDGER.md`. For new-fact accepts, the count is pre-write count + 1 with the new CF-NNNN at the expected id position; for clarificatory retcons (no new CF), the count is unchanged. No duplicate CF ids. No gap in the enumeration.
   3. **CH id enumeration contiguity**: grep `^change_id: CH-\d+` in the change log section. Count is pre-write count + 1; the new CH-NNNN appears at the TAIL of the change log (newest entry is always the last YAML block per sub-step 3.iii's grep-and-tail discoverability commitment); no duplicate CH ids; no gap in the enumeration.
   4. **New CF approval flag**: for new-fact accepts, grep the new CF's block for `source_basis:` → `direct_user_approval: true` (literal line). This is the moment-of-canon-mutation marker per sub-step 3.ii; its absence means the new CF record was persisted in a pre-approval state and Phase 15a's commit contract has been violated.
   5. **Modification_history trace completeness**: for every CF identified by the Phase 12a three-axis scan (see SKILL.md Procedure step 5 §Phase 12a modification_history scan), grep the CF's block for the new `Modified YYYY-MM-DD by CH-NNNN (CF-NNNN)` notes-field line AND the matching `change_id: CH-NNNN` entry in the `modification_history:` array. Both must be present per artifact 2's "REQUIRED TOGETHER" rule; a CF with notes-field trace but absent array entry is the Rule 6 (No Silent Retcons) audit-trail gap this discipline prevents.
   6. **Attribution comment integrity in domain-file patches**: grep each modified domain file for the new `<!-- added by CF-NNNN -->` (new-fact accepts) or `<!-- clarified by CH-NNNN -->` (clarificatory retcons) attribution count matches the number of patches applied to that file. Malformed attributions (truncated comment syntax, broken across lines, accidentally nested inside YAML) render the attribution invisible to future grep-based audit traversal.
   7. **Adjudication record Discovery-section canonical fields**: grep the new PA-NNNN adjudication file for the five canonical field names (`mystery_reserve_touched`, `invariants_touched`, `cf_records_touched`, `open_questions_touched`, `change_id`). Ad-hoc field-name substitution (e.g., `New CF` instead of `cf_records_touched`) breaks the cross-adjudication grep-discoverability pattern future canon-addition runs depend on.

   Any check failure halts before reporting, names the offending file and check, and requires repair + full re-verification before proceeding. Do not selectively re-check — a fix in one area can introduce issues in another. This verification pass is the end-state complement to the Phase 15a write-phase progress discipline (SKILL.md Procedure step 5 §Phase 15a write-phase progress discipline) that checkpoints between macro-phases; the two disciplines are intended to run together on large-delivery accepts and independently catch different classes of drift. For canonical grep commands implementing each of the seven post-write checks (and the complementary inter-step checkpoints), load `references/phase-15a-checkpoint-grep-reference.md` — load at Phase 15a for large-delivery accepts; small-delivery invocations can compose grep ad hoc.
