...
### Engine-added `record_kind` discriminator: do NOT include in payloads

On-disk records under `_source/<subdir>/*.yaml` carry a `record_kind` discriminator field at the top of the YAML body (e.g., `record_kind: "canon_fact"` on every CF record, `record_kind: "change_log"` on every CH record, `record_kind: "mystery_reserve"` / `"open_question"` / `"invariant"` / `"section"` / `"named_entity"` on the corresponding atomic records). The engine adds this field at write time; the authored-record JSON schemas under `tools/validators/src/schemas/` (`canon-fact-record.schema.json`, `change-log-entry.schema.json`, `mystery-reserve.schema.json`, `open-question.schema.json`) declare `additionalProperties: false` and do NOT list `record_kind` in their `properties`. **Patch payloads MUST NOT include `record_kind`** — including it triggers `record_schema_compliance.additionalProperties` failures at validation time. When mirroring an existing record's shape (e.g., reading a CF-NNNN.yaml file from disk to learn the schema by example), strip the `record_kind` field before constructing the patch payload's `cf_record` / `ch_record` / `m_record` / `oq_record` value. The §10 worked example below already omits the field; that omission is intentional.
