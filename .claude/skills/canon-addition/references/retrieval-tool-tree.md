...
- **Retrieval-time `stale_index`**: any retrieval call (`get_context_packet`, `get_record`, `find_named_entities`, etc.) may return `stale_index` when the world index has diverged from on-disk content (typically a prior session's hybrid-file frontmatter edit without index sync). The error's `details.drifted_files[]` names the stale paths. Recovery: `node tools/world-index/dist/src/cli.js sync <world-slug>`; retry the retrieval call. This is identical to the submit-time `index_stale` handling under §Phase 15a — pre-flight retrieval is just the more common firing point because it is the first MCP call most runs make. See `references/engine-envelope-shape.md` §6 for the consolidated failure-mode table.
