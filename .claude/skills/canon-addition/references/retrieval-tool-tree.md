...
- `mcp__worldloom__find_named_entities(names)` for pre-figuring scans of named entities the proposal commits. The tool's input schema accepts only `world_slug` and `names`; inspect each canonical-match's `mentions_by_node_type[]` array for entries with `node_type` in `{character_record, diegetic_artifact_record}` to identify pre-figuring matches client-side. This searches canonical entity names, entity aliases, scoped-reference display names, and scoped-reference aliases; it does not scan prose bodies.
