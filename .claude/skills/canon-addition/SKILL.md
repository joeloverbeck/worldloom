...
1. **Pre-flight Check.** **Normalize the `world_slug` input first**: if the argument begins with `worlds/`, strip the prefix so both `animalia` and `worlds/animalia` resolve identically. Validate that the normalized slug matches `[a-z0-9-]+` before proceeding; if not, abort naming the malformed input. Then verify `worlds/<world-slug>/` exists; if absent, abort: "World directory not found. Run `create-base-world` first, or supply a valid `world_slug`." Verify all six mandatory world files are readable; if any is missing or unreadable, abort naming the specific file. Load `docs/FOUNDATIONS.md` and the six mandatory world files. Scan `CANON_LEDGER.md` for the highest existing `CF-NNNN` and `CH-NNNN`; allocate `next_cf_id` and `next_ch_id`. Scan `worlds/<world-slug>/adjudications/*.md` (if the directory exists) for the highest existing `PA-NNNN`; allocate `next_pa_id` (or `PA-0001` if absent).
    ...
    **Large-file method**: mature worlds accumulate not only ledgers but also any domain file whose accumulated annotations push it past the Read tool's default token budget. Line count is a weak proxy (≥1500 lines is a practical threshold for `CANON_LEDGER.md`, but files with very dense lines may exceed budget at much lower line counts — `INSTITUTIONS.md` at ~260 dense lines, `MYSTERY_RESERVE.md` and `EVERYDAY_LIFE.md` at ~300+ lines have all exceeded budget on the Animalia ledger); rely on the actual Read failure as the trigger. The same grep-then-targeted-read pattern applies across the file class: where a full-file read would fail or waste budget, use `Grep -n` on a structural anchor pattern appropriate to the file, then issue targeted `Read offset/limit` calls anchored at the grepped offsets rather than full-file reads. Anchor patterns by file:
    ...
    - `MYSTERY_RESERVE.md` — `^## M-` for entry enumeration; targeted reads when checking forbidden-answer collisions (Phase 2) or drafting firewall extensions for specific M-N entries (Phase 9 / Phase 13a). For **appending a new cross-application extension to an existing M-N entry**, additionally grep `^<!-- added by CF-\d+ -->` within the M-N section to locate the last existing extension's end — Edit's `old_string` anchors at the last extension's closing line to append without disturbing prior extensions.
    ...
    Load the skill's own templates into working context: `templates/canon-fact-record.yaml`, `templates/change-log-entry.yaml`, `templates/adjudication-report.md`. Also load `templates/critic-prompt.md` and `templates/critic-report-format.md` when escalation is likely (>3 domains named in the proposal, user-stated novelty level is high, or the proposal's underlying world-change touches an invariant). **Each template carries load-bearing operational rules beyond its YAML schema** — the pre-flight load surfaces those rules into working context, not only the field names and shapes: `canon-fact-record.yaml` specifies the **retrofit rule for `modification_history` arrays** (when a CF being modified has prior notes-field modification traces but an empty or absent array, retrofit all prior traces into the array alongside the new one — Rule 6 audit-trail discipline; full rule in `references/accept-path.md` artifact 2); `change-log-entry.yaml` specifies **change_type selection guidance** (use `addition` for any change whose dominant action is appending new CFs, even when paired with qualifications to existing CFs; use `*_retcon` only when the dominant action is modifying an existing CF's scope, cost, perspective, chronology, or ontology; use `clarification` for pure annotation additions with no field changes); `adjudication-report.md` specifies the **Discovery-section canonical field names** (`mystery_reserve_touched`, `invariants_touched`, `cf_records_touched`, `open_questions_touched`, `change_id`) that future canon-addition runs grep to locate prior adjudications on the same surfaces — ad-hoc field-name substitution breaks that grep-discoverability and is treated as template non-compliance. Loading templates upfront avoids mid-flow template reads and keeps both the reference schema AND these operational rules visible during drafting. If escalation turns out not to fire, the critic templates remain unused — cost is a single extra read.

    **Pre-figuring diegetic-artifact check (mandatory when proposal names specific entities).** When the proposal names specific places, characters, artifacts, rituals, organizations, taverns, monuments, or other named entities, glob `worlds/<world-slug>/characters/*.md` and `worlds/<world-slug>/diegetic-artifacts/*.md` and grep each directory for the named entities. Any prior diegetic commitment is a **pre-figuring** — the new CF MUST cite the diegetic-artifact id (DA-NNNN) or character id (CHAR-NNNN) in `source_basis.derived_from` per Rule 6 (No Silent Retcons) audit-trail preservation. A silent CF commitment of a name that was already introduced at diegetic level is a retcon of that diegetic artifact's local-scope resolution into world-level canon without the attribution trail — Rule 6 violation. Phase 0 classification must acknowledge the diegetic-to-world-canon formalization register when a pre-figuring is found (the new CF formalizes diegetic precedent rather than inventing fresh naming). The check does NOT constitute consuming diegetic content as canon (per `references/guardrails.md` no-diegetic-artifact rule — diegetic artifacts remain narrator-voice material whose truth-status is filtered by the artifact's own epistemic discipline); it is a CF-record-level audit-trail obligation. Scope of the scan: stop at `characters/` and `diegetic-artifacts/` sub-directories; do NOT extend to `adjudications/` (skill-outputs about canon, not diegetic artifacts) or `proposals/` / `audits/` (canon-pipeline artifacts, not diegetic texts). If the world has no `characters/` or `diegetic-artifacts/` sub-directories yet, the scan is vacuously satisfied. This check runs at pre-flight — it does not depend on escalation firing, and thus serves as the structural catchment for pre-figuring even on narrow proposals (≤3 domains) that produce no Phase 6b critic synthesis.
...
5. **Branch on verdict.**
   - **Accept outcomes** (ACCEPT / ACCEPT_WITH_REQUIRED_UPDATES / ACCEPT_AS_LOCAL_EXCEPTION / ACCEPT_AS_CONTESTED_BELIEF): load `references/accept-path.md` and execute Phases 12a → 13a → 14a → 15a. **Clarificatory-retcon routing**: when the Phase 0 classification (per `proposal-normalization.md` §Retcon-Proposal Inputs) identified the proposal as `retcon_type: A` (Clarificatory) — i.e., no new CF is created, only domain-file patches and `modification_history` qualifications on existing CFs — follow the Clarificatory-Retcon Variant section in `accept-path.md` alongside the main Phase 12a-15a flow. The variant adapts Phase 13a artifact 1 (skipped), Phase 14a test applicability (tests 1/2/3/8 are N/A-adapted), and Phase 15a sub-step 3.ii (skipped when no new CF). The main flow (domain-file patches, adjudication record, Change Log Entry, Phase 12a modification_history scan) still applies. Two sub-disciplines are load-bearing and therefore inlined here rather than left to the reference:
     - **Phase 12a modification_history scan (mandatory)**: enumerate CFs requiring modification_history entries along three axes — (a) every CF in the proposal's `derived_from_cfs`; (b) every CF named in Phase 8 soft-contradiction findings; (c) every CF whose stabilizer, scope, consequence, or standing role is substantively extended by the new CF per Phase 9 repairs. Missing this scan produces silent qualification gaps; a CF that should have received a modification_history entry but didn't is a Rule 6 (No Silent Retcons) violation even when the underlying prose change was correctly attributed.
        ...
        **Named-polity-instance commitment (distinctive axis-(c) shape).** When a new CF commits a specific named polity's adoption status on an existing CF's abstract polity-asymmetric distribution (e.g., a new named city-CF commits "Brinewick (CF-NNNN) adopts PUNITIVE-WITH-LICENSING-EXPERIMENT on CF-MMMM crafter-tolerance axis"; a new named polity-CF commits "Polity-X adopts LICENSING-UNDER-TIGHT-CHARTER on CF-MMMM mercenary-licensure axis"), the commitment qualifies the axis CF's standing-role distribution and warrants a `modification_history` entry under axis (c). This is distinct from cross-reference (mere acknowledgment that the new CF sits within the axis's abstract distribution without committing a specific adoption status). Decision test for this shape: does the new CF's adoption-status statement add named-polity-instance content the axis CF's distribution section would need to cross-reference back to when future readers enumerate which polities have been committed on that axis? If YES → substantive extension (`modification_history` required on the axis CF). If NO → cross-reference only. Worked precedent: CF-0038 Brinewick committed ADOPTING on CF-0031 contamination-registry — CF-0031's distribution section remains abstract-polity-category but its modification_history now records Brinewick as the first named-polity-instance; future readers of CF-0031 alone need to know Brinewick is specifically committed to read the distribution correctly. The same shape applied to CF-0021, CF-0024, CF-0028, CF-0033 at CH-0013 — five parallel named-polity-instance commitments in one change.
        ...
        **Retrofit discipline for previously-unpopulated arrays (cross-reference to `accept-path.md` artifact 2).** When the Phase 12a scan identifies a CF whose `modification_history` array is absent or empty (`modification_history: []`) but whose `notes` field contains prior `Modified YYYY-MM-DD by CH-NNNN (CF-NNNN): ...` lines, retrofit ALL prior notes-documented modifications into the array alongside the new entry — the machine-readable trace must not disagree with the human-readable trace in the same record. A partially-populated `modification_history` array that lists only the current modification while the notes field references earlier ones is the Rule 6 audit-trail gap this discipline exists to prevent. Retrofitting is a one-time catchup operation: once the array is complete, subsequent modifications append normally. See `accept-path.md` artifact 2 for the practical grep-and-enumerate check at Phase 15a sub-step (i).
