...
- `tools/<package>/tests/**/*.ts` — additionally in scope when the surviving finding's likely Files-to-Touch list would propose new or modified tests (i.e., when the finding implicates an existing-code path that has a test file at HEAD). The tests-scope extension is OPTIONAL rather than mandatory — audits that aren't filing test-touching tickets don't pay the extra grep cost — but when fired it surfaces the existing test-file naming convention so the ticket's Files-to-Touch list can cite the exact path rather than guess at a plausible-looking but stale path (worked precedent: MCPENH-047 in this session — the ticket's Files-to-Touch entry cited `tools/world-index/tests/parse/prose.test.ts` while the actual existing path was `tools/world-index/tests/prose-domain-file.test.ts`; a tests-scope grep at Phase 5 would have surfaced the correct path and avoided the post-Write linter correction)
