...
Then cross-check each finding against the target skill's Guardrails for known-deferred-debt disclosures:
- A disclosure of the form `"<surface> via <tool>; <NAMESPACE>-NNN lands the support — see Guardrails §Known integration debt"` matches a finding when the surface and the routed namespace+ID align.
- A disclosure of the form `"until <NAMESPACE>-NNN lands; manual scan as fallback"` matches when the candidate finding's session-evidence one-liner cites that same fallback.
- Matched findings are **dropped as already-disclosed-not-a-finding** — they appear in the Phase 6 summary table with status `dropped (disclosed via <NAMESPACE>-NNN)` so the audit trail records the disclosure was honored.
