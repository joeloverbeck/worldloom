...
Then cross-check each finding against the target skill's Guardrails for known-deferred-debt disclosures, plus two additional drop categories that don't fit the known-deferred-debt shape (design-intent and archive-grep-confirmed):
...
- **All four matched categories are dropped as already-disclosed-not-a-finding** — they appear in the Phase 6 summary table with the corresponding status string so the audit trail records the disclosure (or design-intent, or archive-grep) was honored.
