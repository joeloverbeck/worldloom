...
- **Commitment-Route Coverage** — for every audited branch, tabulate `arc.arc_contract.commitment_family` and `arc.arc_contract.commitment_class` across realized arcs by resolving each page's `storylet_realized` SLT. Derive family from class for legacy/implicit records. Report per-branch and bundle-aggregate family distribution and class distribution separately. Commitment families or classes from the canonical vocabularies that appear in 0 realized arcs are `info` under-utilization findings. Any single `commitment_class` appearing in more than 40% of pages is a `warning` for commitment-monoculture; any single family dominating the branch should be reported as broad-route concentration unless the story premise lawfully has a narrow route surface. `commitment_detail` is optional precision and is not a coverage quota axis.
...
- Compose the card per `templates/remediation-storylet-proposal-card.md`. Required frontmatter (parallels `storylet-pool-authoring`'s `source_audit_path` parse-time consumer schema): `rsp_id`, `audit_id`, `story_id`, `finding_ids[]`, `target_obligation` (or null), `target_thread` (or null), `target_consequence` (or null), `target_relationship` (or null), `target_commitment_family` (or null; value from the canonical `commitment_family` vocabulary), `target_commitment_class` (or null; value from the closed base `commitment_class` vocabulary), `target_commitment_detail` (or null; optional story-specific precision and not a deterministic join key), `target_arc_archetype` (or null; orienting pattern label from the storylet archetype library or a story-specific snake_case label), `proposed_intensity` (`tame | mature | explicit`), `target_branch` (a branch_path | `"all branches"` | `"global pool"`), `proposed_visibility` (with `scope: global_author_pool | branch_scoped | branch_prefix_scoped` and `visible_branch_path_prefix`), `sketch` (with `hard_preconds`, `fact_effects`, `pays_off_obligations`, `opens_obligations`, `addresses_consequences`, `sketch_arc_contract`, and `sketch_dramatic_unit`), `rationale`. Body sections: Diagnosis / Proposed remediation / Routing.
...
7. When present, `target_commitment_family` and `target_commitment_class` are values from the canonical vocabularies exposed by `mcp__worldloom__get_canonical_vocabulary`; when both are present, the family matches the class-to-family mapping. `target_commitment_detail`, when present, is a non-empty story-specific precision label and is never used as the only deterministic join key. `target_arc_archetype` is a non-empty snake_case orienting pattern label; library values are preferred when they fit, but story-specific labels are valid when the card rationale or sketch explains the dramatic structure.
...
- Branch PG-NNNN: top family <commitment_family>=<N pages>; top class <commitment_class>=<N pages>; missing families/classes <count>/<count>; over-represented classes <list or none>
