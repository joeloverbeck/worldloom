...
- `worlds/<world_slug>/stories/<story_slug>/_source/<class>/*.yaml` — every other record class read per Phase 2 sub-phase needs (SF, BEL, OBL, CNSQ, THR, SREL, STENT, STSTAT, STINT, STLOC, STOBJ, DA, CLK, STSEC, STQ, STPLAN, STEMO, STCHAR, SLT, CHC, SCN)
- `story_bundle_context.scene_coverage` (SPEC-99 / SPEC-95) — bounded, prose-free index surface for the Phase 2c scene-coverage / prose-debt sub-check: per-`PG` `scene_ids[]` + `unscened` over the active branch path, and per-`SCN` `publication_indicator` + `superseded`. Carries no scene prose. Retrieve full `SCN-*` bodies via `mcp__worldloom__get_record` only when a finding needs the scene's PG range or title; `null` when the bundle has no coverage rows or the layer was trimmed under budget pressure (skip the sub-check, not a finding)
...
Targeted retrieval discipline: `story_bundle_context` is an index and summary surface, not full audit authority. When it identifies a material `STPLAN` / `STEMO` / `STSEC` / `STQ` / `CLK` / `SCN` record, retrieve the full body with `mcp__worldloom__get_record`, `mcp__worldloom__get_records`, or a filtered `mcp__worldloom__list_records(..., include_full_body=true)` before issuing CHC-grounding, SLT predicate/effect, PG/SE, prose-receipt, scene-coverage, or health findings that depend on basis, blockers, appraisal, orientation, clue, payoff, clock payload, or scene PG-range detail. The `scene_coverage` layer's prose-free per-`PG` / per-`SCN` projection is sufficient for the Phase 2c scene-coverage / prose-debt sub-check; pull an `SCN` body only when a finding's rationale needs the scene's PG range or title.
...
**Scene-coverage / prose-debt sub-check (per SPEC-99 §2 item 5; reads `story_bundle_context.scene_coverage`)**

This sub-check measures *prose-rendering debt* — the drift between committed causal ticks (`PG` snapshots) and the rendered-scene layer that publishes them. Per FOUNDATIONS §Story Bundles §4a, rendered prose is strictly downstream of committed state: the check measures debt, it never gates, mutates, or supersedes any `PG` / `SCN` / `SE` record. It reads only the bounded, prose-free `story_bundle_context.scene_coverage` index surface (`pages[]` = per-`PG` `scene_ids[]` + `unscened` over the active branch path, root-to-leaf; `active_scenes[]` = per-`SCN` `publication_indicator` + `superseded`). Retrieve a full `SCN` body with `mcp__worldloom__get_record` only when a finding needs the scene's PG range or title for its rationale — never read scene prose. When `scene_coverage` is `null` (no coverage rows, or the layer was trimmed first under budget pressure), record a `scene-coverage-absent` note and skip the sub-check — absence of scenes is never itself a finding.

Resolve each page's effective coverage from the packet: a page is **truly unscened** when `unscened: true` (empty `scene_ids`); a page is **covered-only-by-superseded** when every `SCN` in its `scene_ids[]` carries `superseded: true` in `active_scenes[]`. Both leave a committed tick without a live rendered scene.

- `unscened_run` — `N=3` or more consecutive truly-unscened pages along a branch's active path (committed ticks have drifted ahead of the rendered-scene layer). WARNING; `repair_kind: prose_revision` (plan the range with `branching-story-scene-plan`, then render + attach with `branching-story-scene-prose-attach`). Cite the branch id, the page window, and the run length. A run shorter than `N` is normal rendering lag and is not flagged.
- `scene_planned_no_prose` — an active `SCN` whose `publication_indicator: planned` (a scene was planned but no prose is present or attached). WARNING; `repair_kind: prose_revision`. Cite the `SCN` id and branch id; retrieve the `SCN` body for its PG range only if the operator needs it.
- `scene_receipt_warn` — an active `SCN` whose `publication_indicator: attached:WARN` (prose was attached but its receipt carried warnings). WARNING; `repair_kind: prose_revision`. An `attached:FAIL` indicator surfaces here as the same family at ERROR severity, cross-referenced to Phase 3 prose mode (`prose_receipt_failed`), which owns the full receipt-verdict detail; the structural index-surface signal ensures a default `structural` run still surfaces a failed or warned receipt without requiring `prose` mode.
- `superseded_scene_not_resuperseded` — an active page covered only by `superseded` `SCN`(s) with no live successor scene covering the same range (the prior scene was superseded but never re-rendered). WARNING; `repair_kind: prose_revision`. Cite the affected page id(s), the superseded `SCN` id(s), and the branch id.

Rule 7 firewall: this sub-check reads scene-coverage *membership metadata only* (page↔`SCN` bindings, publication indicators, supersession flags). It resolves, narrows, or touches no Mystery Reserve entry, and it writes nothing outside `audits/`.
...
This chain-level pass scans each scoped PG chain for sequences of pages where every `turn_driver.kind = player_action | player_write_in` despite high-urgency active non-player records being available as lawful drivers. The scan treats the same non-player pressure candidate set as `branching-story-turn-cycle` Phase 0 / FOUNDATIONS §Story Bundles §5c ("Driver salience is local"): `STPLAN` with a due `current_step`, `STEMO` at high intensity carrying behavioral pressure, `CLK` at or past a threshold, active high-urgency `THR`, reveal-ready `STSEC`, `OBL` falling due, pending `CNSQ`, and active high-urgency `STINT` whose holder can act. A page counts for the inertness window when the page-resolving event's `SE.turn_driver` selected/deferred/rejected dispositions (and the event `world_logic_rationale`) show one or more such records deferred, rejected, or displaced by player initiative while the committed `SE.turn_driver.kind` remains player-driven. (Dispositions live on the committed event, not in a page-plan render table — the 19-section page-plan contract that carried the former §7a "active-pressure disposition table" is retired; see `_shared-templates/story-state-contract.md` §8.)

Emit `reactivity_inertness_sequence` when 3+ consecutive pages match the pattern. Severity is WARNING, with `repair_kind: commitment_block` when a missing author-pool move would let the pressure act, or `repair_kind: turn_repair` when the page chain likely needs a repair turn or rationale repair. The finding cites the branch id, page window, player-driven SE ids, active non-player record ids, their `SE.turn_driver` dispositions, and the reason the window is a pressure-inertness pattern rather than a normal player-led sequence.
...
Run the SPEC-80 §3 coverage check against the current bundle state. Emit WARNINGS, not hard fails, for uncovered driver-kind, pressure-source-class, and composition gaps. This phase aligns with Phase 2n: Phase 2n detects the downstream symptom of active pressure sitting inert across a player-driven chain, while Phase 2o detects the upstream cause where the pool cannot express the demanded driver-kind or source-class at all. Phase 2o has two checks: (1) coverage-gap detection (pool vs demand) and (2) permanently-inert storylet detection (each pool supply entry vs the committed history).
...
**Permanently-inert storylet detection.** A coverage gap measures the pool against *demand*; this sub-check measures each *supply* entry against the bundle's committed history to surface blocks the state lifecycle has rendered dead. For every `SLT` whose `scope.visibility` is `global_author_pool` or `branch_prefix_scoped` (skip `branch_scoped` JIT blocks — their eligibility is local to their own branch by design), test whether the block was eligible on **at least one** committed page across the audited scope. Use the same bind-then-instantiate eligibility discipline as Phase 2l / `branching-story-turn-cycle`: the block's `preconditions.hard[]` must be satisfiable against that page's `state_snapshot.active_records` (existential aliases bound against active records before any `bound:<alias>` reference is considered). Emit `storylet_permanently_inert` (`severity: warning`, `repair_kind: commitment_block`) for each pool block eligible on **no** committed page in scope. Cite the SLT id, the specific unsatisfiable hard predicate(s), and the pages checked.

Distinguish **never-eligible** (flagged) from **eligible-but-unused** (a block whose preconditions *were* satisfiable on one or more committed pages but that was never selected — this is normal pool slack and is **never** flagged). Eligibility, not selection, is the test.

Add a sub-reason `inert_cause: unsatisfiable_affect_kind` when the cause is an exact-value affect predicate — `any_emotion_active(kind=…)` or `emotion_active(kind=…)` — whose named `affect_kind` appears in no active `STEMO` (status `active`, not superseded) anywhere across the audited committed history. The finding then additionally names the absent `affect_kind` and the lifecycle-adjacent affect kinds that *are* active in scope (e.g. an `any_emotion_active(kind=dread)` block where the bundle's `STEMO` lifecycle matured `dread` into `fear`/`anxiety` and no active `dread` remains). The actionable hint: "broaden the predicate via `commitment-block-authoring` (drop the over-narrow `kind`, or use an `any[…]` over lifecycle-adjacent affect kinds) so the block can fire across the fear-family pressure it was authored for." This is the upstream-of-Rule-5 signal: a permanently-inert seed block is dead schema weight that silently shrinks the eligible move pool and is a latent consequence-capacity risk (FOUNDATIONS §Story Bundles §5a/§5b, Rule 5).
...
## Scene coverage / prose debt

(Phase 2c scene-coverage / prose-debt sub-check findings: `unscened_run`, `scene_planned_no_prose`, `scene_receipt_warn`, `superseded_scene_not_resuperseded`. Note `scene-coverage-absent` here when the `scene_coverage` layer was null/trimmed and the sub-check was skipped.)
...
- **Rule 5 (No Consequence Evasion)** — Phase 2a (Choice Consequence Integrity replay) + Phase 2c (debt and saliency health + scene-coverage / prose-debt sub-check) + Phase 2f (continuation / terminal proof) + Phase 2g (causal dependency health) + Phase 2n (reactivity inertness) + Phase 2o (storylet pool coverage). Mechanism: cosmetic accepted-choice findings + unactionable / invalidated / ignored / saliency-starved debt findings + scene-coverage / prose-debt findings (`unscened_run` / `scene_planned_no_prose` / `scene_receipt_warn` / `superseded_scene_not_resuperseded`) read from `story_bundle_context.scene_coverage` + terminal-without-rationale + orphan-debt-at-terminal findings + clobbered CHC / affordance / OBL / SLT dependency findings + 3+ consecutive player-driven pages despite available high-urgency non-player pressure + coverage warnings when the author pool cannot express active driver/source pressure + permanently-inert pool blocks that are eligible on no committed page.
...
| Rule 5 (No Consequence Evasion) | Phase 2a, 2c, 2f, 2g, 2n, 2o | Choice Consequence Integrity replay findings + debt and saliency health + scene-coverage / prose-debt findings (unscened runs and un-rendered scenes are a real second-order effect of advancing committed state) + continuation-or-terminal-proof findings + causal dependency findings for clobbered CHC / affordance / OBL / SLT dependencies + chain-level reactivity inertness findings + storylet-pool coverage warnings for active pressure that no SLT can express + permanently-inert pool-block findings. |
...
| §Story Bundles §4a (Plan-Authority Boundary) | All phases, Phase 2c scene-coverage sub-check | Audit reads `PG` records as authoritative; never mutates them. Drift between prose and state is reported in findings, not in PG records. The Phase 2c scene-coverage / prose-debt sub-check measures prose-rendering debt (unscened runs, `planned` / `attached:WARN` scenes, un-resuperseded supersessions) from `story_bundle_context.scene_coverage` — rendered prose is strictly downstream of committed state, so the check reports debt and never gates or supersedes any `PG` / `SCN`. |
