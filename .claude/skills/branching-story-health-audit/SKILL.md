...
9. Re-surfaced findings cite their prior `SAU-NNNN` via `prior_audit_reference`. (Loop → Phase 7)
...
- **Sibling interop**:
  - **Consumes (existing)**: `branching-story-bootstrap` outputs (story bundle structure including `STORY_KERNEL.md.audited_thread_obligation_sketch`, `_source/<class>/`, `pages-prose/`); `branching-story-page-cycle` outputs (PG / SE / CHC records and `narrative_health` flags including `flagged_for_audit`); `storylet-pool-authoring` outputs (SLT records — visibility scopes audited at Phase 3 Storylet-Scope Leakage, and `runtime_jit` provenance counted for high-JIT-rate branch summaries).
