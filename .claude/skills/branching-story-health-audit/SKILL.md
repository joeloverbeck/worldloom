...
7. When present, `target_commitment_class` is a value from the canonical vocabulary exposed by `mcp__worldloom__get_canonical_vocabulary`. `target_arc_archetype` is a non-empty snake_case orienting pattern label; library values are preferred when they fit, but story-specific labels are valid when the card rationale or sketch explains the dramatic structure.
