...
Pre-flight Check (load FOUNDATIONS; resolve worlds/<slug>/stories/<story-slug>/;
                  ...
                  list_records; load `task_type='other'` context packet
                  for governing CFs [until MCPENH-017]; read prior
...
- **Premise-bounded world-canon retrieval** via `mcp__worldloom__get_context_packet(world_slug, task_type='other', seed_nodes=[<resolved entity:slug ids from cast-bound STENTs' world_ent_id + recent page-history named entities>], token_budget=12000)` — supplies governing CFs the audit cross-references for Phase 4 canon-baseline drift. **Until MCPENH-017 lands**, `task_type='other'` (defaultRankingProfile, 8000-token default budget — increase to 12000 explicitly) is the correct interim choice; the deferred ticket registers `branching_story_health_audit` as a task_type whose ranking profile prioritizes governing CFs touching cast/location/period and CFs added since the bundle's `canon_revision` baseline.
...
- Load premise-bounded world-canon retrieval via `mcp__worldloom__get_context_packet(world_slug, task_type='other', seed_nodes=[<resolved ids>], token_budget=12000)`. Apply the packet-too-large fallback per §World-State Prerequisites if signal-overflow.
