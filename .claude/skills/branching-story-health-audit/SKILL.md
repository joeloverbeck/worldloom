...
- **Prose-Ledger Consistency (Rule 1 + Rule 7 boundary)** — for every in-scope `pages-prose/PG-*.md`, compare the rendered prose to the corresponding `PG.state_snapshot` and POV-accessible context:
  ...
  - Grounded offstage reference is clean: mere mention, memory, rumor, inscription, DA content, or offstage reference to an absent character is allowed when grounded in `objective_facts`, `apparent_facts`, `disputed_facts`, `reader_known_facts`, `belief_state_by_actor`, DA content, or POV-accessible world context.
