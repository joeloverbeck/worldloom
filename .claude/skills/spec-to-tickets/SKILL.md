...
**Flow**: compose every ticket's full content in mind first, THEN emit all Write calls together in 1-2 batched messages (N parallel Write tool calls per message). Do NOT alternate compose → Write → compose → Write per ticket — that produces N sequential one-Write-per-message sends, which the §Batch All Write Calls in One Message sub-rule below explicitly forbids. The flow distinction is what the rule requires; the per-ticket field list and content composition that follow this Flow directive describe WHAT to put in each ticket, but the WHEN-to-emit timing is the batched-after-all-composed shape — not interleaved with composition.
