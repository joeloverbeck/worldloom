...
**Worked precedent**: spec-to-tickets on SPEC-21 (2026-05-08 session) — 7 tickets emitted as 7 sequential one-Write-per-message sends despite this rule; correct shape would have been 1 batched message with 7 parallel Write calls (single round-trip), or 2 batched messages given that 1 ticket exceeded 300 lines and 2 others were near-300. Cost of the violation: 7× round-trips and ~5× wall-clock vs. the 1-2-message canonical shape; output correctness preserved (all 7 tickets landed with correct content), but the audit-trail signal is that the operator composed ticket 1 → wrote ticket 1 → composed ticket 2 → wrote ticket 2... rather than composing all 7 first and batching the writes. The Flow directive at the top of this §Step 5 names this anti-pattern explicitly so a future operator catches the mistake before the first Write call lands.
