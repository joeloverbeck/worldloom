...
- **(g) New-class / new-op / new-id-class spot-check**: when the spec introduces a new record class, patch-engine operation kind, ID class, or schema node-type, spot-check that an analogous existing sibling class's full registry seam (id-prefix pattern, node-type constant, op-kind enum, parser / overlay / index / retrieval registration sites across `tools/*/src/**`) is covered by the spec's deliverables — parallel to (e)'s rename/removal blast-radius spot-check, and applicable even when `/reassess-spec` ran. Route any gap per (e): add the site to the relevant ticket's Files to Touch, split it into a follow-up ticket with an explicit dependency, or flag it as an Issue at Step 2.
