...
- **New-class / new-op / new-id-class parity scan**: when the spec introduces a new record class, patch-engine operation kind, ID class, or schema node-type, pick an analogous *existing* sibling class and grep the codebase for every registry, parser, overlay, and enumeration site that sibling occupies — its id-prefix pattern, its node-type constant, its op-kind enum membership, its pre-apply / commit / index / retrieval registrations across `tools/*/src/**`. The bullets above confirm the surfaces the spec *names* exist; this scan confirms the spec *named all* the surfaces. Under-enumeration of the machine-layer seam is an **Issue** for disposition (typically `expand-scope-in-place`) — it is the most common way a spec's deliverable list silently understates a ticket's true Files to Touch. This adapts the cross-package-count-grep and blast-radius discipline `reassess-spec` documents at its `references/codebase-validation.md`.
