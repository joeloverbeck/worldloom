...
(b) Step 2 (Codebase Validation) has completed, and every surfaced Issue (missing file, renamed type, stale skill reference, broken Deps chain, FOUNDATIONS-violating assumption) has an explicit user disposition — fix before decomposition, defer to a follow-up ticket with a named dependency, reject with rationale, or expand scope in place (when validation surfaced materially understated spec scope; the spec narrative is not edited, but the decomposition is sized against the wider implementation surface the codebase actually requires).
...
- **(g) New-class / new-op / new-id-class / new-field spot-check**: when the spec introduces a new record class, patch-engine operation kind, ID class, schema node-type, or a new field on an existing record class or schema, spot-check that the analogous existing sibling's full registry seam — for a new class/op/id-class/node-type, the sibling class's id-prefix pattern, node-type constant, op-kind enum, parser / overlay / index / retrieval registration sites across `tools/*/src/**`; for a new field, the host record/schema's schema files, contract-doc surface, validators, and producing/consuming skills — is covered by the spec's deliverables — parallel to (e)'s rename/removal blast-radius spot-check, and applicable even when `/reassess-spec` ran. Route any gap per (e): add the site to the relevant ticket's Files to Touch, split it into a follow-up ticket with an explicit dependency, or flag it as an Issue at Step 2.
...
Send **a small number of assistant messages containing parallel Write tool calls (one per ticket)**, not N sequential one-Write-per-message sends. For typical spec sizes (up to ~9 tickets) **with moderate ticket bodies (~100-200 lines each)**, a single batched message is the target; for 10+ tickets OR for any decomposition where ticket bodies are very large (~300+ lines each, common for full-package-introduction tickets that include package.json snippets, multi-file `What to Change` sections, and detailed Verification Layers), 2-3 batched messages are acceptable so individual assistant messages stay legible. The batching threshold is body-size-aware, not just count-aware. Each ticket file write is independent because each creates a new file, so they can run concurrently. A single batched message with N Write calls costs one round-trip; N sequential one-Write-per-message sends cost N round-trips — the ban is on per-ticket sequential messaging, not on any multi-batch approach. Compose every ticket's full content first, then emit all Write calls in batched messages.
