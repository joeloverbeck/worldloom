...
- `.claude/skills/_shared-templates/story-record-schemas.md` §4.5.17 / §4.5.18, `.claude/skills/_shared-templates/story-state-contract.md` §5a, `docs/CONTEXT-PACKET-CONTRACT.md`, and `docs/MACHINE-FACING-LAYER.md` — STPLAN/STEMO may explain evidence context but do not become promotion source classes
...
- Any active or superseded `STPLAN` / `STEMO` records named by the source records or authoring `SE.world_logic_rationale`. Treat them as explanatory evidence only; do not place them in `candidate.source_basis.derived_from[]`, do not add them to `pre_figured_by[]`, and do not create a new source kind from them.
