...
arguments:
  ...
  - name: source_diegetic_artifact_id
    ...
    required: false
  - name: source_arc_id
    description: "Required when source_kind=arc_effect_promotion. The SLT-NNNN scene-commitment arc whose effect_model.variants[] contains the promoted variant."
    required: false
  - name: source_page_id
...
- Page-prose retrieval along `promotion_branch_path` via direct `Read` of `worlds/<world-slug>/stories/<story-slug>/pages-prose/PG-*.md` — Phase 1 gathers supporting prose excerpts that the proposal_package surfaces as evidence (the page-prose surface is not under any `_source/` and not under context-packet retrieval; direct read is the established pattern from `branching-story-page-cycle` Phase 1). Every PG cited as supporting evidence must have `prose_status == "rendered"` (or no `prose_status` field, for pre-PROSESPLIT grandfathered pages); Pre-flight step 6 enforces this as a HARD-ABORT before Phase 1. The aborted-pre-flight directive instructs the user to run `branching-story-page-prose-finalize` on the pending page(s) before re-running canon promotion — world-canon mutation requires prose evidence per FOUNDATIONS Rule 6 + §Default Reality.
...
6. **HARD-CHECK: every supporting evidence page is rendered.** Assemble the set of PG-NNNN ids that will be cited as `provenance.supporting_pages[]` and `provenance.supporting_prose_excerpts[].page` in the Phase 8 proposal_package. This set is derivable at Pre-flight from `source_kind`-specific rules: for `story_fact`, the SF's branch_path pages along `promotion_branch_path` carrying SF establishment / corroboration evidence; for `mystery_resolution`, `resolving_page_id` plus the page that emitted the resolving choice; for `character_arc_outcome`, the PG ids where STINT history shifted along `promotion_branch_path` plus `source_branch_leaf_page`; for `artifact_canonization`, the PG ids where the DA was created or modified plus `source_branch_leaf_page`; for `arc_effect_promotion`, `source_page_id` (load the page's prose for the proposal_package excerpt). For every PG-NNNN in that set, load `worlds/<world-slug>/stories/<story-slug>/_source/pages/PG-<NNNN>.yaml` and inspect `prose_status`. If any PG has `prose_status != "rendered"` (and the field is present — pre-PROSESPLIT pages lacking the field are grandfathered as `rendered`), HARD-ABORT with the directive message: `"Cannot promote <SF-NNNN | M-NNNN | STENT-NNNN | DA-NNNN | SLT-NNNN+PG-NNNN> to canon — supporting evidence page <PG-NNNN> has prose_status=<status>. Run branching-story-page-prose-finalize on the pending page(s) before re-running canon promotion. World-canon mutation requires prose evidence (FOUNDATIONS Rule 6: No Silent Retcons; FOUNDATIONS §Default Reality)."` List every pending PG id, not just the first. The skill emits no SP ledger entry, no SP allocation occurs (this step is BEFORE step 7's SP allocation), and the pre-flight failure is reported back to the user.
7. Allocate `SP-NNNN` via `mcp__worldloom__allocate_next_id(world_slug, 'SP', story_slug=<story_slug>)`.
8. Load world canon via `mcp__worldloom__get_context_packet(task_type='story_fact_promotion_to_canon', world_slug=<slug>, seed_nodes=[<source-relevant CFs+Ms+INVs+OQs>], token_budget=8000)`. If it returns `delivery_status='persisted_with_summary'`, recover through `governing_summary`, `get_records(record_ids=[...])`, and `get_persisted_packet_slice(persisted_path=task_header.persisted_output_path, slice_path='<dot-path>')` instead of reading the raw persisted JSON into the main context.
9. Load whole-class M for the Phase 4 mystery firewall: `mcp__worldloom__list_records(world_slug, record_type='mystery_record', include_full_body=true)`. Mystery Firewall is class-bounded — partial loads cannot satisfy the `forbidden`-status hard-reject discipline.
10. Confirm the content_policy block (NC-21 verbatim per the source proposal §Content Policy) is loaded for downstream LLM critic prompts and the proposal-package handoff.

If any step fails, the skill aborts before Phase 1 and emits no SP ledger entry (the failure is a pre-flight failure, not a promotion outcome). The step-6 prose_status hard-check fires BEFORE SP-NNNN allocation, so an aborted pre-flight on a pending-evidence promotion never consumes an SP id slot.
...
- Load the scene-commitment arc `SLT-<source_arc_id>.yaml`, including `arc_contract`, `dramatic_unit`, `execution_envelope`, `effect_model.variants[<applied_variant_id>]`, and the selected `required_effects[<effect_index>]` entry.
...
| Rule 6: No Silent Retcons | Pre-flight (prose-evidence hard-check) + Phase 9 + Phase 10 + (downstream) canon-addition Phase 13a | Quadruple-redundant audit: Pre-flight step 6's prose_status HARD-ABORT (every cited supporting evidence page must be `rendered` — pending pages never enter the proposal_package, preserving the FOUNDATIONS contract that world-canon mutation requires prose evidence); SP-NNNN ledger (Phase 10); CH-NNNN Change Log (canon-addition); PA-NNNN adjudication (canon-addition). The story-local source record's superseding entry adds `promoted_to_cf: CF-NNNN` (Phase 10 Step 2) — the link is recorded, not erased. Story-local truth and world-level truth are tracked separately even after promotion. |
