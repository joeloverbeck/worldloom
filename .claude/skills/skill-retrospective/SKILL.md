...
**Optional:**
- `namespace_overrides` — comma-separated `<finding-id>=<namespace>` pairs (e.g., `F2=VALENH,F4=BSBOOT`). Applied at Phase 3 after auto-routing. A namespace in an override may be any existing prefix found in `tickets/` + `tickets/archived/` + `archive/tickets/` OR one of the fixed pipeline namespaces; an unknown prefix is treated as a fresh-prefix proposal and surfaced for confirmation at Phase 6. Rare; the default per-finding routing is usually correct.
...
Namespace allocation is DEFERRED to Phase 7 (per-finding ticket numbers are unknown until dispositions are collected at Phase 6); Pre-flight only confirms `tickets/`, `tickets/archived/`, and `archive/tickets/` are scannable. If any check fails, abort before Phase 1.
...
Then emit a brief drops table covering every candidate dropped at any phase — Phase 3a operator-mistake drops, Phase 3a operator-judgment-borderline drops, Phase 4 drop-category drops, Phase 5 false-positive reclassifications, and within-invocation subsumption drops (where one Phase 2 candidate's concern is consumed by another finding's filed primary edit and re-filing would duplicate within this batch). One row per dropped candidate naming the lens, the drop origin, and a brief rationale. Canonical status-string shapes: `dropped (operator-mistake — <one-line reason>)`; `dropped (operator-judgment-within-spec — <one-line reason>)`; `dropped (<Phase 4 category name per the taxonomy above> — <one-line reason>)`; `dropped — false-positive (<Phase 5 verification one-liner>)`; `dropped (subsumed by <other-finding-id>'s primary edit — re-filing would duplicate within this batch)`. The drops table is required for Rule 6 (No Silent Retcons) audit-trail completeness — it makes "honestly dropped" verifiable in chat rather than implicit in working memory, and the full-phase coverage prevents Phase 3a drops from being silently retconned out of the chat record when a narrower literal scope would otherwise exclude them. The within-invocation-subsumption category is skill-retrospective-local — distinct from category (6) cross-skill `in-session-sibling-filed-work` in the sibling-owned Phase 4 taxonomy — because skill-retrospective's broader 7-lens scope produces more intra-invocation subsumption than the sibling's narrower 6-pattern scope. When zero candidates were dropped at any phase, emit a one-line `No candidates dropped at any phase.` confirmation in place of the drops table so the absence is still recorded.
...
1. Allocate its ticket number by scanning `tickets/` + `tickets/archived/` + `archive/tickets/` for the next free integer in its namespace; padding follows existing per-namespace precedent (most live namespaces use 3-digit `<NNN>`). Collisions abort with a specific-id error before any Write.
...
| Per-class ID format conventions (FOUNDATIONS-002) | Phase 7 | Ticket IDs follow each namespace's existing `<NNN>` precedent, allocated by scanning `tickets/` + `tickets/archived/` + `archive/tickets/` for the next free integer. |
