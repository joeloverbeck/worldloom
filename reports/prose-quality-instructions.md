> **Derivative artifact.** This document is regenerated by re-extracting from the canonical sources listed below. Do not edit it directly; edit the sources and re-extract. The skills' references remain source-of-truth for in-skill consumers; this report is the canonical-source bundle that the plan-authoring skills inline into per-page plan §2 / §3 / §19. The rendered-page workflow does not concatenate this file at render time — the per-page plan body is self-contained.
>
> Canonical sources:
>
...
> - `.claude/skills/branching-story-bootstrap/references/phase-7-root-page-plan.md` §LLM prompt assembly INSTRUCTION block — scene-setter flavor.
> - `.claude/skills/branching-story-page-cycle/references/phase-7-page-plan.md` §LLM Prompt Assembly INSTRUCTION block — multi-beat arc flavor.

# Prose Quality Instructions for External Renderer

This is the canonical source bundle for the plan-authoring skills (`branching-story-bootstrap` Phase 7 and `branching-story-page-cycle` Phase 7). It bundles the content policy, the eleven-rule Prose Craft Contract, a render-time instruction template that works for both root scene-setter and multi-beat-arc rendering, an anti-pathology checklist, the voice-and-register caveat, and a usage guide.
