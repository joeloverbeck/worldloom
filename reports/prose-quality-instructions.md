...
**The plan IS the prompt.** The plan-authoring skills (`branching-story-bootstrap` Phase 7 and `branching-story-page-cycle` Phase 7) inline §Content Policy, §Prose Craft Contract, and §Render-Time Instruction Template from this report into the per-page plan body as §2, §3, and §19 respectively. The rendered plan at `worlds/<world-slug>/stories/<story-slug>/pages-prose-plans/PG-NNNN.md` is therefore self-contained.
...
The plan body §1-§19 supplies every block the renderer needs: the content policy (§2), prose craft contract (§3), story kernel context (§4), world canon snapshot (§5), invariants (§6), mysteries-in-play firewall posture (§7), cast (§8), story-local facts (§9), open obligations (§10), active threads (§11), pending consequences (§12), locations and objects (§13), recent prose continuity (§14), the selected scene-commitment arc (§15) or root-case entry pressure framing (§15-alt), chosen variant (§16), governor nudge (§17), scene direction (§18), and the trailing render-time instruction block (§19).
...
The rendered prose lands at `pages-prose/PG-NNNN.md`. Run `branching-story-page-prose-finalize` to validate and merge — that skill runs the deferred prose-coupled validators (`prose_ledger_consistency`, `arc_trace_evidence_alignment`, `prose_critic_8_axis`) against the rendered prose and routes re-prompts via the standard 3-attempt budget if any axis surfaces a `SOFT_FAIL` or `HARD_FAIL` verdict. The external renderer does not own the critic loop; it owns only the production of one prose draft per invocation.
