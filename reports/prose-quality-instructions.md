> **Derivative artifact.** This document is regenerated by re-extracting from the canonical sources listed below. Do not edit it directly; edit the sources and re-extract. The skills' references remain source-of-truth for in-skill consumers; this report is the external-renderer-facing bundle.
>
> Canonical sources:
>
> - `.claude/skills/branching-story-page-cycle/templates/content-policy.txt` (and byte-identical `.claude/skills/branching-story-bootstrap/templates/content-policy.txt`) — content_policy block.
> - `.claude/skills/branching-story-page-cycle/references/prose-craft-contract.md` — Prose Craft Contract Rules 1-11 + Diagnostic Vocabulary.
> - `.claude/skills/branching-story-bootstrap/references/phase-7-root-page-render.md` §LLM prompt assembly INSTRUCTION block — scene-setter flavor.
> - `.claude/skills/branching-story-page-cycle/references/phase-7-page-render.md` §LLM Prompt Assembly INSTRUCTION block — multi-beat arc flavor.

# Prose Quality Instructions for External Renderer

This is the external prose renderer's discipline contract. It bundles the content policy, the eleven-rule Prose Craft Contract, a render-time instruction template that works for both root scene-setter and multi-beat-arc rendering, an anti-pathology checklist, the voice-and-register caveat, and a usage guide.

The external renderer concatenates the per-page plan (`pages-prose-plans/PG-NNNN.md` §1-§19) with §3 of this document and produces continuous prose. No commentary, no markdown headers, no engine vocabulary.
...
The external renderer (manual or automated — e.g., OpenRouter Opus 4.7) assembles a single prompt per page in this concatenation order:

1. **§Content Policy of this report** — the `<content_policy>...</content_policy>` block, FIRST so it binds the model before any other instruction.
2. **§1-§19 of the per-page plan** — `worlds/<world-slug>/stories/<story-slug>/pages-prose-plans/PG-NNNN.md`. The plan body supplies the page-specific structural blocks (story kernel snapshot, arc contract or entry pressure framing, dramatic unit, beat plan, execution envelope, stop policy, chosen variant required effects, cast bound, scene context, recent prose continuity, governor nudge, forbidden_resolutions list).
3. **§Prose Craft Contract of this report** — the eleven rules plus the Diagnostic Vocabulary table, verbatim.
4. **§Render-Time Instruction Template of this report** — the `INSTRUCTION:` block as the trailing directive.
...
The rendered prose returns to the worldloom pipeline as the body of `pages-prose/PG-NNNN.md`. The post-render critic and the Phase 7.6 validation gates inside `branching-story-page-cycle` consume that body and route re-prompts via the standard 3-attempt budget if any axis surfaces a `SOFT_FAIL` or `HARD_FAIL` verdict. The external renderer does not own the critic loop; it owns only the production of one prose draft per invocation.

Per-story `forbidden_resolutions[]` are inlined into each plan file at plan-authoring time, not into this report. This report is render-discipline only — it carries no per-story or per-bundle context.
