...
worlds/<world-slug>/             ← generated world bundles (contents gitignored; folder preserved)
  ...
  └── stories/<story-slug>/      ← branching-story bundles (per-bundle layout below)
       ...
       ├── _source/               ← atomic-YAML story-bundle records; record-class inventory is in flux while the story-skill family is being rebuilt per docs/plans/2026-05-13-streamlined-story-skills-greenfield-plan.md
       ├── pages-prose-plans/     ← comprehensive prose plans; written by the story-skill family (rebuilding)
       ├── pages-prose/           ← rendered prose; supplied externally (manual or LLM); validated by a prose-attach step (rebuilding)
...
- `branching-story-bootstrap` — bootstraps a new story bundle inside an existing world. Writes story-bundle records (STENT/STINT/SF/BEL/SE/OBL/CNSQ/THR/SREL/STLOC/STOBJ/optional DA/BR/PG/CHC/optional SLT) via the patch engine + `STORY_KERNEL.md` + `pages-prose-plans/PG-0001.md` + per-bundle `INDEX.md` + per-world `stories/INDEX.md` first-run create-or-append. Rendered prose is supplied externally; no prose-rendering lifecycle. References `.claude/skills/_shared-templates/story-state-contract.md` as the canonical schema + gate contract.
- *Story-skill family rebuild — the remaining six greenfield skills (`branching-story-turn-cycle`, `branching-story-prose-attach`, `commitment-block-authoring`, `branching-story-health-audit`, `story-fact-promotion-to-canon`, `story-promotion-closeout`) are pending authoring per `docs/plans/2026-05-13-streamlined-story-skills-greenfield-plan.md`. Bullets will be reintroduced as each new skill lands.*
...
- **Prose pages are author-supplied.** Rendered prose at `pages-prose/PG-NNNN.md` is supplied externally (manual or LLM) and validated by a prose-attach step distinct from state mutation. The story-skill family is being rebuilt per `docs/plans/2026-05-13-streamlined-story-skills-greenfield-plan.md`; under the rebuilt family the page snapshot is the fork primitive — turn-cycle may advance from any committed page snapshot without requiring rendered parent prose, replacing the prior pending-prose-blocks-next-plan serialization.
