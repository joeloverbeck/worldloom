...
3. Cross-skill boundary under audit: parser (`tools/world-index/src/parse/atomic.ts`) ↔ schema (`tools/world-index/src/schema/types.ts` + `migrations/`) ↔ downstream MCP retrieval surface (`tools/world-mcp/`, in archive/tickets/SPEC81INDSTOCAN-002.md). The parser produces what schema defines; downstream MCP retrieval reads what the parser produces. Edge-type and projection-column additions stay strictly within `tools/world-index/`'s public API surface; no cross-package coupling changes.
4. FOUNDATIONS §Tooling Recommendation ("LLM agents should never operate on prose alone... directly or via the documented context-packet + targeted-retrieval pattern") motivates indexed projection — this ticket lands the index half of the retrieval pattern that archive/tickets/SPEC81INDSTOCAN-002.md then exposes through MCP.
...
4. Single-layer ticket — additional layer mapping is not applicable; downstream MCP retrieval layer is owned by archive/tickets/SPEC81INDSTOCAN-002.md.
...
- `archive/tickets/SPEC81INDSTOCAN-002.md` (modify dependency handoff note)

## Out of Scope

- The new MCP tool `select_storylet_candidates` — landed in archive/tickets/SPEC81INDSTOCAN-002.md.
...
- Updated `archive/tickets/SPEC81INDSTOCAN-002.md` to state that source-record-id filtering uses existing `storylet_predicate_ref` edges, not a new fifth edge.
