...
At intake, `emergent-pressure-events` still told maintainers that `branching-story-bootstrap` and `branching-story-page-cycle` were not-yet-implemented brainstorming proposals, and that `story_fuel` cards remained inert until those siblings shipped. That was true when EPE shipped, but post-review evidence showed the story siblings exist and the page-cycle context-packet profile is registered.

Leaving that stale prose in Guardrails risked misleading future maintainers about the state of the story-fuel handoff. The `story_fuel` routing remains passive and does not invoke downstream skills, but its consumer status is now described as shipped-story-engine inputs rather than planned-only placeholders.

## Assumption Reassessment (2026-05-02)

1. At intake, live stale prose appeared in `.claude/skills/emergent-pressure-events/SKILL.md` §Guardrails: it said `branching-story-bootstrap` and `branching-story-page-cycle` exist as `brainstorming/*.md` proposals and that `story_fuel` cards are inert until those siblings ship.
...
9. Same-seam correction: reassessment also found `.claude/skills/emergent-pressure-events/SKILL.md` §Final Rule still described `story_fuel` as for "future story-engine siblings"; this was the same planned-consumer prose drift and was corrected in this ticket.
...
Updated `.claude/skills/emergent-pressure-events/SKILL.md` §Guardrails to say `branching-story-bootstrap` and `branching-story-page-cycle` now exist as workflow skills, and that `story_fuel` cards are passive inputs for separately invoked story-engine workflows.

### 2. Preserve passive-routing and non-canon constraints

Kept the no-chaining rule, candidate-only framing, and sidecar conditionality intact. The EPE skill still does not invoke story siblings, and story_fuel / ambient cards still keep `proposal_card_extract` null.

### 3. Truth final summary wording

Updated `.claude/skills/emergent-pressure-events/SKILL.md` §Final Rule so `story_fuel` is described as input for separately invoked story-engine workflows, not "future story-engine siblings."
...
- `.claude/skills/emergent-pressure-events/SKILL.md` §Guardrails now describes `story_fuel` cards as passive inputs for separately invoked `branching-story-bootstrap` / `branching-story-page-cycle` workflows.
- The stale planned-consumer guardrail and final-summary phrase were removed.
- The no-chaining rule, candidate-only framing, and sidecar conditionality remain unchanged.

## Verification Result

1. `rg -n 'not-yet-implemented|brainstorming/\*\.md proposals|inert until those siblings ship' .claude/skills/emergent-pressure-events/SKILL.md` returned no hits.
2. `rg -n 'Skills do not chain|downstream_routing|story_fuel|proposal_card_extract|Story-fuel routing|future story-engine siblings|future-consumer filtering' .claude/skills/emergent-pressure-events/SKILL.md` confirmed the passive-routing, non-canon, and sidecar-null language remains present while the stale `future story-engine siblings` and `future-consumer filtering` phrases no longer appear.
3. Manual review of `.claude/skills/emergent-pressure-events/SKILL.md` §Guardrails and §Final Rule confirmed EPE remains a candidate-emitting workflow and does not invoke downstream story-engine skills.

## Deviations

1. Same-seam final-summary stale prose was corrected in addition to the drafted §Guardrails edit. No schema, template, runtime, MCP, or HARD-GATE behavior changed.
2. Archived on 2026-05-02 during post-ticket review after the completed closeout and live EPE skill prose were rechecked.
