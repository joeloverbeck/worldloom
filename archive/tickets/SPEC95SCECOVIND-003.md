...
At intake, two registered validators carried `page-plan`/`page` names but validated **PG/SE causal state**, not page-plan markdown: `page_plan_turn_driver_consistency` (validates `PG.input.resolved_event_id` ↔ `SE.created_at_page` / `turn_driver`) and `page_affordance_integrity` (validates `PG.state_snapshot.visible_affordances` against active records). With page-plan authoring retired (SPEC-93), the names falsely implied a page-plan architecture. This ticket renamed them to causal-state vocabulary while preserving behavior. (SPEC-95 §2 D3, AC#4.)
...
3. Cross-skill / cross-artifact boundary under audit: the **validator registry name-contract** (the snake_case registered names consumed by the framework run-loop and asserted by `registry.test.ts`), the imported-symbol contract (the camelCase exports consumed by colocated + integration tests), and the shared `_shared-templates/story-state-contract.md` §7 gate-9 reference that named `page_plan_turn_driver_consistency` as an enforcing validator. **Blast-radius enumeration confirmed by grep (the spec's widened §5 sweep):** beyond the spec's illustrative §5 list, five sibling-spec integration tests imported the camelCase symbol `pagePlanTurnDriverConsistency` directly — `spec85-multi-actor-collision-confrontation`, `spec85-secret-reveal-ledger-clue`, `spec85-offstage-bridge-sabotage`, `spec85-clock-fire-route-closes`, and `spec76-red-kiln-ambush`. The final stale-name sweep also found `tools/validators/tests/integration/spec93-capstone.test.ts` asserting the old source path as an allowed legacy-read surface; that path assertion moved with the source rename. These updates are in-scope by the §5 sweep's "renamed-validator references are in scope" triage rule; the rename is behavior-preserving (the validators stay live and registered; tests assert the same firing behavior under the new symbol/name), so updating these sibling-spec tests carries no production-behavior change — symbol/import/path/assertion strings updated only (Rule 6: rename, not removal).
...
6. (was template item 7 — rename blast radius) Pipeline-wide grep for the old symbols/names across `tools/`, `.claude/skills/` (excluding `dist/`, `archive/`) yielded the source/registry, colocated tests, registry test, integration tests, README, shared-template, and SPEC-93 capstone path assertion enumerated in Files to Touch. After the rename, `rg -n "page_plan_turn_driver_consistency|page_affordance_integrity|pagePlanTurnDriverConsistency|pageAffordanceIntegrity|page-plan-turn-driver-consistency|page-affordance-integrity" tools/ .claude/skills/ | rg -v "dist|archive"` returned zero matches.
...
2. New names consistently present → grep-proof: `pg_se_turn_driver_consistency` / `pg_affordance_integrity` resolve in the renamed source files, `registry.test.ts`, and the README inventory; `pgSeTurnDriverConsistency` / `pgAffordanceIntegrity` resolve in `registry.ts`.
...
Renamed `page-plan-turn-driver-consistency.ts` → `pg-se-turn-driver-consistency.ts` (export `pagePlanTurnDriverConsistency` → `pgSeTurnDriverConsistency`; `const VALIDATOR = "page_plan_turn_driver_consistency"` → `"pg_se_turn_driver_consistency"`); renamed `page-affordance-integrity.ts` → `pg-affordance-integrity.ts` (export `pageAffordanceIntegrity` → `pgAffordanceIntegrity`; `name: "page_affordance_integrity"` → `"pg_affordance_integrity"`). The validator logic and `applies_to` behavior were not changed.

### 2. Update the registry (`tools/validators/src/public/registry.ts`)

Updated the two `import { ... } from "../structural/<old-file>.js"` lines to the new filenames + symbols, and the two registered-validators array entries.

### 3. Rename + update colocated tests

Renamed `tools/validators/tests/structural/page-plan-turn-driver-consistency.test.ts` → `pg-se-turn-driver-consistency.test.ts` and `tools/validators/tests/structural/page-affordance-integrity.test.ts` → `pg-affordance-integrity.test.ts`; updated their imports, symbol references, and asserted name strings.

### 4. Update test consumers of the names (registry + integration)

Updated the name-list assertion in `tools/validators/tests/structural/registry.test.ts`; updated the snake-name assertions in `spec44-append-only-supersession.test.ts`, `spec92-scene-layer-capstone.test.ts`, `validate-patch-plan.test.ts`; updated the camelCase-symbol imports/usages in `spec85-multi-actor-collision-confrontation.test.ts`, `spec85-secret-reveal-ledger-clue.test.ts`, `spec85-offstage-bridge-sabotage.test.ts`, `spec85-clock-fire-route-closes.test.ts`, `spec76-red-kiln-ambush.test.ts`; updated the old source-path assertion in `spec93-capstone.test.ts`.

### 5. Update docs + shared template

Updated the validator inventory in `tools/validators/README.md` (the two old names → new names) and the §7 Hard-Gate-9 enforcing-validator row in `.claude/skills/_shared-templates/story-state-contract.md` (`page_plan_turn_driver_consistency` → `pg_se_turn_driver_consistency`).
...
- `tools/validators/tests/integration/spec93-capstone.test.ts` (modify)
...
1. `cd tools/validators && npm test` passed (build + `node --test`) with all colocated and integration tests green on the renamed symbols/names: 1058 tests, 0 failures.
2. `rg -n "page_plan_turn_driver_consistency|page_affordance_integrity|pagePlanTurnDriverConsistency|pageAffordanceIntegrity|page-plan-turn-driver-consistency|page-affordance-integrity" tools/ .claude/skills/ | rg -v "dist|archive"` returned zero matches.
3. `rg -n "pgSeTurnDriverConsistency|pgAffordanceIntegrity|pg_se_turn_driver_consistency|pg_affordance_integrity" tools/validators/src/public/registry.ts tools/validators/README.md` found the new registry symbols and README inventory names.

### Invariants

1. The validator set's COUNT is unchanged (rename, not add/remove); both validators remain registered.
2. Each validator's verdict behavior is unchanged — no `applies_to`, threshold, or logic change.
...
3. `tools/validators/tests/structural/registry.test.ts` + the 9 integration tests — name/symbol/path-string updates only; assertion logic unchanged.

### Commands

1. `cd tools/validators && npm test`
2. `rg -n "page_plan_turn_driver_consistency|page_affordance_integrity|pagePlanTurnDriverConsistency|pageAffordanceIntegrity|page-plan-turn-driver-consistency|page-affordance-integrity" tools/ .claude/skills/ | rg -v "dist|archive"` (expect zero matches)
...
Renamed the two PG/SE causal-state validators and all current operational consumers from `page_plan_turn_driver_consistency` / `page_affordance_integrity` to `pg_se_turn_driver_consistency` / `pg_affordance_integrity`. The source modules, exported symbols, registry entries, structural tests, integration/capstone assertions, README inventory, and shared story-state contract gate row now use the new names.

## Verification Result

1. `npm run clean` in `tools/validators` removed stale compiled `dist` output before proof.
2. `npm test` in `tools/validators` passed: 1058 tests, 0 failures.
3. `rg -n "page_plan_turn_driver_consistency|page_affordance_integrity|pagePlanTurnDriverConsistency|pageAffordanceIntegrity|page-plan-turn-driver-consistency|page-affordance-integrity" tools/ .claude/skills/ | rg -v "dist|archive"` returned zero matches.
4. `rg -n "pgSeTurnDriverConsistency|pgAffordanceIntegrity|pg_se_turn_driver_consistency|pg_affordance_integrity" tools/validators/src/public/registry.ts tools/validators/README.md` confirmed the new registry symbols and README inventory names.

## Deviations

- The final stale-name sweep found one additional same-seam current test outside the original illustrative file list: `tools/validators/tests/integration/spec93-capstone.test.ts` asserted the old source path as a legacy-read allowlist entry. It was updated to the renamed source path; this is rename proof-surface truthing, not behavior change.
