...
2. `archive/specs/SPEC-36-story-pipeline-ninth-iteration-fixes.md` §D1 specifies the validator (`severity_mode: "fail"`, four verdict codes), 8 tests (one per subcase + one accept-path per subcase), registry registration, registry test extension, and skill-prose updates at the two named sites. SPEC-35's §Risks & Open Questions explicitly defers validator-hardening-II's two items (D1, D2) — this spec discharges that cluster.
