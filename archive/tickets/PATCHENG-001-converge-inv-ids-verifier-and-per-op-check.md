...
**Priority**: HIGH
**Effort**: Small
**Engine Changes**: Yes — modified `tools/patch-engine/src/apply.ts` `verifyExpectedIdAllocations`; added regression coverage in `tools/patch-engine/tests/integration/end-to-end-canon-addition.test.ts`; removed the phantom-`INV-1` workaround documentation from `.claude/skills/create-base-world/`.
**Deps**: None (engine-internal bug fix; no upstream tickets blocking).

## Problem

At intake, when `create-base-world` submitted a genesis envelope creating ≥1 invariant per FOUNDATIONS §Invariants category (one each: ontological, causal, distribution, social, aesthetic_thematic), the `expected_id_allocations.inv_ids` array had two layers of pre-apply enforcement that **disagreed on the expected ID format**:
...
**Historical workaround discovered before this ticket**: place a phantom `INV-1` as the first entry, followed by the real category-prefix ids:
```yaml
...
Before this ticket, the workaround was documented at `.claude/skills/create-base-world/references/engine-envelope-shape.md` §3 (added by the create-base-world skill audit on 2026-05-01) and mirrored in any future canon-addition run that emitted multiple invariants in one plan. That prose has now been replaced with the category-prefix-only contract.

The intended end-state was to align `verifyExpectedIdAllocations`'s `inv_ids` handling with how `sec_ids` is already handled at `tools/patch-engine/src/apply.ts:211–222`:
...
The `sec_ids` verifier iterates each id, scans per-prefix in the index, and verifies the id is the next free for its specific prefix. It correctly handles the multi-prefix case (`SEC-GEO-001`, `SEC-PAS-001`, `SEC-INS-001`, ...). The landed `inv_ids` verifier now does the same for invariant category prefixes, with an added same-plan per-prefix counter so sequential allocations such as `ONT-2`, `ONT-3` are accepted when `ONT-1` already exists.
...
6. Pipeline-wide grep for current `inv_ids` callers: at intake, `create-base-world` Phase 11 step 4 emitted the phantom-`INV-1` first entry pattern. No other skill currently calls `submit_patch_plan` with `inv_ids` (canon-addition runs typically extend invariants via `append_extension` rather than creating new ones). The final grep/manual review shows the create-base-world skill surfaces now describe category-prefix-only `inv_ids`.
...
8. Final implementation note: `verifyExpectedIdAllocations` now removes `inv_ids` from the flat `INV-N` verifier table and verifies every `inv_ids` entry against `^(ONT|CAU|DIS|SOC|AES)-\d+$` per prefix. Same-plan monotonicity is handled by counting already-accepted IDs per prefix, so `ONT-2`, `ONT-3` passes when `ONT-1` already exists and `ONT-3` alone fails with the expected `ONT-2` race.
9. Final proof-surface correction: instead of a live `create-base-world` dry-run that would create local world content, the landed verification uses package integration tests around `submitPatchPlan` plus grep/manual review of the create-base-world skill prose. This proves the engine contract and the skill reference without generating a new world.

## Architecture Check

1. Aligning `inv_ids` verification with the existing `sec_ids` per-prefix taxonomy is the cleanest convergence — it keeps both multi-prefix id classes prefix-aware. The landed invariant path also accounts for same-plan sequential IDs under one prefix, because genesis and later invariant batches can create more than one invariant in the same category. No new top-level abstraction was introduced.
...
1. After fix: `verifyExpectedIdAllocations` accepts category-prefix-only `inv_ids` without the phantom `INV-1` first entry, including same-plan sequential IDs such as `ONT-2`, `ONT-3` when `ONT-1` already exists → patch-engine integration test (`tools/patch-engine/tests/integration/end-to-end-canon-addition.test.ts`).
...
`tools/patch-engine/tests/integration/end-to-end-canon-addition.test.ts`:
...
- `tools/patch-engine/tests/integration/end-to-end-canon-addition.test.ts` (modify — `submitPatchPlan` `inv_ids` per-prefix regression tests)
...
2. `rg -n 'expected_id_allocations|inv_ids|INV-1|verifyExpectedIdAllocations' .claude/skills/create-base-world/SKILL.md .claude/skills/create-base-world/references/engine-envelope-shape.md` shows category-prefix-only skill guidance and no phantom workaround.
3. Phantom-`INV-1` first entry is REJECTED (regression test).

### Invariants

1. `inv_ids` and `sec_ids` are both prefix-aware verifier paths. `inv_ids` additionally accounts for same-plan sequential allocations under one category prefix, removing the old flat `INV-N` divergence without claiming a byte-for-byte duplicate of the `sec_ids` loop.
...
1. `tools/patch-engine/tests/integration/end-to-end-canon-addition.test.ts` — new per-prefix verifier behavior tests in the existing `submitPatchPlan` integration lane.

### Commands

1. `cd tools/patch-engine && npm test` — package-local pass.
2. `rg -n 'phantom.*INV-1|INV-1.*phantom|INV-N sentinel|inv_ids.*phantom' .claude/skills/create-base-world/` — no hits.
3. `rg -n 'expected_id_allocations|inv_ids|INV-1|verifyExpectedIdAllocations' .claude/skills/create-base-world/SKILL.md .claude/skills/create-base-world/references/engine-envelope-shape.md` — manual review of the remaining skill guidance.

## Outcome

Completed: 2026-05-01.

`tools/patch-engine/src/apply.ts` now verifies `inv_ids` per invariant category prefix instead of through the old flat `INV-N` row. The verifier rejects `INV-N` sentinels, detects skipped category-prefix IDs, and accepts same-plan sequential IDs for the same prefix.

The create-base-world skill and `engine-envelope-shape.md` now describe category-prefix-only `inv_ids`; the phantom `INV-1` workaround prose and example were removed from the skill surfaces.

## Verification Result

1. `cd tools/patch-engine && npm test` — passed; 53 tests passed, including the new `submitPatchPlan` `inv_ids` accept/reject cases.
2. `rg -n 'phantom.*INV-1|INV-1.*phantom|INV-N sentinel|inv_ids.*phantom' .claude/skills/create-base-world/` — no hits.
3. `rg -n 'expected_id_allocations|inv_ids|INV-1|verifyExpectedIdAllocations' .claude/skills/create-base-world/SKILL.md .claude/skills/create-base-world/references/engine-envelope-shape.md` — remaining skill hits describe category-prefix-only `inv_ids` and reject `INV-N` sentinels.

## Deviations

The drafted acceptance criterion for a live `create-base-world` dry-run was replaced with package integration tests plus skill-reference grep/manual review. That keeps verification on the engine contract and avoids generating local world content just to prove a verifier branch.
