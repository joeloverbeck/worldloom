...
At intake, `tools/world-index/src/enumerate.ts` still treated `pages-prose`, `pages-prose-plans` (markdown) and `pages-prose-receipts` (yaml) as indexable story-bundle directories. Page-plan authoring was retired by SPEC-93, no live bundle produces these artifacts (0 page-prose files exist on disk across all worlds), and the scene-first explorer (SPEC-96/97) replaces the page-prose read routes that justified keeping them. The entries were dead inventory that perpetuated the page-as-prose model. This ticket removed them so the indexable surface reflects the scene-first reality. (SPEC-95 §2 D1, §4.)

## Assumption Reassessment (2026-05-29)

1. At intake, `tools/world-index/src/enumerate.ts` defined `STORY_BUNDLE_MARKDOWN_DIRECTORIES` as a `Set` containing `pages-prose`, `pages-prose-plans`, `scene-prose`, `scene-prose-plans`, plus `audits` / `character-proposals` / `story-characters` / `storylet-batches` / `story-promotions`, and `STORY_BUNDLE_YAML_DIRECTORIES = new Set(["pages-prose-receipts", "scene-prose-receipts"])`. `isIndexablePath` reads both Sets for the `stories/<slug>/<dir>/<file>` length-4 markdown branch and the length-4 yaml branch; removing the three entries from the Sets was the operative change, and non-listed dirs fall through to the `unexpected` bucket (verified: `enumerate.ts` `walk` pushes non-`isIndexablePath` files to `unexpected`). Confirmed `STORY_BUNDLE_MARKDOWN_DIRECTORIES` / `STORY_BUNDLE_YAML_DIRECTORIES` are consumed ONLY by `enumerate.ts` (no other `tools/world-index/src/**` reader).
...
5. (was template item 7 — removal blast radius) The removed inventory is referenced across the pipeline; the authoritative gate is SPEC-95 §5's completeness sweep. In-scope here: `enumerate.ts` inventory sets, `enumerate.test.ts`, `schema.test.ts`, and the command test assertion that the shared atomic fixture's legacy page-prose artifacts are now reported as `unexpected_path`. The shared fixture retained its legacy page-prose artifacts as intentional unexpected-path evidence rather than deleting them. Out of scope (noted, not edited): `tools/story-explorer/src/**` page-prose read routes (SPEC-96), `story-fact-promotion-to-canon` legacy receipt reads (deferred per SPEC95SCECOVIND-004), `docs/REPOSITORY-MAP.md` legacy-dir annotations and `_shared-templates/*` legacy-prose mentions (SPEC-99 docs closeout). No production reader of the removed indexable entries exists outside `enumerate.ts` itself.
...
Removed `"pages-prose"` and `"pages-prose-plans"` from `STORY_BUNDLE_MARKDOWN_DIRECTORIES`; removed `"pages-prose-receipts"` from `STORY_BUNDLE_YAML_DIRECTORIES`. Retained `scene-prose`, `scene-prose-plans` (markdown) and `scene-prose-receipts` (yaml). `isIndexablePath`'s branch structure remains unchanged; it reads the Sets, and the retained length-4 markdown/yaml scene branches still resolve correctly.

### 2. Add the de-index migration (`tools/world-index/src/schema/migrations/008_<slug>.sql` — new) + version bump

Bumped `CURRENT_INDEX_VERSION` in `tools/world-index/src/schema/version.ts` from `7` to `8`. Added migration `008_deindex_legacy_page_prose.sql` after `007_slt_projection_columns.sql`. The migration deletes `nodes` rows whose `file_path` matches the de-indexed `pages-prose` / `pages-prose-plans` / `pages-prose-receipts` story-bundle dirs, deletes dependent rows that reference those node ids, and clears the corresponding `file_versions` rows so incremental sync reparses under the new inventory.

### 3. Update colocated tests + fixtures

Updated `tools/world-index/tests/enumerate.test.ts` to drop assertions that `pages-prose*` paths are indexable and to assert that `pages-prose` / `pages-prose-plans` / `pages-prose-receipts` paths classify as `unexpected`. Added `schema.test.ts` coverage for the migration's stale-row/file-version invalidation. Updated `commands.test.ts` so the shared atomic fixture's legacy page-prose artifacts are asserted as expected `unexpected_path` validation rows.
...
2. `enumerate.test.ts` case: a fixture story bundle with `pages-prose/PG-0001.md` + `pages-prose-receipts/PG-0001.yaml` resolves those paths into `unexpected`, and `scene-prose/SCN-0001.md` + `scene-prose-receipts/SCN-0001.yaml` into `indexable`.
...
1. `tools/world-index/tests/enumerate.test.ts` — dropped indexable-page-prose assertions; added the AC#1 `unexpected`-classification assertion for `pages-prose*` and retained `indexable` assertion for `scene-prose*`.
2. `tools/world-index/tests/schema.test.ts` — added a migration test proving legacy page-prose rows, dependent edges, and file versions are removed while scene-prose rows remain.
3. `tools/world-index/tests/commands.test.ts` — updated the atomic fixture build assertion to expect the retained legacy fixture artifacts as `unexpected_path` rows.

### Commands

1. `cd tools/world-index && npm run build && npm test`
2. `grep -rn "pages-prose" tools/world-index/src/enumerate.ts` (expected zero matches; returned no matches)
...
Removed the legacy page-prose inventory entries from `tools/world-index/src/enumerate.ts`, bumped the world-index schema version to 8, and added `008_deindex_legacy_page_prose.sql` to remove stale legacy page-prose nodes/dependent rows/file versions from pre-existing indexes. Updated enumerate, schema migration, and command tests so `pages-prose*` is now treated as unexpected while `scene-prose*` remains indexable.

## Verification Result

1. `grep -n "pages-prose" tools/world-index/src/enumerate.ts` returned no matches; `grep -nE "scene-prose|scene-prose-plans|scene-prose-receipts" tools/world-index/src/enumerate.ts` returned the three retained scene entries.
2. `cd tools/world-index && npm run build` passed.
3. `cd tools/world-index && npm test` initially failed in `commands.test.ts` because the shared atomic fixture's legacy page-prose artifacts were newly reported as `unexpected_path`; after updating that same-seam assertion, rerunning `npm run build` and `npm test` passed with 128 non-CLI compiled tests plus the serial CLI compiled tests passing.

## Deviations

- `tools/world-index/tests/helpers/atomic-fixture.ts` was not changed. Its legacy page-prose artifacts were intentionally retained as fixture evidence that the command path reports those paths as unexpected after de-indexing.
- Added `tools/world-index/tests/commands.test.ts` as same-seam proof fallout discovered by the broad package test; it was not in the original file list, but without it the package's command fixture proof contradicted the new inventory contract.
