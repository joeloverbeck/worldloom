...
Record cards in the new `<RecordPicker>` (and the extended `RecordCard`) must show "involved cast" so a non-character record (a consequence, a clock) is as identifiable as a character — the report's "pick a consequence should be as good as pick a character" bar (`reports/manual-story-studio-third-iteration.md` §12 / §31). At intake, `ManualRecordSummary` carried only `id, title, active, importance, tags, summary, prompt_visibility`; `refs` lived only on the full `ManualRecord`. This ticket added the one backend change SPEC-112 requires: the read summary now exposes derived involved-cast ids.

## Assumption Reassessment (2026-06-02)

1. `ManualRecordSummary` is defined in `tools/manual-story-studio/src/schema/manual-story.ts` and mirrored in the web type at `tools/manual-story-studio/web/src/types/manual-story.ts`; its sole producer is `toSummary` in `tools/manual-story-studio/src/read/records.ts`, which already parses the full record (so deriving `involved_cast` from `refs.characters` is local — no extra read). `RecordRefs` exists on the web `ManualRecord` type.
...
4. (was template item 6 — schema extension) Schema extended: `ManualRecordSummary` (a read-side projection, not a canon CF/CH/story-bundle record). The landed field is optional `involved_cast?: string[]`, derived from `refs.characters`, so every existing consumer is additive-safe — consumers read existing fields and ignore the new one. `toSummary` populates it from the parsed record and defaults to `[]` when `refs.characters` is absent or malformed. No consumer constructs `ManualRecordSummary` literally (verified during reassessment: zero `test/` matches), so requiredness would have been non-breaking either way; optional keeps the additive guarantee explicit.
...
### 1. Added the field to the backend schema

In `tools/manual-story-studio/src/schema/manual-story.ts`, added optional `involved_cast?: string[]` to `ManualRecordSummary`. The field is a derived involved-cast summary rather than the full `refs` object, so the summary stays a minimal projection.

### 2. Populated it in `toSummary`

In `tools/manual-story-studio/src/read/records.ts`, `toSummary` now reads `refs.characters` off the parsed record through a defensive helper and sets `involved_cast`. It defaults to `[]` when the field is absent or not an array.

### 3. Mirrored the field in the web type

In `tools/manual-story-studio/web/src/types/manual-story.ts`, added the identical optional field to the web `ManualRecordSummary` interface.

### 4. Extended the read test

In `tools/manual-story-studio/test/read/records.test.ts`, asserted `listRecords`/`toSummary` surfaces involved-cast for a record with `refs.characters` and `[]` for one without.
...
The manual-story-studio read summary contract now includes optional `involved_cast?: string[]` on both backend and web `ManualRecordSummary` declarations. `toSummary` derives it from parsed `refs.characters`, returning `[]` when no character refs are available, and the read-layer test asserts both populated and empty summary outputs. No persisted record shape, route filter, or storage layout changed.

## Verification Result

1. `npm run test:backend` from `tools/manual-story-studio` — passed; compiled backend and ran 75 compiled test files, including `dist/test/read/records.test.js`.
2. `npm --prefix web test` from `tools/manual-story-studio` — passed; web `tsc -p tsconfig.json --noEmit` accepted the mirrored summary type.
3. `npm test` from `tools/manual-story-studio` — passed; backend build plus 454 compiled backend tests plus web type check.
4. `npm run build` from `tools/manual-story-studio` — passed; web install check, web production build, and backend build succeeded.
...
None. The field landed as the ticket's preferred derived `involved_cast` projection rather than exposing the full `refs` object.
