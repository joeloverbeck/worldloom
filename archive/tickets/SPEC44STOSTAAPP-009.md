...
**Priority**: MEDIUM
**Effort**: Medium
**Engine Changes**: No production code introduced — added `tools/validators/tests/integration/spec44-append-only-supersession.test.ts` exercising the post-SPEC-44 contract end-to-end. Parallels the existing `spec43-midstory-introduction.test.ts` integration test.
...
5. Live command-shape correction: the package script compiles `tests/**/*` to `dist/tests/**/*.js` before running `node --test`; the targeted proof used `npm run build` followed by `node --test dist/tests/integration/spec44-append-only-supersession.test.js`. The full package proof remained `npm test` from `tools/validators`. The repo-root red-bunny smoke was exercised inside the new compiled test and passed in this checkout because `worlds/erotica-world/_index/world.db` was present.
...
2. **Fixture-world isolation per §Spec-Integration Ticket Shape.** The validator-composition scenarios use synthetic indexed records and patch-plan context, not direct writes to the real `worlds/<slug>/` tree. The optional red-bunny smoke copies the gitignored local world to a temp root before running the CLI.
3. **Bounded expected counts.** The synthetic failure fixtures assert the intended verdict code/severity for each enforcement surface. The active-records shape gap fixture intentionally omits exactly CLK / STSEC / STQ / DA and asserts four warn verdicts.
...
### 1. Landed integration test module

Created `tools/validators/tests/integration/spec44-append-only-supersession.test.ts`. The module follows the SPEC-43 integration test's synthetic-fixture structure and uses in-test helpers; no `tests/fixtures/spec44/` directory was needed.
...
- **Scenario 2: Clock resolution via supersession** — Author a prior active clock → author a resolved successor clock (status: "resolved", resolution_event: SE-X, supersedes the prior clock) + SE with appropriate state_delta. Assert clean.
...
- **Scenario 4: Secret clue discovery via supersession** — Author a prior secret with clue_carrier status "available" → author a successor secret with the same carrier status "discovered". Assert clean.
- **Scenario 5: Secret reveal via supersession** — Author a hidden prior secret → author a revealed successor secret with reveal_event SE-X and reveal_records [BEL-12, SF-3]. Assert clean.
...
- **Scenario 7: Question abandon via supersession** — Author an open prior question → author an abandoned successor question with an abandonment rationale. Assert clean.
...
No fixture files were added. The synthetic scenarios are built in the test module.
...
1. `npm run build` followed by `node --test dist/tests/integration/spec44-append-only-supersession.test.js` from `tools/validators` passes the capstone scenarios (7 compiled subtests covering 7 supersession transitions, 5 enforcement-surface fires, and checkout-local red-bunny smoke).
2. `npm test` from `tools/validators` exits 0 (full validator suite regression).
...
1. `tools/validators/tests/integration/spec44-append-only-supersession.test.ts` (new) — 7 compiled `node:test` subtests covering 7 supersession transitions, 5 enforcement-surface fires, and checkout-local red-bunny smoke.
2. No fixture directory was added; fixture records are in-test synthetic objects.
...
1. `npm run build` from `tools/validators` — compilation check and compiled-test freshness.
2. `node --test dist/tests/integration/spec44-append-only-supersession.test.js` from `tools/validators` — targeted compiled integration test.
3. `npm test` from `tools/validators` — full validator suite regression.
4. Red-bunny CLI smoke is embedded in the compiled capstone test and runs `dist/src/cli/world-validate.js erotica-world --story red-bunny --structural --json` against a temp copy when the local gitignored world and derived index exist; it passed in this checkout.

## Outcome

Completed: 2026-05-18

- Added `tools/validators/tests/integration/spec44-append-only-supersession.test.ts`.
- The capstone composes the SPEC-44 validator surfaces over synthetic records: append-only supersession for all seven lifecycle transitions, `no_story_state_in_place_mutation`, `state_delta_class_integrity`, `page_affordance_integrity`, `expected_witness_coverage`, and `active_records_full_shape`.
- The red-bunny structural CLI smoke runs from the capstone against a temp repo copy when the local gitignored world/index exists; it passed in this checkout.

## Verification Result

- `npm run build` from `tools/validators` — passed.
- `node --test dist/tests/integration/spec44-append-only-supersession.test.js` from `tools/validators` — passed: 7 tests, 0 failures.
- `npm test` from `tools/validators` — passed: 537 tests, 0 failures.
- `node tools/validators/dist/src/cli/world-validate.js erotica-world --story red-bunny --structural --json` from the repo root — passed with `fail_count: 0`, `warn_count: 15`, `info_count: 10`.

## Deviations

- The drafted `npm test --prefix tools/validators -- spec44-append-only-supersession` targeted command was replaced with the package-truthful compiled-test lane because the package script builds then runs `dist/tests/**/*.test.js`.
- No `tools/validators/tests/fixtures/spec44/` directory was created; inline synthetic fixtures proved the same invariant with less fixture churn.
- The red-bunny smoke remains checkout-local because `worlds/` content is gitignored; it is skipped by the test when the local world or derived index is absent.
