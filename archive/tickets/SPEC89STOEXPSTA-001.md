...
2. SPEC-89 §4 (Tabs) and §11 (Accessibility) name the four-tab structure and WAI-ARIA tablist contract; §14 (Frontend component layout) prescribes `XRayPanel.tsx`, `XRayTabs.tsx`, and `tabs/<TabName>.tsx` files under `tools/story-explorer/web/src/components/xray/`. The SPEC-89 spec file is archived at `archive/specs/SPEC-89-story-explorer-state-xray-layer.md`.
