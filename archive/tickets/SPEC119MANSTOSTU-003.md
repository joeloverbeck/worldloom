...
3. **Cross-artifact boundary under audit**: this ticket reads the archived SPEC119MANSTOSTU-001 enriched ledger (`resolution.excluded[i].reason`, `.suppressed[i].reason`) and shares `PromptPreview.tsx` with archived SPEC119MANSTOSTU-002; the title-match uses the enriched `title` field on included/excluded/suppressed ledger entries. Live reassessment showed `resolution.blocked` entries carry `ref` / `reason` rather than `title`, so blocked lookup matches `ref`.
...
Added a labeled search input in the inspector aside. On query, it matches the typed title (case-insensitive, trimmed) against the enriched ledger entries' `title` across `resolution.included` / `excluded` / `suppressed`; included entries report that they are included, and excluded/suppressed entries report their deterministic reason in author-readable form. `resolution.blocked` has no title field, so blocked lookup matches `ref` and reports the blocked reason. For a query that matches no ledger entry or blocked ref, the UI reports the generic "not in the working set (not selected, pinned, or active in the current context)" answer. It renders no inferred or scored narrative.

### 2. Extend the web inspector test (`test/web/prompt-inspector.test.ts`)

Added assertions for the ledger-present included/excluded/suppressed/blocked paths, the absent-title generic fallback, and the no-new-data-source invariant.
...
- Added the read-only "Why is this missing?" search affordance to `PromptPreview`.
- Implemented deterministic matching over `composeResult.resolution` only: included/excluded/suppressed by title, blocked entries by `ref`, and absent entries by the generic not-in-working-set fallback.
- Kept the lookup frontend-only with no fetch, record-list, backend, or composer change.
- Extended `test/web/prompt-inspector.test.ts` to assert the lookup paths and no-new-data-source invariant.
...
- `cd tools/manual-story-studio && npm test` — PASS; backend/static suite reported 488 passing tests and the web typecheck passed.

## Deviations

- The drafted ticket said blocked entries would be matched by title. Live `PromptBlockedInput` entries carry `ref` and `reason`, not record identity, so the implemented blocked path matches `ref`. Record-title lookup remains deterministic for included/excluded/suppressed ledger entries, and absent titles still receive the generic not-in-working-set answer.
