...
9. Final worktree refresh note: same-family skill-budget prose now shows `token_budget=16000` and a `packet_incomplete_required_classes` retry note in `.claude/skills/character-generation/SKILL.md` / `references/world-state-prerequisites.md`. CHARGENMCP-001 did not change the engine's `character_generation` default budget; the package test still proves the 8000-budget path returns structured insufficiency and succeeds at the computed retry budget.
