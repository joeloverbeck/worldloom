...
1. **Invariant: chained `update_record_field` ops on the same story-bundle record in one envelope each produce a distinct mutation that lands in the final file.** → `tools/patch-engine/tests/ops/update-record-field.test.ts` (new test: seed a story-bundle PG record with namespaced node_id `"red-bunny:PG-1"` and `file_path: "stories/red-bunny/_source/pages/PG-1.yaml"`; submit a 3-op envelope that sets `prose_path`, then `prose_status`, then `deferred_validation_trace.prose_ledger_consistency` with bare `target_record_id: "PG-1"`; assert the final file has all three field changes).
2. **Invariant: bare `target_record_id: "PG-NNNN"` resolves to the namespaced indexed record for story-bundle classes.** → `tools/patch-engine/tests/ops/update-record-field.test.ts` (new test: same seed as above; submit a 1-op envelope with bare `target_record_id: "PG-1"`; assert the op succeeds and the field is mutated).
...
- bare `target_record_id: "PG-1"` resolving a production-shaped namespaced index row;
...
3. `tools/patch-engine/tests/ops/update-record-field.test.ts` — modify the existing PG prose-finalize transitional-field and unrelated-PG-field attestation tests to use namespaced seed rows and run the assertions against bare `target_record_id: "PG-1"`. This makes the existing story-bundle update tests production-shaped.
