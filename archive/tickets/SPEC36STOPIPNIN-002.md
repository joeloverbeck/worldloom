...
At intake, `tools/world-mcp/tests/tools/get-context-packet.story-pipeline.test.ts` exercised the story-local-seed filter at `tools/world-mcp/src/tools/get-context-packet.ts` with only SF and STENT seed-id types. The filter pattern already matched 20 story-local types (SF, BEL, SE, OBL, CNSQ, THR, SREL, STINT, STENT, STSTAT, STLOC, STOBJ, BR, PG, CHC, SLT, SLB, SAU, SP, RSP). The ninth-iteration audit (`reports/story-related-improvements-ninth-iteration.md` §11.3 P2 test #5) flagged that without PG/BEL/SE seed-variant coverage, a regression that broke filtering specifically for these high-frequency story-record types could slip past CI. SPEC-36 §D6 is now covered by explicit PG/BEL/SE tests.
...
### 1. Appended three new test cases to `tools/world-mcp/tests/tools/get-context-packet.story-pipeline.test.ts`

Following the existing SF / STENT pattern, this ticket added:

- `getContextPacket ignores PG seed nodes for story-pipeline task types` — supplies a scoped `PG-1` seed plus `CF-1`; asserts the PG seed is filtered out of `local_authority`, `CF-1` is preserved, and `story_local_seed_nodes_ignored` is emitted.
- `getContextPacket ignores BEL seed nodes for story-pipeline task types` — supplies a scoped `BEL-1` seed plus `CF-1`; asserts the BEL seed is filtered out, `CF-1` is preserved, and the warning is emitted.
- `getContextPacket ignores SE seed nodes for story-pipeline task types` — supplies a scoped `SE-1` seed plus `CF-1`; asserts the SE seed is filtered out, `CF-1` is preserved, and the warning is emitted.

The landed tests use the same fixture-building helpers, task-type setup, and assertion shape as the existing SF / STENT tests. No production-code change or new helper function was required.
...
1. The three new test cases (`getContextPacket ignores PG seed nodes for story-pipeline task types`, `getContextPacket ignores BEL seed nodes for story-pipeline task types`, `getContextPacket ignores SE seed nodes for story-pipeline task types`) pass under `npm run build` followed by the focused compiled test-file run and under `npm test` in `tools/world-mcp/`.
...
1. `tools/world-mcp/tests/tools/get-context-packet.story-pipeline.test.ts` — appended three new test cases per the landed change list above; existing SF / STENT cases remain unchanged.

### Commands

1. `cd tools/world-mcp && npm run build` — refresh compiled output.
2. `cd tools/world-mcp && node --test dist/tests/tools/get-context-packet.story-pipeline.test.js` — targeted compiled test-file run.
3. `cd tools/world-mcp && npm test` — full-suite proof.

## Outcome

Completed on 2026-05-16.

This ticket appended three story-local seed filtering tests for scoped PG, BEL, and SE seed IDs in `tools/world-mcp/tests/tools/get-context-packet.story-pipeline.test.ts`. Each test pairs the story-local seed with world-canon `CF-1`, verifies the story-local seed is filtered from world-scope `local_authority`, verifies `CF-1` remains available, and verifies `story_local_seed_nodes_ignored` is emitted.

No production-code change was needed because `STORY_LOCAL_SEED_NODE_PATTERN` already matched PG / BEL / SE. No new helper functions were added.

## Verification Result

Commands run from `tools/world-mcp/`:

1. `npm run build` — passed; refreshed compiled `dist/` output.
2. `node --test dist/tests/tools/get-context-packet.story-pipeline.test.js` — passed; 8 tests passed including the three new PG / BEL / SE cases.
3. `npm test` — passed; package build plus full compiled test suite passed with 373 passing tests.

## Deviations

None. The ticket landed as a test-only extension and did not require production-code changes.
