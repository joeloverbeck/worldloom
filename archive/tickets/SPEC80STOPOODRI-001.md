...
3. **Cross-skill boundary under audit**: the bootstrap-author-side coverage rule must remain symmetrically paralleled by `commitment-block-authoring`'s Phase 1 #15 cast-role coverage criterion (already at `commitment-block-authoring/SKILL.md:159`) and by the new Phase 1 targets #16/#17 that `archive/tickets/SPEC80STOPOODRI-002.md` introduced. The bootstrap-author-side rule guards the GENESIS pool at story creation; the Phase 1 targets guard pool EVOLUTION during direct_batch additions. Both must use the same §3.1/§3.2/§3.3 trigger maps so the two enforcement points stay consistent. The shared contract surface: SPEC-80 §3 trigger maps (single source of truth).
...
3. **Single source of truth for trigger maps**: SPEC-80 §3 trigger maps are the canonical reference for both the bootstrap-author-side rule and the `commitment-block-authoring` Phase 1 #16/#17 targets that `archive/tickets/SPEC80STOPOODRI-002.md` introduced. The Phase 6 reference cites the spec's §3 sections rather than restating the maps in skill prose (which would create dual-maintenance burden).
...
- Editing `commitment-block-authoring/SKILL.md` Phase 1 — that surface is owned by `archive/tickets/SPEC80STOPOODRI-002.md`.
