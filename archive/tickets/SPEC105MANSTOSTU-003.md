...
### 1. Created `tools/manual-story-studio/src/server/read-error-http.ts`
...
### 2. Created `tools/manual-story-studio/test/server/read-error-http.test.ts`

Unit tests covering each row of the dispatch table + the unrecognized-code fall-through. The tests use a `FastifyReply`-shaped test double rather than a real Fastify instance.
...
This ticket added the shared `mapReadErrorToHttpReply` adapter and a focused unit test suite. The helper maps all six SPEC-105 read-error codes to the required HTTP status/body shapes, returns `HealthReport` for 409 responses, and logs then returns 500 for an unrecognized code.

No deviations from the planned file set.
...
2. `cd tools/manual-story-studio && npm test` — passed; backend reported 356 tests passing and web `tsc --noEmit` passed.
3. `grep -nE "^export function" tools/manual-story-studio/src/server/read-error-http.ts` — passed; the only exported function listed is `mapReadErrorToHttpReply`.
