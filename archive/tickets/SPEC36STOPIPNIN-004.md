...
2. `archive/specs/SPEC-36-story-pipeline-ninth-iteration-fixes.md` §D3 specifies regex tightening across all story-bundle schemas, fixture conversion in the two named files PLUS the §D3 sub-rule 3 "Authoring-time sibling sweep" extending the conversion to additional padded sites surfaced by grep, and two negative golden tests proving padded IDs fail `record_schema_compliance`. FOUNDATIONS.md §Canonical Storage Layer (FOUNDATIONS-002) confirms the unpadded-ID convention is canonical.
