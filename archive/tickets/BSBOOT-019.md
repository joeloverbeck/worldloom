...
At intake, downstream consumers mostly already treated `canon_revision` as an audit field, but two live consumer surfaces did not fully define the `null` case:
...
That left a small contract gap: bootstrap could emit `null`, while page-cycle and health-audit did not state the exact downstream handling for a genesis-world branch before first canonization.
...
2. At intake, `branching-story-page-cycle` already advertised the nullable page field in `references/record-schemas.md`, but its pre-flight current-revision bullet only named the latest `CH-NNNN.yaml` path and omitted the no-CH fallback. This ticket updated that bullet to use `list_records(record_type='change_log_entry', fields=['record_id'])`, choose the highest `CH-NNNN`, and write `state_snapshot.canon_revision: null` when no CH records exist.
...
4. `branching-story-health-audit` consumes the field for canon-baseline drift. This ticket documented explicit ordering semantics: `null` means no world CH was visible at that tick, `null -> CH-NNNN` is a first-canonization advancement, `CH-NNNN -> null` is a regression/corrupt trail, and `null -> null` is stable in an uncanonized genesis world.
...
Updated the current-world-canon-revision bullet so it mirrors bootstrap:

- uses `mcp__worldloom__list_records(world_slug, record_type='change_log_entry', fields=['record_id'])` or the existing context-packet governing context;
- chooses the highest-numbered `CH-NNNN` when present;
- if no CH records exist, sets `state_snapshot.canon_revision: null`;
- direct-listing `_source/change-log/` is documented as offline/debug fallback only.

### 2. `.claude/skills/branching-story-health-audit/SKILL.md`

Updated the Canon-Baseline Drift check to define nullable ordering:
...
- `archive/tickets/BSBOOT-019.md` (closeout/archive record)
...
Updated page-cycle pre-flight prose to define the no-CH genesis fallback for `state_snapshot.canon_revision`, using `list_records(change_log_entry)` or an already supplied context-packet revision and treating direct `_source/change-log/` listing as offline/debug fallback only.

Updated health-audit Canon-Baseline Drift prose to define nullable ordering: `null -> null` is stable, `null -> CH-NNNN` is valid first canonization, same-or-higher `CH-NNNN` is monotonic, and `CH-NNNN -> null` or lower `CH-NNNN` is a regression.

## Verification Result

Passed:

1. `rg -n 'canon_revision.*null|no CH|change_log_entry' .claude/skills/branching-story-page-cycle/references/pre-flight-and-prerequisites.md` — found the existing targeted-retrieval audit-trail note and the updated pre-flight no-CH fallback.
2. `rg -n 'null -> CH|CH-NNNN -> null|null means no CH|Canon-Baseline Drift' .claude/skills/branching-story-health-audit/SKILL.md` — found the updated Canon-Baseline Drift nullable ordering semantics.
3. `rg -n 'canon_revision' .claude/skills/story-fact-promotion-to-canon .claude/skills/storylet-pool-authoring` — no matches; the reviewed downstream skills still do not define ordering/default semantics for this field.

## Deviations

None. The implementation stayed docs/skill-contract only and did not change schemas, validators, engine behavior, or world/story-bundle content.
