...
At intake, canon-addition was the canon-mutating skill that emitted CF records via `create_cf_record` patch ops but did not yet author SPEC-09's `epistemic_profile` / `exception_governance` blocks or record skill-side Tests 11 and 12. With Rules 11 and 12 mechanized as validators (SPEC09CANSAFEXP-003) and the conditionally-mandatory blocks structurally enforced (`archive/tickets/SPEC09CANSAFEXP-002.md`), canon-addition needed to (a) populate `epistemic_profile` and `exception_governance` (or `n_a` with fact-type rationale) at Phase 13a (patch plan assembly) before the validate_patch_plan call at Phase 14a, and (b) append Tests 11 (Action-Space Integrity, judgment-layer) and 12 (Redundancy, judgment-layer) to the existing 10-test contiguous block at Phase 14a. Without that skill update, canon-addition would emit CFs that fail the archived SPEC09CANSAFEXP-002 structural validator and would not produce the skill-side judgment layer that completes the action-space / redundancy enforcement.

## Assumption Reassessment (2026-04-27)

1. At intake, `.claude/skills/canon-addition/SKILL.md` Phase 14a (Validation) contained the contiguous 10-test block (Tests 1-10 documented inline) and the explicit SPEC-09 anticipation comment: *"SPEC-09 Phase 2.5 will append Tests 11 (action-space) and 12 (redundancy) atop this contiguous block — preserve the numbering."* Phase 13a (Patch plan assembly) and Phase 12a (modification_history scan — axis-(c) judgment) were siblings to Phase 14a in the skill's Procedure section.
2. `.claude/skills/canon-addition/references/counterfactual-and-verdict.md` did not contain standalone detailed criteria blocks for Tests 1-10; it contained the PA body-markdown checklist placeholder *"SPEC-09 will append Tests 11/12 here."* The truthful extension target is that PA checklist sentence plus a new adjacent `Phase 14a Tests 11/12 Detailed Criteria` section.
...
5. **HARD-GATE / canon-write ordering surface touched**: canon-addition's HARD-GATE gates `submit_patch_plan` calls. At intake, clause (c) named only "skill-side Tests 9, 10"; this ticket updates it to name "skill-side Tests 9, 10, 11, 12" so the HARD-GATE precondition keeps pace with the new tests.
...
4. Reference-doc extension lands in `counterfactual-and-verdict.md`, not as a new reference file. The live file's SPEC-09 placeholder is in the PA body-markdown template rather than in a standalone Tests 1-10 criteria block, so the landed design replaces the placeholder and adds a new adjacent detail section.
...
4. `references/counterfactual-and-verdict.md` extension lands adjacent to the former SPEC-09 placeholder — codebase grep-proof + manual review of the surrounding PA checklist context to confirm Tests 11/12 detail blocks are present.
5. Phase 14a and reference-doc criteria are manually reviewable as a skill-side dry-run contract — manual review confirms the skill now instructs Phase 13a block population before validation, Phase 14a 12-test recording, and one-line rationale requirements for Tests 11/12. Full synthetic canon-addition execution remains owned by SPEC09CANSAFEXP-008's integration capstone.
...
Locate the PA body-markdown checklist placeholder (greppable substring: `SPEC-09 will append Tests 11/12 here`). Replace the placeholder sentence so the checklist says 12 tests, then add a new adjacent `Phase 14a Tests 11/12 Detailed Criteria` section with PASS criteria, FAIL examples, rationale-quality requirements, and edge cases for both tests. Do not claim parity with non-existent Tests 1-10 detail blocks in this reference file.
...
- `.claude/skills/canon-addition/references/counterfactual-and-verdict.md` (modify) — PA checklist count update plus Tests 11/12 detailed criteria adjacent to the former SPEC-09 placeholder
- `specs/SPEC-09-canon-safety-expansion.md` (modify) — same-seam reference truthing now that placeholders were replaced
- `specs/IMPLEMENTATION-ORDER.md` (modify) — mark canon-addition skill updates complete and truth Phase 13a wording
...
5. `grep -nE "Phase 14a Tests 11/12 Detailed Criteria|Test 11: Action-Space Integrity|Test 12: Redundancy" .claude/skills/canon-addition/references/counterfactual-and-verdict.md` returns matches at the detailed-criteria blocks.
6. Manual review of `.claude/skills/canon-addition/SKILL.md` and `references/counterfactual-and-verdict.md` confirms a synthetic capability-type accept branch would populate `exception_governance` at Phase 13a and record Phase 14a Tests 1-12 with rationale. Full end-to-end synthetic execution remains deferred to SPEC09CANSAFEXP-008's integration capstone.
...
4. References/counterfactual-and-verdict.md extension supplies enough criteria depth for Tests 11/12 (PASS criteria, FAIL examples, and edge cases) without implying the file has pre-existing Tests 1-10 detail blocks.

## Test Plan

### New/Modified Tests

`None — documentation-only ticket; verification is command-based and the full synthetic canon-addition dry-run is owned by SPEC09CANSAFEXP-008.`
...
2. `grep -nE "Phase 14a Tests 11/12 Detailed Criteria|Test 11: Action-Space Integrity|Test 12: Redundancy" .claude/skills/canon-addition/references/counterfactual-and-verdict.md` — reference-doc detail blocks present.
3. Manual review against the new Phase 14a checklist (12 tests with rationale); full synthetic canon-addition execution is covered by SPEC09CANSAFEXP-008.

## Outcome

Completion date: 2026-04-27.

Completed the canon-addition SPEC-09 skill update:

- Extended the HARD-GATE and process-flow text so accept branches require skill-side Tests 9, 10, 11, 12 plus existing judgment layers.
- Added Phase 13a guidance to populate `epistemic_profile` and `exception_governance`, or justified `n_a`, before Phase 14a validation.
- Updated Phase 14a from 10 to 12 tests, appended Action-Space Integrity and Redundancy, and listed Rules 11/12 among the validator mechanical layers.
- Updated the PA body-markdown checklist text and added Tests 11/12 detailed criteria in `counterfactual-and-verdict.md`.
- Truthed same-seam SPEC-09 and implementation-order references that still pointed at removed placeholders or Phase 12 block-authoring wording.

## Verification Result

- `grep -nE "^   11\. \*\*Action-Space Integrity|^   12\. \*\*Redundancy" .claude/skills/canon-addition/SKILL.md` — passed; both appended tests are present in the contiguous Phase 14a checklist.
- `grep -n "Tests 9, 10, 11, 12" .claude/skills/canon-addition/SKILL.md` — passed; HARD-GATE clause (c) references the full skill-side judgment set.
- `grep -n "all 12 tests" .claude/skills/canon-addition/SKILL.md` — passed; Phase 14a introductory text now records 12 tests.
- `grep -n "epistemic_profile\|exception_governance" .claude/skills/canon-addition/SKILL.md` — passed; Phase 13a block-population prose references both blocks.
- `grep -nE "Phase 14a Tests 11/12 Detailed Criteria|Test 11: Action-Space Integrity|Test 12: Redundancy" .claude/skills/canon-addition/references/counterfactual-and-verdict.md` — passed; reference-doc criteria blocks are present.
- `git diff --check` — passed.

## Deviations

- The reference file did not contain an existing Tests 1-10 detailed criteria block, so the implementation replaced the PA checklist placeholder and added a new adjacent Tests 11/12 detail section.
- A full synthetic canon-addition run was not executed in this ticket; the active SPEC09CANSAFEXP-008 integration capstone owns the end-to-end dry-run across prior SPEC-09 tickets.
