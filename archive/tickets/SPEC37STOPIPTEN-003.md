...
- The broad package suite was red before implementation on the stale capability-parity validator registry expectation. This ticket does not edit `tools/world-mcp/tests/server/capability-parity.test.ts`; that surface belongs to the active sibling capability/fingerprint ticket (`tickets/SPEC37STOPIPTEN-004.md`) or an equivalent same-package registry-truthing follow-up.
