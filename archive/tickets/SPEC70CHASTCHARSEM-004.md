...
At intake, SPEC-70 §2.1 + §2.5 required the authoring surfaces to document the Semantic Preservation Contract (every structured operational `CHAR` fact must be copied / transformed / compressed / omitted-with-rationale / story-irrelevant, never surviving only in `## Source Distillation`) and add the §16a "Relevant capabilities / limits for this page" packet line. This is the human/skill-facing contract that the schema (001), subsections (002), and validator (003) enforce — it landed last so it references real, shipped surfaces rather than vaporware.
...
5. Closeout correction: the drafted `grep -n "required because: viewpoint"` proof was case-stale; the live template uses `Required because: viewpoint`. The accepted proof command is the case-correct grep recorded below. Implementation also added explicit `source_operational_fact_map` authoring prose to `story-character-profile` and `branching-story-bootstrap` because 003 made the field validator-consumed; this stays inside the prose contract boundary.
...
Added the contract text (SPEC-70 §2.1 blockquote) to `story-character-profile/SKILL.md` (Phase 1 + Phase 3), `branching-story-bootstrap/SKILL.md` (cast-distillation phase), and `story-state-contract.md` (STCHAR semantics). Updated the `story-character-profile` Phase 1 source-section list and bootstrap cast-distillation source list to explicitly name `dramatic_core` (all 10 engine fields), `## Capabilities`, and `## Signature Scene Behavior`. Added authoring prose for `source_operational_fact_map` entries so the prose skills emit the schema/validator field shipped by 001/003.

### 2. §16a capabilities line (§2.5)

In `story-state-contract.md` §16a, added `- Relevant capabilities / limits for this page:` to the full packet after `- Agency and planning tendency:`. For the reduced `offstage_causal` packet, added the same line only when the offstage character's capability or limit is the mechanism of their causal bearing (authoring judgment note, not a validator gate). Left the closed `required_because` vocabulary unchanged.
...
1. `grep -n "Relevant capabilities / limits for this page" .claude/skills/_shared-templates/story-state-contract.md` — PASS; found the full-packet line and the conditional `offstage_causal` line.
2. `grep -ln "structured operational source fact" .claude/skills/story-character-profile/SKILL.md .claude/skills/branching-story-bootstrap/SKILL.md .claude/skills/_shared-templates/story-state-contract.md` — PASS; found the Semantic Preservation Contract in all three surfaces.
3. `grep -n "Required because: viewpoint" .claude/skills/_shared-templates/story-state-contract.md` — PASS; the closed `required_because` vocabulary line remains present and unchanged in substance.
4. `rg -n "dramatic_core|## Capabilities|## Signature Scene Behavior|source_operational_fact_map" .claude/skills/story-character-profile/SKILL.md .claude/skills/branching-story-bootstrap/SKILL.md .claude/skills/_shared-templates/story-state-contract.md` — PASS; confirms the two authoring skills name the explicit source sections and both STCHAR-producing skills now instruct `source_operational_fact_map` use.

## Outcome

Completed 2026-05-22. Added the Semantic Preservation Contract to the `story-character-profile`, `branching-story-bootstrap`, and shared `story-state-contract` surfaces. The two STCHAR authoring flows now name `dramatic_core`, `## Capabilities`, and `## Signature Scene Behavior` as source material, and they instruct world-CHAR-derived STCHAR profiles to map present structured `dramatic_core` fields through `source_operational_fact_map` rather than leaving retained facts in `## Source Distillation`.

Added the §16a `Relevant capabilities / limits for this page:` line to the full packet template and added the conditional reduced-packet line only for `offstage_causal` cases where a capability or limit is the mechanism of causal bearing. No validator, schema, TypeScript, or test files changed.

## Deviations

- The drafted lowercase `grep -n "required because: viewpoint"` command was corrected to the live template's `Required because: viewpoint` capitalization.
- The implementation added explicit `source_operational_fact_map` prose to the STCHAR frontmatter instructions because 001/003 made that map a real shipped surface; this is same-seam contract prose, not a schema/code change.
