...
At intake, after choosing a world from T005's picker, users landed on the Story Picker placeholder instead of a list of story bundles under `worlds/<slug>/stories/`. The completed route now shows each story card's metadata (title, page count, leaf count, rendered-prose count, latest indexed page, index-freshness state, `PG-1` presence). Without this route, users could navigate INTO a world but had no way to pick a story within it. SPEC-88 §4.2 defines the exact card content; live reassessment narrowed the drafted latest turn/page label to latest page because `StorySummary` exposes no latest-turn field.
...
2. SPEC-88 §4.2 (post-reassessment) names the drafted card content: "story slug + title from `STORY_KERNEL.md` frontmatter (when readable); PG count, leaf-page count, rendered-prose count, latest indexed turn/page; index freshness state; whether `PG-1` exists (warn if not); click → `/worlds/:slug/stories/:storySlug/entry`". The live implementation renders latest indexed page only because `StorySummary` exposes no latest-turn field. The "warn if not" for missing PG-1 is a small inline warning chip per story card, not a blocking error — users should be able to inspect malformed bundles.
...
4. Implementation reassessment corrected one drafted card-detail assumption: the live `StorySummary` client mirror exposes `latestPageId` but no `latestTurnIndex`, so the route renders the latest page label only (`Latest: PG-n` / `Latest: none indexed`). Turn-index display remains owned by later page-level surfaces whose `PageSummary` contains `turnIndex`.
...
### 1. Created `tools/story-explorer/web/src/routes/stories.tsx`

Functional component implementing §4.2's contract. It reads `worldSlug` from URL params through a React Router loader and fetches both `getWorld(worldSlug)` for the world-level `indexStatus` banner and `listStories(worldSlug)` for the story list.

Structure:
- `<h1>{worldDisplayName} Stories</h1>`
- `<IndexStatusBanner status={world.indexStatus} />` (renders nothing when fresh)
- Story-card grid/list with deterministic story index-status badges
- Empty state: "No story bundles under this world." + "Use `/branching-story-bootstrap` to create one." link/hint
- Each card: title (or "Untitled story" fallback when STORY_KERNEL.md unreadable), story slug, PG count, leaf count, rendered-prose count, latest page (`Latest: PG-12` / `Latest: none indexed`), story index-status badge, inline PG-1-missing warning chip (when `rootPageId === null`), click → `/worlds/${worldSlug}/stories/${storySlug}/entry`.

### 2. Updated `tools/story-explorer/web/src/app.tsx`

Replaced the `/worlds/:slug/stories` placeholder with the imported `StoriesRoute`, attached `storyListLoader`, and wrapped the route in the existing `RouteFrame` with `Loading stories...`.

### 3. Created `tools/story-explorer/web/src/routes/stories.test.tsx`
...
- Every story `IndexStatus.kind` maps to deterministic card labels.
...
Added story-grid, story-card, story-status badge, PG-1 warning chip, route kicker, and full-row index-status banner styles using the existing token system.
...
- `tools/story-explorer/web/src/styles/app.css` (modify — story picker and banner styles)
...
1. `cd tools/story-explorer/web && npm test -- stories.test` — all 6 test cases pass.
...
1. `npm test -- stories.test` from `tools/story-explorer/web/` passed: 1 file / 6 tests.
2. `npm test` from `tools/story-explorer/web/` passed: 10 files / 33 tests.
...
The Story Picker route is now live at `/worlds/:slug/stories`: it loads world metadata and story summaries through the existing API client, renders the world-level index banner before the story list, handles empty story lists as designed UI, renders story cards with metadata and deterministic index-status/PG-1 warning chips, and navigates via links to each story's entry route.

Deviations from the original plan: the live `StorySummary` type has `latestPageId` but no latest-turn field, so the card displays latest page only. Ignored web build artifacts were refreshed by `npm run build`; they remain generated artifacts, not tracked ticket-owned source.
