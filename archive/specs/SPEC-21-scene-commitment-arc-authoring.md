...
- **Phase 5 (Diversity Audit)**: shape ≤40% / tone ≤40% / OBL-engagement ≥60%. Under v2, the diversity axes shift: `commitment_class` ≤30% (tighter because the enum is larger — 20 values) / `arc_archetype` ≤25% / tone ≤40% (preserved) / OBL-engagement ≥60% (preserved) / dramatic-unit-coverage (new — measured against `beat_plan.beats[].state_significance` aggregated across all beats in all arcs in the batch, using the 8-axis `strong_axis` enum: `relationship_trajectory`, `obligation_state`, `information_posture`, `risk_cost_exposure`, `route_or_scene_type`, `thread_pressure`, `irreversibility`, `character_intention`; each axis appears as `state_significance` on ≥30% of the batch's arcs).
...
The OBL → `eligible_commitment_classes` mapping is heuristic (LLM-driven against the OBL's `type` and `subjects`), not a closed table — Phase 1 prompts the LLM to enumerate which commitment_classes could plausibly pay off each open OBL given its narrative shape.
...
| 13. Exit-portfolio completeness (≥1 native_seed; engine_discovered_exit_budget block present with min/max/allowed_sources) — these are sub-field minimums enforced by SPEC-22's `arc_schema_compliance` validator (which checks "all seven new structural blocks ... populated; required sub-fields per block are non-empty"); not a separate validator | NEW | n/a | HARD-REJECT |
| 14. Rule 11 spectator-caste leverage (skill-internal deterministic check — no SPEC-22 validator extension): when `effect_model.required_effects` includes a `fact_create` op whose `args.truth_scope.world_level == true` AND `args.exception_governance` is populated (i.e., the arc creates a world-level exceptional-capability fact), arc.notes MUST carry a `leverage:`-prefixed line enumerating ≥3 ordinary-actor leverage forms from the canonical permissible-enum (`locality`, `secrecy`, `legitimacy`, `bureaucracy`, `numbers`, `ritual_authority`, `domain_expertise`, `access`, `timing`, `social_trust`, `deniability`, `infrastructural_control`). Mirrors create-base-world Phase 9's genesis spectator-caste check from SPEC-18. The existing `rule11_action_space` validator's `applies_to: appliesToCanonFacts` does not run over SLT records; this gate is enforced at skill-Phase-4 only | NEW | n/a | HARD-REJECT |
...
| Dramatic-unit-coverage | n/a | each `strong_axis` enum value (`relationship_trajectory`, `obligation_state`, `information_posture`, `risk_cost_exposure`, `route_or_scene_type`, `thread_pressure`, `irreversibility`, `character_intention`) appears as `beat_plan.beats[].state_significance` on ≥30% of the batch's arcs (aggregated across all beats per arc) | new — ensures the batch covers the strong-axis space; measurement source is `beat_plan.beats[].state_significance` rather than `value_delta_target`, because the SLT's `value_delta_target` block has only 4 sub-blocks (relationship, thread_pressure, obligation, information_posture) per archived SPEC-19 §A while the strong-axis space has 8 dimensions |
...
- **Arc-seed completeness**: Phase 2 seeds carry all required fields per §B (`commitment_class`, `arc_archetype`, `target_obligation`, `target_thread`, `entry_pressure_description`, `scene_question`, `value_delta_target_axes`, `tone_register`, `content_intensity_band`, `implied_preconditions`, `dramatic_transaction_summary`). `target_obligation` and `target_thread` MAY be null (arcs that engage neither an OBL nor a THR are valid); the other 9 fields MUST be populated. Test: synthesize 10 seeds; all 10 have the 9 mandatory fields populated and the 2 nullable fields explicitly set (null or a real id).
...
- **Phase 5 diversity thresholds**: a 20-arc batch has commitment_class ≤30% per class, arc_archetype ≤25% per archetype, tone ≤40%, theme ≤50%, OBL-engagement ≥60%, and each of the 8 `strong_axis` enum values (`relationship_trajectory`, `obligation_state`, `information_posture`, `risk_cost_exposure`, `route_or_scene_type`, `thread_pressure`, `irreversibility`, `character_intention`) appears as `beat_plan.beats[].state_significance` on ≥30% of the batch's arcs.
...
- **Rule 11 leverage on capability arcs**: an arc whose `effect_model.required_effects` includes a `fact_create` op with `args.truth_scope.world_level == true` AND `args.exception_governance` populated (a world-level exceptional-capability fact) MUST carry `leverage:` in `notes` with ≥3 forms from the canonical permissible-enum; gate 14 HARD-REJECTs missing or under-3 leverage lines. Arcs whose `required_effects` include only `mystery_progress` or other story-local effects do NOT trigger gate 14.
...
- **Engine ops + validators + canonical-vocabularies + sibling-skill alignment** — owned by SPEC-22. Phase 5b (Engine Pre-Validation, in the existing skill at `.claude/skills/storylet-pool-authoring/SKILL.md`) inherits SPEC-22 Track 2's extensions automatically: `record_schema_compliance` extends to handle SLT v2 (per SPEC-22 §Track 2), and `storylet_predicate_dsl_parsability` extends to include stop-policy predicates via the shared `_shared/predicate-dsl-grammar.ts`. No SPEC-21 changes to Phase 5b's coverage list are required.
...
- **JIT template cascade quality**: JIT arcs are bounded by the archetype library. A JIT call against a commitment_class with no recommended-archetype mapping (rare but possible) falls through to a generic `silent_witness` archetype with a warning logged. Future tickets may add commitment-class-specific JIT prompts.
- **Rule 11 leverage on every capability arc**: gate 14's trigger is narrow by design — fires only when `effect_model.required_effects` includes a `fact_create` op with `args.truth_scope.world_level == true` AND `args.exception_governance` populated. Arcs whose `required_effects` include only `mystery_progress`, `relationship_axis_shift`, `thread_pressure_delta`, or other story-local effects do NOT trigger gate 14, even when the affected mysteries / threads have `safety: low`. Gate 14 is a skill-internal Phase 4 check; the existing `rule11_action_space` engine validator runs only over CF records (`applies_to: appliesToCanonFacts`) and does not backstop SLT-level enforcement. If a future spec wants engine-level Rule 11 backstop on arcs (parallel to canon-addition Phase 14a Test 11 for CFs), it would extend SPEC-22's validator inventory with a new `rule11_action_space_arc` validator; not in v1 scope.
