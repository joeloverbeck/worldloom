...
3. **At intake, `STENT.role_in_story` was an untyped string** in `tools/world-mcp/src/context-packet/shared.ts` despite FOUNDATIONS §Story Bundles §7 naming it as a sibling-scan shared surface. `SPEC23STOSTACON-006` and `SPEC23STOSTACON-007` have since closed the schema and `world-mcp` projection sides; this bullet remains historical context for why Workstream 3 existed.
...
- **`role_in_story` projection in `context-packet/shared.ts`.** Completed by `SPEC23STOSTACON-007`: the public context-packet shape now uses `RoleInStory[]`, and package tests cover the list projection. The TypeScript shape change remains a breaking contract fact for consumers of `story_bundle_context.cast_bind_list[].role_in_story`.
