...
**Implementation note (2026-05-15, SPEC28STOCONHAR-005).** D5 is landed. The live `story-fact-promotion-to-canon/SKILL.md` and `templates/proposal-package.yaml` proposal-package examples keep `source_basis.direct_user_approval: false` throughout promotion, state that Phase 7 approves proposal creation rather than canon acceptance, and document `derived_from: []` for novel candidates with no `null` or branch ids. Remaining D5 current-state wording below is historical intake context for why the ticket existed.
...
**Implementation note (2026-05-15, SPEC28STOCONHAR-006).** D6 is landed. The live `story-promotion-closeout` skill preserves the "supersede only when an amended-schema field must change" rule, replaces the supersession-count gate with `source_record_dispositions:` completeness, documents the closed enum (`superseded | ledger_only | unchanged_no_schema_field_changed`) in the inline closeout-ledger schema, allocates supersession ids only for records that actually need replacement records, and describes output supersessions as disposition-driven rather than verdict-driven. Remaining D6 current-state wording below is historical intake context for why the ticket existed.
...
**Implementation note (2026-05-15, SPEC28STOCONHAR-007).** D7 is landed. The live `branching-story-bootstrap` skill now requires `STORY_KERNEL.md` YAML frontmatter with the machine-read story identity, cast binding, agency, mystery, and invariant fields before the eight markdown sections. `docs/CONTEXT-PACKET-CONTRACT.md` §6 now documents `story_bootstrap` as a target-slug, world-canon-only packet before the bundle exists, and `tools/world-mcp` now leaves `story_bundle_context` null for that task type. During implementation, live reassessment found the same retired storylet-pool `shape` / `content_intensity` vocabulary in the `tools/world-mcp` story-bundle context payload, so D7 also updated that response shape to `by_move_family`, `by_urgency`, and visible-record `move_family` / `urgency` fields. The parser path for `STORY_KERNEL.md` frontmatter did not need changes. Remaining current-state wording below is historical intake context for why the ticket existed.
...
- **D7**: `branching-story-bootstrap/SKILL.md` STORY_KERNEL.md contract specifies the YAML frontmatter block; `docs/CONTEXT-PACKET-CONTRACT.md` §6 no longer lists "shape/intensity counts" and clarifies `story_bootstrap` packet behavior; the frontmatter field names match what `story-bundle-context.ts` parses; the `tools/world-mcp` story-bundle context payload summarizes storylet pools by `move_family` and `saliency.urgency`.
...
- **The CF-schema-level `direct_user_approval` field-without-a-consumer question.** D5 fixes the misleading *story-skill* comments; whether the field should exist at all in the *world-canon* CF schema is a `docs/FOUNDATIONS.md` / `canon-addition` scope question. Captured as follow-up `tickets/FOUNDATIONS-005.md`.
