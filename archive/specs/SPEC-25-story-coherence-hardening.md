...
`archive/reports/story-related-upgrades.md` proposes 13 changes across three priority tiers. That report was produced from `FOUNDATIONS.md` + the story-state contract + the *initial brainstorming docs* — not the *landed skills*. Reassessment against the actual codebase recalibrates it sharply:
...
**Skill**: SPEC25STOCOHHAR-009 landed this cleanup in `story-promotion-closeout` SKILL.md — Phase 2 `flag` / `archive` branch handling is ledger / `INDEX.md`-only, the Output table has no `BR` supersession row, the Phase 5 op list has no `create_br_record`, and the upstream `story-fact-promotion-to-canon` handoff no longer promises BR supersession. No schema, MCP, or validator change.
...
**§9 Prose Length Discipline At Story Scope (C2 — pre-existing doc-drift)**: At D7 intake, §9 still referenced `arc.beat_plan.min_beats`, `max_beats`, and the old `STORY_KERNEL` structural-pacing fields from the archived SPEC-19/20 scene-commitment-arc era. The landed contract §4.4 `SLT` schema has a flat `beats: 1-5` list and no `arc` field. SPEC25STOCOHHAR-010 reconciled §9 against the landed contract: `FOUNDATIONS.md` now references `SLT.beats` / page-plan structural pacing and contains no `arc.beat_plan`, `cadence_policy`, or `max_arcs_without` literals.
...
- **D6**: SPEC25STOCOHHAR-009 landed; `story-promotion-closeout` SKILL.md contains no remaining "supersede a `BR`" reference, and cross-skill BR-supersession / `create_br_record` closeout references are clean.
...
The following `archive/reports/story-related-upgrades.md` items are rejected; all rejections are **structural** (the zero-production-stories greenfield position means no rejection is a pragmatic / migration-cost softening):
...
- **D7 §9 reconciliation** *(resolved by SPEC25STOCOHHAR-010)*: the landed story-pipeline skills and shared story-state contract contain no current `cadence_policy` template field, and `FOUNDATIONS.md` §9 now references `SLT.beats` / page-plan structural pacing instead of the archived scene-commitment-arc vocabulary.
...
SPEC-25 landed as the completed `SPEC25STOCOHHAR-*` ticket family. The final state includes replayable `STSTAT` entity status, schema-backed `SF.authority`, required `OBL` / `CNSQ` urgency, predicate DSL v2 with alias binding, explicit `CHC.grounded_in` gate-7 enforcement, BR-supersession cleanup in `story-promotion-closeout`, and `docs/FOUNDATIONS.md` D7 reconciliation.

The implementation narrowed the external-review report to the structural hardening changes that survived live-repo reassessment. Rejected report items remain out of scope in this spec. The bootstrap seed-block predicate DSL integration was split to and completed by `archive/tickets/SPEC25STOCOHHAR-011.md` after SPEC25STOCOHHAR-007 proved it was a direct bootstrap authoring surface.

Verification is recorded in the archived implementation tickets:

- `archive/tickets/SPEC25STOCOHHAR-001.md` through `archive/tickets/SPEC25STOCOHHAR-011.md` for the landed package, validator, skill, and docs proof surfaces.
- `archive/tickets/SPEC25STOCOHHAR-010.md` for the final FOUNDATIONS reconciliation proof, including `STSTAT` presence and the absence of `arc.beat_plan`, `cadence_policy`, and `max_arcs_without` literals from `docs/FOUNDATIONS.md`.
