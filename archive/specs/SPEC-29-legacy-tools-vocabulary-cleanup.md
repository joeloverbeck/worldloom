...
**Status**: COMPLETED — produced 2026-05-15; decomposed into completed `archive/tickets/SPEC29LEGTOOVOC-001.md`, completed `archive/tickets/SPEC29LEGTOOVOC-002.md`, completed `archive/tickets/SPEC29LEGTOOVOC-003.md`, completed `archive/tickets/SPEC29LEGTOOVOC-004.md`, and completed `archive/tickets/SPEC29LEGTOOVOC-005.md`.
...
**Source**: SPEC-28 follow-up 1 — `docs/triage/2026-05-15-story-related-improvements-triage.md` §"Follow-ups identified (not actioned)" bullet 1. Originally framed by the third-iteration external report (`archive/reports/story-related-improvements-third-iteration.md` P0.10) as "purge legacy ARC vocabulary"; SPEC-28's triage rejected the P0.10 framing because the live story skills and `story-state-contract.md` were already clean (greenfield rebuild), but the tools-layer residue remained and SPEC-28 deferred it as an out-of-report finding. SPEC-29 actions the deferred work.

**Implementation Note (2026-05-15)**: `archive/tickets/SPEC29LEGTOOVOC-001.md` completed D1 plus the same-seam producer registry cleanup in `tools/world-index/src/public/canonical-vocabularies.ts` and focused test residue cleanup in `tools/world-index`, `tools/world-mcp`, and `tools/validators`. `archive/tickets/SPEC29LEGTOOVOC-002.md` completed D2 plus the required world-mcp package-local fallout: `arc_trace_record` / `arc_trace_node` / `ARCTRACE` no longer appear in world-mcp source, registered metadata, package README, package scripts, fixtures, or tests, and `SUPPORTED_LIST_RECORD_TYPES` now has 30 entries. `archive/tickets/SPEC29LEGTOOVOC-003.md` completed the validators, patch-engine, and hooks D4 slice: the validators ARC compliance sentinel and fixture were deleted, patch-engine dropped the `arc_trace_emitted` / `arc_trace_id` / `arc_trace_evidence_alignment` prose-finalize exceptions plus their stale tests, the `create_arc_trace_record` envelope-rejection sub-test was removed, and hook3 now proves story-source blocking with non-ARC storylet paths. `archive/tickets/SPEC29LEGTOOVOC-004.md` completed the world-index package slice: `arc_trace_node` / `ARCTRACE` no longer appear in world-index source or tests, migration `005_arc_trace_nodes.sql` and the dedicated arc-trace indexing test are deleted, the forward migration chain is four files, and fresh `world.db` builds have no arc-trace tables. `archive/tickets/SPEC29LEGTOOVOC-005.md` completed D5 by truthing `docs/MACHINE-FACING-LAYER.md`, `tools/world-mcp/README.md`, and `tools/world-index/README.md` so current documentation no longer advertises retired vocabulary classes, `arc_trace_record`, `arc_trace_node`, `--arc-traces`, or `ARC_TRACE` package surfaces. Older broad-scope prose below remains historical planning context.
...
SPEC-29 landed through five implementation tickets now archived under `archive/tickets/SPEC29LEGTOOVOC-001.md` through `archive/tickets/SPEC29LEGTOOVOC-005.md`. The live tools layer no longer exposes the retired scene-commitment/arc vocabulary classes or `arc_trace_record` / `arc_trace_node` surfaces, the stale regression fixtures and package residues were removed, and the current MCP/world-index documentation was truthed to the surviving vocabulary and record surfaces.

Deviations from the original plan:
- The implementation absorbed same-seam package fallout discovered during ticket reassessment, including world-index public type/version surfaces, world-mcp package README/script/fixture cleanup, and docs/spec status notes.
- The retained `.claude/skills/_shared-templates/story-state-contract.md` prohibition text remains intentionally in place as a FOUNDATIONS-backed contract, not legacy vocabulary exposure.
- Historical planning and triage prose still mention retired terms as historical record; current source and public docs do not advertise them as live surfaces.

Verification:
- `archive/tickets/SPEC29LEGTOOVOC-001.md` through `archive/tickets/SPEC29LEGTOOVOC-005.md` record the package-local build/test, grep, and manual-review evidence for each deliverable.
- Final D5 proof confirmed no retired vocabulary / arc-trace tokens remain in `docs/MACHINE-FACING-LAYER.md`, `tools/world-mcp/README.md`, or `tools/world-index/README.md`.
- The broad retained-token sweep classifies remaining hits as historical planning/triage records or the retained story-state prohibition text.
