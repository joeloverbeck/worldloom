...
- 2026-05-17: `archive/tickets/SPEC39TOOESMMIG-005.md` completed the `tools/world-index` capstone migration. The landed slice added `"type": "module"`, moved world-index tsconfig to `Node16`/`Node16` with `ignoreDeprecations` removed, added `.js` suffixes to all relative source/test imports, replaced source/test CJS path globals with `import.meta.dirname`, converted ESM test-runtime probes where needed, and made `test:spec10-verification` use the tracked Animalia fixture instead of gitignored local `worlds/animalia` state. The package build, 87-test package suite, spec10 verification script, CLI smoke, static ESM greps, and downstream consumer builds all passed.
