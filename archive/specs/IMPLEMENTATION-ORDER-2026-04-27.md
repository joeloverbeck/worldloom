...
**Status**: completed 2026-04-24; spec archived at `archive/specs/SPEC-13-atomic-source-migration.md`. The one-week delayed cleanup of the ignored `.pre-migration-snapshot/animalia/` restore copy completed on 2026-05-02 via `archive/tickets/SPEC13ATOSRCMIG-006.md`; that ticket was a local safety cleanup exception, not a blocker for Phase 2.
...
- **Stream B — Animalia migration execution** (completed 2026-04-24 in private-world commit `99f6a97`):
  ...
  - Snapshot retention for one week, then cleanup (step 17) — completed by `archive/tickets/SPEC13ATOSRCMIG-006.md` on 2026-05-02
...
- Former exception closed: `.pre-migration-snapshot/animalia/` was removed after the one-week stability window by `archive/tickets/SPEC13ATOSRCMIG-006.md`
...
- **SPEC-06** all-skill migration — completed 2026-04-26 and archived at `archive/specs/SPEC-06-skill-rewrite-patterns.md`. All 8 canon-mutating / canon-reading skills (`canon-addition`, `character-generation`, `diegetic-artifact-generation`, `continuity-audit`, `propose-new-canon-facts`, `canon-facts-from-diegetic-artifacts`, `propose-new-characters`, `create-base-world`) rewritten against atomic source via the archived `SPEC06SKIREWPAT-001..008` per-skill ticket family plus the `SPEC06SKIREWPAT-009` static-audit capstone (audit report at `docs/triage/2026-04-26-spec06-phase2-static-acceptance.md`). SPEC-06 Part A / Part B distinction collapsed since atomic-source landed in Phase 1.5. `create-base-world` emits `_source/` directly for new worlds.
- **SPEC-07 Part B** — completed 2026-04-26. `docs/HARD-GATE-DISCIPLINE.md` §Why write order matters rewritten for the post-SPEC-13 3-tier engine ordering (create-all → update-all → adjudication; two-phase commit; per-world write lock); §Execution pattern step 6 de-hedged for the now-Phase-2-complete engine path. `CLAUDE.md` §Non-Negotiables HARD-GATE bullet refined to acknowledge Hook 3 + approval-token structural enforcement (with prose retained for absent-mechanism cases); `_source/` engine-only-surfaces bullet refined to distinguish Hook-3-blocked `_source/<subdir>/*.yaml` from prescriptively-engine-routed hybrid artifacts. SPEC-07 fully resolved (Part A done, Part B done, Part C retired); spec archived at `archive/specs/SPEC-07-docs-updates.md`.
...
- `SPEC-07 Part B` docs updates — completed 2026-04-26; archived at `archive/specs/SPEC-07-docs-updates.md`
...
**Tier 4 (depended on Tier 3 + Tier 3.5)**:
- `SPEC-06` all-skill migration — completed 2026-04-26 and archived at `archive/specs/SPEC-06-skill-rewrite-patterns.md`. Per-skill ticket family archived under `archive/tickets/SPEC06SKIREWPAT-001..008.md` in the historical sub-order below; `SPEC06SKIREWPAT-009` (static-audit capstone) archived at `archive/tickets/SPEC06SKIREWPAT-009.md` with audit report at `docs/triage/2026-04-26-spec06-phase2-static-acceptance.md`.
  1. `canon-addition` (proven pattern; migrated first as the fullest-surface pilot)
  2. `create-base-world` (updated to emit `_source/` directly for new worlds)
  ...
  5. `propose-new-characters` (biggest; landed once pattern was proven)
  6. `continuity-audit` (complex reasoning; landed last on the main-orchestrator side)

**Phase 2 completion gate** (revised per SPEC-13 and SPEC-14; **CLOSED 2026-04-26** via the SPEC06SKIREWPAT-009 static-audit capstone — see `docs/triage/2026-04-26-spec06-phase2-static-acceptance.md`):
- All 8 skills end-to-end via engine against `_source/` atomic records — **MET**
- canon-addition large delivery: zero raw Edit; Hook 3 denies 100% of raw attempts on `_source/` surfaces — **MET**
- **Per SPEC-14**: `world-validate animalia --json` reports **zero findings** (not "zero `fail` with `info` grandfathering as residual" — the 224-finding baseline had full fix paths via SPEC-14 Tier 3 tickets and the grandfathering file emptied post-fix) — **MET**
...
- Atomicity injection tests pass (two-phase commit holds; no partial writes) — **MET** (covered by archived SPEC-03 patch-engine test suite)
- Concurrency test passes (per-world write lock serializes same-world plans) — **MET** (same coverage)
- Every `create_*` / `update_record_field` / `append_extension` / `append_touched_by_cf` op fixture-tested — **MET**
- `touched_by_cf_completeness` validator passes on animalia post-Phase-2 state — **MET**
- **Per SPEC-14**: every record emitted by a rewritten skill passes `record_schema_compliance` end-to-end (cross-package integration test asserts engine output matches validator schema) — **MET**
...
- **Phase 1.5** (SPEC-13 atomic-source migration): COMPLETE 2026-04-24
  ...
  - Deferred exception closed: `SPEC13ATOSRCMIG-006` removed `.pre-migration-snapshot/animalia/` after the one-week stability window
...
| SPEC-13 Atomic-Source Migration | ✓ implemented 2026-04-24; archived at `archive/specs/SPEC-13-atomic-source-migration.md`; delayed snapshot cleanup completed by `archive/tickets/SPEC13ATOSRCMIG-006.md` on 2026-05-02 |
