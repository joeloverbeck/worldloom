...
- **Bootstrap PG-0001 choice-creation coverage gap** — the initial spec set focused on `branching-story-page-cycle`'s Phase 8 rewrite (SPEC-20 §F) but did not address `branching-story-bootstrap`'s Phases 7-9 (root render + initial choice generation + validation gates), even though bootstrap is the second skill that emits CHCs (PG-0001's initial 4-6) and renders the root page. Bootstrap's Phase 8 already delegates to page-cycle Phase 8, so the choice-surface gate propagates mechanically — but neither SPEC-20 nor SPEC-22 specified how the gate behaves at PG-0001 (no parent arc to classify against; no closed arc to draw `native_seeds` from; no ARC_TRACE to extract). Resolved by:
  ...
  - SPEC-22 Track 4's `branching-story-bootstrap` section expanded with Phase 7 (root page render becomes scene-setter mode — no SLT selection at PG-0001; first arc render is at PG-0002 via page-cycle), Phase 8 (delegate to SPEC-20 §F's choice-surface gate in PG-0001 special-case mode; the legacy `phase-8-choice-generation.md`'s Required CHC diversification + Pair-distance discipline sub-sections are SUPERSEDED by SPEC-20 §F), Phase 9 (gate count 12 -> 17, with vacuous-at-root and root-page-exception semantics for the new validators), plus updated deliverables-table rows for the affected reference files.
...
| SPEC-20 | `archive/specs/SPEC-20-scene-commitment-arc-runtime-pipeline.md` | Runtime tier — page-cycle rewrite for arc selection + effect-variant-before-render + multi-beat render + ARC_TRACE extraction + choice-surface gate. |
| SPEC-21 | `archive/specs/SPEC-21-scene-commitment-arc-authoring.md` | Completed authoring tier — storylet-pool-authoring rewrite for arc semantics + arc archetype library (14-20 archetypes) + Rule 11 leverage gate. |
| SPEC-22 | `archive/specs/SPEC-22-scene-commitment-arc-engine-and-cross-skill.md` | Completed machine-layer + sibling-skill + migration tier — patch-engine op + validators + canonical-vocabularies + indexer + MCP retrieval + branching-story-bootstrap/health-audit/promotion alignment + test-story discard. |

Plus:
- `archive/specs/IMPLEMENTATION-ORDER-2026-05-09.md` — completed dated history for the read order, implementation order (4 tiers), and dependency graph.
