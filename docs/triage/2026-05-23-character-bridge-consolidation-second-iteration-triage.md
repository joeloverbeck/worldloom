...
| §11 false-confidence path #4 (composite role evades voice check) | **accept (folded)** | Same finding as Proposal 1's parsing core; folded into SPEC-73. |
| §15 Proposal 6 — expanded negative fixtures | **fold** | Only the fixtures backing accepted work (composite-role-evades-voice-block + composite-offstage-on-present-STENT + unknown-label warn) are warranted. Folded into SPEC-73 §4.4 test plan. Capability-loss / role-demand / overlay fixtures depend on rejected/deferred proposals and fall away with them. |
...
| §18 open questions 1, 3 | **resolved by SPEC-73** | OQ1 (prose line vs YAML block): SPEC-73 keeps the prose line and parses it — exactly the report's own recommendation. OQ3 (legacy migration aggressiveness): SPEC-73 uses warn for unknown labels (one-release window); fails only the genuine voice-block omission and offstage-on-present cases the parsing fix newly catches. |
...
2. The receipt-side verbatim-composite contract added by VALSTCHAR-001 (`story-record-schemas.md:948`) is the authoritative spelling of composite `required_because` values, and SPEC-73's parsing aligns with it.
3. The closed vocabulary at `story-state-contract.md:466` (eight labels) is the intended documented surface; promoting the SPEC-73 warning to a hard schema enum is left to a future spec and not currently warranted.
4. No production page plan today legitimately uses a `Required because:` label outside the documented vocabulary for which a warning would be inappropriate; if one is found, SPEC-73's warn-not-fail policy gives a release window to migrate.
