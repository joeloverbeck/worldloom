...
|---|---|---|---|---|
| [STPOOL-002](../../tickets/STPOOL-002-fix-stop-predicate-args-in-storylet-record-examples.md) | F-01 — stop-predicate args drift in storylet-record.yaml examples | HIGH | Small | Three worked example arcs use predicate `args` that contradict the predicate-DSL grammar this skill owns (`commitment_satisfied` examples use `outcome:` instead of `commitment_class:`; `participant_exits` uses `role:` instead of `participant:`; one entry uses `safety_valve_triggered` which is not in `STOP_PREDICATES` at all). Bootstrap reference flags exactly this divergence and tasks the audit with surfacing it. Correctness fix; will pass existing validators after the edit. |
| [STPOOL-003](../../tickets/STPOOL-003-add-realization-target-to-beat-scaffolds.md) | F-02 — `realization_target` missing from beat scaffolds | HIGH | Small | Main beat scaffold and all three example arcs omit `beat_plan.beats[].realization_target`, which the engine JSON schema at `tools/validators/src/schemas/story-storylet.schema.json` requires per `branching-story-bootstrap/references/phase-6-storylet-pool-seed.md:78`. Records following the template will fail `record_schema_compliance`. |
| [STPOOL-004](../../tickets/STPOOL-004-add-non-empty-interrupt-before-to-examples.md) | F-03 — `interrupt_before` must be non-empty | HIGH | Small | Two of three example arcs omit `interrupt_before` entirely; the third uses an invalid predicate (corrected by STPOOL-002). Per bootstrap reference, validator rejects empty `interrupt_before: []` arrays. After STPOOL-002 lands, none of the three examples demonstrates a passing block — this ticket fixes that. |
...
- **STPOOL-003** (add realization_target) — independent of STPOOL-002. Can run in parallel.
