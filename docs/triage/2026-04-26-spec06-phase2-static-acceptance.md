...
**Open sub-item (separate from CS1)**: SPEC-07 Part B's prescriptive Markdown block at lines 152–161 still describes a 3-tier engine write order whose third tier names `CANON_LEDGER.md`. Post-SPEC-13, canon facts are atomic `_source/canon/CF-NNNN.yaml` records and `CANON_LEDGER.md` no longer exists. Additionally, `docs/HARD-GATE-DISCIPLINE.md:72` still describes the old `CANON_LEDGER.md`-append-last recovery model — Part B's "§Why write order matters rewrite for 3-tier engine ordering" has not been executed against the live doc. This is a SPEC-07 Part B execution gap, not a SPEC06SKIREWPAT-009 finding; it needs its own scoped work and is properly routed through `/reassess-spec` against SPEC-07. Surfacing here so it does not go silent.
...
- §Cross-Spec Follow-Ups: CS1 line-160 dead-file reference resolved; CS1 line 225 kept as a deprecated-pattern flag (correct as-is). One open sub-item under CS1 — the broader SPEC-07 Part B execution gap on the live `docs/HARD-GATE-DISCIPLINE.md` 3-tier rewrite — is routed for separate `/reassess-spec` work against SPEC-07.
