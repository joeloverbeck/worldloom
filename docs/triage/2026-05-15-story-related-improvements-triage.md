...
- **`source_basis.direct_user_approval` is a field-without-a-consumer** (out-of-report finding). SPEC-28 D5 fixes the misleading story-skill comments, but the field is inert across the entire codebase (FOUNDATIONS doesn't define it; `canon-addition` / validators / patch-engine never read it). Whether it should exist in the world-canon CF schema is a `docs/FOUNDATIONS.md` / `canon-addition` scope question.
