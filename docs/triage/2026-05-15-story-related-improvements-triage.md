...
- **Tools-layer arc-vocabulary residue** (out-of-report finding). `tools/world-mcp/`, `tools/validators/`, and `docs/MACHINE-FACING-LAYER.md` still carry SPEC-22-era arc machinery (`arc_trace_record` retrieval, `get_canonical_vocabulary` serving `arc_archetype` / `narrative_point`). The `commitment_family` / `commitment_class` part is confirmed-deliberate (SCAUD-003); the `arc_trace_record` part is ambiguous (create op is validator-rejected, read surface lingers). No arc records exist anywhere, so it is inert. Worth a `tools/`-scoped investigation to confirm deliberate-vs-dead — distinct from SPEC-28's skill+contract scope.
  - 2026-05-15 update: reassessment overturned the "SCAUD-003 deliberate retention" framing — codebase recon confirmed zero live skill consumers for `commitment_family` / `commitment_class` / `arc_archetype` / `narrative_point` / `strong_axis` / `strong_outcome` / `stop_predicate` (all seven are pure orphans in `tools/world-mcp/src/tools/get-canonical-vocabulary.ts`); SCAUD-003's "independent storylet/arc vocabulary metadata" rationale was speculative since the live SLT schema uses `move_family` and the "arc surface" is itself the SPEC-22 legacy the greenfield rebuild superseded. Actioned by `archive/specs/SPEC-29-legacy-tools-vocabulary-cleanup.md` (2026-05-15), which supersedes the SCAUD-003 retention decision.
...
- **P2 hostile test-fixture suite** as a dedicated follow-up spec, sequenced after SPEC-28 (its assertions depend on D2's `SE.commitment` and the D1/D4/D6 contract fixes being in place).
  - 2026-05-15 update: **deferred indefinitely**, not sequenced. SPEC-28 prerequisites all landed (PR #32 merged), and reassessment surfaced that the suite's load-bearing assertions (`every action creates an SE`, `no silent rejection`, `selected SLT recorded`, `no actor uses unavailable knowledge`) are **process invariants** that structurally require skill execution to test — either a mocked-skill harness (does not exist; would need to be built) or a manually-authored pilot bundle (real skill runs, user-priced-out: "not going to spend actual tokens running skills to produce a test story"). The remaining option — static bundle fixtures under `tools/validators/tests/fixtures/` — runs against the existing validator lane but adds *breadth, not depth*: the structural invariants it would catch are already enforced by `record-schema-compliance` + `additionalProperties: false` + `snapshot-replay-equality.test.ts` + `recursive-reference-closure.test.ts` on every bundle. Verdict: defer. If hostile-scenario *documentation* becomes useful for onboarding when a real story bundle is authored, that is a small `docs/` ticket later — not a test suite.
