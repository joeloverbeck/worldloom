...
The story-bundle family — `branching-story-bootstrap`, `branching-story-turn-cycle`, `branching-story-scene-plan`, `branching-story-scene-prose-attach`, `commitment-block-authoring`, `branching-story-health-audit`, `story-fact-promotion-to-canon`, and `story-promotion-closeout` — all carry HARD-GATE blocks. `branching-story-scene-prose-attach` validates externally supplied scene prose and writes a `scene-prose-receipts/SCN-<integer>.yaml` receipt; it never mutates the PG record, never promotes story facts to world canon, and never gates future `branching-story-turn-cycle` planning. Canon promotion remains exclusive to `story-fact-promotion-to-canon`, which hands approved candidates to `canon-addition` under its own HARD-GATE.
...
3. **Validation / Rejection tests** — numbered; each must record PASS with an authority-cited one-line rationale. The rationale must cite the record id, packet layer, validator result, retrieved field, or named loaded authority that supports the judgment. A bare "PASS" is treated as FAIL per the skill's own contract; a PASS/FAIL rationale based only on model memory, vibes, or a prose impression without a cited authority is also treated as FAIL.
...
**Validate MCP path (default)** — call `mcp__worldloom__validate_patch_plan(plan)` with the plan envelope. The current state-turn contract validates story records directly; there is no draft side-channel for new `PG` commits. This is the canonical pre-apply validation path for ordinary plan sizes.
...
**MCP path (default)** — call `mcp__worldloom__submit_patch_plan(plan, approval_token)` with the plan envelope and the issued token. Submit-time pre-apply validators inspect the approved record envelope directly. This is the canonical submission path for ordinary plan sizes.
...
- Same record-envelope validation path: validate and submit route the approved patch-plan envelope through the same pre-apply validators before any write.
