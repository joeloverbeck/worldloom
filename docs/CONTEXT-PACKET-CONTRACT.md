...
This layer is `null` for world-canon task types. For `story_character_profile`, `story_turn_cycle`, `commitment_block_authoring`, `branching_story_health_audit`, and `story_fact_promotion_to_canon`, callers must supply `story_slug`; the layer is then populated from indexed story-bundle records plus `STORY_KERNEL.md` frontmatter. For `story_bootstrap`, callers supply `story_slug` as the target slug before the bundle exists; `story_bundle_context` is `null`, so the packet is world-canon-only. When a non-bootstrap story-pipeline request supplies `parent_page_id`, or includes a `PG-*` id in `seed_nodes`, the layer also includes `selection_shortlist`: a projection-only `select_storylet_candidates` result capped at 24 candidates with the default `player_action` turn driver. This shortlist is additive to the 50-cap visible storylet summary and points consumers to `get_records(record_ids=requires_full_body_ids, story_slug=...)` for full SLT bodies.

This layer carries:

- storylet pool totals, `move_family` distribution, `saliency.urgency` counts, and capped visible storylet records
- `selection_shortlist` when a parent page is available, carrying a SPEC-81 label, `filter_trace`, compact projection records, `shortlisted_candidate_ids`, and `requires_full_body_ids`; it never carries full SLT bodies
- open obligations
...
- active story characters, exposed as `active_story_characters` entries with `id`, `status`, `bound_stent_ids`, `source_kind`, `source_char_id`, `profile_revision`, and `packet_preview`. This is a status-based active STCHAR inventory (`status === "active"`), not a page/branch snapshot of the exact current-page cast.
...
- `packet_incomplete_required_classes` must report `missing_classes`, `requested_budget`, `minimum_required_budget`, `retained_classes`, top-level `fallback_advice`, and `truncation_summary` (listing every droppable layer that was emptied during the failed fit attempt). It includes `retry_with.token_budget` only when the token budget is the binding constraint.
...
The effective inline packet-body ceiling is `harness_ceiling_chars - envelope_overhead_reserve_chars` (`56000` characters by default). The assembler builds all packet content layers, including `story_bundle_context` for story-pipeline task types. If the fully assembled packet would exceed that effective ceiling, the server writes that full packet JSON to its package-local tool-results directory and returns a bounded inline summary with `task_header.delivery_status: persisted_with_summary` and `task_header.persisted_output_path`; story-pipeline summaries include `governing_summary.story_bundle_context_summary`. The same package-owned tool-results policy is used by large follow-up retrieval tools before the external MCP harness can reject the response: unprojected hybrid `get_record` persists full JSON and returns `delivery_status: oversize_with_projection_suggestions`, while oversized `get_records` and unfiltered `describe_envelope_schema` persist full JSON and return `delivery_status: persisted_with_summary` plus a bounded `summary` block. If the `get_record` suggestion list must be shortened to keep the recovery response under the effective ceiling, `suggested_section_paths_omitted_count` reports the number of valid paths omitted from the inline hint.
...
`local_authority` and `task_header` are never dropped. For task types whose `task_header.governing_full_body_priority` reserves invariants or Mystery Reserve entries, `governing_world_context` also becomes required once those governing full bodies are allocated: if the packet cannot fit that layer plus local authority under the requested token or effective harness ceiling, the assembler returns `packet_incomplete_required_classes` instead of silently downgrading those governing full bodies. If even `local_authority` exceeds the requested token budget after every droppable layer is emptied and no persisted-summary response can fit inline, the assembler returns the same structured code. The error details always include `harness_ceiling_chars`, `envelope_overhead_reserve_chars`, `effective_harness_ceiling_chars`, `minimum_required_harness_ceiling_chars`, and top-level `fallback_advice` so operators can distinguish token-budget insufficiency from transport-ceiling insufficiency. They include `retry_with: { token_budget: minimum_required_budget }` only when `minimum_required_harness_ceiling_chars <= effective_harness_ceiling_chars`; when `minimum_required_harness_ceiling_chars > effective_harness_ceiling_chars`, no token-budget retry can satisfy the current operator harness ceiling, so callers should follow `fallback_advice` and retrieve dropped records individually or in batches.
...
Use `mcp__worldloom__get_persisted_packet_slice(persisted_path, slice_path)` for structured extraction from the full packet. Dot paths address object fields (`governing_world_context.nodes`), array indexes (`records[0].record.record`), and node-id selections (`local_authority.nodes[id=entity:donostia]`).

`get_records` uses the same delivery-status vocabulary at its own top level. Inline responses are `{ delivery_status: "inline", records: [...] }`. If the full ordered batch response would exceed the effective inline ceiling, the tool persists that inline-shaped JSON and returns `{ delivery_status: "persisted_with_summary", persisted_output_path, summary }`. The summary carries `record_count`, `requested_record_ids`, per-record metadata / per-id errors, ceiling metadata, and suggested slice paths such as `records[0]`; retrieve full record bodies with `get_persisted_packet_slice(persisted_path, "records[<N>].record.record")`.

`describe_envelope_schema(op_kind?)` also uses `{ delivery_status: "inline", ... }` when its response fits. Unfiltered or otherwise wide responses may return `delivery_status: "persisted_with_summary"` with `summary.available_op_kinds` and suggested slice paths such as `op_schemas.create_cf_record`; callers should usually re-invoke with `op_kind` when they only need one operation schema, or slice the persisted full schema when comparing across multiple operation kinds.

## Index + Follow-Up Retrieval Pattern

The context packet's content layers (`local_authority` through `impact_surfaces`; `task_header` is metadata) deliver an INDEX of locality-relevant nodes plus body-preview snippets sufficient for ranking and citation. For story-pipeline task types, `story_bundle_context` additionally delivers the indexed bundle-local pool / obligation / thread / branch state for the supplied `story_slug`; when a parent page is available, `story_bundle_context.selection_shortlist` delivers the indexed storylet-candidate shortlist produced by `select_storylet_candidates`. For selected task-critical classes, nodes in `local_authority`, `governing_world_context`, and `exact_record_links` may also carry an additive `full_body` string. Skills that need the full body of one load-bearing node that was not delivered retrieve it via `mcp__worldloom__get_record(record_id)`; for large hybrid records, an oversize response returns `suggested_section_paths` so the skill can immediately retry with `section_path` rather than falling back to raw file reads. Skills that already have a known set of ids retrieve them via `mcp__worldloom__get_records(record_ids)` to preserve order and avoid N round trips. Skills that need a single field of a large record retrieve it via `mcp__worldloom__get_record_field(record_id, field_path)`. When a packet returns `delivery_status: persisted_with_summary`, skills retrieve structured slices from the persisted full packet via `mcp__worldloom__get_persisted_packet_slice(persisted_path, slice_path)`. Skills whose validation surface intentionally tests every record of a class, such as whole-class invariant or Mystery Reserve firewall passes, may use `mcp__worldloom__list_records(world_slug, record_type=<type>, include_full_body=true)` as the primary load instead of a seed-local packet plus a known-id batch. This pattern keeps packet sizes within model-context budgets while preserving FOUNDATIONS §Tooling Recommendation completeness guarantees: the packet identifies WHAT must be retrieved; task-aware `full_body`, story-bundle context, targeted retrieval, batched targeted retrieval, storylet-candidate shortlist retrieval, persisted-packet slice retrieval, and whole-class enumeration deliver the required content.
...
The profile remains world-canon read-only for world-level records and requires `story_slug`. `get_context_packet(task_type='story_turn_cycle', story_slug=..., parent_page_id='PG-N')` returns world-canon/indexed context, the governing audit trail needed to interpret the story-local turn safely, and `story_bundle_context` for the indexed bundle-local pool / obligation / thread / recent-page state plus a projection-only `selection_shortlist`.
...
`story_fact_promotion_to_canon` is the registered context-packet profile for `story-fact-promotion-to-canon` Pre-flight. The skill derives `seed_nodes` from source-relevant CF, M, INV, OQ, and named-entity ids gathered while translating a story-local source into a canon-addition proposal package. The profile uses an 8000 default budget and prioritizes canon fact records, governing invariant and Mystery Reserve records, open questions, named-entity grounding, relevant section context, and recent change-log context so promotion triage can detect canon-baseline conflicts before handing a world-canon candidate to `canon-addition`.
...
|---|---|---|
| `list_records(world_slug, record_type, include_full_body=true)` | Whole-class loads where the consumer must test every record of a supported atomic or hybrid class, such as EPE Phase 6 invariant / Mystery Reserve firewall checks, continuity-audit cross-checks, or CHAR/DA/PA/NCP/NCB registry enumeration. | `{ records: [{ record_id, content_hash, file_path, body }], total, truncated: false }`; hybrid `body` is `{ record_kind, frontmatter, body_sections }` |
| `get_records(record_ids, world_slug?)` | Known-id follow-up loads where the packet, claim map, audit window, or dossier trace already names multiple records and whole-class enumeration would be too broad. | Inline: `{ delivery_status: "inline", records: [{ record_id, found, record?, content_hash?, file_path?, error? }] }` in request order. Oversize: `{ delivery_status: "persisted_with_summary", persisted_output_path, summary }` with `records[<N>]` slice hints. |
| `get_persisted_packet_slice(persisted_path, slice_path)` | Structured recovery from package-persisted JSON emitted by `get_context_packet`, `get_records`, or `describe_envelope_schema` `persisted_with_summary` responses. | `{ found, slice?, error? }` for dot paths such as `governing_world_context.nodes`, `records[0].record.record`, `op_schemas.create_cf_record`, or `local_authority.nodes[id=entity:donostia]` |
