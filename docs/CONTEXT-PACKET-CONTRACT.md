...
- active story characters, exposed as `active_story_characters` entries with `id`, `status`, `bound_stent_ids`, `source_kind`, `source_char_id`, `profile_revision`, `profile_hash`, `voice_block_hash`, and `packet_preview`. This is a status-based active STCHAR inventory (`status === "active"`), not a page/branch snapshot of the exact current-page cast; page-specific `page_packet_hash` values live on page-plan §16a packets and prose receipts.
