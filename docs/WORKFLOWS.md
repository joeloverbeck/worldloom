...
- **Surgical story-state maintenance**: use `mcp__worldloom__plan_story_state_maintenance` for bounded append-only story-bundle repairs where no new fictional turn occurred. The tool only returns a patch-plan envelope; validate it, obtain explicit approval, sign the exact plan, and submit through `mcp__worldloom__submit_patch_plan`. Use `branching-story-turn-cycle` `repair_turn` instead when the repair is a real causal tick that needs a new `SE`, `PG`, page plan, and choices.
