...
| `plan_story_state_maintenance` | Review-only planning helper for bounded story-bundle state maintenance. It allocates fresh story-scoped IDs, verifies superseded source records through indexed retrieval, and emits a patch-plan envelope using existing `create_stemo_record`, `create_stplan_record`, `create_srel_record`, and `create_chc_record` ops. It never signs, submits, or writes; operators must validate the returned `patch_plan`, obtain explicit approval, sign that exact plan, and apply it with `submit_patch_plan`. Use this for append-only correction of stale story state when no new fictional turn occurred; use `branching-story-turn-cycle` `repair_turn` when the repair is a real causal tick that needs a new `SE` / `PG` / page plan. |
