...
|---|---|---|
| Retrieval tools report missing nodes, persistent `index_version_mismatch`, or persistent `stale_index` | `_index/world.db` is absent, incompatible, still schema-mismatched after the retrieval layer's one-shot auto-build recovery, or still stale after auto-sync recovery | Run `world-index init <world>` for an empty bootstrap, or `world-index build <world>` / `world-index sync <world>` for populated world state. If a successful retrieval response includes `freshness_audit.pre_call_index_version_was_old: true` or `freshness_audit.pre_call_index_was_stale: true`, the retrieval layer already recovered and no manual retry is needed. |
