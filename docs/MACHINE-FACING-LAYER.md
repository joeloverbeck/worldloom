...
| Localize specific nodes, records, persisted-packet slices, record fields, story-state provenance, entities, storylet candidates, or neighborhoods | `search_nodes`, `get_node`, `get_record`, `get_records`, `get_records_field`, `get_story_state_provenance`, `get_persisted_packet_slice`, `list_records`, `select_storylet_candidates`, `get_record_field`, `get_neighbors`, `find_named_entities` |
| Preview append-only IDs for one or many classes | `allocate_next_id` previews a single world-, story-, pipeline-, or sub-audit-scoped ID; `allocate_many_ids` previews an ordered batch during bootstrap/reservation workflows. The calls are read-only: batch entries use the same `{id_class, story_slug?, audit_id?}` scope rules as the single allocator and repeated same-scope entries increment within one response, but separate calls do not advance the registry baseline. Reservation happens only when a patch plan consuming those IDs lands through `submit_patch_plan` with `envelope.expected_id_allocations`. |
...
| Validate a patch plan envelope without mutating world content | `validate_patch_plan`, which returns `status: "pass"`, `status: "fail"` with validator verdicts, or `status: "skipped"` with a reason when the envelope cannot be validated. `pass` and `fail` responses include `validators_run[]`; `skipped` responses include `validators_run: []`. Envelope-shape validation also checks the canonical record-id field for world-canon `create_*_record` payloads, so `create_ch_record` requires `payload.ch_record.change_id` while the other world-canon create ops require `id`; misshapen create payloads skip before validator delegation with an `invalid_input` field path such as `patch_plan.patches[N].payload.ch_record.change_id`. For story-state turns, validators inspect the record envelope directly; new `PG` commits no longer use a draft side-channel. For envelopes too large for MCP transport, or for a temporary stale-validator-bundle workaround when restart is not immediately available, use the equivalent CLI path: `node tools/world-mcp/dist/src/cli/validate-patch-plan.js [--world-root <path>] <plan-path>`. The CLI resolves the world root by `--world-root` > `WORLDLOOM_ROOT` > auto-discovery from cwd. |
| Apply world-level changes on machine-layer-enabled worlds | `submit_patch_plan` via the patch engine. Submit-time pre-apply validators exercise the approved patch-plan envelope before any write. |
...
`inline-canonical-prose-sections` is the scene-plan authoring helper for Content Policy, Prose Craft Contract, and Render-Time Instruction byte equality. Run `node tools/world-mcp/dist/src/cli/inline-canonical-prose-sections.js --plan <scene-plan-draft.md>` before presenting a scene plan so the renderer-facing draft already contains the canonical content-policy, prose-craft, and render-time-instruction bodies. It is no longer part of `PG` hash stamping.
...
| `SLT` | `storylet_predicate_class` | story-bundle node-type value | Each distinct predicate-referenced node type projected for referenced-class filtering, derived from the closed predicate DSL and exact record-id prefixes. Role and subtype filters such as `holder_role` or `kind` are not class values. |
...
| `get_record` | The full parsed record for a structured id such as CF / CH / M / OQ / SEC / PA / DA / CHAR / NCP / NCB. World-level DA records and proposal records are hybrid markdown records and do not require `story_slug`; story-local `DA-<integer>` records are story-bundle records and require `story_slug`. Other story-bundle ids such as PG / SE / BEL / SF / OBL / CNSQ / THR / SREL / STINT / STENT / STSTAT / STCHAR / STLOC / STOBJ / CLK / STSEC / STQ / STPLAN / STEMO / BR / CHC / SLT / SLB / SAU / SP / RSP require `story_slug` because authored story ids are unique only within `(world_slug, story_slug)`. `ARC_TRACE` is not a valid record class. Use this after context-packet previews before citing record content. Optional `section_path` projects parsed atomic/story records by dotted path, such as `effect_evidence` or `semantic_critic_verdict.status`; for hybrid PA / world-level DA / CHAR / NCP / NCB records it can project `frontmatter`, `body`, `frontmatter.<key>`, or `body.<section>`, and for story-local STCHAR hybrid records it can project `frontmatter`, `body`, or body sections when `story_slug` is supplied. If an unprojected hybrid response would exceed the effective inline ceiling, the bounded response is `delivery_status: "oversize_with_projection_suggestions"` with `persisted_output_path`, `total_chars`, `response_cap_chars`, and `suggested_section_paths`; retry with one of the suggested `section_path` values for structured slice retrieval. When the suggestion list itself must be shortened to keep the recovery response under the cap, `suggested_section_paths_omitted_count` reports how many valid paths were omitted from the inline hint. |
...
| `verify_pg_state_hash` | Verification for one committed PG record. Requires `world_slug`, `story_slug`, and `page_id`, recomputes `state_hash` directly from the parsed PG without modifying plan fields, and returns the recorded/computed state hash pair. State hash mismatch is the verdict-driving tamper signal. |
...
| `list_records` | Indexed records for one supported atomic, hybrid, or story-bundle record type. Hybrid record types include `character_record`, `diegetic_artifact_record`, `adjudication_record`, `character_proposal_card`, and `character_proposal_batch`. Story-bundle record types require `story_slug` and include `story_entity_record`, `story_status_record`, `story_plan_record`, `story_emotion_record`, `scene_record`, `story_character_authority_record`, `belief_record`, `story_fact_record`, `story_event_record`, `obligation_record`, `consequence_record`, `thread_record`, `relationship_record_story`, `intention_record`, `story_location_record`, `story_object_record`, `pressure_clock_record`, `story_secret_record`, `story_question_record`, `branch_record`, `page_record`, `choice_record`, `storylet_record`, `story_diegetic_artifact_record`, `audit_record_story`, `promotion_record`, `storylet_batch_manifest`, and `remediation_storylet_proposal_card`. `section_record` enumerates atomic SEC-*-NNN YAML records only; WORLD_KERNEL.md narrative H2 spans are indexed as narrative sections for search/context and are retrieved directly by `get_record(record_id="<world>:WORLD_KERNEL.md:<H2 text>:0")` when needed. Optional `filters` narrow rows before projection/full-body wrapping by parsed body field path. Filter values are scalars for exact match or arrays for any-of membership; dotted paths address nested fields, such as `{ move_family: ["investigation", "disclosure"], "scope.visibility": "global_author_pool", "exit_options.action_family": ["investigate", "communicate"] }`. Array-valued record fields match when any record value is present in the filter set. Atomic and story-bundle default/projection modes accept `fields` entries that are either top-level parsed-record keys or dotted parsed-body paths from the filtered response rows; projected records always include `record_id` and keep the requested field string as the response key. For example, `fields: ["grounding.compatible_turn_drivers"]` on `storylet_record` returns `{ record_id, "grounding.compatible_turn_drivers": [...] }` per record. Hybrid default/projection mode returns compact metadata (`record_id`, `record_kind`, `title`, `content_hash`, `file_path`) for the hybrid record types; `fields` remains validated against those response-shape top-level keys. Unknown `fields` entries return `invalid_input` with `details.unknown_projection_keys` and sorted `details.accepted_projection_keys` instead of being silently dropped. Empty atomic/story-bundle result sets return `accepted_projection_keys: []` plus a `details.note` because accepted keys cannot be derived from matched rows. Hybrid filters can address frontmatter fields directly or through `frontmatter.<key>`. `include_full_body: true` returns `{ record_id, content_hash, file_path, body }` per record and ignores `fields`; hybrid `body` is `{ record_kind, frontmatter, body_sections }`. Use full-body mode for deliberate whole-class sweeps such as every invariant, every Mystery Reserve firewall block, continuity-audit cross-check, or structured hybrid registry enumeration; for large storylet pools or other predictable hard-filter loads, apply `filters` before falling back to persisted-output recovery. |
| `select_storylet_candidates` | Projection-only indexed shortlist for storylet candidates under one `(world_slug, story_slug, parent_page_id, turn_driver)` context. It applies story scope, branch visibility, driver kind, optional intent action family, predicate shape/class, source-record id, mystery-policy, cooldown, and salience/diversity ranking over `slt_projections` plus storylet edge projections, then returns `filter_trace`, `shortlisted_candidate_ids`, compact `shortlisted_projection_records`, and `requires_full_body_ids`. `filter_trace` carries the aggregate counts plus per-stage `<stage>_rejected_samples` arrays capped at 3 entries for scope, driver kind, action family, predicate shape/class, source-record id, and mystery policy diagnostics; each sample carries `slt_id`, `reason`, and stage-specific `evidence`. `cooldown_active_samples` remains the specialized cooldown diagnostic surface. `intent_signature.grounding_record_classes` is matched against indexed predicate `record_kind` values such as `belief_record` and `thread_record`; recognized predicate short codes such as `BEL` and `THR` are normalized before filtering, and unknown or non-predicate class tokens return `invalid_input` instead of silently producing a zero shortlist. `intent_signature.grounding_record_ids` narrows SLTs that have exact-id predicate refs to intersecting source records; SLTs with only existential predicates have no exact source refs and wildcard-pass the source-record-id stage because alias binding is resolved later against active records. It never returns full SLT bodies; consumers deliberately follow up with `get_records` for the shortlist only. Full predicate evaluation, alias binding, and Observer Firewall checks remain in the consumer-side turn-cycle evaluator. |
...
| `get_record_schema` | JSON Schema for a record node type plus transitively referenced schemas, required fields, and validator-sourced conditional block requirements, including hybrid proposal schemas (`character_proposal_card`, `character_proposal_batch`) and story-bundle schemas such as `belief_record`, `story_status_record`, `story_plan_record`, and `story_emotion_record`. Use to discover field constraints, regex patterns, enum values, required/optional fields, and CF taxonomy rules such as when `epistemic_profile` or `exception_governance` is required before authoring a record draft. For `canon_fact_record`, the current CF `status` enum is `hard_canon`, `derived_canon`, `soft_canon`, and `contested_canon`; Mystery Reserve entries are separate `M-<integer>` records, not CF status values. |
...
| `get_context_packet` | Ranked packet of Kernel, Invariants, relevant records, neighbors, section context, and story-bundle context for story-pipeline task types. `story_bootstrap`, `story_character_profile`, `story_turn_cycle`, `commitment_block_authoring`, `branching_story_health_audit`, and `story_fact_promotion_to_canon` require `story_slug`. For `story_bootstrap`, the slug is the target bundle slug and `story_bundle_context` is `null` because the bundle does not yet exist; for the other story-pipeline task types, `story_bundle_context` is populated from indexed story-bundle records plus `STORY_KERNEL.md` frontmatter. When `parent_page_id` is supplied, or a `PG-*` id appears in `seed_nodes`, non-bootstrap story-pipeline packets also include `story_bundle_context.selection_shortlist`: a projection-only `select_storylet_candidates` result capped at 24 candidates with the default `player_action` turn driver. World-canon task types return `story_bundle_context: null`. Story-pipeline `seed_nodes` are realized world-scope seeds such as `CHAR` ids and world anchors only for bootstrap/profile-source reads; normal turn-cycle, scene prose, and audit runtime consumes active STCHAR authority through `story_slug`, `story_bundle_context.active_story_characters`, and targeted `get_record` / `get_records` / `list_records(record_type="story_character_authority_record", story_slug=...)`. Story-local records should be read via `story_slug` + `story_bundle_context` or targeted retrieval, and story-local misuse is surfaced through `task_header.warnings`. `NCP` / `NCB` authoring-proposal ids are world-scope proposal records rather than realized story cast, so story-pipeline requests warn-and-drop them with `authoring_proposal_seed_nodes_ignored` (see `docs/CONTEXT-PACKET-CONTRACT.md` §Assembly Discipline). Unresolvable world-scope seeds are skipped and named in `task_header.warnings`, so resolvable seeds and seed-independent governing context can still be returned; when every seed is unresolved, an aggregate warning names the seed-independent fallback. Body previews are generally truncated and full text requires `get_record`; selected task-critical classes in `local_authority`, `governing_world_context`, and `exact_record_links` may carry additive `full_body`, with delivered classes reported in `task_header.full_body_classes_delivered` and budget downgrades reported in `truncation_summary.full_body_downgrades`. `character_generation`, `diegetic_artifact_generation`, `story_bootstrap`, `story_character_profile`, `story_turn_cycle`, `commitment_block_authoring`, `branching_story_health_audit`, and `story_fact_promotion_to_canon` reserve governing-context invariant and Mystery Reserve full bodies before opportunistic layers; `task_header.governing_full_body_priority` reports that policy, and insufficient budget/ceiling for those required governing bodies returns `packet_incomplete_required_classes` instead of silent downgrades. Task-specific governing nodes may also carry parsed `record` projections, such as `character_generation` invariant records and Mystery Reserve firewall fields. Omitted budgets use per-task defaults (`story_bootstrap`, `story_turn_cycle`, and `commitment_block_authoring` currently 18000; `canon_addition` currently 16000; `propose_new_canon_facts`, `propose_new_characters`, and `emergent_pressure_events` 15000; `propose_new_worlds_from_preferences`, `canon_facts_from_diegetic_artifacts`, `branching_story_health_audit`, and `story_character_profile` 12000; `story_fact_promotion_to_canon` and remaining task types 8000). Incomplete-packet errors include `retry_with.token_budget` only when the token budget is the binding constraint; if the minimum required harness ceiling exceeds the effective harness ceiling, the error omits `retry_with` and uses top-level `fallback_advice` to point callers to targeted `get_record` / `get_records` / `get_record_field` recovery. Successful packets expose `task_header.delivery_status`, `task_header.harness_ceiling_chars` (default 60000; server override `WORLDLOOM_MCP_HARNESS_CEILING_CHARS`), `task_header.envelope_overhead_reserve_chars` (default 4000), and `task_header.estimator_version`; inline packet bodies are kept under `harness_ceiling_chars - envelope_overhead_reserve_chars`, and when the fully assembled packet would exceed that effective ceiling, `delivery_status: 'persisted_with_summary'` returns `governing_summary` inline plus `task_header.persisted_output_path` for structured slice recovery. Optional `delivery_mode: 'full' \| 'summary_only'` (default `'full'`) selects per-node payload shape — `summary_only` replaces every node's `body_preview` with a <=100-char `summary` for "what's relevant" index passes while preserving eligible `full_body` delivery (see `docs/CONTEXT-PACKET-CONTRACT.md` §Fast-Summary Inline Delivery, §Task-Aware Full-Body Delivery, and §Delivery Modes). |
...
| `plan_story_state_maintenance` | Review-only planning helper for bounded story-bundle state maintenance from a committed `parent_page_id`. It allocates fresh story-scoped IDs, verifies superseded source records through indexed retrieval, emits the requested maintenance record create ops (`create_stemo_record`, `create_stplan_record`, `create_srel_record`, `create_chc_record`), and appends an audit/system repair `create_se_record` plus a planless forkable maintenance `create_pg_record` whose `state_snapshot.active_records` replays the parent snapshot through that SE delta. It never signs, submits, writes, or asks the operator to write a page plan; operators must validate the returned `patch_plan`, obtain explicit approval, sign that exact plan, and apply it with `submit_patch_plan`. Existing orphan records that predate this shape require an operator-driven backfill maintenance PG or bundle-specific annotation; the helper is forward-only. Use this for append-only correction of stale story state when no new fictional turn occurred; use `branching-story-turn-cycle` `repair_turn` when the repair is a real causal tick that needs ordinary fictional state progression and choices. |

`record_type` and `node_type` are related but not interchangeable vocabulary layers. `list_records.record_type` is the operator-facing retrieval vocabulary. `get_record_schema.node_type` exposes only current validator-backed schema files, including `character_proposal_card` / `character_proposal_batch` for NCP/NCB hybrid proposal records, `belief_record` for BEL story records, `story_status_record` for STSTAT story records, and `scene_record` for SCN records.

For story-pipeline packets, `get_context_packet` also returns a prose-free `story_bundle_context.scene_coverage` layer that projects SPEC-95's derived `scene_coverage` world-index view onto the active branch path: active SCN ids per page, unscened pages, and SPEC-94 publication indicators. It never reads scene prose and is the lowest-priority story-bundle layer — trimmed first under budget pressure, with its membership ids preserved in the governing summary. The load-bearing consumer is `branching-story-health-audit`. See `docs/CONTEXT-PACKET-CONTRACT.md` §6 for the full shape.
...
|---|---|---|
| Retrieval tools report missing nodes, persistent `index_version_mismatch`, or persistent `stale_index` | `_index/world.db` is absent, incompatible, still schema-mismatched after the retrieval layer's one-shot auto-build recovery, or still stale after the retrieval layer's auto-sync-then-auto-build recovery | Run `world-index init <world>` for an empty bootstrap, or `world-index build <world>` / `world-index sync <world>` for populated world state. If a successful retrieval response includes `freshness_audit.pre_call_index_version_was_old: true` or `freshness_audit.pre_call_index_was_stale: true`, the retrieval layer already recovered and no manual retry is needed. On `stale_index` the guard runs `world-index sync <world>` and retries once; if the index is still stale it escalates to a full `world-index build <world>` and retries once more. Since IDXSYNC-002 (file-level drift token unified on raw bytes + `last_indexed_at` refreshed for every still-present file on sync) the first `sync` rung converges to a freshness-equivalent state as a full build, so the escalation should rarely fire on routine drift and `recovery_outcome: still_stale_after_build` is now a genuine "index/disk divergence — investigate" signal. It surfaces `recovery_outcome: still_stale_after_build` only when the index is still stale after that build (run a manual `world-index build <world>` and investigate disk/index divergence), or `recovery_outcome: build_failed` when the escalation build exits non-zero. |
