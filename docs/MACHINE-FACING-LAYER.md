...
| Validate a patch plan envelope without mutating world content | `validate_patch_plan`, which returns `status: "pass"`, `status: "fail"` with validator verdicts, or `status: "skipped"` with a reason when the envelope cannot be validated. `pass` and `fail` responses include `validators_run[]`; `skipped` responses include `validators_run: []`. Envelope-shape validation also checks the canonical record-id field for world-canon `create_*_record` payloads, so `create_ch_record` requires `payload.ch_record.change_id` while the other world-canon create ops require `id`; misshapen create payloads skip before validator delegation with an `invalid_input` field path such as `patch_plan.patches[N].payload.ch_record.change_id`. For candidate story page-plan drafts, pass `page_plan_drafts=[{path, content}]` or CLI `--page-plan-drafts <drafts-json-path>` so page-plan structural validators inspect the proposed `pages-prose-plans/PG-<integer>.md` bytes. For envelopes too large for MCP transport, or for a temporary stale-validator-bundle workaround when restart is not immediately available, use the equivalent CLI path: `node tools/world-mcp/dist/src/cli/validate-patch-plan.js [--world-root <path>] [--page-plan-drafts <drafts-json-path>] <plan-path>`. The CLI resolves the world root by `--world-root` > `WORLDLOOM_ROOT` > auto-discovery from cwd. |
| Apply world-level changes on machine-layer-enabled worlds | `submit_patch_plan` via the patch engine. When the approved deliverable includes page-plan draft bytes, pass the same `page_plan_drafts` array or CLI `--page-plan-drafts <drafts-json-path>` used during validation so submit-time pre-apply validators exercise the approved draft before any write. |
...
| `verify_pg_state_hash` | Prose-attach Phase 2 verification for one committed PG record. Requires `world_slug`, `story_slug`, and `page_id`, recomputes `state_hash` directly from the parsed PG without modifying `plan.plan_hash`, and returns the recorded/computed state hash pair plus the SPEC-72 advisory recorded/computed plan-hash pair from `pages-prose-plans/<page_id>.md`. Missing plan files return `computed_plan_hash: null` / `plan_hash_match: null`; state hash mismatch is the verdict-driving tamper signal. |
...
| `plan_story_state_maintenance` | Review-only planning helper for bounded story-bundle state maintenance from a committed `parent_page_id`. It allocates fresh story-scoped IDs, verifies superseded source records through indexed retrieval, emits the requested maintenance record create ops (`create_stemo_record`, `create_stplan_record`, `create_srel_record`, `create_chc_record`), and appends an audit/system repair `create_se_record` plus a forkable maintenance `create_pg_record` whose `state_snapshot.active_records` replays the parent snapshot through that SE delta. It never signs, submits, or writes; operators must validate the returned `patch_plan`, write `maintenance_page_plan.body` exactly to `maintenance_page_plan.target_file`, obtain explicit approval, sign that exact plan, and apply it with `submit_patch_plan`. Existing orphan records that predate this shape require an operator-driven backfill maintenance PG or bundle-specific annotation; the helper is forward-only. Use this for append-only correction of stale story state when no new fictional turn occurred; use `branching-story-turn-cycle` `repair_turn` when the repair is a real causal tick that needs ordinary fictional state progression and choices. |
