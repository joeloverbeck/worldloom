...
| Allocate append-only IDs for one or many classes | `allocate_next_id` for a single world-, story-, pipeline-, or sub-audit-scoped ID; `allocate_many_ids` for ordered batch allocation during bootstrap/reservation workflows. Batch entries use the same `{id_class, story_slug?, audit_id?}` scope rules as the single allocator and repeated same-scope entries increment within the response. |
