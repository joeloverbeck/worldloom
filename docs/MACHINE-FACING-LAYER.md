...
**Persisted tool results**: `get_context_packet` overflow and unprojected hybrid `get_record` overflow use the same package-owned results directory. Set `WORLDLOOM_MCP_TOOL_RESULTS_DIR` to override it; otherwise the default is `/tmp/worldloom-mcp-tool-results/`. These persisted JSON files are recovery artifacts for the current local agent session, not canon or durable project outputs.
