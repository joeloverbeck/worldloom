# Prose Craft Contract - scene-plan canonical source

This is the canonical source for the 11-rule Prose Craft Contract + Diagnostic Vocabulary table inlined verbatim into every `scene-prose-plans/SCN-<integer>.md` scene plan. `branching-story-scene-plan` inlines this file's body byte-for-byte.

The Diagnostic Vocabulary table at the end is dual-purpose: it ships with every scene plan as part of the verbatim Prose Craft Contract block, AND it is the internal citation vocabulary used by scene-prose qualitative craft review. Its axis names are the citation tokens used in verdicts.
