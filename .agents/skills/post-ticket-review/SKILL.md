...
- `ticket_path` (optional): Path to the completed ticket file, for example `tickets/SPEC-01-003.md`. If omitted, use the single unambiguous just-finished ticket from the current session/worktree; otherwise ask for the exact path. Resolve globs or obvious near-matches to the exact live path before use.
...
- Run `git mv`, `git add`, and other git index-mutating commands serially. Do not launch them in parallel tool calls; a concurrent index write can leave `.git/index.lock` and obscure the review-created state you are trying to classify.
...
2. If `ticket_path` is omitted, first look for one unambiguous just-finished ticket in the current session/worktree and use that exact live path. If the immediately prior work was a non-ticket task such as skill maintenance, ignore that non-ticket work unless it changes the completed ticket's factual closeout. Candidate signals include active `tickets/` files marked `COMPLETED`, recent `## Outcome` / `## Verification Result` sections, dirty ticket paths, or a single ticket id repeatedly named in the current session. If there is no single unambiguous candidate after that scan, stop and ask the user for the exact ticket path instead of guessing.
...
4. Inspect `git diff --name-status --cached` and `git diff --name-status` after the move. If the archive move is staged but the archived file also has unstaged closeout edits, keep those states distinct in the final dirty-path classification.
5. Grep active tickets, specs, and docs for the old active path and same-family dependency references. Resolve candidate sweep roots first, and omit missing top-level directories such as an absent `specs/` rather than letting the sweep fail on nonexistent paths; record omissions only when they affect handoff clarity. Also inspect the moved archived ticket itself for the old active path, especially `Commands`, `Test Plan`, `Verification Result`, `Files to Touch`, and closeout snippets; update self-referential proof commands to the archived path when they remain actionable. When the reviewed ticket changed or named skills, tools, validators, hooks, or workflow docs, include those directly touched or ticket-named paths in the literal sweep as read-only review surfaces. Prefer literal-safe searches such as `rg -n -F 'tickets/SPEC-EXAMPLE-001.md' tickets specs docs archive/tickets/SPEC-EXAMPLE-001.md .claude/skills/<skill> .agents/skills/<skill> tools/<package>` or single-quoted regex patterns so ticket IDs, backticks, and path punctuation are not interpreted by the shell. Also sweep for bare ticket IDs in active dependency lists; when the just-archived ticket is now the prerequisite, replace stale bare IDs such as `SPEC-EXAMPLE-001` with the archived path `archive/tickets/SPEC-EXAMPLE-001.md`. Classify each hit as stale, historical, intentionally review-created, or forbidden-surface blocker. Repair `Deps`, target snippets, and actionable handoff instructions to the archived path when the completed ticket is now the prerequisite, but only in allowed review surfaces such as tickets, specs, and docs. Leave ordinary historical ID mentions and intentional follow-up references alone unless they claim a live path or would mislead implementation; report intentional review-created references in the handoff instead of "repairing" them. After literal replacements, run a stricter stale-active-path check that excludes `archive/tickets/` false positives, such as a regex with a negative `archive/` lookbehind when the local `rg` supports PCRE2.
