...
- when running stale-anchor sweeps over markdown/code literals, default to single-quoted search patterns for backticked command names, `$` variables, pipes, parentheses, and other shell-active characters; do not use double-quoted patterns that can execute the literal while searching
