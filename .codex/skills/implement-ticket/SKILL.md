...
- When a package script wraps `node --test`, inspect `package.json` before trusting extra positional args; `pnpm test <path>` or similar passthrough can end up executing a source `.ts` path directly instead of the compiled `dist/...` test and produce misleading `ERR_UNKNOWN_FILE_EXTENSION` failures.
