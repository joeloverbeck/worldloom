...
- after the final verification rerun, do a completed-ticket truth pass:
  ...
  - convert planned work in `What to Change`, `Acceptance Criteria`, and `Test Plan` to landed facts, or explicitly label those sections as the historical plan if preserving the original plan is necessary. For completed implementation tickets, prefer renaming `What to Change` to `Landed Changes` when preserving the section as a final record.
  ...
  - re-read the ticket again after those edits so the final record is internally consistent
...
- before running any `rg` / `grep` stale-anchor sweep over markdown/code literals, check whether the pattern contains backticks, `$`, pipes, parentheses, or other shell-active characters. Use single-quoted literal patterns, escape the active characters, or split the search into simpler safe patterns. Do not use double-quoted patterns that can execute the literal while searching.
