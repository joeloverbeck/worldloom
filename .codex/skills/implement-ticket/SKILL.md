...
- for package public-surface tickets that add or change `package.json` `exports` or exported symbols from an already-exported entrypoint, verify the full producer-consumer contract before trusting the drafted snippet: `require` / `import` / `default` / `types` conditions match the real emitted module format, the producer build actually emits the advertised runtime artifact and `.d.ts` artifact, and the intended consumer package's resolver settings (`moduleResolution`, export-map support, package root) can import that public entry without falling back to a private path
