...
- before running any final `rg` / `grep` pattern copied from Markdown, check for backticks, `$`, pipes, parentheses, or other shell-active characters; use single-quoted literal patterns, escaped patterns, or split simpler patterns before executing the command
- when the completed ticket intentionally preserves old commands, enum values, fallback wording, or failure text as labelled historical intake evidence, scope final negative stale-anchor sweeps to live consumer surfaces such as skills, docs, package sources, and tests; record the ticket-local historical preservation separately instead of treating it as a stale consumer hit
- when stale-anchor sweeps search for markdown/code literals containing backticks, `$`, pipes, parentheses, or similar shell-active characters, use single-quoted literal patterns, escaped patterns, or split simpler patterns; never trust a sweep that may have executed part of the search string in the shell. Prefer command shapes like `rg -n 'old \`literal\` text' path/` over double-quoted patterns.
- when the active ticket completes a named dependency, deferred follow-up, or known-debt item, grep active same-family sibling tickets for the completed ticket id and truth same-seam pending, deferred, or fallback wording before final proof; do not leave sibling tickets pointing future work at an already-completed gap.
- if that completed ticket id appears in active tickets outside the same family, inspect only the local reference sentence or dependency note and truth it when it describes the now-completed dependency, deferred item, or fallback state; do not widen into the other ticket's implementation scope unless live evidence shows same-seam required fallout
