...
- For story-state contract record-shape tickets, inventory both initial emitters and later writers, superseders, and readers of each affected class. Do not stop at bootstrap or turn-cycle creation sites; check lifecycle writers/readers such as prose attach, health audit, promotion closeout, story-fact promotion, and any other named skill that can supersede or interpret the same story records.
- When the user supplies an explicit spec or doc reference for the active ticket, sweep that reference before source edits for current-state claims about the owned seam, especially problem/status statements that will become false if the ticket lands. Truth same-seam stale current-state prose during reassessment when it defines the implementation boundary; record intentionally excluded stale prose before coding instead of discovering it only at closeout.
- For cross-skill or producer/consumer contract tickets, inspect both sides' `Guardrails`, `Known integration debt`, `Sibling interop`, and similar debt/disclosure sections for the active ticket id and stale `until landed`, `future`, or fallback language. Truth same-seam stale debt notes in the active ticket when they are only documentation of the contract's current state; do not change producer behavior unless the ticket actually owns that behavior.
- When an archived dependency or completed ticket claims that a consumer skill, doc, test, or package surface was updated, grep the live consumer for the old phrase, enum value, command, field name, or fallback path before coding. If the old marker still exists, classify it as same-seam required fallout, separate drift, or explicitly outside scope in `Assumption Reassessment` rather than relying on the archived closeout as proof of current state.
...
- When reassessment chooses one implementation strategy from drafted alternatives, such as derivation compatibility vs materialized migration, handler-level proof vs CLI proof, or optional vs required schema fields, grep active follow-up tickets for conditional wording that depends on that choice before source edits or closeout. Truth direct dependency/status wording in the active follow-up while leaving the follow-up's owned implementation prose untouched unless the current ticket truly owns it.
