...
- Read-only audit finding prose, including health-audit Mystery Reserve surfacing, does not require the read when reassessment proves the edit preserves gate order, approval timing, operator PASS/FAIL criteria, `validation_trace`, validator behavior, submit/validate flow, pre-apply behavior, and approval-token semantics. Record `HARD-GATE read: not required` and name the unchanged gate behavior. If the edit changes what counts as PASS/FAIL, what the user approves, or when the gate fires, read `docs/HARD-GATE-DISCIPLINE.md`.
